...
public abstract class SpawnReference implements Component<EntityStore> {
   ...
   static {
      BASE_CODEC = ((BuilderCodec.Builder)BuilderCodec.abstractBuilder(SpawnReference.class).append(new KeyedCodec("SpawnMarker", InvalidatablePersistentRef.CODEC), (reference, entityReference) -> reference.reference = entityReference, (reference) -> reference.reference).add()).build();
   }
}
