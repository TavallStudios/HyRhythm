...
public abstract class EntityEventSupport extends EventSupport<EntityEventType, EntityEventNotification> {
   public void postMessage(EntityEventType type, @Nonnull EntityEventNotification notification, @Nonnull Ref<EntityStore> parent, @Nonnull Store<EntityStore> store) {
      ...
      if (slot != null && slot.isEnabled()) {
         Vector3d parentEntityPosition = ((TransformComponent)store.getComponent(parent, TransformComponent.getComponentType())).getPosition();
         ...
         if (!(distanceSquared > slot.getMaxRangeSquared())) {
            FlockMembership flockMembership = (FlockMembership)store.getComponent(parent, FlockMembership.getComponentType());
