...
public class NewSpawnStartTickingSystem extends TickingSystem<EntityStore> {
   ...
   public NewSpawnStartTickingSystem(@Nonnull ResourceType<EntityStore, QueueResource> queueResourceType) {
      this.dependencies = Set.of(new SystemDependency(Order.AFTER, StepCleanupSystem.class));
   ...
   public void tick(float dt, int systemIndex, @Nonnull Store<EntityStore> store) {
      ObjectList<Ref<EntityStore>> queue = ((QueueResource)store.getResource(this.queueResourceType)).queue;
      if (!queue.isEmpty()) {
         ObjectListIterator var5 = queue.iterator();

         while(var5.hasNext()) {
            Ref<EntityStore> reference = (Ref)var5.next();
   ...
   public static void queueNewSpawn(@Nonnull Ref<EntityStore> reference, @Nonnull Store<EntityStore> store) {
      ...
      ((QueueResource)store.getResource(NewSpawnStartTickingSystem.QueueResource.getResourceType())).queue.add(reference);
   }
