...
public class SteeringSystem extends SteppableTickingSystem {
   ...
   public SteeringSystem(@Nonnull ComponentType<EntityStore, NPCEntity> npcEntityComponent) {
      ...
      this.query = Query.<EntityStore>and(npcEntityComponent, TransformComponent.getComponentType());
   }
   ...
   public void steppedTick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      NPCEntity npcComponent = (NPCEntity)archetypeChunk.getComponent(index, this.npcEntityComponent);
      ...
      if (role != null) {
         ...
         try {
            if (role.getDebugSupport().isDebugMotionSteering()) {
               ...
               role.getActiveMotionController().steer(ref, role, role.getBodySteering(), role.getHeadSteering(), (double)dt, commandBuffer);
               x = position.getX() - x;
               z = position.getZ() - z;
               double l = Math.sqrt(x * x + z * z);
               double v = l / (double)dt;
               double vx = x / (double)dt;
               double vz = z / (double)dt;
               double vh = l > 0.0 ? (double)PhysicsMath.normalizeTurnAngle(PhysicsMath.headingFromDirection(x, z)) : 0.0;
               NPCPlugin.get().getLogger().at(Level.FINER).log("=   Role    = t =%.4f v =%.4f vx=%.4f vz=%.4f h =%.4f nh=%.4f vh=%.4f", dt, v, vx, vz, 57.295776F * yaw, 57.295776F * yaw, 57.2957763671875 * vh);
            } else {
               role.getActiveMotionController().steer(ref, role, role.getBodySteering(), role.getHeadSteering(), (double)dt, commandBuffer);
            }
         } catch (IllegalStateException | IllegalArgumentException e) {
            ((HytaleLogger.Api)NPCPlugin.get().getLogger().at(Level.SEVERE).withCause(e)).log();
