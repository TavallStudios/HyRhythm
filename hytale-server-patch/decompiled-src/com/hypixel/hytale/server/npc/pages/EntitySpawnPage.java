...
public class EntitySpawnPage extends InteractiveCustomUIPage<EntitySpawnPageEventData> {
   ...
   private static final Value<String> TAB_STYLE_ACTIVE = Value.<String>ref("Common.ui", "DefaultTextButtonStyle");
   private static final Value<String> TAB_STYLE_INACTIVE = Value.<String>ref("Common.ui", "SecondaryTextButtonStyle");
   ...
   public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull EntitySpawnPageEventData data) {
   ...
   private void handleSelect(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull EntitySpawnPageEventData data) {
      UICommandBuilder commandBuilder = new UICommandBuilder();
   ...
   private void handleSetItemMaterial(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull EntitySpawnPageEventData data) {
   ...
   private void handleSpawn(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull EntitySpawnPageEventData data) {
      switch (this.activeTab) {
         case "NPC" -> this.spawnNPC(ref, store, data.count);
         case "Items" -> this.spawnItem(ref, store, data.count);
         case "Model" -> this.spawnModel(ref, store, data.count);
      }
      ...
      if (this.selectedNpcRole != null && this.position != null && this.rotation != null) {
         if (count >= 1 && count <= 100) {
            ...
            for(int i = 0; i < count; ++i) {
               NPCPlugin.get().spawnNPC(store, this.selectedNpcRole, (String)null, this.position, spawnRotation);
            }
   ...
   private void buildNPCList(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder) {
      ...
      if (!this.searchQuery.isEmpty()) {
         ...
         this.npcRoles = (List)var10001.sorted(Comparator.comparingInt(map::getInt).reversed()).limit(20L).collect(Collectors.toList());
      } else {
      ...
      for(int bound = this.npcRoles.size(); i < bound; ++i) {
         String id = (String)this.npcRoles.get(i);
   ...
   private void buildModelList(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder) {
      ...
      for(Map.Entry<String, ModelAsset> entry : ModelAsset.getAssetMap().getAssetMap().entrySet()) {
         String modelPath = ((ModelAsset)entry.getValue()).getModel();
         if (modelPath != null && !itemAndBlockModelPaths.contains(modelPath) && !modelPath.startsWith("Items/Projectiles/")) {
            allModels.add((String)entry.getKey());
         }
      }
      ...
      if (!this.searchQuery.isEmpty()) {
         ...
         Stream var10001 = map.keySet().stream().sorted();
         Objects.requireNonNull(map);
         this.modelIds = (List)var10001.sorted(Comparator.comparingInt(map::getInt).reversed()).limit(20L).collect(Collectors.toList());
      } else {
         this.modelIds = (List)allModels.stream().sorted().collect(Collectors.toList());
      }

      int i = 0;

      for(int bound = this.modelIds.size(); i < bound; ++i) {
         String id = (String)this.modelIds.get(i);
   ...
   private void selectNPCRole(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull String npcRole, @Nonnull UICommandBuilder commandBuilder) {
   ...
   public static class EntitySpawnPageEventData {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(EntitySpawnPageEventData.class, EntitySpawnPageEventData::new).append(new KeyedCodec("NPCRole", Codec.STRING), (entry, s) -> entry.npcRole = s, (entry) -> entry.npcRole).add()).append(new KeyedCodec("ModelId", Codec.STRING), (entry, s) -> entry.modelId = s, (entry) -> entry.modelId).add()).append(new KeyedCodec("ItemId", Codec.STRING), (entry, s) -> entry.itemId = s, (entry) -> entry.itemId).add()).append(new KeyedCodec("ItemStackId", Codec.STRING), (entry, s) -> entry.itemStackId = s, (entry) -> entry.itemStackId).add()).append(new KeyedCodec("Type", Codec.STRING), (entry, s) -> entry.type = s, (entry) -> entry.type).add()).append(new KeyedCodec("Tab", Codec.STRING), (entry, s) -> entry.tab = s, (entry) -> entry.tab).add()).append(new KeyedCodec("@SearchQuery", Codec.STRING), (entry, s) -> entry.searchQuery = s, (entry) -> entry.searchQuery).add()).append(new KeyedCodec("@Count", Codec.INTEGER), (entry, s) -> entry.count = s, (entry) -> entry.count).add()).append(new KeyedCodec("@RotationOffset", Codec.FLOAT), (entry, s) -> entry.rotationOffset = s, (entry) -> entry.rotationOffset).add()).append(new KeyedCodec("@Scale", Codec.FLOAT), (entry, s) -> entry.scale = s, (entry) -> entry.scale).add()).build();
      }
   }
}
