...
public class BlockTypeView extends BlockRegionView<BlockTypeView> {
   ...
   public BlockTypeView getUpdatedView(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      BlockTypeView blockTypeView = (BlockTypeView)blackBoardResource.getView(BlockTypeView.class, ref, componentAccessor);
   ...
   public void addSearchedBlockSets(@Nonnull Ref<EntityStore> ref, @Nonnull NPCEntity entity, @Nonnull IntList blockSets) {
      ...
      if (context.isEnabled()) {
         context.log("Registering new entity %s (reference:%s) with partial blackboard view %s, %s", entity.getRoleName(), ref, xOfViewIndex(this.index), zOfViewIndex(this.index));
      }
   ...
   public void removeSearchedBlockSets(@Nonnull Ref<EntityStore> ref, @Nonnull NPCEntity npcComponent, @Nonnull IntList blockSets) {
      if (!this.entities.remove(ref)) {
         throw new IllegalStateException(String.format("Attempting to unregister entity %s (reference:%s) from partial blackboard view %s at %s, %s when not registered", npcComponent.getRoleName(), ref, this.index, xOfViewIndex(this.index), zOfViewIndex(this.index)));
      } else {
         HytaleLogger.Api context = Blackboard.LOGGER.at(Level.FINEST);
         if (context.isEnabled()) {
            context.log("Unregistering entity %s (reference:%s) from partial blackboard view %s, %s", npcComponent.getRoleName(), ref, xOfViewIndex(this.index), zOfViewIndex(this.index));
         }
   ...
   private void removeSearchedBlockSet(int blockSet) {
      ...
      if (newCount < 0) {
         throw new IllegalStateException(String.format("Attempting to unregister blockset %s from partial blackboard view %s at %s, %s when not registered", blockSet, this.index, xOfViewIndex(this.index), zOfViewIndex(this.index)));
      } else {
         if (newCount == 0) {
   ...
   public IBlockPositionData findBlock(int blockSet, double range, double yMax, boolean pickRandom, @Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      ChunkStore chunkStore = ((EntityStore)componentAccessor.getExternalData()).getWorld().getChunkStore();
         ...
         for(int z = entityZ - maxRange & -32; z < entityZ + maxRange; z += 32) {
            ...
            if (chunk != null) {
               ...
               for(int y = minY; y < maxY; y += 32) {
                  ...
                  if (sectionRef != null) {
                     BlockPositionProvider entry = (BlockPositionProvider)chunkStoreStore.getComponent(sectionRef, BlockPositionProvider.getComponentType());
                     if (entry == null || entry.isStale(blockSet, section)) {
                        ...
                        if (indexViewFromChunkCoordinates(x, z) == this.index) {
                           if (this.aggregateNeedsRebuild) {
                              HytaleLogger.Api context = Blackboard.LOGGER.at(Level.FINEST);
                              if (context.isEnabled()) {
                                 context.log("Rebuilding blocktype aggregate in partial blackboard view %s, %s with %s blocksets", xOfViewIndex(this.index), zOfViewIndex(this.index), this.allBlockSets.cardinality());
                              }
                           ...
                           HytaleLogger.Api context = Blackboard.LOGGER.at(Level.FINEST);
                           if (context.isEnabled()) {
                              context.log("Entity %s (reference:%s) generating new entry for chunk %s section %s in view %s, %s", roleName, ref, chunkIndex, sectionIndex, xOfViewIndex(this.index), zOfViewIndex(this.index));
                           }
                           ...
                           if (entry != null) {
                              ...
                              BlockTypeView otherView = (BlockTypeView)this.blackboard.getView(BlockTypeView.class, x, z);
                              if (otherView != null) {
                     ...
                     ResourceView resourceView = (ResourceView)this.blackboard.getIfExists(ResourceView.class, indexViewFromChunkCoordinates(x, z));
                     entry.findBlocks(this.foundBlocks, blockSet, range, (double)yRange, ref, resourceView != null ? this.reservedBlockFilter : null, resourceView, componentAccessor);
                  }
               }
            }
         }
      ...
      if (this.foundBlocks.isEmpty()) {
         ...
         if (pickRandom) {
            data = (IBlockPositionData)RandomExtra.randomElement(this.foundBlocks);
         } else {
            double minDist2 = 1.7976931348623157E308;

            for(int i = 0; i < this.foundBlocks.size(); ++i) {
               IBlockPositionData block = (IBlockPositionData)this.foundBlocks.get(i);
