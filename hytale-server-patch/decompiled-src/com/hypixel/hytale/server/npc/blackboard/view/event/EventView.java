...
public abstract class EventView<ViewType extends IBlackboardView<ViewType>, EventType extends Enum<EventType>, NotificationType extends EventNotification> implements IBlackboardView<ViewType> {
   ...
   protected EventView(Class<EventType> type, EventType[] eventTypes, NotificationType reusableEventNotification, @Nonnull World world) {
      ...
      this.eventRegistry = new EventRegistry(new CopyOnWriteArrayList(), () -> !this.shutdown, String.format("EventView for world %s is not enabled!", world.getName()), NPCPlugin.get().getEventRegistry());
   }
   ...
   public void cleanup() {
      for(EventType eventType : this.eventTypes) {
         ((EventTypeRegistration)this.entityMapsByEventType.get(eventType)).cleanup();
      }
   ...
   public int getSetCount() {
      ...
      for(EventType type : this.eventTypes) {
         count += ((EventTypeRegistration)this.entityMapsByEventType.get(type)).getSetCount();
      }
   ...
   public void forEach(@Nonnull IntObjectConsumer<EventType> setConsumer, @Nonnull Consumer<Ref<EntityStore>> npcConsumer) {
      for(EventType eventType : this.eventTypes) {
         ((EventTypeRegistration)this.entityMapsByEventType.get(eventType)).forEach(setConsumer, npcConsumer);
      }
   ...
   protected void onEvent(int senderTypeId, double x, double y, double z, Ref<EntityStore> initiator, Ref<EntityStore> skip, @Nonnull ComponentAccessor<EntityStore> componentAccessor, EventType type) {
      ...
      ((EventTypeRegistration)this.entityMapsByEventType.get(type)).relayEvent(senderTypeId, this.reusableEventNotification, skip, componentAccessor);
   }
}
