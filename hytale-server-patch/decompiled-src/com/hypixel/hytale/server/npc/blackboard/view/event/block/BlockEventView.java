...
public class BlockEventView extends EventView<BlockEventView, BlockEventType, EventNotification> {
   public BlockEventView(@Nonnull World world) {
      ...
      for(BlockEventType eventType : BlockEventType.VALUES) {
         this.entityMapsByEventType.put(eventType, new EventTypeRegistration(eventType, (set, blockId) -> BlockSetModule.getInstance().blockInSet(set, blockId), NPCEntity::notifyBlockChange));
      }
   ...
   public BlockEventView getUpdatedView(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (!entityWorld.equals(this.world)) {
         ...
         return (BlockEventView)blackboardResource.getView(BlockEventView.class, ref, componentAccessor);
      } else {
         return this;
      }
   }
   ...
   private void processDamagedBlock(@Nonnull Ref<EntityStore> initiatorRef, String block, @Nonnull Vector3i position, @Nonnull BlockEventType type) {
      ...
      Player playerComponent = (Player)store.getComponent(initiatorRef, Player.getComponentType());
      if (playerComponent != null && playerComponent.getGameMode() == GameMode.Creative) {
         PlayerSettings playerSettingsComponent = (PlayerSettings)store.getComponent(initiatorRef, PlayerSettings.getComponentType());
      ...
      if (blockId == -2147483648) {
         ...
         this.onEvent(blockId, (double)position.x, (double)position.y, (double)position.z, initiatorRef, (Ref)null, store, (BlockEventType)type);
      }
   }
   ...
   private void onPlayerInteraction(@Nonnull PlayerInteractEvent event) {
      if (!event.isCancelled()) {
         ...
         if (playerComponent.getGameMode() == GameMode.Creative) {
            PlayerSettings playerSettingsComponent = (PlayerSettings)store.getComponent(ref, PlayerSettings.getComponentType());
            if (playerSettingsComponent == null || !playerSettingsComponent.creativeSettings().allowNPCDetection()) {
               return;
            }
         }
         ...
         if (blockPosition != null && event.getActionType() == InteractionType.Use) {
            ...
            this.onEvent(blockId, (double)targetBlock.x, (double)targetBlock.y, (double)targetBlock.z, ref, (Ref)null, store, (BlockEventType)BlockEventType.INTERACTION);
         }
      }
   }
}
