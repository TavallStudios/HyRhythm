...
public class EventTypeRegistration<EventType extends Enum<EventType>, NotificationType extends EventNotification> {
   ...
   public void relayEvent(int senderTypeId, @Nonnull NotificationType reusableEventNotification, Ref<EntityStore> skipEntityReference, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      Ref<EntityStore> initiator = ((EventNotification)reusableEventNotification).getInitiator();

      for(int set = this.eventSets.nextSetBit(0); set >= 0; set = this.eventSets.nextSetBit(set + 1)) {
         if (this.setTester.test(set, senderTypeId)) {
            List<Ref<EntityStore>> entities = (List)this.entitiesBySet.get(set);
            if (entities != null) {
               reusableEventNotification.setSet(set);

               for(int j = 0; j < entities.size(); ++j) {
                  Ref<EntityStore> entity = (Ref)entities.get(j);
                  if (entity.isValid() && !entity.equals(initiator) && !entity.equals(skipEntityReference)) {
                     NPCEntity npc = (NPCEntity)componentAccessor.getComponent(entity, NPC_COMPONENT_TYPE);
      ...
      for(int set = this.eventSets.nextSetBit(0); set >= 0; set = this.eventSets.nextSetBit(set + 1)) {
         ...
         if (entities != null) {
            for(int i = 0; i < entities.size(); ++i) {
               Ref<EntityStore> entity = (Ref)entities.get(i);
               if (entity.isValid()) {
   ...
   public void cleanup() {
      for(int set = this.eventSets.nextSetBit(0); set >= 0; set = this.eventSets.nextSetBit(set + 1)) {
         List<Ref<EntityStore>> entities = (List)this.entitiesBySet.getOrDefault(set, (Object)null);
         if (entities != null) {
