...
public class EntityEventView extends EventView<EntityEventView, EntityEventType, EntityEventNotification> {
   public EntityEventView(@Nonnull World world) {
      ...
      this.eventRegistry.register((Class)PlayerInteractEvent.class, world.getName(), this::onPlayerInteraction);

      for(EntityEventType eventType : EntityEventType.VALUES) {
         this.entityMapsByEventType.put(eventType, new EventTypeRegistration(eventType, (set, roleIndex) -> TagSetPlugin.get(NPCGroup.class).tagInSet(set, roleIndex), NPCEntity::notifyEntityEvent));
      }
   ...
   public EntityEventView getUpdatedView(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (!entityWorld.equals(this.world)) {
         ...
         return (EntityEventView)blackboardResource.getView(EntityEventView.class, ref, componentAccessor);
      } else {
         return this;
      }
   }
   ...
   public void initialiseEntity(@Nonnull Ref<EntityStore> ref, @Nonnull NPCEntity npcComponent) {
      for(int i = 0; i < EntityEventType.VALUES.length; ++i) {
         ...
         if (eventSets != null) {
            ((EventTypeRegistration)this.entityMapsByEventType.get(type)).initialiseEntity(ref, eventSets);
         }
      }

   }

   protected void onEvent(int senderTypeId, double x, double y, double z, Ref<EntityStore> initiator, @Nonnull Ref<EntityStore> skip, @Nonnull ComponentAccessor<EntityStore> componentAccessor, EntityEventType type) {
      FlockMembership membership = (FlockMembership)componentAccessor.getComponent(skip, FlockMembership.getComponentType());
   ...
   private void onPlayerInteraction(@Nonnull PlayerInteractEvent event) {
      ...
      if (!event.isCancelled()) {
         if (playerComponent.getGameMode() == GameMode.Creative) {
            PlayerSettings playerSettingsComponent = (PlayerSettings)store.getComponent(playerRef, PlayerSettings.getComponentType());
         ...
         if (entity != null && event.getActionType() == InteractionType.Use && entity instanceof NPCEntity) {
            ...
            TransformComponent transformComponent = (TransformComponent)store.getComponent(entityRef, TransformComponent.getComponentType());

            assert transformComponent != null;

            Vector3d pos = transformComponent.getPosition();
            this.onEvent(((NPCEntity)entity).getRoleIndex(), pos.x, pos.y, pos.z, playerRef, entityRef, store, (EntityEventType)EntityEventType.INTERACTION);
         }
      }
   }
   ...
   public void processAttackedEvent(@Nonnull Ref<EntityStore> victim, @Nonnull Ref<EntityStore> attacker, @Nonnull ComponentAccessor<EntityStore> componentAccessor, EntityEventType eventType) {
      ...
      if (componentAccessor.getArchetype(victim).contains(Player.getComponentType())) {
         ...
         NPCEntity npc = (NPCEntity)componentAccessor.getComponent(victim, NPCEntity.getComponentType());
         if (npc == null) {
            return;
         }
      ...
      Vector3d pos = ((TransformComponent)store.getComponent(victim, TransformComponent.getComponentType())).getPosition();
