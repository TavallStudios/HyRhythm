...
public class NPCStepCommand extends AbstractWorldCommand {
   ...
   public NPCStepCommand() {
      ...
      this.dtArg = (OptionalArg)this.withOptionalArg("dt", "server.commands.npc.step.dt.desc", ArgTypes.FLOAT).addValidator(Validators.greaterThan(0.0F));
   }

   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      float dt = this.dtArg.provided(context) ? (Float)this.dtArg.get(context) : 1.0F / (float)world.getTps();
