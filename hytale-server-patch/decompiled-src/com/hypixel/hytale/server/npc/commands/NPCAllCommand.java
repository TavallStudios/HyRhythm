...
public class NPCAllCommand extends AbstractPlayerCommand {
   ...
   public NPCAllCommand() {
      ...
      this.distanceArg = (OptionalArg)this.withOptionalArg("distance", "server.commands.npc.all.distance", ArgTypes.DOUBLE).addValidator(Validators.greaterThan(0.0));
   }

   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      double distance = this.distanceArg.provided(context) ? (Double)this.distanceArg.get(context) : 4.0;
      ...
      if (roles.isEmpty()) {
         ...
         int columns = MathUtil.ceil(Math.sqrt((double)roles.size()));
         double squareSideLength = (double)(columns - 1) * distance;
         ...
         for(int index = 0; index < roles.size(); ++index) {
            String name = (String)roles.get(index);
            if (name != null && !name.isEmpty()) {
               try {
                  double x = px + distance * (double)(index % columns);
                  double z = pz + distance * (double)(index / columns);
                  ...
                  if (!(y < 0.0)) {
                     ...
                     Pair<Ref<EntityStore>, NPCEntity> npcPair = npcModule.spawnEntity(store, roleIndex, pos, (Vector3f)null, (Model)null, (TriConsumer)null);
