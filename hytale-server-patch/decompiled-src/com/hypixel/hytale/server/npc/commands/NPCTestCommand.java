...
public class NPCTestCommand extends AbstractCommandCollection {
   public NPCTestCommand() {
      ...
      this.addSubCommand(new ProbeTestCommand());
   }
   ...
   public static class ProbeTestCommand extends AbstractPlayerCommand {
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
         ...
         boolean testWater = probeWater.probePosition(ref, playerCollider, position, collisionResult, (double)eyeHeight, store);
         boolean validatePosition = CollisionModule.get().validatePosition(world, playerCollider, position, 4, (Object)null, (_this, collisionCode, collision, collisionConfig) -> collisionConfig.blockId != -1, collisionResult) != -1;
         NPCPlugin npcPlugin = NPCPlugin.get();
         String text = "Pos Y [" + position.y + ", " + String.valueOf(playerCollider) + "] Height=" + world.getChunk(ChunkUtil.indexChunkFromBlock(position.x, position.z)).getHeight(MathUtil.floor(position.x), MathUtil.floor(position.z));
         context.sendMessage(Message.raw(text));
         npcPlugin.getLogger().at(Level.INFO).log(text);
         text = "Air " + testAir + " " + String.valueOf(probeAir);
         context.sendMessage(Message.raw(text));
         npcPlugin.getLogger().at(Level.INFO).log(text);
         text = "Water " + testWater + " " + String.valueOf(probeWater);
         context.sendMessage(Message.raw(text));
         npcPlugin.getLogger().at(Level.INFO).log(text);
