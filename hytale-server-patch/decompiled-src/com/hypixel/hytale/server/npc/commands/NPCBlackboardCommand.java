...
public class NPCBlackboardCommand extends AbstractCommandCollection {
   ...
   public static class ChunkCommand extends AbstractWorldCommand {
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
         ...
         if (chunkSection == null) {
            ...
            BlockPositionProvider entry = (BlockPositionProvider)chunkStoreStore.getComponent(chunkSection, BlockPositionProvider.getComponentType());
            if (entry == null) {
               ...
               for(int i = searchedBlockSets.nextSetBit(0); i >= 0; i = searchedBlockSets.nextSetBit(i + 1)) {
                  ...
                  sb.append(((BlockSet)BlockSet.getAssetMap().getAsset(i)).getId());
                  msg.insert(((BlockSet)BlockSet.getAssetMap().getAsset(i)).getId());
                  subsequent = true;
                  if (i == 2147483647) {
                     break;
                  }
               ...
               entry.forEachBlockSet((blockSet, list) -> {
                  ...
                  list.forEach((dataEntry) -> {
                     sb.append("    [ ").append(((BlockType)BlockType.getAssetMap().getAsset(dataEntry.getBlockType())).getId());
                     sb.append(" (").append(dataEntry.getX()).append(", ").append(dataEntry.getY()).append(", ").append(dataEntry.getZ()).append(") ]\n");
                     String var10001 = ((BlockType)BlockType.getAssetMap().getAsset(dataEntry.getBlockType())).getId();
                     msg.insert("    [ " + var10001 + " (" + dataEntry.getX() + ", " + dataEntry.getY() + ", " + dataEntry.getZ() + ") ]");
                  });
               });
            }
         }
   ...
   public static class DropCommand extends AbstractWorldCommand {
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
         ((Blackboard)store.getResource(Blackboard.getResourceType())).clear();
   ...
   public static class ViewCommand extends AbstractWorldCommand {
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
         Vector2i chunkPosition = ((RelativeChunkPosition)this.chunkArg.get(context)).getChunkPosition(context, store);
         long viewIndex = BlockTypeView.indexView(chunkPosition.x, chunkPosition.y);
         Blackboard blackboard = (Blackboard)store.getResource(Blackboard.getResourceType());
         BlockTypeView view = (BlockTypeView)blackboard.getView(BlockTypeView.class, viewIndex);
         ...
         msg.insert(Message.translation("server.commands.npc.blackboard.view.coordinates").param("x1", BlockTypeView.toWorldCoordinate(viewX)).param("z1", BlockTypeView.toWorldCoordinate(viewZ)).param("x2", BlockTypeView.toWorldCoordinate(viewX + 1)).param("z2", BlockTypeView.toWorldCoordinate(viewZ + 1)));
         msg.insert("\n");
         if (view == null) {
            ...
            for(int i = searchedBlockSets.nextSetBit(0); i >= 0; i = searchedBlockSets.nextSetBit(i + 1)) {
               ...
               sb.append(((BlockSet)BlockSet.getAssetMap().getAsset(i)).getId()).append(" (").append(counts.getOrDefault(i, 0)).append(')');
               String var10001 = ((BlockSet)BlockSet.getAssetMap().getAsset(i)).getId();
               msg.insert(var10001 + " (" + counts.getOrDefault(i, 0) + ")");
               subsequent = true;
               if (i == 2147483647) {
                  break;
               }
   ...
   public static class BlockEventsCommand extends AbstractWorldCommand {
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
         BlockEventView blockEventView = (BlockEventView)((Blackboard)store.getResource(Blackboard.getResourceType())).getView(BlockEventView.class, 0L);
         ...
         blockEventView.forEach((b, t) -> {
            ...
            String var10001 = ((BlockSet)BlockSet.getAssetMap().getAsset(b)).getId();
   ...
   public static class ResourceViewCommand extends AbstractWorldCommand {
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
         ...
         Blackboard blackboard = (Blackboard)store.getResource(Blackboard.getResourceType());
         ResourceView view = (ResourceView)blackboard.getView(ResourceView.class, viewIndex);
         ...
         Message msg = Message.translation("server.commands.npc.blackboard.view.title").param("x", viewX).param("z", viewZ);
         msg.insert(Message.translation("server.commands.npc.blackboard.view.coordinates").param("x1", ResourceView.toWorldCoordinate(viewX)).param("z1", ResourceView.toWorldCoordinate(viewZ)).param("x2", ResourceView.toWorldCoordinate(viewX + 1)).param("z2", ResourceView.toWorldCoordinate(viewZ + 1)));
         msg.insert("\n");
         if (view == null) {
   ...
   public static class ReservationCommand extends AbstractPlayerCommand {
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
         ...
         if (npcRef != null) {
            ...
            InteractionView reservationView = (InteractionView)blackBoardResource.getView(InteractionView.class, 0L);
