...
public class MotionControllerDive extends MotionControllerBase {
   ...
   public MotionController.VerticalRange getDesiredVerticalRange(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      double waterLevel = (double)this.moveProbe.getWaterLevel() + 1.0;
      double maxY = waterLevel - this.swimDepth - this.minDepthBelowSurface;
      double groundY = (double)this.moveProbe.getGroundLevel() + this.minDepthAboveGround;
   ...
   protected double computeMove(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nonnull Steering steering, double dt, @Nonnull Vector3d translation, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (this.canAct(ref, componentAccessor)) {
         ...
         if (this.debugModeSteer) {
            LOGGER.at(Level.INFO).log("=== Compute = t =%.4f v =%.4f h =%.4f nh=%.4f dh=%.4f", dt, this.moveSpeed, 57.295776F * heading, 57.295776F * newHeading, 57.295776F * turnAngle);
         }
   ...
   private void computeTranslation(@Nonnull Vector3d translation, double dt, float heading, double moveSpeed, double climbSpeed) {
      translation.x = moveSpeed * dt * (double)PhysicsMath.headingX(heading);
      translation.z = moveSpeed * dt * (double)PhysicsMath.headingZ(heading);
         ...
         if (collision == null) {
            ...
            if (!this.moveProbe.probePosition(ref, this.collisionBoundingBox, this.position, this.collisionResult, this.swimDepth, componentAccessor) || canAct && !this.moveProbe.isInWater()) {
               ...
               if (this.debugModeMove) {
                  LOGGER.at(Level.INFO).log("Move - Dive: No Collision, Bisect pos=%s, blocked=%s", Vector3d.formatShortString(this.position), this.isObstructed);
               }
            ...
            if (!this.moveProbe.probePosition(ref, this.collisionBoundingBox, this.position, this.collisionResult, this.swimDepth, componentAccessor) || canAct && !this.moveProbe.isInWater()) {
               ...
               if (this.debugModeMove) {
                  LOGGER.at(Level.INFO).log("Move - Dive: Collision with solid=%s, Bisect pos=%s, blocked=%s", this.collisionWithSolid, Vector3d.formatShortString(this.position), this.isObstructed);
               }
            } else if (this.debugModeMove) {
               LOGGER.at(Level.INFO).log("Move - Dive: Collision with solid=%s, pos=%s, blocked=%s", this.collisionWithSolid, Vector3d.formatShortString(this.position), this.isObstructed);
            }
   ...
   public double probeMove(@Nonnull Ref<EntityStore> ref, @Nonnull ProbeMoveData probeMoveData, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (!this.probeMoveProbe.probePosition(ref, this.collisionBoundingBox, probePosition, this.collisionResult, this.swimDepth, componentAccessor)) {
         ...
         if (this.debugModeMove) {
            LOGGER.at(Level.INFO).log("Probe Step: pos=%s mov=%s left=%s", Vector3d.formatShortString(probePosition), Vector3d.formatShortString(probeMovement), maxDistance);
         }
         ...
         if (collision == null) {
            ...
            if (this.probeMoveProbe.probePosition(ref, this.collisionBoundingBox, probePosition, this.collisionResult, this.swimDepth, componentAccessor) && this.probeMoveProbe.isInWater()) {
               ...
               if (this.debugModeMove) {
                  LOGGER.at(Level.INFO).log("Probe - Dive: No Collision, valid pos=%s, distanceLeft=%s", Vector3d.formatShortString(probePosition), maxDistance - maxDistance);
               }
            } else {
               distanceTravelled = this.bisect(ref, this.tempPosition, 0.0, probePosition, maxDistance, probePosition, componentAccessor);
               if (this.debugModeMove) {
                  LOGGER.at(Level.INFO).log("Probe - Dive: No Collision, Bisect pos=%s, distanceLeft=%s", Vector3d.formatShortString(probePosition), maxDistance - distanceTravelled);
               }
            }
            ...
            if (this.probeMoveProbe.probePosition(ref, this.collisionBoundingBox, probePosition, this.collisionResult, this.swimDepth, componentAccessor) && this.probeMoveProbe.isInWater()) {
               if (this.debugModeMove) {
                  LOGGER.at(Level.INFO).log("Probe - Dive: Collision, valid pos=%s, distanceLeft=%s, collision start=%s", Vector3d.formatShortString(probePosition), maxDistance - distanceTravelled, collisionStart);
               }
            } else {
               distanceTravelled = this.bisect(ref, this.tempPosition, 0.0, probePosition, distanceTravelled, probePosition, componentAccessor);
               if (this.debugModeMove) {
                  LOGGER.at(Level.INFO).log("Probe - Dive: Collision, Bisect pos=%s, distanceLeft=%s, collision start=%s", Vector3d.formatShortString(probePosition), maxDistance - distanceTravelled, collisionStart);
               }
            }
   ...
   public static double relativeSwimDepthToBoundingBox(double swimDepth, @Nullable Box boundingBox, float eyeHeight) {
      if (boundingBox == null) {
         ...
         return swimDepth < 0.0 ? NPCPhysicsMath.lerp((double)eyeHeight, boundingBox.getMin().getY(), -swimDepth) : NPCPhysicsMath.lerp((double)eyeHeight, boundingBox.getMax().getY(), swimDepth);
      }
   }
