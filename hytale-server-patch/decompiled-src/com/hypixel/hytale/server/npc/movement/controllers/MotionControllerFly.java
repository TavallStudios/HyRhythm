...
public class MotionControllerFly extends MotionControllerBase {
   ...
   protected int lastVerticalPositionX = -2147483648;
   protected int lastVerticalPositionZ = -2147483648;
   ...
   public MotionControllerFly(@Nonnull BuilderSupport builderSupport, @Nonnull BuilderMotionControllerFly builder) {
      ...
      this.sinMaxClimbAngle = (double)TrigMathUtil.sin(this.maxClimbAngle);
      this.maxSinkAngle = builder.getMaxSinkAngle();
      this.sinMaxSinkAngle = (double)(-TrigMathUtil.sin(this.maxSinkAngle));
   ...
   protected double computeMove(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nonnull Steering steering, double dt, @Nonnull Vector3d translation, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (this.forceVelocity.equals(Vector3d.ZERO) && this.appliedVelocities.isEmpty()) {
         ...
         if (this.canAct(ref, componentAccessor)) {
            ...
            double speedLimit = this.computeMaxSpeedFromPitch((double)pitch);
      ...
      if (ix == this.lastVerticalPositionX && iz == this.lastVerticalPositionZ) {
         ...
         if (chunkRef != null && chunkRef.isValid()) {
            ...
            if (chunkColumnComponent != null && blockChunkComponent != null) {
               ...
               double below = (double)WorldUtil.findFarthestEmptySpaceBelow(chunkStoreAccessor, chunkColumnComponent, blockChunkComponent, ix, iy, iz, iy) + this.collisionBoundingBox.min.y;
               double above = (double)WorldUtil.findFarthestEmptySpaceAbove(chunkStoreAccessor, chunkColumnComponent, blockChunkComponent, ix, iy, iz, iy) - this.collisionBoundingBox.max.y + 1.0;
   ...
   protected double doMove(@Nonnull Ref<EntityStore> ref, @Nonnull Vector3d position, @Nonnull Vector3d translation, @Nonnull PositionProbeAir moveProbe, @Nullable ProbeMoveData probeMoveData, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (this.debugModeMove) {
         LOGGER.at(Level.INFO).log("%s - Fly: Execute pos=%s vel=%s onGround=%s blocked=%s ", debugPrefix, Vector3d.formatShortString(position), Vector3d.formatShortString(translation), this.onGround(), this.isObstructed);
      }
      ...
      if (this.debugModeValidatePositions && !this.isValidPosition(position, this.collisionResult, componentAccessor)) {
         ...
         if (collision == null) {
            ...
            distanceFactor = 1.0;
            if (this.debugModeMove) {
               LOGGER.at(Level.INFO).log("%s - Fly: No collision pos=%s vel=%s onGround=%s blocked=%s ", debugPrefix, Vector3d.formatShortString(position), Vector3d.formatShortString(translation), this.onGround(), this.isObstructed);
            }
            ...
            if (this.debugModeMove) {
               LOGGER.at(Level.INFO).log("%s - Fly: Collision pos=%s collStart=%s vel=%s onGround=%s blocked=%s ", debugPrefix, Vector3d.formatShortString(position), distanceFactor, Vector3d.formatShortString(translation), this.onGround(), this.isObstructed);
            }
         ...
         if (!moveProbe.probePosition(ref, this.collisionBoundingBox, position, this.collisionResult, componentAccessor)) {
            double adjust = this.bisect(this.lastValidPosition, position, this, (_this, newPosition) -> _this.moveProbe.probePosition(ref, _this.collisionBoundingBox, newPosition, _this.collisionResult, componentAccessor), position);
            distanceFactor *= adjust;
            if (this.debugModeMove) {
               LOGGER.at(Level.INFO).log("%s - Fly: Bisect step pos=%s distanceFactor=%s adjust=%s", debugPrefix, Vector3d.formatShortString(position), distanceFactor, adjust);
            }
         }
   ...
   protected double executeMove(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, double dt, @Nonnull Vector3d translation, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      double scale = this.doMove(ref, this.position, translation, this.moveProbe, (ProbeMoveData)null, componentAccessor);
      if (scale < 1.0) {
         dt *= scale;
   ...
   public double getCurrentMaxBodyRotationSpeed() {
      return (double)this.maxTurnSpeed * this.effectHorizontalSpeedMultiplier;
   }
   ...
   public double getCurrentTurnRadius() {
      return this.lastSpeed / (double)this.maxTurnSpeed;
   }
   ...
   public double getHeightOverGround() {
      return (double)this.probeMoveProbe.getHeightOverGround();
   }
   ...
   protected double computeMaxSpeedFromPitch(double pitch) {
      double sinePitch = (double)TrigMathUtil.sin(pitch);
