...
public class PathManager {
   ...
   public IPath<?> getPath(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (this.currentPath == null) {
         ...
         WorldGenId worldGenIdComponent = (WorldGenId)componentAccessor.getComponent(ref, WorldGenId.getComponentType());
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(PathManager.class, PathManager::new).append(new KeyedCodec("CurrentPath", Codec.UUID_BINARY), (npcEntity, uuid) -> npcEntity.currentPathHint = uuid, (npcEntity) -> npcEntity.currentPathHint).setVersionRange(5, 5).add()).build();
   }
}
