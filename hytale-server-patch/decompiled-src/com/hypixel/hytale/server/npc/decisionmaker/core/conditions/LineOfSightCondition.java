...
public class LineOfSightCondition extends SimpleCondition {
   ...
   protected boolean evaluate(int selfIndex, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nullable Ref<EntityStore> targetRef, @Nonnull CommandBuffer<EntityStore> commandBuffer, EvaluationContext context) {
      if (targetRef != null && targetRef.isValid()) {
         ...
         NPCEntity selfNpcComponent = (NPCEntity)archetypeChunk.getComponent(selfIndex, NPC_COMPONENT_TYPE);
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(LineOfSightCondition.class, LineOfSightCondition::new, ABSTRACT_CODEC).documentation("A simple boolean condition that returns whether or not there is a line of sight to the target.")).build();
