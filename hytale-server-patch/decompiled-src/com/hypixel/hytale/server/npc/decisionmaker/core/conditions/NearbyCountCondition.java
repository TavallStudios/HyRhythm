...
public class NearbyCountCondition extends ScaledCurveCondition {
   ...
   protected double getInput(int selfIndex, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, Ref<EntityStore> target, CommandBuffer<EntityStore> commandBuffer, EvaluationContext context) {
      NPCEntity selfNpcComponent = (NPCEntity)archetypeChunk.getComponent(selfIndex, NPCEntity.getComponentType());

      assert selfNpcComponent != null;

      PositionCache positionCache = selfNpcComponent.getRole().getPositionCache();
      return (double)positionCache.countEntitiesInRange(0.0, this.range, this.includePlayers, NearbyCountCondition::filterNPC, selfNpcComponent.getRole(), this, commandBuffer);
   }
   ...
   public String toString() {
      double var10000 = this.range;
      return "NearbyCountCondition{range=" + var10000 + ", npcGroup=" + this.npcGroup + ", npcGroupIndex=" + this.npcGroupIndex + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(NearbyCountCondition.class, NearbyCountCondition::new, ScaledCurveCondition.ABSTRACT_CODEC).documentation("A scaled curve condition that returns a utility value based on the number of NPCs nearby belonging to a specific **NPCGroup**.")).append(new KeyedCodec("Range", Codec.DOUBLE), (condition, d) -> condition.range = d, (condition) -> condition.range).documentation("The range within which to count NPCs.").addValidator(Validators.nonNull()).addValidator(Validators.greaterThan(0.0)).add()).append(new KeyedCodec("NPCGroup", Codec.STRING), (condition, s) -> condition.npcGroup = s, (condition) -> condition.npcGroup).documentation("The NPCGroup to count NPCs from.").addValidator(Validators.nonNull()).addValidator(NPCGroup.VALIDATOR_CACHE.getValidator()).add()).afterDecode((condition) -> condition.npcGroupIndex = NPCGroup.getAssetMap().getIndex(condition.npcGroup))).build();
   }
}
