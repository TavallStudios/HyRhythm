...
public abstract class SimpleCondition extends Condition {
   ...
   public String toString() {
      double var10000 = this.falseValue;
      return "SimpleCondition{falseValue=" + var10000 + ", trueValue=" + this.trueValue + "} " + super.toString();
   }

   static {
      ABSTRACT_CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.abstractBuilder(SimpleCondition.class, BASE_CODEC).appendInherited(new KeyedCodec("FalseValue", Codec.DOUBLE), (condition, d) -> condition.falseValue = d, (condition) -> condition.falseValue, (condition, parent) -> condition.falseValue = parent.falseValue).documentation("The utility value to use when the condition evaluates false.").addValidator(Validators.range(0.0, 1.0)).add()).appendInherited(new KeyedCodec("TrueValue", Codec.DOUBLE), (condition, d) -> condition.trueValue = d, (condition) -> condition.trueValue, (condition, parent) -> condition.trueValue = parent.trueValue).documentation("The utility value to use when the condition evaluates true.").addValidator(Validators.range(0.0, 1.0)).add()).build();
   }
}
