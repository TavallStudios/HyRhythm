...
public abstract class Option {
   ...
   public void sortConditions() {
      ...
      for(int i = 0; i < this.conditions.length; ++i) {
         ...
         if (condition == null) {
            String var10002 = this.conditions[i];
            throw new IllegalStateException("Condition '" + var10002 + "' does not exist!");
         }
   ...
   public double calculateUtility(int selfIndex, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, Ref<EntityStore> target, CommandBuffer<EntityStore> commandBuffer, @Nonnull EvaluationContext context) {
      ...
      UUIDComponent uuidComponent = (UUIDComponent)archetypeChunk.getComponent(selfIndex, UUIDComponent.getComponentType());
   ...
   public String toString() {
      ...
      return "Option{description=" + var10000 + ", conditions=" + Arrays.toString(this.conditions) + ", sortedConditions=" + Arrays.toString(this.sortedConditions) + ", weightCoefficient=" + this.weightCoefficient + "}";
   }

   static {
      ABSTRACT_CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.abstractBuilder(Option.class).append(new KeyedCodec("Description", Codec.STRING), (option, s) -> option.description = s, (option) -> option.description).documentation("A friendly description of this option's outcome.").add()).appendInherited(new KeyedCodec("Conditions", Condition.CHILD_ASSET_CODEC_ARRAY), (option, s) -> option.conditions = s, (option) -> option.conditions, (option, parent) -> option.conditions = parent.conditions).documentation("The list of conditions for evaluating this option's utility.").addValidator(Validators.nonNull()).addValidator(Validators.nonEmptyArray()).addValidator(Condition.VALIDATOR_CACHE.getArrayValidator()).add()).appendInherited(new KeyedCodec("WeightCoefficient", Codec.DOUBLE), (option, d) -> option.weightCoefficient = d, (option) -> option.weightCoefficient, (option, parent) -> option.weightCoefficient = parent.weightCoefficient).documentation("An additional weighted ranking that can be used to greatly increase the utility of this option.").addValidator(Validators.greaterThanOrEqual(1.0)).add()).build();
   }
   ...
   private static class ConditionReference {
      ...
      private Condition get() {
         ...
         if (condition == null) {
            condition = (Condition)Condition.getAssetMap().getAsset(this.index);
            this.reference = condition.getReference();
         }
