...
public abstract class ScaledCurveCondition extends Condition {
   ...
   public double calculateUtility(int selfIndex, ArchetypeChunk<EntityStore> archetypeChunk, Ref<EntityStore> target, CommandBuffer<EntityStore> commandBuffer, EvaluationContext context) {
      ...
      return input == 1.7976931348623157E308 ? 0.0 : this.responseCurve.computeY(input);
   }
   ...
   public String toString() {
      String var10000 = String.valueOf(this.responseCurve);
      return "ScaledCurveCondition{responseCurve=" + var10000 + "} " + super.toString();
   }

   static {
      ABSTRACT_CODEC = ((BuilderCodec.Builder)BuilderCodec.abstractBuilder(ScaledCurveCondition.class, BASE_CODEC).appendInherited(new KeyedCodec("Curve", ScaledResponseCurve.CODEC), (condition, s) -> condition.responseCurve = s, (condition) -> condition.responseCurve, (condition, parent) -> condition.responseCurve = parent.responseCurve).documentation("The scaled response curve used to evaluate the condition.").addValidator(Validators.nonNull()).add()).build();
   }
}
