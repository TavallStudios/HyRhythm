...
public class HasTargetCondition extends SimpleCondition {
   ...
   public String toString() {
      String var10000 = this.targetSlot;
      return "HasTargetCondition{targetSlot=" + var10000 + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(HasTargetCondition.class, HasTargetCondition::new, ABSTRACT_CODEC).documentation("A simple boolean condition that returns whether the NPC has a target locked in the given slot.")).appendInherited(new KeyedCodec("TargetSlot", Codec.STRING), (condition, s) -> condition.targetSlot = s, (condition) -> condition.targetSlot, (condition, parent) -> condition.targetSlot = parent.targetSlot).documentation("The target slot to check.").addValidator(Validators.nonNull()).addValidator(Validators.nonEmptyString()).add()).build();
   }
}
