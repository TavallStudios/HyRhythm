...
public abstract class Evaluator<OptionType extends Option> {
   ...
   @Nullable
   public Evaluator<OptionType>.OptionHolder evaluate(int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, CommandBuffer<EntityStore> commandBuffer, @Nonnull EvaluationContext context) {
      ...
      for(int i = 0; i < this.options.size(); ++i) {
         Evaluator<OptionType>.OptionHolder optionHolder = (OptionHolder)this.options.get(i);
      ...
      if (bestOption == null) {
         ...
         if (predictability == 1.0F) {
            ...
            for(int i = 0; i < nonMatchingIndex; ++i) {
               Evaluator<OptionType>.OptionHolder optionHolder = (OptionHolder)this.options.get(i);
               if (optionHolder.utility >= threshold) {
            ...
            for(int i = 0; i < nonMatchingIndex; ++i) {
               Evaluator<OptionType>.OptionHolder optionHolder = (OptionHolder)this.options.get(i);
               if (!(optionHolder.utility < threshold)) {
                  randomWeight = optionHolder.tryPick(randomWeight, threshold);
                  if (randomWeight <= 0.0) {
                     bestOption = optionHolder;
                     break;
                  }
