...
public abstract class Condition implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, Condition>> {
   public static final double NO_TARGET = 1.7976931348623157E308;
   ...
   public static AssetStore<String, Condition, IndexedLookupTableAssetMap<String, Condition>> getAssetStore() {
      if (ASSET_STORE == null) {
         ASSET_STORE = AssetRegistry.<String, Condition, IndexedLookupTableAssetMap<String, Condition>>getAssetStore(Condition.class);
      }
   ...
   static {
      CODEC = new AssetCodecMapCodec<String, Condition>(Codec.STRING, (t, k) -> t.id = k, (t) -> t.id, (t, data) -> t.data = data, (t) -> t.data);
      BASE_CODEC = ((BuilderCodec.Builder)BuilderCodec.abstractBuilder(Condition.class).afterDecode((condition) -> condition.reference = new WeakReference(condition))).build();
      CHILD_ASSET_CODEC = new ContainedAssetCodec(Condition.class, CODEC);
      CHILD_ASSET_CODEC_ARRAY = new ArrayCodec<String[]>(CHILD_ASSET_CODEC, (x$0) -> new String[x$0]);
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(Condition::getAssetStore));
