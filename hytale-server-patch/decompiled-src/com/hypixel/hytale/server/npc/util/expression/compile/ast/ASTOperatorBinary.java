...
public class ASTOperatorBinary extends ASTOperator {
   ...
   public static void fromBinaryOperator(@Nonnull Parser.ParsedToken operator, @Nonnull CompileContext compileContext) throws ParseException {
      ...
      try {
         AST rhs = (AST)operandStack.pop();
         AST lhs = (AST)operandStack.pop();
         ...
         if (operatorBinary == null) {
            throw new ParseException("Type mismatch for operator " + String.valueOf(operator.token), operator.tokenPosition);
         } else {
            if (lhs.isConstant() && rhs.isConstant()) {
               ...
               lhs.genCode(instructionList, (Scope)null);
               rhs.genCode(instructionList, (Scope)null);
               instructionList.add((ExecutionContext.Instruction)operatorBinary.getCodeGen().apply((Object)null));
