...
public class Parser {
   ...
   @Nonnull
   private Deque<ParsedToken> operatorStack = new ArrayDeque();
   @Nonnull
   private Deque<ParsedToken> bracketStack = new ArrayDeque();
   ...
   public void parse(@Nonnull String expression, @Nonnull ParsedTokenConsumer tokenConsumer) throws ParseException {
      ...
      ParsedToken bracket;
      for(bracket = (ParsedToken)this.bracketStack.peek(); !token.isEndToken(); token = parsedToken.token) {
         if (token.isOperand()) {
            ...
            bracket = (ParsedToken)this.bracketStack.peek();
            ...
            for(ParsedToken first = (ParsedToken)this.operatorStack.pop(); !first.token.isOpenBracket(); first = (ParsedToken)this.operatorStack.pop()) {
            ...
            for(ParsedToken first = this.peekOperator(); !first.token.isOpenBracket(); first = this.peekOperator()) {
               bracket.operandCount = this.adjustOperandCount(first, bracket.operandCount);
               tokenConsumer.processOperator(first);
      ...
      if (bracket.token != Token.END) {
         ...
         while(!this.operatorStack.isEmpty()) {
            parsedToken = (ParsedToken)this.operatorStack.pop();
   ...
   public ParsedToken peekOperator() {
      return this.operatorStack.isEmpty() ? null : (ParsedToken)this.operatorStack.peek();
   }
   ...
   public interface ParsedTokenConsumer {
      void pushOperand(ParsedToken var1);

      void processOperator(ParsedToken var1) throws ParseException;

      void processFunction(int var1) throws ParseException;

      void processTuple(ParsedToken var1, int var2);
