...
public class ASTOperatorFunctionCall extends ASTOperator {
   ...
   public static void fromParsedFunction(int argumentCount, @Nonnull CompileContext compileContext) throws ParseException {
      ...
      AST functionNameAST = (AST)operandStack.get(len - argumentCount - 1);
      if (!(functionNameAST instanceof ASTOperandIdentifier identifier)) {
         throw new ParseException("Expected identifier for function name but found type " + String.valueOf(functionNameAST.getValueType()), functionNameAST.getTokenPosition());
      } else {
         ...
         if (resultType == null) {
            ...
            if (isConstant) {
               ...
               for(int i = firstArgument; i < len; ++i) {
                  ((AST)operandStack.get(i)).genCode(instructionList, (Scope)null);
               }

               instructionList.add(ExecutionContext.genCALL(functionName, argumentCount, (Scope)null));
