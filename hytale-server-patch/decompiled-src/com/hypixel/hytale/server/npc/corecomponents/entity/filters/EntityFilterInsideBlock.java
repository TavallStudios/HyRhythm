...
public class EntityFilterInsideBlock extends EntityFilterBase {
   ...
   public boolean matchesEntity(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> targetRef, @Nonnull Role role, @Nonnull Store<EntityStore> store) {
      this.chunkStore = ((EntityStore)store.getExternalData()).getWorld().getChunkStore();
      this.matches = false;
      Vector3d position = ((TransformComponent)store.getComponent(targetRef, TRANSFORM_COMPONENT_TYPE)).getPosition();
      Box boundingBox = ((BoundingBox)store.getComponent(targetRef, BOUNDING_BOX_COMPONENT_TYPE)).getBoundingBox();
   ...
   private static boolean accept(int x, int y, int z, @Nonnull EntityFilterInsideBlock filter) {
      ...
      if (index != filter.chunkIndex) {
         ...
         filter.blockChunk = (BlockChunk)filter.chunkStore.getChunkComponent(index, BLOCK_CHUNK_COMPONENT_TYPE);
      }
