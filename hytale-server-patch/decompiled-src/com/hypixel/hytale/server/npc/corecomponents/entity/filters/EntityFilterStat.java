...
public class EntityFilterStat extends EntityFilterBase {
   ...
   protected final EntityStatTarget statTarget;
   protected final int relativeTo;
   protected final EntityStatTarget relativeToTarget;
   ...
   public boolean matchesEntity(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> targetRef, @Nonnull Role role, @Nonnull Store<EntityStore> store) {
      EntityStatMap entityStatMap = (EntityStatMap)store.getComponent(targetRef, ENTITY_STAT_MAP_COMPONENT_TYPE);
      ...
      double ratio = (double)(statValue / relativeStatValue);
