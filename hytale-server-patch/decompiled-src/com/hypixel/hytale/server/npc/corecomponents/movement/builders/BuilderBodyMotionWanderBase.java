...
public abstract class BuilderBodyMotionWanderBase extends BuilderBodyMotionBase {
   ...
   public Builder<BodyMotion> readCommonConfig(@Nonnull JsonElement data) {
      ...
      this.getDouble(data, "MinWalkTime", this.minWalkTime, 2.0, DoubleSingleValidator.greaterEqual0(), BuilderDescriptorState.Stable, "Minimum time to wander for a segment.", (String)null);
      this.getDouble(data, "MaxWalkTime", this.maxWalkTime, 4.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Maximum time to wander for a segment.", (String)null);
      this.getFloat(data, "MinHeadingChange", this.minHeadingChange, 0.0, DoubleRangeValidator.between(0.0, 180.0), BuilderDescriptorState.Stable, "Approximate minimum heading change between segments", (String)null);
      this.getFloat(data, "MaxHeadingChange", this.maxHeadingChange, 90.0, DoubleRangeValidator.between(0.0, 180.0), BuilderDescriptorState.Stable, "Approximate maximum heading change between segments", (String)null);
      this.getBoolean(data, "RelaxHeadingChange", this.relaxHeadingChange, true, BuilderDescriptorState.Stable, "Allow other directions when preferred directions blocked", (String)null);
      this.getDouble(data, "RelativeSpeed", this.relativeSpeed, 0.5, DoubleRangeValidator.fromExclToIncl(0.0, 2.0), BuilderDescriptorState.Stable, "Relative wander speed", (String)null);
      this.getDouble(data, "MinMoveDistance", this.minMoveDistance, 0.5, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Minimum distance to move in a segment", (String)null);
      this.getDouble(data, "StopDistance", this.stopDistance, 0.5, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Distance to stop at target", (String)null);
      this.getInt(data, "TestsPerTick", this.testsPerTick, 1, IntSingleValidator.greater0(), BuilderDescriptorState.Stable, "Direction tests per tick", (String)null);
      ...
      this.getBoolean(data, "RelaxedMoveConstraints", this.isRelaxedMoveConstraints, false, BuilderDescriptorState.Stable, "NPC can do movements like wading (depends on motion controller type)", (String)null);
