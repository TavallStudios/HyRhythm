...
public class BuilderBodyMotionFind extends BuilderBodyMotionFindWithTarget {
   ...
   public BuilderBodyMotionFind readConfig(@Nonnull JsonElement data) {
      ...
      this.getBoolean(data, "Reachable", this.reachable, false, BuilderDescriptorState.Experimental, "Target must be reachable so that hitboxes can overlap", (String)null);
      this.getDoubleRange(data, "HeightDifference", this.heightDifference, DEFAULT_HEIGHT_DIFFERENCE, DoubleSequenceValidator.monotonic(), BuilderDescriptorState.Experimental, "Height difference allowed to target", (String)null);
      this.getDouble(data, "SlowDownDistance", this.slowDownDistance, 8.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Distance when to slow down when approaching", (String)null);
      this.getDouble(data, "StopDistance", this.stopDistance, 10.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Distance to stop at", (String)null);
      this.getDouble(data, "AbortDistance", this.abortDistance, 96.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Distance to abort behaviour", (String)null);
      this.getDouble(data, "Falloff", this.falloff, 3.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Deceleration when approaching target", (String)null);
      this.getDouble(data, "SwitchToSteeringDistance", this.switchToSteeringDistance, 20.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Distance below NPC can test if target is reachable and abort existing path", (String)null);
