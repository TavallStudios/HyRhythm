...
public abstract class BuilderMotionSequence<T extends Motion> extends BuilderMotionBase<T> {
   ...
   public BuilderMotionSequence<T> readConfig(@Nonnull JsonElement data) {
      this.getBoolean(data, "Looped", (b) -> this.looped = b, true, BuilderDescriptorState.Stable, "When true restart after last motion is finished", (String)null);
      this.getBoolean(data, "RestartOnActivate", (b) -> this.restartOnActivate = b, false, BuilderDescriptorState.Experimental, "Restart from first motion when NPC is activated.", (String)null);
      this.requireArray(data, "Motions", this.steps, ArrayNotEmptyValidator.get(), BuilderDescriptorState.Stable, "Array of motions", (String)null, this.validationHelper);
      return this;
   }
   ...
   public boolean validate(String configName, @Nonnull NPCLoadTimeValidationHelper validationHelper, @Nonnull ExecutionContext context, Scope globalScope, @Nonnull List<String> errors) {
      return super.validate(configName, validationHelper, context, globalScope, errors) & this.steps.validate(configName, validationHelper, this.builderManager, context, globalScope, errors);
   }
