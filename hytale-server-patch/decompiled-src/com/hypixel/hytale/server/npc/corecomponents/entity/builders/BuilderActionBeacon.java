...
public class BuilderActionBeacon extends BuilderActionBase {
   ...
   public BuilderActionBeacon readConfig(@Nonnull JsonElement data) {
      ...
      this.getDouble(data, "Range", (d) -> this.range = d, 64.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Experimental, "The maximum range to send the message", (String)null);
      this.requireAssetArray(data, "TargetGroups", this.targetGroups, 0, 2147483647, TagSetExistsValidator.withConfig(AssetValidator.ListCanBeEmpty), BuilderDescriptorState.Experimental, "The target group(s) to send the message to", (String)null);
      ...
      this.getDouble(data, "ExpirationTime", (d) -> this.expirationTime = d, 1.0, DoubleOrValidator.greaterEqual0OrMinus1(), BuilderDescriptorState.Experimental, "The number of seconds that the message should last. -1 represents infinite time.", "The number of seconds that the message should last and be acknowledged by the receiving NPC. -1 represents infinite time.");
   ...
   public int getTargetToSendSlot(@Nonnull BuilderSupport support) {
      ...
      return slotName == null ? -2147483648 : support.getTargetSlot(slotName);
   }
}
