...
public abstract class BuilderBodyMotionFindWithTarget extends BuilderBodyMotionFindBase {
   ...
   public BuilderBodyMotionFindBase readConfig(@Nonnull JsonElement data) {
      ...
      this.getDouble(data, "WaitDistance", this.minMoveDistanceWait, 1.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Experimental, "Minimum distance target needs to move before recomputing path when no path can be found", (String)null);
      this.getDouble(data, "RecomputeDistance", this.minMoveDistanceRecompute, 10.0, DoubleSingleValidator.greaterEqual0(), BuilderDescriptorState.Experimental, "Maximum distance target can move before path is recomputed or 0 to supress recomputation", (String)null);
      this.getDouble(data, "ReprojectDistance", this.minMoveDistanceReproject, 0.5, DoubleSingleValidator.greaterEqual0(), BuilderDescriptorState.Experimental, "Maximum distance target can move before position is reprojected", (String)null);
      this.getBoolean(data, "AdjustRangeByHitboxSize", this.adjustRangeByHitboxSize, false, BuilderDescriptorState.Stable, "Correct range by hitbox sizes of involved entities", (String)null);
      this.getFloat(data, "RecomputeConeAngle", this.recomputeConeAngle, 0.0, DoubleRangeValidator.between(0.0, 360.0), BuilderDescriptorState.Experimental, "Recompute path when target leaves cone from initial position to target", (String)null);
   ...
   public double getRecomputeConeAngle(@Nonnull BuilderSupport support) {
      return (double)(this.recomputeConeAngle.get(support.getExecutionContext()) * 0.017453292F);
   }
