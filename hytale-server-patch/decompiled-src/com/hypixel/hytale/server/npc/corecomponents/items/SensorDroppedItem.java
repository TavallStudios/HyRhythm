...
public class SensorDroppedItem extends SensorBase {
   ...
   public boolean matches(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, double dt, @Nonnull Store<EntityStore> store) {
      if (!super.matches(ref, role, dt, store)) {
         ...
         Ref<EntityStore> droppedItem = role.getPositionCache().getClosestDroppedItemInRange(ref, 0.0, this.range, (sensorDroppedItem, itemRef, role1, componentAccessor) -> sensorDroppedItem.filterItem(ref, itemRef, role1, componentAccessor), role, this, store);
         if (droppedItem == null) {
            this.positionProvider.clear();
            return false;
         } else {
   ...
   protected boolean filterItem(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> itemRef, @Nonnull Role role, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (!itemRef.isValid()) {
         ...
         if (this.viewCone > 0.0F) {
            TransformComponent selfTransformComponent = (TransformComponent)componentAccessor.getComponent(ref, TRANSFORM_COMPONENT_TYPE);

            assert selfTransformComponent != null;

            Vector3d selfPosition = selfTransformComponent.getPosition();
            TransformComponent itemTransformComponent = (TransformComponent)componentAccessor.getComponent(itemRef, TRANSFORM_COMPONENT_TYPE);
         ...
         if (this.hasLineOfSight && !role.getPositionCache().hasLineOfSight(ref, itemRef, componentAccessor)) {
            ...
            ItemComponent itemComponent = (ItemComponent)componentAccessor.getComponent(itemRef, ItemComponent.getComponentType());
