...
public class SensorBlockChange extends SensorEvent {
   ...
   protected Ref<EntityStore> getPlayerTarget(@Nonnull Ref<EntityStore> parent, @Nonnull Store<EntityStore> store) {
      PlayerBlockEventSupport blockEventSupportComponent = (PlayerBlockEventSupport)store.getComponent(parent, PlayerBlockEventSupport.getComponentType());
      ...
      return !blockEventSupportComponent.hasMatchingMessage(this.playerEventMessageSlot, position, this.range) ? null : blockEventSupportComponent.pollMessage(this.playerEventMessageSlot);
   }

   @Nullable
   protected Ref<EntityStore> getNpcTarget(@Nonnull Ref<EntityStore> parent, @Nonnull Store<EntityStore> store) {
      NPCBlockEventSupport blockEventSupportComponent = (NPCBlockEventSupport)store.getComponent(parent, NPCBlockEventSupport.getComponentType());
      ...
      Vector3d position = transformComponent.getPosition();
      return !blockEventSupportComponent.hasMatchingMessage(this.npcEventMessageSlot, position, this.range) ? null : blockEventSupportComponent.pollMessage(this.npcEventMessageSlot);
   }
}
