...
public class BuilderBodyMotionMoveAway extends BuilderBodyMotionFindWithTarget {
   ...
   public BuilderBodyMotionMoveAway readConfig(@Nonnull JsonElement data) {
      ...
      this.getDouble(data, "SlowDownDistance", this.slowdownDistance, 8.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Distance from target when NPC should start to slowdown", (String)null);
      this.getDouble(data, "StopDistance", this.stopDistance, 10.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Distance from target when NPC should halt", (String)null);
      this.getDouble(data, "Falloff", this.falloff, 3.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Rate how fast the slowdown should happen relative to distance", (String)null);
      this.getDoubleRange(data, "HoldDirectionTimeRange", this.holdDirectionDurationRange, DEFAULT_HOLD_DIRECTION_DURATION_RANGE, DoubleSequenceValidator.betweenWeaklyMonotonic(0.0, 1.7976931348623157E308), BuilderDescriptorState.Stable, "How often to change heading", (String)null);
      this.getDouble(data, "ChangeDirectionViewSector", this.changeDirectionViewSector, 230.0, DoubleRangeValidator.between(0.0, 360.0), BuilderDescriptorState.Stable, "The view sector the NPC uses to decide if it should switch direction", (String)null);
      this.getDouble(data, "DirectionJitter", this.directionJitter, 45.0, DoubleRangeValidator.between(0.0, 180.0), BuilderDescriptorState.Stable, "How much jitter in degrees to add to the heading the NPC uses", (String)null);
      this.getDouble(data, "ErraticDistance", this.erraticDistance, 4.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "If the player is closer than this distance, the NPC will behave more erratically using the additional jitter parameter", (String)null);
      this.getDouble(data, "ErraticExtraJitter", this.erraticExtraJitter, 45.0, DoubleRangeValidator.between(0.0, 180.0), BuilderDescriptorState.Stable, "Extra jitter to add to the NPC heading on top of the standard when the target is too close", (String)null);
      this.getDouble(data, "ErraticChangeDurationMultiplier", this.erraticChangeDurationMultiplier, 0.5, DoubleRangeValidator.fromExclToIncl(0.0, 1.0), BuilderDescriptorState.Stable, "A multiplier to decrease the duration between direction changes when the target is too close", (String)null);
