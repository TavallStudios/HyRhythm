...
public abstract class BuilderSensorWithEntityFilters extends BuilderSensorBase {
   protected final BuilderObjectListHelper<IEntityFilter> filters = new BuilderObjectListHelper<IEntityFilter>(IEntityFilter.class, this);
   ...
   public IEntityFilter[] getFilters(@Nonnull BuilderSupport support, @Nullable ISensorEntityPrioritiser prioritiser, ComponentContext context) {
      if (this.filters.isPresent() && !this.filters.isEmpty()) {
         ...
         support.setCurrentComponentContext((ComponentContext)null);
         return (IEntityFilter[])builtFilters.toArray((x$0) -> new IEntityFilter[x$0]);
   ...
   protected BuilderValidationHelper createFilterValidationHelper(ComponentContext context) {
      ...
      return new BuilderValidationHelper(this.fileName, (FeatureEvaluatorHelper)null, this.internalReferenceResolver, (StateMappingHelper)null, instructionContextHelper, this.extraInfo, this.evaluators, this.readErrors);
   }
}
