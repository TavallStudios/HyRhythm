...
public class SensorEntityPrioritiserAttitude implements ISensorEntityPrioritiser {
   ...
   @Nonnull
   public Ref<EntityStore> pickTarget(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nonnull Vector3d position, @Nonnull Ref<EntityStore> playerRef, @Nonnull Ref<EntityStore> npcRef, boolean useProjectedDistance, @Nonnull Store<EntityStore> store) {
      ...
      if (playerPriority != npcPriority) {
         ...
         TransformComponent playerTransformComponent = (TransformComponent)store.getComponent(playerRef, TRANSFORM_COMPONENT_TYPE);

         assert playerTransformComponent != null;

         TransformComponent npcTransformComponent = (TransformComponent)store.getComponent(npcRef, TRANSFORM_COMPONENT_TYPE);
   ...
   public static class AttitudePrioritiser implements IEntityByPriorityFilter {
      private final Attitude[] attitudeByPriority;
      private final Ref<EntityStore>[] targetsByAttitude;
      @Nullable
      private WorldSupport support;

      public AttitudePrioritiser(Attitude[] attitudeByPriority) {
         this.targetsByAttitude = new Ref[Attitude.VALUES.length];
      ...
      public void cleanup() {
         this.support = null;
         Arrays.fill(this.targetsByAttitude, (Object)null);
      }
   }
}
