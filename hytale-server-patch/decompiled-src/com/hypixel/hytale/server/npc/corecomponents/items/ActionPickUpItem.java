   ...
   public boolean canExecute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      if (!super.canExecute(ref, role, sensorInfo, dt, store)) {
         ...
         if (!this.hoover) {
            ...
            ItemComponent itemComponent = (ItemComponent)store.getComponent(targetRef, ITEM_COMPONENT_TYPE);
            if (itemComponent == null || !itemComponent.canPickUp()) {
               return false;
            }

            TransformComponent selfTransformComponent = (TransformComponent)store.getComponent(ref, TRANSFORM_COMPONENT_TYPE);
