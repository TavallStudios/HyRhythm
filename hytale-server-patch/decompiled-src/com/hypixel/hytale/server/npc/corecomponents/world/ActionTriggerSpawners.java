...
public class ActionTriggerSpawners extends ActionBase {
   ...
   public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      ...
      if (this.count <= 0) {
         for(int i = 0; i < spawners.size(); ++i) {
            Ref<EntityStore> spawnMarkerRef = this.filterMarker((Ref)spawners.get(i), store);
            if (spawnMarkerRef != null) {
               SpawnMarkerEntity spawnMarkerEntityComponent = (SpawnMarkerEntity)store.getComponent(spawnMarkerRef, SPAWN_MARKER_ENTITY_COMPONENT_TYPE);
         ...
         for(int i = 0; i < this.triggerList.size(); ++i) {
            Ref<EntityStore> spawnMarkerRef = (Ref)this.triggerList.get(i);
            SpawnMarkerEntity spawnMarkerEntityComponent = (SpawnMarkerEntity)store.getComponent(spawnMarkerRef, SPAWN_MARKER_ENTITY_COMPONENT_TYPE);
   ...
   protected Ref<EntityStore> filterMarker(@Nonnull Ref<EntityStore> targetRef, @Nonnull Store<EntityStore> store) {
      if (!targetRef.isValid()) {
         ...
         TransformComponent parentTransformComponent = (TransformComponent)store.getComponent(this.parentRef, TRANSFORM_COMPONENT_TYPE);
