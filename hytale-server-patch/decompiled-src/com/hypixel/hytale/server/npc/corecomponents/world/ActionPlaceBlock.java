   ...
   public boolean canExecute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      if (super.canExecute(ref, role, sensorInfo, dt, store) && sensorInfo != null && sensorInfo.hasPosition()) {
         ...
         if (blockToPlace == null) {
            ...
            BlockType placedBlockType = (BlockType)BlockType.getAssetMap().getAsset(blockToPlace);
            if (placedBlockType == null) {
               ...
               BoundingBox hitBox = (BoundingBox)store.getComponent(ref, BOUNDING_BOX_COMPONENT_TYPE);
               if (hitBox != null) {
               ...
               if (transformComponent.getPosition().distanceSquaredTo((double)x, (double)y, (double)z) > maxDistance * maxDistance) {
                  return false;
               } else if (sensorInfo instanceof CachedPositionProvider && !((CachedPositionProvider)sensorInfo).isFromCache()) {
                  return true;
               } else {
                  return !BlockPlacementHelper.canPlaceUnitBlock(world, placedBlockType, this.allowEmptyMaterials, x, y, z) ? false : BlockPlacementHelper.canPlaceBlock(world, placedBlockType, 0, this.allowEmptyMaterials, x, y, z);
               }
            }
         }
      } else {
