...
public class BuilderActionDropItem extends BuilderActionWithDelay {
   ...
   public BuilderActionDropItem readConfig(@Nonnull JsonElement data) {
      this.getAsset(data, "Item", this.item, (String)null, ItemExistsValidator.withConfig(AssetValidator.CanBeEmpty), BuilderDescriptorState.Stable, "A specific item to drop", (String)null);
      this.getAsset(data, "DropList", this.dropList, (String)null, ItemDropListExistsValidator.withConfig(AssetValidator.CanBeEmpty), BuilderDescriptorState.Stable, "A reference to an item drop list", (String)null);
      this.getFloat(data, "ThrowSpeed", (s) -> this.throwSpeed = s, 1.0F, DoubleRangeValidator.fromExclToIncl(0.0, 3.4028234663852886E38), BuilderDescriptorState.Stable, "The throw speed to use", (String)null);
      this.getDoubleRange(data, "Distance", (s) -> this.distance = s, DEFAULT_THROW_DISTANCE, DoubleSequenceValidator.betweenWeaklyMonotonic(0.0, 1.7976931348623157E308), BuilderDescriptorState.Stable, "The range from which to pick a distance to throw the item", (String)null);
      this.getDoubleRange(data, "DropSector", (s) -> this.dropSector = s, DEFAULT_DROP_SECTOR, DoubleSequenceValidator.betweenWeaklyMonotonic(-360.0, 360.0), BuilderDescriptorState.Stable, "The sector to spread drops in relative to view direction of NPC", "The sector to spread drops in relative to view direction of NPC in degrees.");
      this.getBoolean(data, "PitchHigh", (v) -> this.highPitch = v, false, BuilderDescriptorState.Stable, "Whether to pitch high or pitch low instead", (String)null);
   ...
   public boolean validate(String configName, @Nonnull NPCLoadTimeValidationHelper validationHelper, ExecutionContext context, Scope globalScope, @Nonnull List<String> errors) {
      ...
      double newThrowSpeed = AimingHelper.ensurePossibleThrowSpeed(this.distance[1], (double)height, 32.0, (double)this.throwSpeed);
      if (newThrowSpeed > (double)this.throwSpeed) {
