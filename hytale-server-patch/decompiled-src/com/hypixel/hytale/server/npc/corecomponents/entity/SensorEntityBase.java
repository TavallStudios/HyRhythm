...
public abstract class SensorEntityBase extends SensorWithEntityFilters {
   ...
   public boolean matches(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, double dt, @Nonnull Store<EntityStore> store) {
      if (!super.matches(ref, role, dt, store)) {
         ...
         if (debugSupport.isVisSensorRanges()) {
            this.currentVisSensorColorIndex = debugSupport.recordSensorRange(this.range, this.minRange, (double)this.visViewAngle);
         } else {
            this.currentVisSensorColorIndex = -1;
         }
   ...
   protected boolean filterEntityWithRange(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> targetRef, @Nonnull Vector3d position, @Nonnull Role role, @Nonnull Store<EntityStore> store) {
      ...
      if (playerComponent != null) {
         ...
         if (gameMode == GameMode.Creative) {
            PlayerSettings playerSettingsComponent = (PlayerSettings)store.getComponent(targetRef, PlayerSettings.getComponentType());
      ...
      return !(squaredDistance < this.minRange * this.minRange) && !(squaredDistance > this.range * this.range) ? this.filterEntity(ref, targetRef, role, store) : false;
   }
   ...
   protected boolean filterEntity(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> targetRef, @Nonnull Role role, @Nonnull Store<EntityStore> store) {
      if (store.getArchetype(targetRef).contains(DEATH_COMPONENT_TYPE)) {
         ...
         NPCEntity npcComponent = (NPCEntity)store.getComponent(targetRef, NPC_COMPONENT_TYPE);
         return this.isExcludingOwnType() && npcComponent != null && this.ownRole == npcComponent.getRoleIndex() ? false : this.matchesFilters(ref, targetRef, role, store);
      }
   }
