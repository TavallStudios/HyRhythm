...
public class ActionDropItem extends ActionWithDelay {
   ...
   public ActionDropItem(@Nonnull BuilderActionDropItem builder, @Nonnull BuilderSupport support) {
      ...
      this.dropSectorStart = 0.017453292F * (float)dropSector[0];
      float end = 0.017453292F * (float)dropSector[1];
   ...
   public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      ...
      if (this.item != null) {
         this.newDirection(ref, this.pickDistance(), (double)height, store);
   ...
   protected void newDirection(@Nonnull Ref<EntityStore> ref, double distance, double height, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (!AimingHelper.computePitch(distance, height, (double)this.throwSpeed, 32.0, this.pitch)) {
         throw new IllegalStateException(String.format("Error in computing pitch with distance %s, height %s, and speed %s that was not caught in validation", distance, height, this.throwSpeed));
      } else {
