...
public class SensorEntityEvent extends SensorEvent {
   ...
   protected Ref<EntityStore> getPlayerTarget(@Nonnull Ref<EntityStore> parent, @Nonnull Store<EntityStore> store) {
      PlayerEntityEventSupport entityEventSupportComponent = (PlayerEntityEventSupport)store.getComponent(parent, PlayerEntityEventSupport.getComponentType());
      ...
      return !entityEventSupportComponent.hasFlockMatchingMessage(this.playerEventMessageSlot, position, this.range, this.flockOnly) ? null : entityEventSupportComponent.pollMessage(this.playerEventMessageSlot);
   }
   ...
   protected Ref<EntityStore> getNpcTarget(@Nonnull Ref<EntityStore> parent, @Nonnull Store<EntityStore> store) {
      ...
      Vector3d position = transformComponent.getPosition();
      return !entityEventSupportComponent.hasFlockMatchingMessage(this.npcEventMessageSlot, position, this.range, this.flockOnly) ? null : entityEventSupportComponent.pollMessage(this.npcEventMessageSlot);
   }
}
