...
public class SensorAnimation extends SensorBase {
   ...
   public boolean matches(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, double dt, @Nonnull Store<EntityStore> store) {
      ActiveAnimationComponent activeAnimationComponent = (ActiveAnimationComponent)store.getComponent(ref, ActiveAnimationComponent.getComponentType());

      assert activeAnimationComponent != null;

      return super.matches(ref, role, dt, store) && Objects.equals(activeAnimationComponent.getActiveAnimations()[this.slot.getMappedSlot().ordinal()], this.animationId);
   }
