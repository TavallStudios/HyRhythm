...
public abstract class BodyMotionFindWithTarget extends BodyMotionFindBase<AStarWithTarget> {
   ...
   public boolean canComputeMotion(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider infoProvider, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      this.targetDeltaSquared = this.haveValidTargetPosition ? role.getActiveMotionController().waypointDistanceSquared(this.getLastTargetPosition(), this.lastPathedPosition) : 1.7976931348623157E308;
   ...
   public AStarBase.Progress startComputePath(@Nonnull Ref<EntityStore> ref, Role role, @Nonnull MotionController activeMotionController, @Nonnull Vector3d position, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      return (this.aStar).initComputePath(ref, position, this.lastPathedPosition, this, activeMotionController, this.probeMoveData, this.sharedNodePoolProvider, componentAccessor);
   }
