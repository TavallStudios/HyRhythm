...
public class HeadMotionAim extends HeadMotionBase implements DebugSupport.DebugFlagsChangeListener {
   ...
   public boolean computeSteering(@Nonnull Ref<EntityStore> ref, @Nonnull Role support, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Steering desiredSteering, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (sensorInfo != null && sensorInfo.hasPosition() && sensorInfo.getPositionProvider() != null) {
         ...
         if (targetRef != null) {
            Velocity targetVelocityComponent = (Velocity)componentAccessor.getComponent(targetRef, Velocity.getComponentType());
         ...
         if (ballisticData != null) {
            ...
            if (!isNearTarget) {
               ProjectileComponent.computeStartOffset(ballisticData.isPitchAdjustShot(), ballisticData.getVerticalCenterShot(), ballisticData.getHorizontalCenterShot(), ballisticData.getDepthShot(), lookRotation.getYaw(), lookRotation.getPitch(), this.startOffset);
            } else {
            ...
            if (this.aimingData.isHaveAttacked()) {
               ...
               if (this.spread > 0.0 && random.nextDouble() > this.hitProbability) {
                  ...
                  double start = 0.1;
                  double end = 0.9;
                  this.targetOffset.assign(NPCPhysicsMath.lerp(boundingBox.getMin().x, boundingBox.getMax().x, random.nextDouble(0.1, 0.9)), NPCPhysicsMath.lerp(boundingBox.getMin().y, boundingBox.getMax().y, random.nextDouble(0.1, 0.9)), NPCPhysicsMath.lerp(boundingBox.getMin().z, boundingBox.getMax().z, random.nextDouble(0.1, 0.9)));
               }
         ...
         if (isNearTarget && ballisticData != null) {
            ...
            double dotXZ = x * x + z * z;
            double dotXYZ = dotXZ + y * y;
            float yaw = dotXZ >= 1.0E-4 ? PhysicsMath.normalizeTurnAngle(PhysicsMath.headingFromDirection(x, z)) : lookRotation.getYaw();
            float pitch = dotXYZ >= 1.0E-4 ? PhysicsMath.pitchFromDirection(x, y, z) : lookRotation.getPitch();
            this.aimingData.setOrientation(yaw, pitch);
            this.aimingData.setTarget((Ref)null);
         }
