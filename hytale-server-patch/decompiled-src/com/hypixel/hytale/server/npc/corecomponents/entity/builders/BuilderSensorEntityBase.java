...
public abstract class BuilderSensorEntityBase extends BuilderSensorWithEntityFilters {
   ...
   protected final BuilderObjectReferenceHelper<ISensorEntityPrioritiser> prioritiser = new BuilderObjectReferenceHelper<ISensorEntityPrioritiser>(ISensorEntityPrioritiser.class, this);
   protected final BuilderObjectReferenceHelper<ISensorEntityCollector> collector = new BuilderObjectReferenceHelper<ISensorEntityCollector>(ISensorEntityCollector.class, this);

   @Nonnull
   public Builder<Sensor> readConfig(@Nonnull JsonElement data) {
      this.getDouble(data, "MinRange", this.minRange, 0.0, DoubleSingleValidator.greaterEqual0(), BuilderDescriptorState.Stable, "Minimum range to test entities in", (String)null);
      this.requireDouble(data, "Range", this.range, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Maximum range to test entities in", (String)null);
      this.getBoolean(data, "LockOnTarget", this.lockOnTarget, false, BuilderDescriptorState.Stable, "Matched target becomes locked target", (String)null);
      this.getString(data, "LockedTargetSlot", this.lockedTargetSlot, "LockedTarget", StringNullOrNotEmptyValidator.get(), BuilderDescriptorState.Stable, "The target slot to use for locking on or unlocking", (String)null);
      this.getBoolean(data, "AutoUnlockTarget", this.autoUnlockTarget, false, BuilderDescriptorState.Stable, "Unlock locked target when sensor not matching it anymore", (String)null);
      this.getBoolean(data, "OnlyLockedTarget", this.onlyLockedTarget, false, BuilderDescriptorState.Stable, "Test only locked target", (String)null);
      this.getString(data, "IgnoredTargetSlot", this.ignoredTargetSlot, (String)null, StringNullOrNotEmptyValidator.get(), BuilderDescriptorState.Stable, "The target slot to use for ignoring", (String)null);
      this.getBoolean(data, "UseProjectedDistance", this.useProjectedDistance, false, BuilderDescriptorState.Stable, "Use the projected movement direction vector for distance, rather than the Euclidean distance", (String)null);
      this.getObject(data, "Prioritiser", this.prioritiser, BuilderDescriptorState.Stable, "A prioritiser for selecting results based on additional parameters", (String)null, this.validationHelper);
      this.getObject(data, "Collector", this.collector, BuilderDescriptorState.Stable, "A collector which can process all checked entities and act on them based on whether they match or not", (String)null, this.validationHelper);
      BuilderValidationHelper builderHelper = this.createFilterValidationHelper(ComponentContext.SensorEntity);
      this.getArray(data, "Filters", this.filters, (ArrayValidator)null, BuilderDescriptorState.Stable, "A series of entity filter sensors to test", (String)null, builderHelper);
   ...
   public boolean validate(String configName, @Nonnull NPCLoadTimeValidationHelper validationHelper, @Nonnull ExecutionContext context, Scope globalScope, @Nonnull List<String> errors) {
      boolean result = this.prioritiser.validate(configName, validationHelper, this.builderManager, context, globalScope, errors) & super.validate(configName, validationHelper, context, globalScope, errors) & this.collector.validate(configName, validationHelper, this.builderManager, context, globalScope, errors);
   ...
   public int getLockedTargetSlot(@Nonnull BuilderSupport support) {
      return !this.lockOnTarget.get(support.getExecutionContext()) && !this.onlyLockedTarget.get(support.getExecutionContext()) ? -2147483648 : support.getTargetSlot(this.lockedTargetSlot.get(support.getExecutionContext()));
   }
   ...
   public ISensorEntityPrioritiser getPrioritiser(@Nonnull BuilderSupport support) {
      return (ISensorEntityPrioritiser)(!this.prioritiser.isPresent() ? new SensorEntityPrioritiserDefault() : (ISensorEntityPrioritiser)this.prioritiser.build(support));
   }

   @Nullable
   public ISensorEntityCollector getCollector(@Nonnull BuilderSupport support) {
      return this.collector.isPresent() ? (ISensorEntityCollector)this.collector.build(support) : ISensorEntityCollector.DEFAULT;
   }
}
