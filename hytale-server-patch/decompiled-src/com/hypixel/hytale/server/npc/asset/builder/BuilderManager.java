...
public class BuilderManager {
   ...
   private final Int2ObjectConcurrentHashMap<BuilderInfo> builderCache = new Int2ObjectConcurrentHashMap<BuilderInfo>();
   ...
   public BuilderManager() {
      ...
      this.nameToIndexMap.defaultReturnValue(-2147483648);
   ...
   public String getCategoryName(@Nonnull Class<?> factoryClass) {
      for(Map.Entry<String, Class<?>> stringClassEntry : this.categoryNames.entrySet()) {
         if (stringClassEntry.getValue() == factoryClass) {
            return (String)stringClassEntry.getKey();
         }
      }
         ...
         if (info != null) {
            ...
            try {
               ...
               do {
                  ...
                  entry = (Object2IntMap.Entry)iterator.next();
               } while(entry.getIntValue() != index);

               var5 = (String)entry.getKey();
            } finally {
               this.indexLock.readLock().unlock();
            }
   ...
   public boolean loadBuilders(@Nonnull AssetPack pack, final boolean includeTests) {
      ...
      try {
         ...
         for(BuilderInfo builderInfo : this.builderCache.values()) {
            try {
               ...
               valid = false;
               errors.add(String.format("%s: %s", builderInfo.getKeyName(), ((RuntimeException)e).getMessage()));
            }
         }
         ...
         if (!errors.isEmpty()) {
            valid = false;
            ObjectListIterator var17 = errors.iterator();

            while(var17.hasNext()) {
               String error = (String)var17.next();
      ...
      ObjectIterator var15 = typeCounter.object2IntEntrySet().iterator();

      while(var15.hasNext()) {
         Object2IntMap.Entry<String> entry = (Object2IntMap.Entry)var15.next();
   ...
   public void assetEditorLoadFile(@Nonnull Path fileName) {
      ...
      try {
         int builderIndex = this.loadFile(fileName, errors, (Object2IntMap)null, true, true);
         if (builderIndex < 0) {
            return;
         }
         ...
         String var10001 = builderNameFromPath(fileName);
         failedBuilderTexts.add(var10001 + ": " + e.getMessage());
      }
   ...
   public int loadFile(@Nonnull Path fileName, boolean reloading, @Nonnull List<String> errors) {
      return this.loadFile(fileName, errors, (Object2IntMap)null, false, reloading);
   }
   ...
   public int loadFile(@Nonnull Path fileName, @Nonnull List<String> errors, @Nullable Object2IntMap<String> typeCounter, boolean includeTests, boolean reloading) {
      ...
      try {
         ...
         errors.add(var10001 + ": Failed to load NPC builder: " + e.getMessage());
         return -2147483648;
      }
      ...
      if (testTypeElement != null) {
         try {
            testType = (TestType)Enum.valueOf(TestType.class, testTypeElement.getAsString().toUpperCase());
         } catch (Exception e) {
            String var46 = String.valueOf(fileName);
            errors.add(var46 + ": " + e.getMessage());
         }
      ...
      Class<?> category = (Class)this.categoryNames.get(categoryName);
      if (category == null) {
         String var51 = String.valueOf(fileName);
         errors.add(var51 + ": Failed to load NPC builder, unknown class " + categoryName);
         return -2147483648;
      } else {
         if (typeCounter != null) {
         ...
         BuilderFactory<Object> factory = this.<Object>getFactory(category);

         Builder<?> builder;
         try {
            builder = factory.createBuilder(content);
         } catch (Exception e) {
            String var47 = String.valueOf(fileName);
            errors.add(var47 + ": " + e.getMessage());
            return -2147483648;
         }
         ...
         AssetExtraInfo.Data extraInfoData = new AssetExtraInfo.Data((Class)null, keyName, (Object)null);
         AssetExtraInfo<Object> extraInfo = new AssetExtraInfo<Object>(extraInfoData);
         ...
         try {
            builder.readConfig((BuilderContext)null, content, this, builderParameters, validationHelper);
         } catch (Exception e) {
         ...
         ObjectListIterator e = evaluators.iterator();

         while(e.hasNext()) {
            Evaluator<?> evaluator = (Evaluator)e.next();
         ...
         if (testType == BuilderManager.TestType.FAILING) {
            ...
            if (failReasonElement == null) {
               ...
               String var50 = String.valueOf(fileName);
               errors.add(var50 + ": Should have failed validation: " + failReasonElement.getAsString());
               return -2147483648;
            } else if (errors.size() - errorCount > 1) {
               String var49 = String.valueOf(fileName);
               errors.add(var49 + ": Should have failed validation: " + failReasonElement.getAsString() + ", but additional errors were also detected.");
               return -2147483648;
            } else {
               String error = (String)errors.removeLast();
               if (!error.contains(failReasonElement.getAsString())) {
                  String var48 = String.valueOf(fileName);
                  errors.add(var48 + ": Should have failed validation: " + failReasonElement.getAsString() + ", but was instead: " + error);
                  return -2147483648;
               } else {
            ...
            return errors.size() > errorCount ? -2147483648 : this.cacheBuilder(keyName, builder, fileName);
         }
      }
   }
   ...
   public boolean validateBuilder(@Nonnull BuilderInfo builderInfo) {
      if (builderInfo.isValidated()) {
         ...
         return builder.getDependencies().isEmpty() && !builder.hasDynamicDependencies() ? builderInfo.setValidated(true) : this.validateBuilder(builderInfo, new IntOpenHashSet(), new IntArrayList());
      }
   }
   ...
   public <T> BuilderFactory<T> getFactory(@Nonnull Class<?> clazz) {
      if (clazz == null) {
         ...
         BuilderFactory<T> factory = (BuilderFactory)this.factoryMap.get(clazz);
         if (factory == null) {
      ...
      if (!this.setup) {
         ...
         try {
            ...
            while(iterator.hasNext()) {
               Object2IntMap.Entry<String> next = (Object2IntMap.Entry)iterator.next();
   ...
   public List<String> getTemplateNames() {
      return (List)this.collectMatchingBuilders(new ObjectArrayList(), (builderInfo) -> true, (builderInfo, strings) -> strings.add(builderInfo.getKeyName()));
   }
   ...
   public Schema generateSchema(@Nonnull SchemaContext context) {
      ...
      try {
         ...
         BuilderFactory<?> roleFactory = (BuilderFactory)this.factoryMap.get(Role.class);
         Schema schema = roleFactory.toSchema(context, true);
         ObjectSchema check = new ObjectSchema();
         check.setRequired(new String[]{"Class", "Type"});
         StringSchema keys = new StringSchema();
         keys.setEnum((String[])this.categoryNames.keySet().toArray((x$0) -> new String[x$0]));
         ...
         for(Map.Entry<String, Class<?>> cats : this.categoryNames.entrySet()) {
            BuilderFactory<Object> factory = this.<Object>getFactory((Class)cats.getValue());
            Schema s = factory.toSchema(context, true);
            Schema cond = new Schema();
            ObjectSchema classCheck = new ObjectSchema();
            classCheck.setProperties(Map.of("Class", StringSchema.constant((String)cats.getKey())));
      ...
      for(BuilderFactory<?> builderFactory : this.factoryMap.values()) {
         ...
         if (defaultBuilder != null) {
            try {
               ...
               NPCPlugin.get().getLogger().at(Level.SEVERE).log("Failed to build descriptor for %s %s: %s", categoryName, categoryName, ((RuntimeException)e).getMessage());
            }
         }
         ...
         for(String builderName : builderFactory.getBuilderNames()) {
            ...
            if (!name.equals("Component")) {
               try {
                  ...
                  NPCPlugin.get().getLogger().at(Level.SEVERE).log("Failed to build descriptor for %s %s: %s", categoryName, name, ((RuntimeException)e).getMessage());
               }
            }
         }
      }
   ...
   public static void saveDescriptors(List<BuilderDescriptor> builderDescriptors, @Nonnull Path fileName) {
      try {
         BufferedWriter fileWriter = Files.newBufferedWriter(fileName);
         ...
         try {
            ...
            gsonBuilder.registerTypeAdapter(Duration.class, (JsonSerializer)(src, typeOfSrc, context) -> new JsonPrimitive(src.toString()));
            gsonBuilder.registerTypeAdapter(Period.class, (JsonSerializer)(src, typeOfSrc, context) -> new JsonPrimitive(src.toString()));
   ...
   public Builder<Role> tryGetCachedValidRole(int builderIndex) {
      return this.<Role>tryGetCachedValidBuilder(builderIndex, Role.class);
   }
      ...
      if (validateDependents) {
         ...
         loadedBuilders.forEach((index, builderInfo) -> {
            for(BuilderInfo info : this.builderCache.values()) {
               ...
               } catch (IllegalStateException | IllegalArgumentException | SkipSentryException e) {
                  NPCPlugin.get().getLogger().at(Level.WARNING).log("Could not check if builder %s was dependent: %s", this.lookupName(info.getIndex()), ((RuntimeException)e).getMessage());
                  continue;
               }
   ...
   private static void validateAllSpawnableNPCs(@Nonnull Int2ObjectMap<BuilderInfo> builders, @Nonnull List<String> errors) {
      builders.forEach((index, builderInfo) -> {
         ...
         if (builder.isSpawnable() && builder instanceof SpawnableWithModelBuilder<?> spawnableBuilder) {
            ...
            } catch (IllegalStateException | SkipSentryException e) {
               errors.add(String.format("%s: %s", fileName, ((RuntimeException)e).getMessage()));
   ...
   private boolean validateBuilder(@Nonnull BuilderInfo builderInfo, @Nonnull IntSet validatedDependencies, @Nonnull IntArrayList path) {
      ...
      if (path.contains(index)) {
         ...
         try {
            ...
            NPCPlugin.get().getLogger().at(Level.SEVERE).log("Builder '%s' validation failed: %s", this.lookupName(path.getInt(0)), ((RuntimeException)e).getMessage());
            return builderInfo.setValidated(false);
         }
   ...
   private IntSet computeAllDependencies(@Nonnull Builder<?> builder, int builderIndex, @Nonnull IntSet dependencies, @Nonnull IntArrayList path) {
      if (path.contains(builderIndex)) {
         String var10004 = this.lookupName(builderIndex);
         throw new SkipSentryException(new IllegalArgumentException("Cyclic reference detected for builder: " + var10004));
      } else {
   ...
   private void iterateDependencies(@Nonnull IntIterator iterator, @Nonnull IntSet dependencies, @Nonnull IntArrayList path) {
      while(iterator.hasNext()) {
         ...
         if (!dependencies.contains(dependency)) {
            ...
            if (child == null) {
               String var10004 = this.lookupName(dependency);
               throw new SkipSentryException(new IllegalStateException("Reference to unknown builder: " + var10004));
            }
   ...
   private class BuilderAssetMonitorHandler implements AssetMonitorHandler {
      ...
      public void accept(Map<Path, EventKind> map) {
         ...
         for(Map.Entry<Path, EventKind> entry : map.entrySet()) {
            ...
            } else if (Files.isRegularFile(path, new LinkOption[0]) && !BuilderManager.isIgnoredFile(path)) {
               try {
                  int builderIndex = BuilderManager.this.loadFile(path, errors, (Object2IntMap)null, this.includeTests, true);
                  if (builderIndex >= 0) {
                     ...
                     for(BuilderInfo builderInfo : BuilderManager.this.builderCache.values()) {
                        try {
                           ...
                           NPCPlugin.get().getLogger().at(Level.WARNING).log("Could not check if builder %s was dependent: %s", BuilderManager.this.lookupName(builderInfo.getIndex()), ((RuntimeException)e).getMessage());
                        }
                     }
                  ...
                  String var10001 = BuilderManager.builderNameFromPath(path);
                  failedBuilderTexts.add(var10001 + ": " + e.getMessage());
               }
            }
         }
