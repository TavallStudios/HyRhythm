...
public class BuilderObjectMapHelper<K, V> extends BuilderObjectArrayHelper<Map<K, V>, V> {
      ...
      if (this.hasNoElements()) {
         ...
         for(BuilderObjectReferenceHelper<V> builderObjectReferenceHelper : this.builders) {
            if (!builderObjectReferenceHelper.excludeFromRegularBuild()) {
               ...
               K key = (K)this.id.apply(value);
               if (objects.containsKey(key)) {
                  String var10002 = String.valueOf(key);
                  throw new IllegalArgumentException("Duplicate key \"" + var10002 + "\" at " + this.getBreadCrumbs() + ": " + this.builderParameters.getFileName());
               }
   ...
   public <T, U> T testEach(@Nonnull BiFunction<Builder<V>, U, T> test, @Nonnull BuilderManager builderManager, ExecutionContext executionContext, U meta, T successResult, T emptyResult, Builder<?> parentSpawnable) {
      if (this.hasNoElements()) {
         ...
         for(BuilderObjectReferenceHelper<V> builderObjectReferenceHelper : this.builders) {
            T result = (T)test.apply(builderObjectReferenceHelper.getBuilder(builderManager, executionContext, parentSpawnable), meta);
