...
public class SubTypeTypeAdapterFactory implements TypeAdapterFactory {
   ...
   public <T> TypeAdapter<T> create(@Nonnull Gson gson, @Nonnull TypeToken<T> type) {
      if (type.getRawType() != this.baseClassType) {
         ...
         final Map<Class<?>, Map.Entry<String, TypeAdapter<?>>> delegateMap = new HashMap();
         ...
         return (new TypeAdapter<T>() {
            public void write(JsonWriter out, @Nonnull T value) throws IOException {
               Map.Entry<String, TypeAdapter<?>> entry = (Map.Entry)delegateMap.get(value.getClass());
               if (entry == null) {
                  throw new IllegalArgumentException();
               } else {
                  JsonObject result = new JsonObject();
                  JsonObject obj = ((TypeAdapter)entry.getValue()).toJsonTree(value).getAsJsonObject();
                  result.addProperty(SubTypeTypeAdapterFactory.this.typeFieldName, (String)entry.getKey());
