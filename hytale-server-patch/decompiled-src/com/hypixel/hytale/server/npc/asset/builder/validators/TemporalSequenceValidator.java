...
public class TemporalSequenceValidator extends TemporalArrayValidator {
   ...
   public static boolean compare(@Nonnull LocalDateTime value, @Nonnull RelationalOperator op, LocalDateTime c) {
      boolean var10000;
      switch (op) {
         case NotEqual -> var10000 = !value.equals(c);
         case Less -> var10000 = value.isBefore(c);
         case LessEqual -> var10000 = !value.isAfter(c);
         case Greater -> var10000 = value.isAfter(c);
         case GreaterEqual -> var10000 = !value.isBefore(c);
         case Equal -> var10000 = value.equals(c);
   ...
   public String errorMessage(String name, TemporalAmount[] value) {
      return name + (this.relationLower == null ? "" : " values should be " + this.relationLower.asText() + " " + String.valueOf(this.lower) + " and") + (this.relationUpper == null ? "" : " values should be " + this.relationUpper.asText() + " " + String.valueOf(this.upper) + " and") + (this.relationSequence == null ? "" : " succeeding values should be " + this.relationSequence.asText() + " preceding values and") + " values must all either be periods or durations but is " + Arrays.toString(value);
   }
}
