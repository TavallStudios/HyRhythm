...
public class BuilderSupport {
   ...
   private final ReferenceSlotMapper<Timer> timerSlotMapper = new ReferenceSlotMapper<Timer>(Timer::new);
   ...
   public BuilderSupport(BuilderManager builderManager, @Nonnull NPCEntity npcEntity, Holder<EntityStore> holder, ExecutionContext executionContext, @Nonnull Builder<?> roleBuilder, RoleStats roleStats) {
      ...
      this.instructionSlotMappings.defaultReturnValue(-2147483648);
   }
   ...
   public Alarm getAlarm(String name) {
      NPCEntity npc = (NPCEntity)this.holder.getComponent(NPCEntity.getComponentType());
      AlarmStore alarmStore = npc.getAlarmStore();
      return (Alarm)alarmStore.get(this.npcEntity, name);
   }
   ...
   public Tickable[] allocateTimers() {
      ...
      return referenceList.isEmpty() ? null : (Tickable[])referenceList.toArray((x$0) -> new Tickable[x$0]);
   }
   ...
   public void addComponentLocalStateMachine(int defaultState) {
      if (this.componentLocalStateMachines == null) {
         ...
         this.componentLocalStateMachines.defaultReturnValue(-2147483648);
      }
   ...
   public int getInstructionSlot(@Nullable String name) {
      ...
      if (slot == -2147483648) {
         ...
         this.instructions.add((Object)null);
      }
   ...
   public Instruction[] getInstructionSlotMappings() {
      Instruction[] slots = (Instruction[])this.instructions.toArray((x$0) -> new Instruction[x$0]);

      for(int i = 0; i < slots.length; ++i) {
         Instruction instruction = slots[i];
         if (instruction == null) {
            Object var10002 = this.instructionNameMappings.get(i);
            throw new IllegalStateException("Instruction: " + (String)var10002 + " doesn't exist");
         }
      }
   ...
   public StatePair getMappedStatePair(int index) {
      ...
      for(int i = this.modifiedStateMap.size() - 1; i >= 0; --i) {
         Map.Entry<StateMappingHelper, StatePair[]> entry = (Map.Entry)this.modifiedStateMap.get(i);
         result = ((StatePair[])entry.getValue())[index];
         index = ((StateMappingHelper)entry.getKey()).getComponentImportStateIndex(result.getFullStateName());
         if (index < 0) {
            break;
         }
      }
      ...
      if (this.blockSensorResetBlockSets != null) {
         this.blockSensorResetBlockSets.forEach((blockSet) -> {
            if (!this.blackboardBlockSets.contains(blockSet)) {
               throw new IllegalStateException(String.format("No block sensors match BlockSet %s in ResetBlockSensors action", ((BlockSet)BlockSet.getAssetMap().getAsset(blockSet)).getId()));
            }
         });
      }
   ...
   public IntSet getBlockChangeSets(BlockEventType type) {
      IntSet playerEventSets = this.playerBlockEventSlotMapper != null ? (IntSet)this.playerBlockEventSlotMapper.getEventSets().get(type) : null;
      IntSet npcEventSets = this.npcBlockEventSlotMapper != null ? (IntSet)this.npcBlockEventSlotMapper.getEventSets().get(type) : null;
   ...
   public IntSet getEventNPCGroups(EntityEventType type) {
      IntSet playerEventSets = this.playerEntityEventSlotMapper != null ? (IntSet)this.playerEntityEventSlotMapper.getEventSets().get(type) : null;
      IntSet npcEventSets = this.npcEntityEventSlotMapper != null ? (IntSet)this.npcEntityEventSlotMapper.getEventSets().get(type) : null;
      if (playerEventSets == null && npcEventSets == null) {
         return null;
      } else {
         IntOpenHashSet set = new IntOpenHashSet();
