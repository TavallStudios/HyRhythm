...
public class BuilderInstruction extends BuilderBase<Instruction> {
   ...
   protected final BuilderObjectReferenceHelper<Sensor> sensorBuilderObjectReferenceHelper = new BuilderObjectReferenceHelper<Sensor>(Sensor.class, this);
   protected final BuilderObjectReferenceHelper<BodyMotion> bodyMotionBuilderObjectReferenceHelper = new BuilderObjectReferenceHelper<BodyMotion>(BodyMotion.class, this);
   protected final BuilderObjectReferenceHelper<HeadMotion> headMotionBuilderObjectReferenceHelper = new BuilderObjectReferenceHelper<HeadMotion>(HeadMotion.class, this);
   protected final BuilderObjectReferenceHelper<ActionList> actionsBuilderObjectReferenceHelper = new BuilderObjectReferenceHelper<ActionList>(ActionList.class, this);
   protected final BuilderObjectListHelper<Instruction> steps = new BuilderObjectListHelper<Instruction>(Instruction.class, this);
   ...
   public Builder<Instruction> readConfig(@Nonnull JsonElement data) {
      ...
      this.getBoolean(data, "Enabled", this.enabled, true, BuilderDescriptorState.Stable, "Whether this instruction should be enabled on the NPC", (String)null);
      ...
      this.getBoolean(data, "Continue", (v) -> this.continueAfter = v, false, BuilderDescriptorState.WorkInProgress, "Continue after this instruction was executed", (String)null);
      this.getDouble(data, "Weight", this.chance, 1.0, DoubleSingleValidator.greater0(), BuilderDescriptorState.Stable, "Weighted chance of picking this instruction in a random instruction", (String)null);
      this.getBoolean(data, "TreeMode", (v) -> this.treeMode = v, false, BuilderDescriptorState.Stable, "Whether this instruction and its contents should be treated like a traditional behaviour tree.", "Whether this instruction and its contents should be treated like a traditional behaviour tree, i.e. will continue if all child instructions fail");
      this.getBoolean(data, "InvertTreeModeResult", this.invertTreeModeResult, false, BuilderDescriptorState.Stable, "Whether or not to invert the result of TreeMode evaluation when passing up to parent TreeMode instructions", (String)null);
      this.decreaseDepth();
   ...
   public boolean validate(String configName, @Nonnull NPCLoadTimeValidationHelper validationHelper, @Nonnull ExecutionContext context, Scope globalScope, @Nonnull List<String> errors) {
      if (!this.enabled.get(context)) {
         ...
         boolean result = this.sensorBuilderObjectReferenceHelper.validate(configName, validationHelper, this.builderManager, context, globalScope, errors) & this.bodyMotionBuilderObjectReferenceHelper.validate(configName, validationHelper, this.builderManager, context, globalScope, errors) & this.headMotionBuilderObjectReferenceHelper.validate(configName, validationHelper, this.builderManager, context, globalScope, errors) & this.actionsBuilderObjectReferenceHelper.validate(configName, validationHelper, this.builderManager, context, globalScope, errors) & this.steps.validate(configName, validationHelper, this.builderManager, context, globalScope, errors);
         if (this.currentStateName != null) {
   ...
   public Instruction[] getSteps(@Nonnull BuilderSupport support) {
      ...
      return stepList == null ? Instruction.EMPTY_ARRAY : (Instruction[])stepList.toArray((x$0) -> new Instruction[x$0]);
   }
