...
public class AStarBase {
   ...
   public Progress initComputePath(@Nonnull Ref<EntityStore> ref, @Nonnull Vector3d start, AStarEvaluator evaluator, @Nonnull MotionController motionController, @Nonnull ProbeMoveData probeMoveData, @Nonnull AStarNodePoolProvider nodePoolProvider, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      this.clearPath();
      this.iterations = 0;
      ...
      this.positionToIndexOffsetX = (double)twoX + positionToIndexOffset;
      this.positionToIndexOffsetY = (double)twoY + positionToIndexOffset;
      this.positionToIndexOffsetZ = (double)twoZ + positionToIndexOffset;
      ...
      if (this.searchDirections == null || this.searchDirectionIs2D != this.is2D || this.searchDirectionIsDiagonalMoves != this.canMoveDiagonal || !this.searchDirectionsWorldNormal.equals(motionController.getWorldNormal())) {
         ...
         if (this.is2D) {
            ...
            for(int i = 0; i < this.searchDirections.length; ++i) {
               Vector3d direction = this.searchDirections[i];
               int endIndex = directionIndex + this.normalsPerDirection;
               Vector3d oneNormal = null;

               for(int j = 0; j < this.searchDirections.length; ++j) {
                  Vector3d otherDirection = this.searchDirections[j];
                  if (i != j && direction.dot(otherDirection) == 0.0 && (oneNormal == null || oneNormal.dot(otherDirection) == 0.0)) {
                     this.normalDirections[directionIndex++] = j;
      ...
      this.tempPositionVector.assign(this.projectedX ? start.x : (double)startBlockX + 0.5, this.projectedY ? start.y : (double)startBlockY + 0.5, this.projectedZ ? start.z : (double)startBlockZ + 0.5);
      ...
      if (position != null) {
         ...
         this.tempPositionVector.x = this.projectedX ? start.x : (double)MathUtil.fastFloor(2.0 * start.x) / 2.0;
         this.tempPositionVector.y = this.projectedY ? start.y : (double)MathUtil.fastFloor(2.0 * start.y) / 2.0;
         this.tempPositionVector.z = this.projectedZ ? start.z : (double)MathUtil.fastFloor(2.0 * start.z) / 2.0;
         ...
         if (this.openNodes.isEmpty()) {
            double startX = this.tempPositionVector.x + (double)(this.projectedX ? 0 : -1);
            double endX = this.tempPositionVector.x + (double)(this.projectedX ? 0 : 1);
            double startY = this.tempPositionVector.y + (double)(this.projectedY ? 0 : -1);
            double endY = this.tempPositionVector.y + (double)(this.projectedY ? 0 : 1);
            double startZ = this.tempPositionVector.z + (double)(this.projectedZ ? 0 : -1);
            double endZ = this.tempPositionVector.z + (double)(this.projectedZ ? 0 : 1);
   ...
   public Progress computePath(@Nonnull Ref<EntityStore> ref, @Nonnull MotionController motionController, @Nonnull ProbeMoveData probeMoveData, int nodesToProcess, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (this.progress != AStarBase.Progress.COMPUTING) {
         ...
         while(!this.openNodes.isEmpty() && nodesToProcess-- > 0) {
            ...
            AStarNode node = (AStarNode)this.openNodes.get(idx);
            node.close();
                  ...
                  if (successors[directionIndex] == null) {
                     ...
                     if (!(distance < halfThreshold)) {
                        ...
                        if (halfPositionIndex != -1L) {
                           AStarNode otherNode = (AStarNode)this.visitedBlocks.get(halfPositionIndex);
                           if (otherNode != null) {
                              ...
                              if (this.is2D) {
                                 ...
                                 for(int normalIndex = firstNormalIndex; normalIndex < firstNormalIndex + this.normalsPerDirection; ++normalIndex) {
                                    ...
                                    AStarNode diagonalNode = (AStarNode)this.visitedBlocks.get(diagonalVertexIndex);
   ...
   public AStarNode findBestVisitedNode(@Nonnull ToFloatFunction<AStarNode> weight, @Nonnull BiFloatPredicate predicate, float initialValue) {
      if (this.visitedBlocks.isEmpty()) {
         ...
         ObjectIterator<Long2ObjectMap.Entry<AStarNode>> iterator = Long2ObjectMaps.fastIterator(this.visitedBlocks);
         float value = initialValue;
         AStarNode resultNode = null;

         while(iterator.hasNext()) {
            AStarNode node = (AStarNode)((Long2ObjectMap.Entry)iterator.next()).getValue();
   ...
   public static String positionIndexToString(long index) {
      double x = (double)(xFromIndex(index) - 1024) * 0.5 + 0.5;
      double y = (double)(yFromIndex(index) - 1024) * 0.5 + 0.5;
      double z = (double)(zFromIndex(index) - 1024) * 0.5 + 0.5;
      return "[" + x + "/" + y + "/" + z + "]";
   }

   protected Progress setProgress(Progress progress) {
      this.progress = progress;
      return this.progress;
   }

   @Nullable
   protected Vector3d canAdvance(@Nonnull Ref<EntityStore> ref, @Nonnull Vector3d startPosition, @Nonnull Vector3d destination, @Nonnull MotionController motionController, @Nonnull ProbeMoveData probeMoveData, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
   ...
   protected void addOpenNode(@Nonnull AStarNode parentNode, int directionIndex, @Nonnull Vector3d position, long positionIndex, float cost, MotionController motionController) {
      int inverseSearchDirection = this.inverseSearchDirections[directionIndex];
      AStarNode node = this.nodePool.allocate().initWithPredecessor(parentNode, directionIndex, position, positionIndex, inverseSearchDirection, cost, this.evaluator.estimateToGoal(this, position, motionController));
      this.addOpenNode(node, positionIndex);
   }
   ...
   protected void addOpenNode(@Nonnull AStarNode node, long index) {
      ...
      for(float totalCost = node.getTotalCost(); predecessor >= 0 && ((AStarNode)this.openNodes.get(predecessor)).getTotalCost() < totalCost; --predecessor) {
      }
   ...
   protected void addOrUpdateNode(@Nonnull AStarNode node, int directionIndex, @Nonnull Vector3d position, @Nonnull MotionController motionController, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      long positionIndex = this.positionToIndex(position);
      if (positionIndex != -1L && positionIndex != node.positionIndex) {
         AStarNode targetNode = (AStarNode)this.visitedBlocks.get(positionIndex);
         if (targetNode == null) {
   ...
   protected long addOffsetToIndex(long index, long xSteps, long ySteps, long zSteps) {
      long x = (long)xFromIndex(index) + xSteps;
      long y = (long)yFromIndex(index) + ySteps;
      long z = (long)zFromIndex(index) + zSteps;
