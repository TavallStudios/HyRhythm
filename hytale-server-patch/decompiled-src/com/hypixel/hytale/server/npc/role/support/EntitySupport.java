...
public class EntitySupport {
   ...
   public void tick(float dt) {
      ...
      while(i < this.delayingComponents.size()) {
         IComponentExecutionControl component = (IComponentExecutionControl)this.delayingComponents.get(i);
   ...
   public static void setDisplayName(@Nonnull Holder<EntityStore> holder, @Nullable String displayName, boolean override) {
      ...
      if (displayName != null) {
         Nameplate nameplateComponent = (Nameplate)holder.ensureAndGetComponent(Nameplate.getComponentType());
   ...
   public static void setDisplayName(@Nonnull Ref<EntityStore> ref, @Nullable String displayName, boolean override, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (ref.isValid()) {
         DisplayNameComponent displayNameComponent = (DisplayNameComponent)componentAccessor.getComponent(ref, DisplayNameComponent.getComponentType());
         if (displayNameComponent != null) {
            Message displayNameMessage = displayNameComponent.getDisplayName();
            if (displayNameMessage != null && !displayNameMessage.getAnsiMessage().isEmpty() && !override) {
               return;
         ...
         if (displayName != null) {
            Nameplate nameplateComponent = (Nameplate)componentAccessor.ensureAndGetComponent(ref, Nameplate.getComponentType());
            nameplateComponent.setText(displayName);
         } else {
   ...
   public static StdScope createScope(@Nonnull NPCEntity entity) {
      ...
      scope.addSupplier("blocked", (BooleanSupplier)(() -> entity.getRole().getActiveMotionController().isObstructed()));
      scope.addSupplier("health", (DoubleSupplier)(() -> {
         EntityStatValue healthStat = EntityStatsModule.get(entity).get(DefaultEntityStatTypes.getHealth());
         return (double)((EntityStatValue)Objects.requireNonNull(healthStat)).asPercentage();
      }));
      return scope;
   }
}
