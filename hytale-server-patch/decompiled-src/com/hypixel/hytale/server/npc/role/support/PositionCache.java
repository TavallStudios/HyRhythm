...
public class PositionCache {
   ...
   private final List<Ref<EntityStore>> droppedItems = new ObjectArrayList();
   private final List<Ref<EntityStore>> spawnMarkers = new ObjectArrayList();
   private final List<Ref<EntityStore>> spawnBeacons = new ObjectArrayList();
   private final Object2ByteMap<Ref<EntityStore>> lineOfSightCache = new Object2ByteOpenHashMap();
   private final Object2ByteMap<Ref<EntityStore>> inverseLineOfSightCache = new Object2ByteOpenHashMap();
   private final Object2ByteMap<Ref<EntityStore>> friendlyFireCache = new Object2ByteOpenHashMap();
   ...
   public PositionCache(@Nonnull Role role) {
      this.role = role;
      this.players = new EntityList((BucketItemPool)null, IS_VALID_PLAYER);
      this.npcs = new EntityList((BucketItemPool)null, IS_VALID_NPC);
   }
   ...
   private static <T> boolean testLineOfSightRays(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> targetRef, @Nonnull RayPredicate<T> predicate, @Nonnull T t, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      Vector3d position = transformComponent.getPosition();
      ModelComponent modelComponent = (ModelComponent)componentAccessor.getComponent(ref, MODEL_COMPONENT_TYPE);
      ...
      ModelComponent targetModelComponent = (ModelComponent)componentAccessor.getComponent(targetRef, MODEL_COMPONENT_TYPE);
      if (targetModelComponent != null) {
         ...
         double oy = 0.0;
         double oz = 0.0;
         BoundingBox boundingBoxComponent = (BoundingBox)componentAccessor.getComponent(targetRef, BOUNDING_BOX_COMPONENT_TYPE);
   ...
   public boolean isFriendlyBlockingLineOfSight(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> targetRef, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (cached) {
         ...
         boolean blocking = testLineOfSightRays(ref, targetRef, (sx, sy, sz, tx, ty, tz, _this, accessor) -> {
   ...
   protected static boolean rayIsIntersectingEntity(@Nonnull Ref<EntityStore> ref, @Nonnull Vector3d pos, @Nonnull Vector3d dir, @Nonnull Vector2d minMax, double length2, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (boundingBoxComponent == null) {
         ...
         if (dotProduct <= 0.0) {
            ...
            return dotProduct * dotProduct >= dist2 * length2 ? false : CollisionMath.intersectRayAABB(pos, dir, px, py, pz, boundingBoxComponent.getBoundingBox(), minMax);
         }
      }
   }
