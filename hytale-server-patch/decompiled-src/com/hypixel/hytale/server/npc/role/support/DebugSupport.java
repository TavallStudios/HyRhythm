...
public class DebugSupport {
   ...
   protected List<DebugFlagsChangeListener> debugFlagsChangeListeners = new ArrayList();
   ...
   public void recordEntityCheck(@Nonnull Ref<EntityStore> entityRef, int sensorColorIndex, boolean matched) {
      ...
      ((List)this.entityVisDataMap.computeIfAbsent(entityRef, (k) -> new ArrayList())).add(new EntityVisData(sensorColorIndex, matched));
   }
   ...
   public static record SensorVisData(double range, double minRange, int colorIndex, double viewAngle) {
   }

   public static record EntityVisData(int sensorColorIndex, boolean matched) {
   }
