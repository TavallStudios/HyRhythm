...
public class SpawnNPCInteraction extends SimpleBlockInteraction {
   ...
   private void spawnNPC(@Nonnull Store<EntityStore> store, @Nonnull Vector3i targetBlock) {
      ...
      if (this.weightedSpawnMap != null) {
         entityToSpawn = (String)this.weightedSpawnMap.get((Random)ThreadLocalRandom.current());
      }

      if (entityToSpawn != null) {
         NPCPlugin.get().spawnNPC(store, entityToSpawn, (String)null, spawnData.position(), spawnData.rotation());
      }
   }
      ...
      if (chunkRef != null && chunkRef.isValid()) {
         ...
         if (blockType == null) {
            ...
            if (blockChunkComponent == null) {
               ...
               Vector3f rotation = new Vector3f(0.0F, (float)(rotationTuple.yaw().getRadians() + Math.toRadians((double)this.spawnYawOffset)), 0.0F);
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SpawnNPCInteraction.class, SpawnNPCInteraction::new, SimpleBlockInteraction.CODEC).documentation("Spawns an NPC on the block that is being interacted with.")).append(new KeyedCodec("EntityId", Codec.STRING), (spawnNPCInteraction, s) -> spawnNPCInteraction.entityId = s, (spawnNPCInteraction) -> spawnNPCInteraction.entityId).documentation("The ID of the entity asset to spawn.").addValidator(NPCRoleValidator.INSTANCE).add()).append(new KeyedCodec("WeightedEntityIds", new ArrayCodec(SpawnNPCInteraction.WeightedNPCSpawn.CODEC, (x$0) -> new WeightedNPCSpawn[x$0])), (spawnNPCInteraction, o) -> spawnNPCInteraction.weightedSpawns = o, (spawnNPCInteraction) -> spawnNPCInteraction.weightedSpawns).documentation("A weighted list of entity IDs from which an entity will be selected for spawning. Supersedes any provided EntityId.").add()).append(new KeyedCodec("SpawnOffset", Vector3d.CODEC), (spawnNPCInteraction, s) -> spawnNPCInteraction.spawnOffset.assign(s), (spawnNPCInteraction) -> spawnNPCInteraction.spawnOffset).documentation("The offset to apply to the spawn position of the NPC, relative to the block's rotation and center.").add()).append(new KeyedCodec("SpawnYawOffset", Codec.FLOAT), (spawnNPCInteraction, f) -> spawnNPCInteraction.spawnYawOffset = f, (spawnNPCInteraction) -> spawnNPCInteraction.spawnYawOffset).documentation("The yaw rotation offset in radians to apply to the NPC rotation, relative to the block's yaw.").add()).append(new KeyedCodec("SpawnChance", Codec.FLOAT), (spawnNPCInteraction, f) -> spawnNPCInteraction.spawnChance = f, (spawnNPCInteraction) -> spawnNPCInteraction.spawnChance).documentation("The chance of the NPC spawning when the interaction is triggered.").add()).afterDecode((interaction) -> {
         if (interaction.weightedSpawns != null && interaction.weightedSpawns.length > 0) {
            WeightedMap.Builder<String> mapBuilder = WeightedMap.<String>builder(ArrayUtil.EMPTY_STRING_ARRAY);
   ...
   protected static class WeightedNPCSpawn implements IWeightedElement {
      private static final BuilderCodec<WeightedNPCSpawn> CODEC;
      private String id;
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(WeightedNPCSpawn.class, WeightedNPCSpawn::new).append(new KeyedCodec("Id", Codec.STRING), (spawn, s) -> spawn.id = s, (spawn) -> spawn.id).documentation("The Role ID of the NPC to spawn.").addValidator(Validators.nonNull()).addValidator(NPCRoleValidator.INSTANCE).add()).append(new KeyedCodec("Weight", Codec.DOUBLE, true), (spawn, d) -> spawn.weight = d, (spawn) -> spawn.weight).documentation("The relative weight of this NPC (chance of being spawned is this value relative to the sum of all weights).").addValidator(Validators.nonNull()).addValidator(Validators.greaterThan(0.0)).add()).build();
      }
   }
}
