...
public class ContextualUseNPCInteraction extends SimpleInstantInteraction {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ContextualUseNPCInteraction.class, ContextualUseNPCInteraction::new, SimpleInstantInteraction.CODEC).documentation("Interacts with the target NPC passing in context for it to use.")).appendInherited(new KeyedCodec("Context", Codec.STRING), (interaction, s) -> interaction.context = s, (interaction) -> interaction.context, (interaction, parent) -> interaction.context = parent.context).documentation("The provided context for the use action.").addValidator(Validators.nonNull()).add()).build();
   }
}
