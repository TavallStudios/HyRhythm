...
public class ActionTriggerSpawnBeacon extends ActionBase {
   ...
   public boolean canExecute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      return super.canExecute(ref, role, sensorInfo, dt, store) && (this.targetSlot == -2147483648 || role.getMarkedEntitySupport().hasMarkedEntityInSlot(this.targetSlot));
   }
   ...
   public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      ...
      for(Ref<EntityStore> spawnBeaconRef : role.getPositionCache().getSpawnBeaconList()) {
         SpawnBeacon spawnBeaconComponent = (SpawnBeacon)store.getComponent(spawnBeaconRef, SpawnBeacon.getComponentType());
         ...
         if (spawnWrapper.getSpawnIndex() == this.beaconId) {
            ...
            FloodFillPositionSelector floodFillPositionSelectorComponent = (FloodFillPositionSelector)store.getComponent(spawnBeaconRef, FloodFillPositionSelector.getComponentType());
