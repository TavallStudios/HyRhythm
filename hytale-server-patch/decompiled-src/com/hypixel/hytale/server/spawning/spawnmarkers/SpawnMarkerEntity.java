...
public class SpawnMarkerEntity implements Component<EntityStore> {
   ...
   public boolean tickRespawnTimer(float dt) {
      return (this.respawnCounter -= (double)dt) <= 0.0;
   }
   ...
   public boolean tickTimeToDeactivation(float dt) {
      return (this.timeToDeactivation -= (double)dt) <= 0.0;
   }

   public boolean tickSpawnLostTimeout(float dt) {
      return (this.spawnLostTimeoutCounter -= (double)dt) <= 0.0;
   }
   ...
   public boolean spawnNPC(@Nonnull Ref<EntityStore> ref, @Nonnull SpawnMarker marker, @Nonnull Store<EntityStore> store) {
      ...
      if (configs == null) {
         ...
         SpawnMarker.SpawnConfiguration spawn = (SpawnMarker.SpawnConfiguration)configs.get((Random)ThreadLocalRandom.current());
         if (spawn == null) {
            ...
            if (roleName != null && !roleName.isEmpty()) {
               ...
               if (builderInfo == null) {
                  ...
                  if (role == null) {
                     ...
                     SpatialResource<Ref<EntityStore>, EntityStore> spatialResource = (SpatialResource)store.getResource(EntityModule.get().getPlayerSpatialResourceType());
                           ...
                           if (testResult != SpawnTestResult.TEST_OK) {
                              ...
                              if (this.spawnPosition.distanceSquaredTo(position) > marker.getMaxDropHeightSquared()) {
                                 ...
                                 TriConsumer<NPCEntity, Ref<EntityStore>, Store<EntityStore>> postSpawn = (_entity, _ref, _store) -> {
                                    SpawnMarkerReference spawnMarkerReference = (SpawnMarkerReference)_store.ensureAndGetComponent(_ref, SpawnMarkerReference.getComponentType());
                                    spawnMarkerReference.getReference().setEntity(ref, _store);
                                    spawnMarkerReference.refreshTimeoutCounter();
                                    WorldGenId worldGenIdComponent = (WorldGenId)_store.getComponent(ref, WorldGenId.getComponentType());
                                 ...
                                 Pair<Ref<EntityStore>, NPCEntity> npcPair = npcModule.spawnEntity(store, roleIndex, this.spawnPosition, rotation, (Model)null, postSpawn);
                                 if (npcPair == null) {
                                    ...
                                    Ref<EntityStore> flockReference = FlockPlugin.trySpawnFlock(npcRef, npcComponent, store, roleIndex, this.spawnPosition, rotation, spawn.getFlockDefinition(), postSpawn);
                                    EntityGroup group = flockReference == null ? null : (EntityGroup)store.getComponent(flockReference, EntityGroup.getComponentType());
                                    ...
                                    if (this.storedFlock != null) {
                                       ...
                                       if (group != null) {
                                          group.forEachMember((index, member, referenceArray) -> {
                                             ...
                                             reference.setEntity(member, store);
                                    ...
                                    SpawningPlugin.get().getLogger().at(Level.FINE).log("Marker %s spawned %s and set respawn to %s", uuid, npcComponent.getRoleName(), realtime ? this.respawnCounter : this.gameTimeRespawn);
                                    this.refreshTimeout();
                                    return true;
                                 }
                              }
               ...
               SpawningPlugin.get().getLogger().at(Level.FINE).log("Marker %s performed noop spawn and set repawn to %s", uuid, realtime ? this.respawnCounter : this.gameTimeRespawn);
               this.refreshTimeout();
               return true;
            }
         }
   ...
   private void fail(@Nonnull Ref<EntityStore> self, @Nonnull UUID uuid, @Nonnull String role, @Nonnull Vector3d position, @Nonnull Store<EntityStore> store, @Nonnull FailReason reason) {
   ...
   public Component<EntityStore> cloneSerializable() {
      SpawnMarkerEntity spawnMarker = new SpawnMarkerEntity((SpawningContext)null);
      spawnMarker.spawnMarkerId = this.spawnMarkerId;
      spawnMarker.respawnCounter = this.respawnCounter;
      spawnMarker.spawnCount = this.spawnCount;
      spawnMarker.gameTimeRespawn = this.gameTimeRespawn;
   ...
   public String toString() {
      String var10000 = this.spawnMarkerId;
      return "SpawnMarkerEntity{spawnMarkerId='" + var10000 + "', cachedMarker=" + String.valueOf(this.cachedMarker) + ", respawnCounter=" + this.respawnCounter + ", gameTimeRespawn=" + String.valueOf(this.gameTimeRespawn) + ", spawnAfter=" + String.valueOf(this.spawnAfter) + ", spawnCount=" + this.spawnCount + ", spawnLostTimeoutCounter=" + this.spawnLostTimeoutCounter + ", failedSpawns=" + this.failedSpawns + ", context=" + String.valueOf(this.context) + ", spawnPosition=" + String.valueOf(this.spawnPosition) + ", storedFlock=" + String.valueOf(this.storedFlock) + "} " + super.toString();
   }
   ...
   static {
      NPC_REFERENCES_CODEC = new ArrayCodec<InvalidatablePersistentRef>(InvalidatablePersistentRef.CODEC, (x$0) -> new InvalidatablePersistentRef[x$0]);
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SpawnMarkerEntity.class, SpawnMarkerEntity::new).addField(new KeyedCodec("SpawnMarker", Codec.STRING), (spawnMarkerEntity, s) -> spawnMarkerEntity.spawnMarkerId = s, (spawnMarkerEntity) -> spawnMarkerEntity.spawnMarkerId)).addField(new KeyedCodec("RespawnTime", Codec.DOUBLE), (spawnMarkerEntity, d) -> spawnMarkerEntity.respawnCounter = d, (spawnMarkerEntity) -> spawnMarkerEntity.respawnCounter)).addField(new KeyedCodec("SpawnCount", Codec.INTEGER), (spawnMarkerEntity, i) -> spawnMarkerEntity.spawnCount = i, (spawnMarkerEntity) -> spawnMarkerEntity.spawnCount)).addField(new KeyedCodec("GameTimeRespawn", Codec.DURATION), (spawnMarkerEntity, duration) -> spawnMarkerEntity.gameTimeRespawn = duration, (spawnMarkerEntity) -> spawnMarkerEntity.gameTimeRespawn)).addField(new KeyedCodec("SpawnAfter", Codec.INSTANT), (spawnMarkerEntity, instant) -> spawnMarkerEntity.spawnAfter = instant, (spawnMarkerEntity) -> spawnMarkerEntity.spawnAfter)).addField(new KeyedCodec("NPCReferences", NPC_REFERENCES_CODEC), (spawnMarkerEntity, array) -> spawnMarkerEntity.npcReferences = array, (spawnMarkerEntity) -> spawnMarkerEntity.npcReferences)).addField(new KeyedCodec("PersistedFlock", StoredFlock.CODEC), (spawnMarkerEntity, o) -> spawnMarkerEntity.storedFlock = o, (spawnMarkerEntity) -> spawnMarkerEntity.storedFlock)).addField(new KeyedCodec("SpawnPosition", Vector3d.CODEC), (spawnMarkerEntity, v) -> spawnMarkerEntity.spawnPosition.assign(v), (spawnMarkerEntity) -> spawnMarkerEntity.storedFlock == null ? null : spawnMarkerEntity.spawnPosition)).build();
   }
