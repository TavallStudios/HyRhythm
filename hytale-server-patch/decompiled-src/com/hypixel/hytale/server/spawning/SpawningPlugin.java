...
public class SpawningPlugin extends JavaPlugin {
   ...
   public SpawningPlugin(@Nonnull JavaPluginInit init) {
      super(init);
      this.config = this.withConfig("SpawningModule", SpawningPlugin.NPCSpawningConfig.CODEC);
   }

   public void setup() {
      instance = this;
      this.getCommandRegistry().registerCommand(new SpawnCommand());
      this.getEventRegistry().register((Class)LoadedAssetsEvent.class, Environment.class, this::onEnvironmentChange);
   ...
   public void start() {
      NPCSpawningConfig config = this.config.get();
   ...
   public BeaconSpawnWrapper getBeaconSpawnWrapper(int configId) {
      return (BeaconSpawnWrapper)this.beaconSpawnManager.getSpawnWrapper(configId);
   }
            ...
            if (!beaconSpawn) {
               ...
               if (environment != -2147483648) {
                  ...
                  if (environmentSpawnData != null) {
                     ...
                     if (npcSpawnData != null && (double)npcSpawnData.getActual() > npcSpawnData.getExpected() * 1.25 + 4.0) {
   ...
   public static <T extends NPCSpawn> void validateSpawnsConfigurations(@Nonnull String type, @Nonnull Map<String, T> spawns, @Nonnull List<String> errors) {
      for(Map.Entry<String, T> spawn : spawns.entrySet()) {
         RoleSpawnParameters[] spawnParameters = ((NPCSpawn)spawn.getValue()).getNPCs();
   ...
   public static void validateSpawnMarkers(@Nonnull Map<String, SpawnMarker> markers, @Nonnull List<String> errors) {
      for(Map.Entry<String, SpawnMarker> marker : markers.entrySet()) {
         IWeightedMap<SpawnMarker.SpawnConfiguration> configs = ((SpawnMarker)marker.getValue()).getWeightedConfigurations();
         if (configs == null) {
            errors.add("Spawn marker " + (String)marker.getKey() + ": No configurations defined");
         } else {
            configs.forEach((config) -> {
               try {
                  ...
                  String var10001 = (String)marker.getKey();
                  errors.add("Spawn marker " + var10001 + ": " + e.getMessage());
               }
   ...
   protected void onSpawnMarkersChange(@Nonnull LoadedAssetsEvent<String, SpawnMarker, DefaultAssetMap<String, SpawnMarker>> event) {
      ...
      Universe.get().getWorlds().forEach((name, world) -> world.execute(() -> world.getEntityStore().getStore().forEachChunk(SpawnMarkerEntity.getComponentType(), (BiConsumer)((archetypeChunk, commandBuffer) -> {
               for(int index = 0; index < archetypeChunk.size(); ++index) {
                  SpawnMarkerEntity spawnMarkerEntity = (SpawnMarkerEntity)archetypeChunk.getComponent(index, SpawnMarkerEntity.getComponentType());
                  if (loadedAssets.containsKey(spawnMarkerEntity.getSpawnMarkerId())) {
                     Holder<EntityStore> holder = commandBuffer.removeEntity(archetypeChunk.getReferenceTo(index), EntityStore.REGISTRY.newHolder(), RemoveReason.UNLOAD);
   ...
   protected static void onSpawnMarkersRemove(@Nonnull RemovedAssetsEvent<String, SpawnMarker, DefaultAssetMap<String, SpawnMarker>> event) {
      ...
      Universe.get().getWorlds().forEach((name, world) -> world.execute(() -> world.getEntityStore().getStore().forEachChunk(SpawnMarkerEntity.getComponentType(), (BiConsumer)((archetypeChunk, commandBuffer) -> {
               for(int index = 0; index < archetypeChunk.size(); ++index) {
                  SpawnMarkerEntity spawnMarkerEntityComponent = (SpawnMarkerEntity)archetypeChunk.getComponent(index, SpawnMarkerEntity.getComponentType());
   ...
   private void onEnvironmentChange(@Nonnull LoadedAssetsEvent<String, Environment, IndexedLookupTableAssetMap<String, Environment>> event) {
      ...
      for(Map.Entry<String, Environment> entry : event.getLoadedAssets().entrySet()) {
         String environment = (String)entry.getKey();
         int index = environmentAssetMap.getIndex(environment);
         if (index == -2147483648) {
            throw new IllegalArgumentException("Unknown key! " + environment);
         }

         this.worldSpawnManager.updateSpawnParameters(index, (Environment)entry.getValue());
      }
      ...
      if (!NPCPlugin.get().getBuilderManager().isEmpty()) {
         ...
         Universe.get().getWorlds().forEach((name, world) -> world.execute(() -> {
               ...
               store.forEachChunk(LegacySpawnBeaconEntity.getComponentType(), (BiConsumer)((archetypeChunk, commandBuffer) -> {
                  for(int index = 0; index < archetypeChunk.size(); ++index) {
                     LegacySpawnBeaconEntity legacySpawnBeaconComponent = (LegacySpawnBeaconEntity)archetypeChunk.getComponent(index, LegacySpawnBeaconEntity.getComponentType());
   ...
   private void setUpWithAllRoles() {
      ...
      for(Map.Entry<String, WorldNPCSpawn> entry : assetMap.entrySet()) {
         WorldNPCSpawn value = (WorldNPCSpawn)entry.getValue();
      ...
      for(Map.Entry<String, BeaconNPCSpawn> entry : beaconSpawnAssetMap.entrySet()) {
         BeaconNPCSpawn value = (BeaconNPCSpawn)entry.getValue();
         if (this.beaconSpawnManager.addSpawnWrapper(new BeaconSpawnWrapper(value))) {
            ++beaconSetupCount;
         }
   ...
   private void rebuildBeaconSpawnConfigurations(@Nullable IntSet changeSet) {
      if (changeSet != null && !changeSet.isEmpty()) {
         ...
         while(var3.hasNext()) {
            ...
            BeaconNPCSpawn spawn = (BeaconNPCSpawn)BeaconNPCSpawn.getAssetMap().getAssetOrDefault(configIndex, (JsonAssetWithMap)null);
   ...
   private void onModelAssetChange(@Nonnull LoadedAssetsEvent<String, ModelAsset, DefaultAssetMap<String, ModelAsset>> event) {
      if (this.spawnMarkerModel != null) {
         ...
         ModelAsset modelAsset = (ModelAsset)modelMap.get(this.spawnMarkerModel.getModelAssetId());
         if (modelAsset != null) {
            this.spawnMarkerModel = Model.createUnitScaleModel(modelAsset);
         }
      }
   ...
   public static class LegacySpawnSuppressorEntityMigration extends EntityModule.MigrationSystem {
      ...
      public void onEntityAdd(@Nonnull Holder<EntityStore> holder, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store) {
         UnknownComponents<EntityStore> unknownComponent = (UnknownComponents)holder.getComponent(this.unknownComponentsComponentType);

         assert unknownComponent != null;

         Map<String, BsonDocument> unknownComponents = unknownComponent.getUnknownComponents();
         BsonDocument spawnSuppressor = (BsonDocument)unknownComponents.remove("SpawnSuppressor");
         if (spawnSuppressor != null) {
            ...
            if (!archetype.contains(this.persistentModelComponentType)) {
               Model.ModelReference modelReference = (Model.ModelReference)Entity.MODEL.get(spawnSuppressor).get();
            ...
            if (!archetype.contains(this.nameplateComponentType)) {
               holder.addComponent(this.nameplateComponentType, new Nameplate((String)Entity.DISPLAY_NAME.get(spawnSuppressor).get()));
            }

            if (!archetype.contains(this.uuidComponentType)) {
               holder.addComponent(this.uuidComponentType, new UUIDComponent((UUID)Entity.UUID.get(spawnSuppressor).get()));
            }
   ...
   public static class NPCSpawningConfig {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(NPCSpawningConfig.class, NPCSpawningConfig::new).append(new KeyedCodec("SpawnBudgetFactor", Codec.DOUBLE), (o, i) -> o.spawnBudgetFactor = i, (o) -> o.spawnBudgetFactor).add()).append(new KeyedCodec("MaxActiveJobs", Codec.INTEGER), (o, i) -> o.maxActiveJobs = i, (o) -> o.maxActiveJobs).add()).append(new KeyedCodec("DefaultSpawnMarkerModel", Codec.STRING), (o, i) -> o.defaultMarkerModel = i, (o) -> o.defaultMarkerModel).add()).append(new KeyedCodec("LocalSpawnControllerJoinDelay", Codec.DOUBLE), (o, i) -> o.localSpawnControllerJoinDelay = i, (o) -> o.localSpawnControllerJoinDelay).add()).build();
      }
   }
}
