...
public class FloodFillPositionSelector implements Component<EntityStore> {
   ...
   private static final int TOO_HIGH = 2147483647;
   private static final int TOO_LOW = -2147483648;
   ...
   private final Int2ObjectMap<ObjectArrayList<WeightedPosition>> positionsByRole = new Int2ObjectOpenHashMap();
   ...
   public boolean tickCalculatePositionsAfter(float dt) {
      return (this.calculatePositionsAfter -= (double)dt) <= 0.0;
   }
   ...
   public boolean prepareSpawnContext(@Nonnull Vector3d playerPosition, int spawnsThisRound, int roleIndex, @Nonnull SpawningContext spawningContext, @Nonnull BeaconSpawnWrapper spawnWrapper) {
      ObjectArrayList<WeightedPosition> positions = (ObjectArrayList)this.positionsByRole.get(roleIndex);
      if (this.positionsByRole.isEmpty()) {
         ...
         for(int i = 0; i < positions.size(); ++i) {
            WeightedPosition entry = (WeightedPosition)positions.get(i);
         ...
         WeightedPosition[] sortBuffer = ((SortBufferProvider)sortBufferProvider.get()).getBuffer(size);
         ...
         if (sum == 0.0) {
            ...
            if (entry == null) {
               ...
               SpawnSuppressionController suppressionController = (SpawnSuppressionController)this.world.getEntityStore().getStore().getResource(SpawnSuppressionController.getResourceType());
               ...
               if ((suppressionEntry == null || !suppressionEntry.isSuppressingRoleAt(roleIndex, position.y)) && spawningContext.set(this.world, (double)position.x, (double)position.y, (double)position.z) && spawningContext.canSpawn() == SpawnTestResult.TEST_OK) {
                  return true;
               } else {
   ...
   public void buildPositionCache(@Nonnull Vector3d origin, @Nonnull FloodFillEntryPoolSimple pool) {
      ...
      if (this.chunk != null) {
         ...
         if (this.debug != FloodFillPositionSelector.Debug.DISABLED && (this.irregularCase || this.debug == FloodFillPositionSelector.Debug.ALL)) {
            ...
            HytaleLogger.Api var10000 = SpawningPlugin.get().getLogger().at(Level.WARNING);
            String var10001 = String.valueOf(origin);
            var10000.log("Spawn beacon at: " + var10001 + (this.irregularCase ? " is an irregular case" : ""));
         }
   ...
   private void floodFill(int worldX, int worldY, int worldZ, int setX, int setZ, @Nonnull FloodFillEntryPoolSimple pool) {
      if (this.chunk != null) {
         ...
         while(!this.floodFillQueue.isEmpty()) {
            int[] state = (int[])this.floodFillQueue.poll();
            worldX = state[0];
            worldY = state[1];
            worldZ = state[2];
            setX = state[3];
            ...
            if (setX >= 0 && setX < this.size && setZ >= 0 && setZ < this.size) {
               ...
               if (this.heightGrid[index] != -1) {
                  ...
                  if (block != 0 && ((BlockType)BlockType.getAssetMap().getAsset(block)).getMaterial() == BlockMaterial.Solid) {
                     while(block != 0) {
                        ++worldY;
                        block = this.chunk.getBlock(worldX, worldY, worldZ);
                        if (worldY > this.maxY) {
                           this.heightGrid[index] = 2147483647;
                           pool.deallocate(state);
                           continue label66;
                        }
                     }
                     ...
                     while(block == 0) {
                        ...
                        block = this.chunk.getBlock(worldX, worldY, worldZ);
                        if (worldY < this.minY) {
                           this.heightGrid[index] = -2147483648;
                           pool.deallocate(state);
                           continue label66;
                        }
                     }
   ...
   private void findPositions(int originX, int originZ) {
      ...
      do {
         resolution *= 2;
         segments = this.buildLowerResolutionMap((BitSet)this.resolutionMaps.get(resolution), this.size / resolution, (BitSet)this.resolutionMaps.get(resolution / 2), this.size / (resolution / 2));
      } while(resolution < 8 && segments / this.desiredPositionCount > 1);

      BitSet lowestResolutionMap = (BitSet)this.resolutionMaps.get(resolution);
      ...
      for(int roleIndex : this.roleIndexes) {
         ...
         if (spawnable != null && spawnable.isSpawnable() && spawnable instanceof ISpawnableWithModel && this.spawningContext.setSpawnable((ISpawnableWithModel)spawnable, true)) {
            ObjectArrayList<WeightedPosition> positionList = (ObjectArrayList)this.positionsByRole.get(roleIndex);
            ...
            if (this.debug != FloodFillPositionSelector.Debug.DISABLED && ((double)positionCount < (double)this.desiredPositionCount * 0.3 || (double)positionCount > (double)this.desiredPositionCount * 5.0)) {
   ...
   private boolean canSpawn(int x, int y, int z, int roleIndex, @Nullable ChunkSuppressionEntry suppressionEntry) {
      if (!((BeaconNPCSpawn)this.spawnWrapper.getSpawn()).isOverrideSpawnSuppressors() && suppressionEntry != null && suppressionEntry.isSuppressingRoleAt(roleIndex, y)) {
         return false;
      } else if (this.spawningContext.set(this.world, (double)x, (double)y, (double)z) && this.spawningContext.canSpawn() == SpawnTestResult.TEST_OK && this.spawnWrapper.withinLightRange(this.spawningContext)) {
         if (this.spawningContext.ySpawn > (double)this.maxY) {
            ...
            if (spawnBlockSet == null && spawnFluidTag == -2147483648) {
               return true;
            } else if (spawnBlockSet != null && spawnBlockSet.contains(this.spawningContext.groundBlockId)) {
               return true;
            } else {
               return spawnFluidTag != -2147483648 && Fluid.getAssetMap().getIndexesForTag(spawnFluidTag).contains(this.spawningContext.groundFluidId);
            }
         }
      } else {
         return false;
   ...
   public static class SortBufferProvider {
      ...
      public WeightedPosition[] getBuffer(int size) {
         if (size <= this.buffer.length) {
            ...
            this.buffer = (WeightedPosition[])ObjectArrays.grow(this.buffer, size);
            return this.buffer;
         }
      }
   }
