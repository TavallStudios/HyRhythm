...
public class LocalSpawnControllerSystem extends TickingSystem<EntityStore> {
               ...
               for(int i = 0; i < possibleBeacons.size(); ++i) {
                  ...
                  if (possibleBeacon.spawnParametersMatch(store)) {
                     for(int j = 0; j < existingBeacons.size(); ++j) {
                        ...
                        LegacySpawnBeaconEntity existingBeaconComponent = (LegacySpawnBeaconEntity)store.getComponent(existingBeaconReference, this.spawnBeaconComponentType);

                        assert existingBeaconComponent != null;

                        TransformComponent existingBeaconTransformComponent = (TransformComponent)store.getComponent(existingBeaconReference, this.transformComponentype);

                        assert existingBeaconTransformComponent != null;

                        double existingY = existingBeaconTransformComponent.getPosition().getY();
                        if (!(existingY > (double)maxY) && !(existingY < (double)minY)) {
                     ...
                     for(int j = 0; j < pendingSpawns.size(); ++j) {
                        LegacySpawnBeaconEntity pending = (LegacySpawnBeaconEntity)pendingSpawns.get(j);
                        Ref<EntityStore> pendingReference = pending.getReference();
                        TransformComponent pendingTransformComponent = (TransformComponent)store.getComponent(pendingReference, TransformComponent.getComponentType());
                        ...
                        if (!(pendingY > (double)maxY) && !(pendingY < (double)minY)) {
                     ...
                     if (spawnLightLevelMatches(world, xPos, yPos, zPos, sunlightFactor, possibleBeacon, averageLightValues)) {
                        Pair<Ref<EntityStore>, LegacySpawnBeaconEntity> beaconEntityPair = LegacySpawnBeaconEntity.create(possibleBeacon, transformComponent.getPosition(), transformComponent.getRotation(), store);
                        Ref<EntityStore> beaconRef = (Ref)beaconEntityPair.first();
   ...
   private static boolean spawnLightLevelMatches(@Nonnull World world, int x, int y, int z, double sunlightFactor, @Nonnull BeaconSpawnWrapper wrapper, @Nonnull Object2ByteMap<LightType> averageValues) {
      ...
      if (lightRangePredicate.isTestLightValue()) {
         byte lightValue = getCachedAverageLightValue(LightType.Light, world, x, y, z, sunlightFactor, (_x, _y, _z, _chunk, _sunlightFactor) -> LightRangePredicate.calculateLightValue(_chunk, _x, _y, _z, _sunlightFactor), averageValues);
      ...
      if (lightRangePredicate.isTestSkyLightValue()) {
         byte lightValue = getCachedAverageLightValue(LightType.SkyLight, world, x, y, z, sunlightFactor, (_x, _y, _z, _chunk, _sunlightFactor) -> _chunk.getSkyLight(_x, _y, _z), averageValues);
      ...
      if (lightRangePredicate.isTestSunlightValue()) {
         byte lightValue = getCachedAverageLightValue(LightType.Sunlight, world, x, y, z, sunlightFactor, (_x, _y, _z, _chunk, _sunlightFactor) -> (byte)((int)((double)_chunk.getSkyLight(_x, _y, _z) * _sunlightFactor)), averageValues);
      ...
      if (lightRangePredicate.isTestRedLightValue()) {
         byte lightValue = getCachedAverageLightValue(LightType.RedLight, world, x, y, z, sunlightFactor, (_x, _y, _z, _chunk, _sunlightFactor) -> _chunk.getRedBlockLight(_x, _y, _z), averageValues);
      ...
      if (lightRangePredicate.isTestGreenLightValue()) {
         byte lightValue = getCachedAverageLightValue(LightType.GreenLight, world, x, y, z, sunlightFactor, (_x, _y, _z, _chunk, _sunlightFactor) -> _chunk.getGreenBlockLight(_x, _y, _z), averageValues);
      ...
      if (lightRangePredicate.isTestBlueLightValue()) {
         byte lightValue = getCachedAverageLightValue(LightType.BlueLight, world, x, y, z, sunlightFactor, (_x, _y, _z, _chunk, _sunlightFactor) -> _chunk.getBlueBlockLight(_x, _y, _z), averageValues);
   ...
   private static byte getCachedAverageLightValue(LightType lightType, @Nonnull World world, int x, int y, int z, double sunlightFactor, @Nonnull TriIntObjectDoubleToByteFunction<BlockChunk> valueCalculator, @Nonnull Object2ByteMap<LightType> averageValues) {
      ...
      if (cachedValue < 0) {
            ...
            for(int zOffset = z - 4; zOffset < z + 4; ++zOffset) {
               ...
               if (chunk != null) {
                  ...
                  for(int yOffset = y; yOffset < y + 4; ++yOffset) {
                     ...
                     if (blockId == 0 || ((BlockType)BlockType.getAssetMap().getAsset(blockId)).getMaterial() != BlockMaterial.Solid) {
                        ++counted;
         ...
         cachedValue = counted > 0 ? (byte)((int)((float)total / (float)counted)) : 0;
