...
public class SpawnSuppressorEntry {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SpawnSuppressorEntry.class, SpawnSuppressorEntry::new).append(new KeyedCodec("Position", Vector3d.CODEC), (entry, v) -> entry.position = v, (entry) -> entry.position).add()).append(new KeyedCodec("Suppression", Codec.STRING), (entry, s) -> entry.suppressionId = s, (entry) -> entry.suppressionId).add()).build();
   }
}
