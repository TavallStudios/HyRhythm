...
public class WorldSpawnTrackingSystem extends RefSystem<EntityStore> {
   ...
   public WorldSpawnTrackingSystem(@Nonnull ResourceType<EntityStore, WorldSpawnData> worldSpawnDataResourceType, @Nonnull ComponentType<ChunkStore, ChunkSpawnData> chunkSpawnDataComponentType, @Nonnull ComponentType<ChunkStore, ChunkSpawnedNPCData> chunkSpawnedNPCDataComponentType) {
      ...
      this.query = Query.<EntityStore>and(this.npcComponentType, this.transformComponentType);
   }
   ...
   public void onEntityAdded(@Nonnull Ref<EntityStore> ref, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      if (!isTracked) {
         ...
         WorldSpawnData worldSpawnData = (WorldSpawnData)store.getResource(this.worldSpawnDataResourceType);
         switch (reason) {
            case SPAWN:
               ...
               Vector3d position = ((TransformComponent)store.getComponent(ref, this.transformComponentType)).getPosition();
               ...
               if (count > 0.0) {
                  ...
                  for(Iterator<Ref<ChunkStore>> iterator = chunkOptions.iterator(); iterator.hasNext() && count > 0.0; count = trackNewNPC((Ref)iterator.next(), environmentIndex, count, this.chunkSpawnDataComponentType, this.chunkSpawnedNPCDataComponentType, chunkComponentStore)) {
                  }
                  ...
                  if (count > 0.0 && context.isEnabled()) {
                     ...
                     WorldNPCSpawnStat npcSpawnStat = (WorldNPCSpawnStat)worldEnvironmentSpawnData.getNpcStatMap().get(npcComponent.getRoleIndex());
         ...
         switch (reason) {
            case REMOVE:
               ...
               if (count > 0.0) {
                  ...
                  for(Iterator<Ref<ChunkStore>> iterator = chunkOptions.iterator(); iterator.hasNext() && count > 0.0; count = untrackRemovedNPC((Ref)iterator.next(), environmentIndex, count, this.chunkSpawnDataComponentType, this.chunkSpawnedNPCDataComponentType, chunkComponentStore)) {
                  }
   ...
   private static double trackNewNPC(@Nonnull Ref<ChunkStore> ref, int environmentIndex, double count, @Nonnull ComponentType<ChunkStore, ChunkSpawnData> chunkSpawnDataComponentType, @Nonnull ComponentType<ChunkStore, ChunkSpawnedNPCData> chunkSpawnedNPCDataComponentType, @Nonnull Store<ChunkStore> store) {
      ChunkSpawnData chunkSpawnData = (ChunkSpawnData)store.getComponent(ref, chunkSpawnDataComponentType);
      if (chunkSpawnData == null) {
         return count;
      } else {
         ChunkEnvironmentSpawnData spawnData = (ChunkEnvironmentSpawnData)chunkSpawnData.getChunkEnvironmentSpawnDataMap().get(environmentIndex);
         if (spawnData == null) {
            return count;
         } else {
            ChunkSpawnedNPCData chunkSpawnedNPCDataComponent = (ChunkSpawnedNPCData)store.getComponent(ref, chunkSpawnedNPCDataComponentType);
