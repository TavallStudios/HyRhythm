...
public class WorldSpawnJobSystems {
   ...
   private static Result trySpawn(@Nonnull ISpawnableWithModel spawnable, IntSet spawnBlockSet, int spawnFluidTag, @Nonnull SpawnJobData spawnJobData, @Nonnull WorldChunk worldChunk, @Nonnull ChunkEnvironmentSpawnData environmentSpawnData, @Nonnull WorldSpawnData worldSpawnData) {
      ...
      try {
         ...
         LOGGER.at(Level.WARNING).log("%s with spawnable=%s spwnCfg=%s X/Y/Z=%s/%s/%s", ((RuntimeException)e).getMessage(), getSpawnableName(spawnJobData.getRoleIndex()), spawnJobData.getSpawnConfig().getSpawn().getId(), spawningContext.xSpawn, spawningContext.ySpawn, spawningContext.zSpawn);
   ...
   private static Result spawn(@Nonnull SpawnJobData spawnJobData, @Nonnull WorldChunk worldChunk, @Nonnull WorldSpawnData worldSpawnData) {
      ...
      try {
         ...
         Pair<Ref<EntityStore>, NPCEntity> npcPair = npcModule.spawnEntity(store, roleIndex, position, rotation, spawningContext.getModel(), (_npc, _holder, _store) -> preAddToWorld(_npc, _holder, roleIndex, spawnJobData), (TriConsumer)null);
         if (npcPair == null) {
            LOGGER.at(Level.SEVERE).log("Spawn job %s: Failed to create %s: The spawned entity returned null", spawnJobData.getJobId(), npcModule.getName(roleIndex));
            rejectSpawn(spawnJobData.getRejectionMap(), SpawnRejection.OTHER);
            return endProbing(WorldSpawnJobSystems.Result.FAILED, spawnJobData, worldChunk, worldSpawnData);
         }

         NPCEntity npcComponent = (NPCEntity)npcPair.right();
         Ref<EntityStore> npcRef = (Ref)npcPair.left();
   ...
   static {
      TICKING_QUERY = Query.<ChunkStore>and(Query.not(ChunkStore.REGISTRY.getNonTickingComponentType()), QUERY);
   }
   ...
   public static class EntityRemoved extends HolderSystem<ChunkStore> {
      ...
      public void onEntityRemoved(@Nonnull Holder<ChunkStore> entityHolder, @Nonnull RemoveReason reason, @Nonnull Store<ChunkStore> store) {
         SpawnJobData spawnJobData = (SpawnJobData)entityHolder.getComponent(this.spawnJobDataComponentType);
         WorldChunk worldChunk = (WorldChunk)entityHolder.getComponent(this.worldChunkComponentType);
         WorldSpawnData worldSpawnData = (WorldSpawnData)((ChunkStore)store.getExternalData()).getWorld().getEntityStore().getStore().getResource(this.worldSpawnDataResourceType);
   ...
   public static class TickingState extends RefChangeSystem<ChunkStore, NonTicking<ChunkStore>> {
      ...
      public void onComponentAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull NonTicking<ChunkStore> component, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         SpawnJobData spawnJobData = (SpawnJobData)store.getComponent(ref, this.spawnJobDataComponentType);
         WorldChunk worldChunk = (WorldChunk)store.getComponent(ref, this.worldChunkComponentType);
         WorldSpawnData worldSpawnData = (WorldSpawnData)((ChunkStore)store.getExternalData()).getWorld().getEntityStore().getStore().getResource(this.worldSpawnDataResourceType);
         WorldSpawnJobSystems.endProbing(WorldSpawnJobSystems.Result.FAILED, spawnJobData, worldChunk, worldSpawnData);
