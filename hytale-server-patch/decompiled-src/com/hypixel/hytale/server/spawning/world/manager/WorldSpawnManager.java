...
public class WorldSpawnManager extends SpawnManager<WorldSpawnWrapper, WorldNPCSpawn> {
   protected final Int2ObjectConcurrentHashMap<EnvironmentSpawnParameters> environmentSpawnParametersMap = new Int2ObjectConcurrentHashMap<EnvironmentSpawnParameters>();
   ...
   public WorldSpawnManager() {
      this.npcEnvCombinations.defaultReturnValue(-2147483648);
   }
   ...
   public boolean addSpawnWrapper(@Nonnull WorldSpawnWrapper spawnWrapper) {
      ...
      while(var7.hasNext()) {
         ...
         IntIterator var9 = environments.iterator();

         while(var9.hasNext()) {
            Integer environmentIndex = (Integer)var9.next();
            if (this.haveCombination(npcIndex, environmentIndex)) {
               SpawningPlugin.get().getLogger().at(Level.SEVERE).log("Spawning Configuration %s can't be utilised: Combination NPC %s with Environment %s defined before in configuration %s", spawn.getId(), NPCPlugin.get().getName(npcIndex), ((Environment)assetMap.getAsset(environmentIndex)).getId(), this.getCombination(npcIndex, environmentIndex));
               return false;
            }
         }
      }

      EnvironmentSpawnParameters environmentSpawnParameters;
      for(IntIterator var13 = environments.iterator(); var13.hasNext(); environmentSpawnParameters.getSpawnWrappers().add(spawnWrapper)) {
         Integer environmentIndex = (Integer)var13.next();
         environmentSpawnParameters = this.environmentSpawnParametersMap.get(environmentIndex);
      ...
      for(World world : Universe.get().getWorlds().values()) {
         ...
         while(var20.hasNext()) {
            Int2ObjectMap.Entry<RoleSpawnParameters> roleSpawnParametersEntry = (Int2ObjectMap.Entry)var20.next();
   ...
   public void rebuildConfigurations(@Nullable IntSet changeSet) {
      if (changeSet != null && !changeSet.isEmpty()) {
         ...
         while(var3.hasNext()) {
            ...
            WorldNPCSpawn spawn = (WorldNPCSpawn)WorldNPCSpawn.getAssetMap().getAssetOrDefault(configIndex, (JsonAssetWithMap)null);
   ...
   public static void trackNPCs(@Nonnull IntSet spawnConfigs) {
      Universe.get().getWorlds().forEach((name, world) -> world.execute(() -> {
            ...
            store.forEachChunk(NPCEntity.getComponentType(), (BiConsumer)((archetypeChunk, commandBuffer) -> {
   ...
   private static void onRoleRemoved(@Nonnull World world, int roleIndex, @Nonnull IntSet environments) {
      world.execute(() -> {
         world.getChunkStore().getStore().forEachEntityParallel(SpawnJobData.getComponentType(), (index, chunk, commandBuffer) -> {
            SpawnJobData spawnJobData = (SpawnJobData)chunk.getComponent(index, SpawnJobData.getComponentType());
   ...
   private String getCombination(int npc, int environment) {
      return ((WorldNPCSpawn)WorldNPCSpawn.getAssetMap().getAsset(this.npcEnvCombinations.get(combinedIndex(npc, environment)))).getId();
   }
}
