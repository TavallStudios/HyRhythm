...
public class ChunkSpawningSystems {
               ...
               if (worldEnvironmentSpawnData != null) {
                  ...
                  if (!chunkEnvironmentSpawnData.wasProcessedAsUnspawnable()) {
                     ...
                     if (worldEnvironmentSpawnData.getSegmentCount() < 0) {
                        Environment environment = (Environment)Environment.getAssetMap().getAsset(worldEnvironmentSpawnData.getEnvironmentIndex());
                        String environmentName = environment != null ? environment.getId() : null;
                        LOGGER.at(Level.SEVERE).log("Block count for environment %s dropped below 0 to %s", environmentName, worldEnvironmentSpawnData.getSegmentCount());
                     }
   ...
   protected static boolean processStartedChunk(@Nonnull Ref<ChunkStore> ref, @Nonnull Store<ChunkStore> store, @Nonnull WorldChunk worldChunk, @Nonnull WorldSpawnData worldSpawnData, @Nonnull ComponentType<ChunkStore, ChunkSpawnData> chunkSpawnDataComponentType, @Nonnull ComponentType<ChunkStore, ChunkSpawnedNPCData> chunkSpawnedNPCDataComponentType, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
      ...
      if (chunkSpawnData.isStarted()) {
         ...
         while(iterator.hasNext()) {
            Int2ObjectMap.Entry<ChunkEnvironmentSpawnData> entry = (Int2ObjectMap.Entry)iterator.next();
            ((ChunkEnvironmentSpawnData)entry.getValue()).init(entry.getIntKey(), worldChunk);
         }
