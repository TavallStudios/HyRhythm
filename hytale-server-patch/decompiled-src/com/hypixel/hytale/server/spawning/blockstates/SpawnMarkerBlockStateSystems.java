...
public class SpawnMarkerBlockStateSystems {
   ...
   private static void createMarker(@Nonnull Ref<ChunkStore> ref, @Nonnull SpawnMarkerBlockState state, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
      StateData data = state.getBlockType().getState();
      if (data instanceof SpawnMarkerBlockState.Data stateData) {
         SpawnMarker marker = (SpawnMarker)SpawnMarker.getAssetMap().getAsset(stateData.getSpawnMarker());
         if (marker == null) {
   ...
   public static class TickHeartbeat extends EntityTickingSystem<ChunkStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         SpawnMarkerBlockState state = (SpawnMarkerBlockState)archetypeChunk.getComponent(index, this.componentType);
         if (state.getSpawnMarkerReference() == null) {
            Ref<ChunkStore> ref = archetypeChunk.getReferenceTo(index);
            SpawnMarkerBlockStateSystems.createMarker(ref, state, ((ChunkStore)store.getExternalData()).getWorld().getEntityStore().getStore(), commandBuffer);
         }

         if (state.getSpawnMarkerReference().getEntity(((ChunkStore)store.getExternalData()).getWorld().getEntityStore().getStore()) != null) {
   ...
   public static class SpawnMarkerAddedFromExternal extends RefSystem<EntityStore> {
      ...
      public SpawnMarkerAddedFromExternal(ComponentType<EntityStore, SpawnMarkerBlockReference> componentType) {
         this.query = Query.<EntityStore>and(componentType, Query.or(FromWorldGen.getComponentType(), FromPrefab.getComponentType()));
      }
   ...
   public static class SpawnMarkerTickHeartbeat extends EntityTickingSystem<EntityStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         SpawnMarkerBlockReference marker = (SpawnMarkerBlockReference)archetypeChunk.getComponent(index, this.componentType);
         Vector3i pos = marker.getBlockPosition();
         WorldChunk chunk = ((EntityStore)store.getExternalData()).getWorld().getChunkIfInMemory(ChunkUtil.indexChunkFromBlock(pos.x, pos.z));
         if (chunk != null) {
