...
public class SpawnPopulateCommand extends AbstractWorldCommand {
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      int environmentIndex = this.environmentArg.provided(context) ? Environment.getAssetMap().getIndex(((Environment)this.environmentArg.get(context)).getId()) : -2147483648;
      store.forEachEntityParallel(NPCEntity.getComponentType(), (index, archetypeChunk, commandBuffer) -> {
         ...
         if (npcEnvironment >= 0 && (environmentIndex == -2147483648 || environmentIndex == npcEnvironment)) {
