...
public class BeaconSpawnManager extends SpawnManager<BeaconSpawnWrapper, BeaconNPCSpawn> {
   @Nonnull
   private final Int2ObjectConcurrentHashMap<List<BeaconSpawnWrapper>> wrappersByEnvironment = new Int2ObjectConcurrentHashMap<List<BeaconSpawnWrapper>>();
   ...
   public BeaconSpawnWrapper removeSpawnWrapper(int spawnConfigurationIndex) {
      ...
      if (wrapper == null) {
         ...
         IntSet environmentIds = ((BeaconNPCSpawn)wrapper.getSpawn()).getEnvironmentIds();
         environmentIds.forEach((environment) -> ((List)this.wrappersByEnvironment.get(environment)).remove(wrapper));
         return wrapper;
      }
   }

   public boolean addSpawnWrapper(@Nonnull BeaconSpawnWrapper spawnWrapper) {
      IntSet environmentIds = ((BeaconNPCSpawn)spawnWrapper.getSpawn()).getEnvironmentIds();
      environmentIds.forEach((environment) -> ((List)this.wrappersByEnvironment.computeIfAbsent(environment, (key) -> new ObjectArrayList())).add(spawnWrapper));
