...
public class BeaconSpawnController extends SpawnController<NPCBeaconSpawnJob> {
   ...
   public NPCBeaconSpawnJob createRandomSpawnJob(@Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      LegacySpawnBeaconEntity legacySpawnBeaconComponent = (LegacySpawnBeaconEntity)componentAccessor.getComponent(this.ownerRef, LegacySpawnBeaconEntity.getComponentType());
         ...
         if (spawn == null) {
            ...
            if (roleIndex >= 0 && !this.unspawnableRoles.contains(roleIndex)) {
               ...
               if (this.activeJobs.size() < this.getMaxActiveJobs() && this.nextPlayerIndex < this.playersInRegion.size() && predictedTotal < this.currentScaledMaxTotalSpawns) {
                  job = this.idleJobs.isEmpty() ? new NPCBeaconSpawnJob() : (NPCBeaconSpawnJob)this.idleJobs.pop();
                  job.beginProbing((PlayerRef)this.playersInRegion.get(this.nextPlayerIndex++), this.currentScaledMaxConcurrentSpawns, roleIndex, spawn.getFlockDefinition());
   ...
   public void notifySpawnedEntityExists(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (context.isEnabled()) {
         UUIDComponent ownerUuidComponent = (UUIDComponent)componentAccessor.getComponent(this.ownerRef, UUIDComponent.getComponentType());
