...
public class ConcurrentSizedTimeoutCache<K, V> implements Cache<K, V> {
   ...
   public V get(K key) {
      if (this.future.isCancelled()) {
         ...
         return (V)this.buckets[hash & this.bucketMask].compute(key, this.computeKey, this.computeValue, this.destroyer);
      }
   }
   ...
   private static class Bucket<K, V> {
      ...
      public V compute(@Nonnull K key, @Nonnull Function<K, K> computeKey, @Nonnull Function<K, V> computeValue, @Nonnull BiConsumer<K, V> destroyer) {
         ...
         try {
            ...
            if (entry != null) {
               Object var10 = entry.markAndGet(timestamp);
               return (V)var10;
            }
         } finally {
         ...
         Object newKey = computeKey.apply(key);
         V newValue = (V)computeValue.apply(key);
         Object resultValue = newValue;
         long writeStamp = this.lock.writeLock();

         try {
            CacheEntry<K, V> newEntry = this.pool.isEmpty() ? new CacheEntry() : (CacheEntry)this.pool.poll();
            Objects.requireNonNull(newEntry, "CacheEntry pool returned null entry!");
            newEntry.key = (K)newKey;
            newEntry.value = newValue;
            newEntry.timestamp = timestamp;
            CacheEntry<K, V> currentEntry = (CacheEntry)this.map.putIfAbsent(newKey, newEntry);
