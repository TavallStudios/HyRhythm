...
public class CaveNodeShapeUtils {
   ...
   protected static int getCoverHeight(int lowest, int lowestPossible, int highest, int highestPossible, boolean heightLimited, @Nonnull CaveNodeType.CaveNodeCoverEntry cover, @Nonnull CaveNodeType.CaveNodeCoverEntry.Entry entry) {
      switch (cover.getType()) {
         case FLOOR:
            if (lowest != 2147483647 && lowestPossible == lowest) {
         ...
         case CEILING:
            if (heightLimited) {
               ...
               if (highest != -2147483648 && highestPossible == highest) {
      ...
      if (y >= 0 && y <= 319) {
         ...
         if (priority == 3) {
            ...
            if (supportsMap == null) {
               ...
               boolean var10000;
               switch (type) {
                  case FLOOR -> var10000 = supportsMap.containsKey(BlockFace.DOWN);
                  case CEILING -> var10000 = supportsMap.containsKey(BlockFace.UP);
