...
public class Cave {
   ...
   public void addNode(@Nonnull CaveNode element) {
      ...
      element.forEachChunk((chunk) -> {
         List<CaveNode> nodes = (List)this.rawChunkNodeMap.get(chunk);
         if (nodes == null) {
   ...
   private void compileNodeMap() {
      ...
      ObjectIterator var1 = this.rawChunkNodeMap.long2ObjectEntrySet().iterator();

      while(var1.hasNext()) {
         Long2ObjectMap.Entry<List<CaveNode>> entry = (Long2ObjectMap.Entry)var1.next();
         CaveNode[] array = (CaveNode[])((List)entry.getValue()).toArray((x$0) -> new CaveNode[x$0]);
   ...
   public String toString() {
      String var10000 = String.valueOf(this.rawChunkNodeMap);
      return "Cave{rawChunkNodeMap=" + var10000 + ", chunkNodeMap=" + String.valueOf(this.chunkNodeMap) + ", caveType=" + String.valueOf(this.caveType) + ", bounds=" + String.valueOf(this.bounds) + ", nodeCount=" + this.nodeCount + "}";
   }
}
