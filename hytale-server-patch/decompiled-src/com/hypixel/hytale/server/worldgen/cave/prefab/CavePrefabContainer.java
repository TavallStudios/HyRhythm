   ...
   public static class CavePrefabEntry {
      ...
      public static class CavePrefabConfig {
         ...
         public double getDisplacement(int seed, int x, int z, @Nonnull CaveNode caveNode) {
            return this.displacementSupplier.get(seed, x, z, (long)(seed + caveNode.getSeedOffset()));
         }

         public int getHeight(int seed, int x, int z, @Nonnull CaveNode caveNode) {
            int y = this.placement.getFunction().generate(seed, (double)x, (double)z, caveNode);
            return y == -1 ? -1 : (int)((double)y + this.getDisplacement(seed, x, z, caveNode));
         }
