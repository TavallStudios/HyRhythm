...
public class DistortedCaveNodeShape implements CaveNodeShape {
   ...
   public double getFloorPosition(int seed, double x, double z) {
      ...
      if (this.shape.isValidProjection(t)) {
         ...
         if (shapeHeight > 0.0) {
            int minY = this.getBounds().getLowBoundY();
            return (double)(this.getFloor(seed, x, z, centerY, shapeHeight, minY) - 1);
         }
      }
   ...
   public double getCeilingPosition(int seed, double x, double z) {
      ...
      if (this.shape.isValidProjection(t)) {
         ...
         if (shapeHeight > 0.0) {
            int maxY = this.getBounds().getHighBoundY();
            return (double)(this.getCeiling(seed, x, z, centerY, shapeHeight, maxY) + 1);
         }
      }
   ...
   public void populateChunk(int seed, @Nonnull ChunkGeneratorExecution execution, @Nonnull Cave cave, @Nonnull CaveNode node, @Nonnull Random random) {
      ...
      for(int x = minX; x <= maxX; ++x) {
         ...
         for(int z = minZ; z <= maxZ; ++z) {
            ...
            double t = this.shape.getProjection((double)x, (double)z);
            if (this.shape.isValidProjection(t)) {
               double centerY = this.shape.getYAt(t);
               double shapeHeight = this.shape.getHeightAtProjection(seed, (double)x, (double)z, t, centerY, caveType, this.distortion);
               if (!(shapeHeight <= 0.0)) {
                  int floorY = this.getFloor(seed, (double)x, (double)z, centerY, shapeHeight, minY);
                  int ceilingY = this.getCeiling(seed, (double)x, (double)z, centerY, shapeHeight, maximumY);
   ...
   public static class DistortedCaveNodeShapeGenerator implements CaveNodeShapeEnum.CaveNodeShapeGenerator {
      ...
      public CaveNodeShape generateCaveNodeShape(Random random, CaveType caveType, @Nullable CaveNode parentNode, @Nonnull CaveNodeType.CaveNodeChildEntry childEntry, @Nonnull Vector3d position, float yaw, float pitch) {
         ...
         Vector3d direction = getDirection((double)yaw, (double)pitch, length);
         ...
         DistortedShape shape = this.shapeFactory.create(origin, direction, length, startWidth, startHeight, midWidth, midHeight, endWidth, endHeight, this.interpolation);
      ...
      private static Vector3d getDirection(double yaw, double pitch, double length) {
         if (length == 0.0) {
            ...
            return (new Vector3d((double)(TrigMathUtil.sin(pitch) * TrigMathUtil.cos(yaw)), (double)TrigMathUtil.cos(pitch), (double)(TrigMathUtil.sin(pitch) * TrigMathUtil.sin(yaw)))).scale(length);
         }
      }
