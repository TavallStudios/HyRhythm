...
public class BlockPopulator {
   public static void populate(int seed, @Nonnull ChunkGeneratorExecution execution) {
      Random random = new FastRandom(HashUtil.hash((long)seed, (long)execution.getX(), (long)execution.getZ(), 5647422603192711886L));
   ...
   private static void generateBlockColumn(int seed, @Nonnull ChunkGeneratorExecution execution, int cx, int cz, @Nonnull Random random) {
      ...
      for(empty = true; y >= min; --y) {
         double threshold = (double)interpolator.getHeightThreshold(seed, x, z, y);
   ...
   private static void generateCovers(int seed, @Nonnull ChunkGeneratorExecution execution, int cx, int cz, int x, int z, @Nonnull Random random, @Nonnull Biome biome, @Nonnull IntList surfaceBlockList) {
      ...
      if (size != 0) {
         for(WaterContainer.Entry waterContainer : biome.getWaterContainer().getEntries()) {
            for(CoverContainer.CoverContainerEntry coverContainerEntry : coverContainer.getEntries()) {
               if (coverContainerEntry.isOnWater() && isMatchingCoverColumn(seed, coverContainerEntry, random, x, z)) {
                  ...
                  if (isMatchingCoverHeight(seed, coverContainerEntry, random, x, y, z) && isMatchingParentCover(execution, coverContainerEntry, cx, cz, y, waterContainer.getBlock(), waterContainer.getFluid())) {
         ...
         for(int i = 0; i < size; ++i) {
            ...
            for(CoverContainer.CoverContainerEntry coverContainerEntry : coverContainer.getEntries()) {
               if (!coverContainerEntry.isOnWater() && isMatchingParentCover(execution, coverContainerEntry, cx, cz, y, 0, 0) && isMatchingCoverColumn(seed, coverContainerEntry, random, x, z) && isMatchingCoverHeight(seed, coverContainerEntry, random, x, y, z)) {
                  CoverContainer.CoverContainerEntry.CoverContainerEntryPart coverEntry = coverContainerEntry.get(random);
                  execution.setBlock(cx, y + coverEntry.getOffset(), cz, (byte)3, coverEntry.getEntry());
                  execution.setFluid(cx, y + coverEntry.getOffset(), cz, (byte)3, coverEntry.getEntry().fluidId());
               }
            ...
            for(LayerContainer.DynamicLayer layer : layers.getDynamicLayers()) {
               ...
               if (entry != null) {
                  ...
                  for(NoiseBlockArray.Entry blockArrayEntry : blockArray.getEntries()) {
                     int repetitions = blockArrayEntry.getRepetitions(seed, (double)x, (double)z);
                     ...
                     for(int j = 0; j < repetitions; ++j) {
                        ...
                        execution.setFluid(cx, y, cz, (byte)2, blockArrayEntry.getBlockEntry().fluidId(), (int)environmentId);
                        --y;
                     }
                  }
               }
         ...
         for(LayerContainer.StaticLayer layer : layers.getStaticLayers()) {
            ...
            if (entry != null) {
               ...
               for(NoiseBlockArray.Entry blockEntry : blockEntries) {
                  int repetitions = blockEntry.getRepetitions(seed, (double)x, (double)z);
                  if (repetitions > 0) {
                     ...
                     for(int i = 0; i < repetitions; ++i) {
                        ...
                        if (currentBlock != 0) {
                           ...
                           execution.setFluid(cx, layerY, cz, (byte)2, block.fluidId(), (int)environmentId);
                        }
               ...
               if (blockEntries.length == 0 && environmentId != -2147483648) {
