...
public class PrefabPopulator {
   ...
   private int minPriority = 2147483647;
   @Nullable
   private Biome biome;
   @Nullable
   ...
   public PrefabPopulator() {
      ...
      this.prefabs = new ObjectArrayList();
   ...
   public void run(int seed, @Nonnull ChunkGeneratorExecution execution) {
      this.worldSeed = seed;
      this.minPriority = 2147483647;
      this.uniquePrefabs = (UniquePrefabContainer.UniquePrefabEntry[])Objects.requireNonNullElse(execution.getChunkGenerator().getUniquePrefabs(seed), EMPTY_UNIQUE_PREFABS);
   ...
   private void generatePrefabs(int seed, @Nonnull ChunkGeneratorExecution execution) {
      for(int i = 0; i < this.prefabs.size(); ++i) {
         if (!this.conflicts.get(i)) {
            Candidate prefab = (Candidate)this.prefabs.get(i);
            int x = prefab.x;
            int y = prefab.y;
            int z = prefab.z;
   ...
   private void collectBiome(@Nonnull Biome biome) {
      ...
      for(int i = 0; i < this.biomes.size(); ++i) {
         int id = ((Biome)this.biomes.get(i)).getId();
         if (insert.getId() == id) {
            return;
         }

         if (insert.getId() < id) {
            insert = (Biome)this.biomes.set(i, insert);
         }
      }
   ...
   private void collectPrefab(double px, double pz) {
      ...
      this.random.setSeed(HashUtil.hash(this.prefabSeed, (long)x, (long)z) * 1609272495L);
      ...
      if (isMatchingNoiseDensity(this.worldSeed, x, z, patternGenerator)) {
         if (!isWithinUniquePrefabExclusionRange(x, z, patternGenerator, this.uniquePrefabs)) {
            ...
            if (isMatchingBiome(this.biome, result)) {
               ...
               WorldGenPrefabSupplier supplier = (WorldGenPrefabSupplier)prefabs.get((Random)this.random);
               if (supplier != null) {
   ...
   private void collectConflicts() {
      for(int i = 0; i < this.prefabs.size(); ++i) {
         Candidate candidate = (Candidate)this.prefabs.get(i);
         ...
         if (candidate.priority > this.minPriority && !this.conflicts.get(i)) {
            for(int j = 0; j < this.prefabs.size(); ++j) {
               Candidate other = (Candidate)this.prefabs.get(j);
               if (candidate.priority > other.priority && intersects(minX, minY, minZ, maxX, maxY, maxZ, other.x + other.buffer.getMinX(other.rotation), other.y + other.buffer.getMinY(), other.z + other.buffer.getMinZ(other.rotation), other.x + other.buffer.getMaxX(other.rotation), other.y + other.buffer.getMaxY(), other.z + other.buffer.getMaxZ(other.rotation))) {
   ...
   private static boolean isWithinUniquePrefabExclusionRange(int x, int z, @Nonnull PrefabPatternGenerator generator, @Nonnull UniquePrefabContainer.UniquePrefabEntry[] uniquePrefabs) {
      long radius = (long)generator.getExclusionRadius();
      if (radius <= 0L) {
         return false;
      } else {
         ...
         for(UniquePrefabContainer.UniquePrefabEntry unique : uniquePrefabs) {
            if (priority < unique.getCategory().priority()) {
               long dx = (long)(x - unique.getPosition().getX());
               long dz = (long)(z - unique.getPosition().getZ());
   ...
   private static int getHeight(int seed, int x, int z, @Nonnull ChunkGeneratorExecution execution, @Nonnull Biome biome, @Nonnull PrefabPatternGenerator prefabPatternGenerator, Random random) {
      ...
      if (prefabPatternGenerator.isOnWater() && prefabPatternGenerator.isDeepSearch()) {
         ...
         for(WaterContainer.Entry waterContainer : biome.getWaterContainer().getEntries()) {
            ...
            if (max != -2147483648 && prefabPatternGenerator.getHeightCondition().eval(seed, x, z, max, random)) {
               height = max;
               break;
            }
         }
   ...
   private static void generatePrefabAt(int seed, int x, int z, int height, @Nonnull ChunkGeneratorExecution execution, @Nonnull WorldGenPrefabSupplier supplier, BlockMaskCondition configuration, PrefabRotation rotation, ICoordinateRndCondition heightCondition, int environmentId, boolean fitHeightmap, boolean submerge) {
      ...
      long externalSeed = HashUtil.hash((long)x, (long)z) * -1058827062L;
