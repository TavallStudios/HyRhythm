...
public class ChunkGenerator implements IBenchmarkableWorldGen, ValidatableWorldGen, MetricProvider, IWorldMapProvider {
   ...
   public ChunkGenerator(ZonePatternProvider zonePatternProvider, Path dataFolder) {
      ...
      this.executor = new ChunkThreadPoolExecutor(POOL_SIZE, POOL_SIZE, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue(), new ChunkWorkerThreadFactory(this, "ChunkGenerator-%d-Worker-%d"), this::onExecutorShutdown);
   ...
   public Transform[] getSpawnPoints(int seed) {
      return (Transform[])CompletableFuture.supplyAsync(() -> {
         ...
         Transform[] array = (Transform[])list.toArray((x$0) -> new Transform[x$0]);
         Random random = getResource().random;
         random.setSeed((long)seed * 1494360372L);
         ArrayUtli.shuffleArray((Object[])array, random);
         return array;
      }, this.executor).join();
   }
   ...
   public CompletableFuture<GeneratedChunk> generate(int seed, long index, int x, int z, @Nullable LongPredicate stillNeeded) {
      return CompletableFuture.supplyAsync(() -> {
         if (stillNeeded != null && !stillNeeded.test(index)) {
            ...
            GeneratedChunk generatedChunk = (GeneratedChunk)this.generatedChunkSupplier.get();
   ...
   public ZoneBiomeResult generateZoneBiomeResultAt(int seed, int x, int z, @Nonnull ZoneBiomeResult result) {
      ...
      ZoneGeneratorResult zoneResult = zonePatternGenerator.generate(seed, (double)x, (double)z, tempZoneResult != null ? tempZoneResult : new ZoneGeneratorResult());
      Biome biome = zoneResult.getZone().biomePatternGenerator().generateBiomeAt(zoneResult, seed, x, z);
      double heightThresholdContext = biome.getHeightmapInterpreter().getContext(seed, (double)x, (double)z);
      double heightmapNoise = biome.getHeightmapNoise().get(seed, (double)x, (double)z);
   ...
   public static float generateInterpolatedThreshold(int seed, int x, int z, int y, @Nonnull InterpolatedBiomeCountList biomeCounts) {
      ...
      for(int size = biomes.size(); i < size; ++i) {
         ...
         threshold += r.biome.getHeightmapInterpreter().getThreshold(seed, (double)x, (double)z, y, r.heightThresholdContext) * (float)r.count;
         counter += r.count;
      }
   ...
   public int generateHeight(int seed, int x, int z) {
      ...
      for(int y = this.generateHighestThresholdDependent(biomeCounts); y > 0; --y) {
         ...
         if ((double)threshold > heightNoise || (double)threshold == 1.0) {
            return y;
         }
      }
   ...
   public UniquePrefabContainer.UniquePrefabEntry[] generateUniquePrefabs(int seed) {
      ...
      return (UniquePrefabContainer.UniquePrefabEntry[])entries.toArray((x$0) -> new UniquePrefabContainer.UniquePrefabEntry[x$0]);
   }
   ...
   public static ChunkGeneratorResource getResource() {
      return (ChunkGeneratorResource)THREAD_LOCAL.get();
   }
   ...
   public String toString(boolean timings, boolean zonePatternGenerator) {
      String var10000 = String.valueOf(timings ? this.timings : "-hidden-");
      return "ChunkGenerator{timings=" + var10000 + ", zonePatternProvider=" + String.valueOf(zonePatternGenerator ? this.zonePatternProvider : "-hidden-") + ", generatorCache=" + String.valueOf(this.generatorCache) + ", caveGeneratorCache=" + String.valueOf(this.caveGeneratorCache) + ", uniquePrefabCache=" + String.valueOf(this.uniquePrefabCache) + "}";
   }
