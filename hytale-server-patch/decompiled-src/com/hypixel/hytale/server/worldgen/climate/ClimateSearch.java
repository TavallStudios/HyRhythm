...
public class ClimateSearch {
   ...
   public static CompletableFuture<Result> search(int seed, int cx, int cy, int startRadius, int searchRadius, @Nonnull Rule rule, @Nonnull ClimateNoise noise, @Nonnull ClimateGraph graph) {
      return CompletableFuture.supplyAsync(() -> {
         ...
         FastRandom rng = new FastRandom(HashUtil.hash((long)seed, 1659788403585L));
         long start_ms = System.currentTimeMillis();

         for(int r = startRadius; r <= radius; r += 100) {
            int steps = (int)Math.floor(6.283185307179586 * (double)r / 100.0);
            double inc = 6.283185307179586 / (double)steps;
            double off = 6.283185307179586 * rng.nextDouble();

            for(int i = 0; i < steps; ++i) {
               double t = (double)i * inc + off;
               int x = cx + (int)(Math.cos(t) * (double)r);
               int y = cy + (int)(Math.sin(t) * (double)r);
   ...
   private static double collect(int seed, int x, int y, ClimateNoise noise, ClimateGraph graph, Rule rule) {
      double continent = noise.continent.get(seed, (double)x, (double)y);
      if (!rule.continent.test(continent)) {
         return 0.0;
      } else {
         double temperature = noise.temperature.get(seed, (double)x, (double)y);
         if (!rule.temperature.test(temperature)) {
            return 0.0;
         } else {
            double intensity = noise.intensity.get(seed, (double)x, (double)y);
   ...
   public static record Result(Vector2i position, double score, long time_ms) {
