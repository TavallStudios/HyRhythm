...
public class GeneratorChunkWorldMap extends ChunkWorldMap {
   ...
   public CompletableFuture<Map<String, MapMarker>> generatePointsOfInterest(@Nonnull World world) {
      ...
      return uniquePrefabs != null && uniquePrefabs.length != 0 ? CompletableFuture.supplyAsync(() -> {
   ...
   public WorldMapSettings getWorldMapSettings() {
      ...
      for(Zone zone : this.generator.getZonePatternProvider().getZones()) {
         for(Biome biome : zone.biomePatternGenerator().getBiomes()) {
            ...
            BiomeData old = (BiomeData)biomeDataMap.putIfAbsent((short)biomeId, biomeData);
            if (old != null) {
               String var10002 = String.valueOf(biomeData);
               throw new IllegalArgumentException("Multiple biomes with the same ID! New: " + var10002 + ", Old: " + String.valueOf(old));
            }
         }
      }
      ...
      return new WorldMapSettings((Box2D)null, 3.0F, 2.0F, 3, 32, settingsPacket);
   }
}
