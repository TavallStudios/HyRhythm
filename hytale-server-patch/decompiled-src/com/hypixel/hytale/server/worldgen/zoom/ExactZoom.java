...
public class ExactZoom {
   ...
   public boolean inBounds(double x, double y) {
      ...
      if (!(x < 0.0) && !(y < 0.0)) {
         x /= this.zoomX;
         y /= this.zoomY;
         return (double)(this.source.getWidth() - 1) >= x && (double)(this.source.getHeight() - 1) >= y;
      } else {
         return false;
      }
   }
   ...
   public Zone.UniqueCandidate[] generateUniqueZoneCandidates(Zone.UniqueEntry[] entries, int maxPositions) {
      ...
      for(int i = 0; i < entries.length; ++i) {
         ...
         if (size <= maxPositions) {
            candidates[i] = new Zone.UniqueCandidate(entry, (Vector2i[])positions.toArray((x$0) -> new Vector2i[x$0]));
         } else {
   ...
   public String toString() {
      ...
      return "ExactZoom{source=" + var10000 + ", distanceProvider=" + String.valueOf(this.distanceProvider) + ", zoomX=" + this.zoomX + ", zoomY=" + this.zoomY + ", offsetX=" + this.offsetX + ", offsetY=" + this.offsetY + "}";
   }
}
