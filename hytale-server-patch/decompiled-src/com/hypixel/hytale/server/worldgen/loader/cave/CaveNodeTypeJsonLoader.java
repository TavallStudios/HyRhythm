...
public class CaveNodeTypeJsonLoader extends JsonLoader<SeedStringResource, CaveNodeType> {
   ...
   public CaveNodeType load() {
      CaveNodeType caveNodeType = new CaveNodeType(this.name, this.loadPrefabs(), this.loadFillings(), this.loadShapeGenerator(), this.loadHeightCondition(), this.loadChildCountBounds(), this.loadCovers(), this.loadPriority(), this.loadEnvironment());
   ...
   protected IWeightedMap<BlockFluidEntry> loadFillings() {
      WeightedMap.Builder<BlockFluidEntry> builder = WeightedMap.<BlockFluidEntry>builder(BlockFluidEntry.EMPTY_ARRAY);
      JsonElement fillingElement = this.get("Filling");
      if (fillingElement != null && !fillingElement.isJsonNull()) {
         if (fillingElement.isJsonObject()) {
   ...
   protected CaveNodeShapeEnum.CaveNodeShapeGenerator loadShapeGenerator() {
      if (this.has("Type")) {
         ...
         try {
            Object var10000;
            switch (CaveNodeShapeEnum.valueOf(typeString.toUpperCase())) {
               case PIPE -> var10000 = (new PipeCaveNodeShapeGeneratorJsonLoader(this.seed, this.dataFolder, this.json)).load();
               case CYLINDER -> var10000 = (new CylinderCaveNodeShapeGeneratorJsonLoader(this.seed, this.dataFolder, this.json)).load();
               case PREFAB -> var10000 = (new PrefabCaveNodeShapeGeneratorJsonLoader(this.seed, this.dataFolder, this.json)).load();
               case ELLIPSOID -> var10000 = (new EllipsoidCaveNodeShapeGeneratorJsonLoader(this.seed, this.dataFolder, this.json)).load();
               case EMPTY_LINE -> var10000 = (new EmptyLineCaveNodeShapeGeneratorJsonLoader(this.seed, this.dataFolder, this.json)).load();
               case DISTORTED -> var10000 = (new DistortedCaveNodeShapeGeneratorJsonLoader(this.seed, this.dataFolder, this.json)).load();
               default -> throw new MatchException((String)null, (Throwable)null);
            }

            return (CaveNodeShapeEnum.CaveNodeShapeGenerator)var10000;
         } catch (Throwable e) {
            throw new Error(String.format("Could not find Shape by the name %s in Json: %s", typeString, typeElement), e);
         }
      } else {
