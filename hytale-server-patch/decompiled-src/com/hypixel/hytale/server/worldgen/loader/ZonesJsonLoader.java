...
public class ZonesJsonLoader extends Loader<SeedStringResource, Zone[]> {
   ...
   public Zone[] load() {
      ...
      for(Map.Entry<String, ZoneFileContext> zoneEntry : zoneRegistry) {
         ZoneFileContext zoneContext = (ZoneFileContext)zoneEntry.getValue();

         try {
            JsonObject zoneJson = (JsonObject)FileIO.load((Path)zoneContext.getPath().resolve("Zone.json"), JsonLoader.JSON_OBJ_LOADER);
