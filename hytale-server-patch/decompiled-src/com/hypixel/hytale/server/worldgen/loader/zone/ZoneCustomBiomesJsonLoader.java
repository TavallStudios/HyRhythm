...
public class ZoneCustomBiomesJsonLoader extends JsonLoader<SeedStringResource, CustomBiome[]> {
   ...
   public CustomBiome[] load() {
      ...
      for(Map.Entry<String, BiomeFileContext> biomeEntry : this.zoneContext.getCustomBiomes()) {
         BiomeFileContext biomeContext = (BiomeFileContext)biomeEntry.getValue();

         try {
            JsonElement biomeJson = (JsonElement)FileIO.load((Path)biomeContext.getPath(), JsonLoader.JSON_OBJ_LOADER);
