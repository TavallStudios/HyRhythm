...
public class ClimateTypeJsonLoader<K extends SeedResource> extends JsonLoader<K, ClimateType> {
   ...
   protected String loadName() {
      return this.mustGetString("Name", (String)null);
   }

   @Nonnull
   protected ClimateColor loadIslandColor(@Nonnull ClimateColor color) {
      return this.has("Island") ? (new ClimateColorJsonLoader(this.seed, this.dataFolder, this.get("Island"), this.parent != null ? this.parent.island : null)).load() : color;
   }
   ...
   protected ClimatePoint loadPoint(int index, JsonElement pointsJson) {
      try {
         ...
         throw error(t, "Invalid climate point at index: %d", new Object[]{index});
      }
   }
