...
public class ClimateNoiseJsonLoader<K extends SeedResource> extends JsonLoader<K, ClimateNoise> {
   ...
   protected NoiseProperty loadContinentNoise() {
      return (new NoisePropertyJsonLoader(this.seed, this.dataFolder, this.mustGetObject("Continent", (JsonObject)null))).load();
   }

   @Nonnull
   protected NoiseProperty loadTemperatureNoise() {
      return (new NoisePropertyJsonLoader(this.seed, this.dataFolder, this.mustGetObject("Temperature", (JsonObject)null))).load();
   }

   @Nonnull
   protected NoiseProperty loadIntensityNoise() {
      return (new NoisePropertyJsonLoader(this.seed, this.dataFolder, this.mustGetObject("Intensity", (JsonObject)null))).load();
   }

   @Nonnull
   protected ClimateNoise.Thresholds loadThresholds() {
      return (new ContinentThresholdsJsonLoader(this.seed, this.dataFolder, this.mustGetObject("Thresholds", (JsonObject)null))).load();
   }
