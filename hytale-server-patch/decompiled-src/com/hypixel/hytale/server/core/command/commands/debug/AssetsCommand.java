      ...
      protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext context) {
         return CompletableFuture.runAsync(() -> {
            for(Map.Entry<Class<? extends JsonAssetWithMap>, AssetStore<?, ?, ?>> e : AssetRegistry.getStoreMap().entrySet()) {
               ...
               context.sendMessage(Message.translation("server.commands.assets.longest.result").param("type", ((Class)e.getKey()).getSimpleName()).param("assetName", longestName).param("length", longestName.length()));
            }
