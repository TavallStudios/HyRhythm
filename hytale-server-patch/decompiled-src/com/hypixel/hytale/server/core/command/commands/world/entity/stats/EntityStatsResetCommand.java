...
public class EntityStatsResetCommand extends AbstractTargetEntityCommand {
   ...
   public EntityStatsResetCommand() {
      ...
      this.entityStatNameArg = this.withRequiredArg("statName", "server.commands.entity.stats.reset.statName.desc", ArgTypes.STRING);
   }
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull ObjectList<Ref<EntityStore>> entities, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      String entityStat = (String)this.entityStatNameArg.get(context);
   ...
   public static void resetEntityStat(@Nonnull CommandContext context, @Nonnull List<Ref<EntityStore>> entities, @Nonnull String entityStat, @Nonnull Store<EntityStore> store) {
      ...
      if (entityStatIndex == -2147483648) {
         ...
         context.sendMessage(Message.translation("server.general.failed.didYouMean").param("choices", StringUtil.sortByFuzzyDistance(entityStat, EntityStatType.getAssetMap().getAssetMap().keySet(), CommandUtil.RECOMMEND_COUNT).toString()));
      } else {
         for(Ref<EntityStore> entity : entities) {
            EntityStatMap entityStatMap = (EntityStatMap)store.getComponent(entity, EntityStatsModule.get().getEntityStatMapComponentType());
