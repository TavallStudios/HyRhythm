...
public class EntityEffectCommand extends AbstractTargetEntityCommand {
   ...
   public EntityEffectCommand() {
      ...
      this.durationArg = (DefaultArg)this.withDefaultArg("duration", "server.commands.entity.effect.duration.desc", ArgTypes.FLOAT, 100.0F, "server.commands.entity.effect.duration.default").addValidator(Validators.greaterThan(0.0F));
   }
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull ObjectList<Ref<EntityStore>> entities, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      ...
      ObjectListIterator var7 = entities.iterator();

      while(var7.hasNext()) {
         Ref<EntityStore> entityRef = (Ref)var7.next();
         EffectControllerComponent effectControllerComponent = (EffectControllerComponent)store.getComponent(entityRef, EffectControllerComponent.getComponentType());
