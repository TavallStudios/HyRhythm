...
public class AuthPersistenceCommand extends CommandBase {
   ...
   static {
      MESSAGE_SINGLEPLAYER = Message.translation("server.commands.auth.persistence.singleplayer").color(Color.RED);
   }
   ...
   private static class SetPersistenceVariant extends CommandBase {
      ...
      SetPersistenceVariant() {
         ...
         this.typeArg = this.withRequiredArg("type", "server.commands.auth.persistence.type.desc", ArgTypes.STRING);
      }
      ...
      protected void executeSync(@Nonnull CommandContext context) {
         ...
         if (authManager.isSingleplayer()) {
            ...
            BuilderCodec<? extends AuthCredentialStoreProvider> codec = (BuilderCodec)AuthCredentialStoreProvider.CODEC.getCodecFor(typeName);
            if (codec == null) {
