...
public class CommandManager implements CommandOwner {
   ...
   public Map<String, Set<String>> createVirtualPermissionGroups() {
      ...
      for(AbstractCommand command : this.commandRegistration.values()) {
         for(Map.Entry<String, Set<String>> entry : command.getPermissionGroupsRecursive().entrySet()) {
            Set<String> permissionsForGroup = (Set)permissionsByGroup.computeIfAbsent((String)entry.getKey(), (k) -> new HashSet());
   ...
   public CommandRegistration register(@Nonnull AbstractCommand command) {
      ...
      if (name != null && !name.isEmpty()) {
         ...
         try {
            ...
            if (var7 instanceof GeneralCommandException) {
               GeneralCommandException generalException = (GeneralCommandException)var7;
            ...
            ((HytaleLogger.Api)HytaleLogger.getLogger().at(Level.SEVERE).withCause(var7)).log("Failed to register command: %s%s", command.getName(), errorMessage != null ? " - " + errorMessage : "");
            return null;
         }
         ...
         return new CommandRegistration(command, () -> true, () -> {
            AbstractCommand remove = (AbstractCommand)this.commandRegistration.remove(name);
            if (remove != null) {
   ...
   public CompletableFuture<Void> handleCommand(@Nonnull PlayerRef playerRef, @Nonnull String command) {
      ...
      if (ref == null) {
         ...
         return this.handleCommand((CommandSender)playerComponent, command);
      }
   }
   ...
   private void runCommand(@Nonnull CommandSender commandSender, @Nonnull String commandInput, @Nonnull AbstractCommand abstractCommand, @Nonnull CompletableFuture<Void> future) {
      try {
            ...
            commandFuture.whenComplete((aVoid, throwable) -> {
               if (throwable != null) {
                  if (!CompletableFutureUtil.isCanceled(throwable) && !isInternalException(throwable)) {
                     ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(new SkipSentryException(throwable))).log("Failed to execute command %s for %s", commandInput, commandSender.getDisplayName());
         ...
         if (var9 instanceof CommandException commandException) {
            ...
            ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(var9)).log("Failed to execute command %s for %s", commandInput, commandSender.getDisplayName());
            Message errorMsg = var9.getMessage() == null ? Message.translation("server.modules.command.noProvidedExceptionMessage") : Message.raw(var9.getMessage());
   ...
   private static boolean isInternalException(@Nonnull Throwable throwable) {
      if (throwable instanceof CommandException) {
         ...
         return throwable instanceof CompletionException && throwable.getCause() != null && throwable.getCause() != throwable ? isInternalException(throwable.getCause()) : false;
      }
   }
   ...
   private CompletableFuture<Void> handleCommands0(@Nonnull CommandSender sender, @Nonnull Deque<String> commands) {
      return commands.isEmpty() ? CompletableFuture.completedFuture((Object)null) : this.handleCommand(sender, (String)commands.poll()).thenCompose((aVoid) -> this.handleCommands0(sender, commands));
   }
