...
public final class ArgTypes {
      ...
      public UUID parse(@Nonnull String input, @Nonnull ParseResult parseResult) {
         try {
            ...
            for(World world : Universe.get().getWorlds().values()) {
               ...
               PlayerRef playerRef = (PlayerRef)NameMatching.DEFAULT.find(playerRefs, input, PlayerRef::getUsername);
               if (playerRef != null) {
   ...
   public static final SingleArgumentType<CompletableFuture<ProfileServiceClient.PublicGameProfile>> GAME_PROFILE_LOOKUP_ASYNC = new SingleArgumentType<CompletableFuture<ProfileServiceClient.PublicGameProfile>>("server.commands.parsing.argtype.playerUuidLookup.name", "server.commands.parsing.argtype.playerUuidLookup.usage", new String[]{java.util.UUID.randomUUID().toString(), "john_doe", "user123"}) {
      @Nullable
   ...
   public static final SingleArgumentType<ProfileServiceClient.PublicGameProfile> GAME_PROFILE_LOOKUP = new SingleArgumentType<ProfileServiceClient.PublicGameProfile>("server.commands.parsing.argtype.playerUuidLookup.name", "server.commands.parsing.argtype.playerUuidLookup.usage", new String[]{java.util.UUID.randomUUID().toString(), "john_doe", "user123"}) {
      @Nullable
      public ProfileServiceClient.PublicGameProfile parse(@Nonnull String input, @Nonnull ParseResult parseResult) {
         try {
            return (ProfileServiceClient.PublicGameProfile)((CompletableFuture)ArgTypes.GAME_PROFILE_LOOKUP_ASYNC.parse(input, parseResult)).join();
         } catch (Exception e) {
            Throwable cause = (Throwable)(e.getCause() != null ? e.getCause() : e);
            if (cause instanceof IllegalStateException) {
               ...
               parseResult.fail(Message.translation("server.commands.parsing.argtype.playerUuidLookup.lookupError").param("input", input).param("error", cause.getMessage() != null ? cause.getMessage() : cause.getClass().getSimpleName()));
            }
   ...
   public static final SingleArgumentType<PlayerRef> PLAYER_REF = new SingleArgumentType<PlayerRef>("server.commands.parsing.argtype.player.name", "server.commands.parsing.argtype.player.usage", new String[]{"john_doe", "user123"}) {
      ...
      public PlayerRef processedGet(CommandSender sender, @Nonnull CommandContext context, Argument<?, PlayerRef> argument) {
         PlayerRef playerRef = (PlayerRef)context.get(argument);
         if (playerRef != null) {
            return playerRef;
   ...
   public static final SingleArgumentType<IntegerComparisonOperator> INTEGER_COMPARISON_OPERATOR;
   public static final SingleArgumentType<IntegerOperation> INTEGER_OPERATION;
   ...
   static {
      ENTITY_ID = new ArgWrapper<EntityWrappedArg, UUID>(UUID.withOverriddenUsage("server.commands.parsing.argtype.entityid.usage"), EntityWrappedArg::new);
      INTEGER_COMPARISON_OPERATOR = new SingleArgumentType<IntegerComparisonOperator>("Integer Comparison Operator", "A mathematical sign for integer comparison", new String[]{">", "<", ">=", "<=", "%", "=", "!="}) {
         @Nullable
         public IntegerComparisonOperator parse(String input, @Nonnull ParseResult parseResult) {
            IntegerComparisonOperator integerComparisonOperator = ArgTypes.IntegerComparisonOperator.getFromStringRepresentation(input);
      ...
      INTEGER_OPERATION = new SingleArgumentType<IntegerOperation>("Integer Operator", "A mathematical sign for performing an operation", new String[]{"+", "-", "*", "/", "%", "="}) {
         @Nullable
         public IntegerOperation parse(String input, @Nonnull ParseResult parseResult) {
            IntegerOperation integerOperation = ArgTypes.IntegerOperation.getFromStringRepresentation(input);
      ...
      INT_RANGE = new MultiArgumentType<Pair<Integer, Integer>>("Integer Range", "Two integers representing a minimum and maximum of a range", new String[]{"-2 8", "5 5", "1 5"}) {
         private final WrappedArgumentType<Integer> minValue;
         private final WrappedArgumentType<Integer> maxValue;

         {
            this.minValue = this.withParameter("min", "Minimum value, must be less than or equal to max value", ArgTypes.INTEGER);
            this.maxValue = this.withParameter("max", "Maximum value, must be greater than or equal to min value", ArgTypes.INTEGER);
         }

         @Nullable
         public Pair<Integer, Integer> parse(@Nonnull MultiArgumentContext context, @Nonnull ParseResult parseResult) {
            if ((Integer)context.get(this.minValue) > (Integer)context.get(this.maxValue)) {
               String var10001 = String.valueOf(context.get(this.minValue));
               parseResult.fail(Message.raw("You cannot set the minimum value as larger than the maximum value. Min: " + var10001 + " Max: " + String.valueOf(context.get(this.maxValue))));
               return null;
            } else {
               return Pair.of((Integer)context.get(this.minValue), (Integer)context.get(this.maxValue));
            }
         }
      };
      RELATIVE_INT_RANGE = new MultiArgumentType<RelativeIntegerRange>("Integer Range", "Two integers representing a minimum and maximum of a range", new String[]{"~-2 ~8", "~5 ~5", "~1 ~5"}) {
         private final WrappedArgumentType<RelativeInteger> minValue;
         private final WrappedArgumentType<RelativeInteger> maxValue;

         {
            this.minValue = this.withParameter("min", "Minimum value, must be less than or equal to max value", ArgTypes.RELATIVE_INTEGER);
            this.maxValue = this.withParameter("max", "Maximum value, must be greater than or equal to min value", ArgTypes.RELATIVE_INTEGER);
         }
         ...
         public RelativeIntegerRange parse(@Nonnull MultiArgumentContext context, @Nonnull ParseResult parseResult) {
            ...
            if (min != null && max != null) {
               if (min.isRelative() != max.isRelative()) {
                  ...
                  return new RelativeIntegerRange((RelativeInteger)context.get(this.minValue), (RelativeInteger)context.get(this.maxValue));
               }
            } else {
      ...
      VECTOR2I = new MultiArgumentType<Vector2i>("Integer Vector 2D", "Two integers, generally corresponding to x/z axis", new String[]{"124 232", "5 -3", "1 1"}) {
         private final WrappedArgumentType<Integer> xValue;
         private final WrappedArgumentType<Integer> zValue;

         {
            this.xValue = this.withParameter("x", "X value", ArgTypes.INTEGER);
            this.zValue = this.withParameter("z", "Z value", ArgTypes.INTEGER);
         }

         @Nonnull
         public Vector2i parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return new Vector2i((Integer)context.get(this.xValue), (Integer)context.get(this.zValue));
         }
      };
      VECTOR3I = new MultiArgumentType<Vector3i>("Integer Vector", "Three integers, generally corresponding to x/y/z axis", new String[]{"124 232 234", "5 0 -3", "1 1 1"}) {
         private final WrappedArgumentType<Integer> xValue;
         private final WrappedArgumentType<Integer> yValue;
         private final WrappedArgumentType<Integer> zValue;

         {
            this.xValue = this.withParameter("x", "X value", ArgTypes.INTEGER);
            this.yValue = this.withParameter("y", "Y value", ArgTypes.INTEGER);
            this.zValue = this.withParameter("z", "Z value", ArgTypes.INTEGER);
         }

         @Nonnull
         public Vector3i parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return new Vector3i((Integer)context.get(this.xValue), (Integer)context.get(this.yValue), (Integer)context.get(this.zValue));
         }
      };
      RELATIVE_VECTOR3I = new MultiArgumentType<RelativeVector3i>("Relative Integer Vector", "Three optionally relative integers, generally corresponding to x/y/z axis", new String[]{"124 ~232 234", "~5 0 ~-3", "1 ~1 1"}) {
         private final WrappedArgumentType<RelativeInteger> xValue;
         private final WrappedArgumentType<RelativeInteger> yValue;
         private final WrappedArgumentType<RelativeInteger> zValue;

         {
            this.xValue = this.withParameter("x", "X value", ArgTypes.RELATIVE_INTEGER);
            this.yValue = this.withParameter("y", "Y value", ArgTypes.RELATIVE_INTEGER);
            this.zValue = this.withParameter("z", "Z value", ArgTypes.RELATIVE_INTEGER);
         }

         @Nonnull
         public RelativeVector3i parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return new RelativeVector3i((RelativeInteger)context.get(this.xValue), (RelativeInteger)context.get(this.yValue), (RelativeInteger)context.get(this.zValue));
         }
      };
      RELATIVE_BLOCK_POSITION = new MultiArgumentType<RelativeIntPosition>("server.commands.parsing.argtype.relativeBlockPosition.name", "server.commands.parsing.argtype.relativeBlockPosition.usage", new String[]{"124 232 234", "~5 ~ ~-3", "~ ~ ~"}) {
         private final WrappedArgumentType<IntCoord> xValue;
         private final WrappedArgumentType<IntCoord> yValue;
         private final WrappedArgumentType<IntCoord> zValue;

         {
            this.xValue = this.withParameter("x", "server.commands.parsing.argtype.xCoord.usage", ArgTypes.RELATIVE_INT_COORD);
            this.yValue = this.withParameter("y", "server.commands.parsing.argtype.yCoord.usage", ArgTypes.RELATIVE_INT_COORD);
            this.zValue = this.withParameter("z", "server.commands.parsing.argtype.zCoord.usage", ArgTypes.RELATIVE_INT_COORD);
         }

         @Nonnull
         public RelativeIntPosition parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return new RelativeIntPosition((IntCoord)context.get(this.xValue), (IntCoord)context.get(this.yValue), (IntCoord)context.get(this.zValue));
         }
      };
      RELATIVE_POSITION = new MultiArgumentType<RelativeDoublePosition>("server.commands.parsing.argtype.relativePosition.name", "server.commands.parsing.argtype.relativePosition.usage", new String[]{"124.63 232.27 234.22", "~5.5 ~ ~", "~ ~ ~"}) {
         private final WrappedArgumentType<Coord> xValue;
         private final WrappedArgumentType<Coord> yValue;
         private final WrappedArgumentType<Coord> zValue;

         {
            this.xValue = this.withParameter("x", "server.commands.parsing.argtype.xCoord.usage", ArgTypes.RELATIVE_DOUBLE_COORD);
            this.yValue = this.withParameter("y", "server.commands.parsing.argtype.yCoord.usage", ArgTypes.RELATIVE_DOUBLE_COORD);
            this.zValue = this.withParameter("z", "server.commands.parsing.argtype.zCoord.usage", ArgTypes.RELATIVE_DOUBLE_COORD);
         }

         @Nonnull
         public RelativeDoublePosition parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return new RelativeDoublePosition((Coord)context.get(this.xValue), (Coord)context.get(this.yValue), (Coord)context.get(this.zValue));
         }
      };
      RELATIVE_CHUNK_POSITION = new MultiArgumentType<RelativeChunkPosition>("server.commands.parsing.argtype.relativeChunkPosition.name", "server.commands.parsing.argtype.relativeChunkPosition.usage", new String[]{"5 10", "~c2 ~c-3", "~ ~"}) {
         private final WrappedArgumentType<IntCoord> xValue;
         private final WrappedArgumentType<IntCoord> zValue;
         ...
         public RelativeChunkPosition parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return new RelativeChunkPosition((IntCoord)context.get(this.xValue), (IntCoord)context.get(this.zValue));
         }
      };
      ROTATION = new MultiArgumentType<Vector3f>("server.commands.parsing.argtype.rotation.name", "server.commands.parsing.argtype.rotation.usage", new String[]{"124.63 232.27 234.22"}) {
         private final WrappedArgumentType<Float> pitch;
         private final WrappedArgumentType<Float> yaw;
         private final WrappedArgumentType<Float> roll;

         {
            this.pitch = this.withParameter("server.commands.parsing.argtype.pitch.name", "server.commands.parsing.argtype.pitch.usage", ArgTypes.FLOAT);
            this.yaw = this.withParameter("server.commands.parsing.argtype.yaw.name", "server.commands.parsing.argtype.yaw.usage", ArgTypes.FLOAT);
            this.roll = this.withParameter("server.commands.parsing.argtype.roll.name", "server.commands.parsing.argtype.roll.usage", ArgTypes.FLOAT);
         }

         @Nonnull
         public Vector3f parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return new Vector3f((Float)context.get(this.pitch), (Float)context.get(this.yaw), (Float)context.get(this.roll));
         }
      };
      BLOCK_TYPE_KEY = new SingleArgumentType<String>("Block Type Key", "A block type", new String[]{"Wood_Drywood_Planks_Half"}) {
         @Nullable
      ...
      BLOCK_ID = new ProcessedArgumentType<String, Integer>("Block Id", Message.raw("A block type, converted to an int id"), BLOCK_TYPE_KEY, new String[]{"Wood_Drywood_Planks_Half"}) {
         @Nonnull
         public Integer processInput(String blockTypeKey) {
            return BlockType.getAssetMap().getIndex(blockTypeKey);
         }
      };
      COLOR = new SingleArgumentType<Integer>("server.commands.parsing.argtype.color.name", "server.commands.parsing.argtype.color.usage", new String[]{"#FF0000", "#00FF00FF", "16711680", "0xFF0000"}) {
         @Nullable
               ...
               try {
                  ...
                  switch (hexString.length()) {
                     case 3:
                        ...
                        int b = (int)(value & 15L);
                        return -16777216 | r << 20 | r << 16 | g << 12 | g << 8 | b << 4 | b;
                     case 4:
      ...
      LAYER_ENTRY_TYPE = new MultiArgumentType<Pair<Integer, String>>("Layer Entry Type", "A thickness for a corresponding block pattern", new String[]{"1 Rock_Stone", "3 50%Rock_Basalt;50%Rock_Stone"}) {
         private final WrappedArgumentType<Integer> thickness;
         private final WrappedArgumentType<String> blockPattern;

         {
            this.thickness = this.withParameter("thickness", "How thick the layer should be", ArgTypes.INTEGER);
            this.blockPattern = this.withParameter("blockPattern", "The block pattern to use for the layer. If using with percentages, separate values with a ';'", ArgTypes.STRING);
         }

         @Nonnull
         public Pair<Integer, String> parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return Pair.of((Integer)context.get(this.thickness), (String)context.get(this.blockPattern));
         }
      };
      WEIGHTED_BLOCK_TYPE = new MultiArgumentType<Pair<Integer, String>>("Weighted Block Type", "A weight corresponding to a blocktype", new String[]{"5 Empty", "20 Rock_Stone", "2 Rock_Shale"}) {
         private final WrappedArgumentType<Integer> weight;
         private final WrappedArgumentType<String> blockType;

         {
            this.weight = this.withParameter("weight", "The relative weight of this entry. Think of it as a lottery ticket", ArgTypes.INTEGER);
            this.blockType = this.withParameter("blockType", "The BlockTypeKey associated with the weight", ArgTypes.BLOCK_TYPE_KEY);
         }

         @Nonnull
         public Pair<Integer, String> parse(@Nonnull MultiArgumentContext context, ParseResult parseResult) {
            return Pair.of((Integer)context.get(this.weight), (String)context.get(this.blockType));
         }
      };
      WEIGHTED_BLOCK_ENTRY = new SingleArgumentType<String>("Weighted Block Entry", "A block with optional weight prefix (e.g., 20%Rock_Stone or Rock_Stone)", new String[]{"Rock_Stone", "20%Rock_Stone", "50%Rock_Stone|Yaw=90"}) {
         @Nullable
         public String parse(@Nonnull String input, @Nonnull ParseResult parseResult) {
            if (input.isEmpty()) {
      ...
      BLOCK_PATTERN = new ProcessedArgumentType<List<String>, BlockPattern>("Block Pattern", Message.raw("A list of blocks with optional weights (e.g., [20%Rock_Stone, 80%Rock_Shale])"), new ListArgumentType(WEIGHTED_BLOCK_ENTRY), new String[]{"[Rock_Stone]", "[20%Rock_Stone, 80%Rock_Shale]", "[50%Rock_Stone|Yaw=90, 50%Fluid_Water]"}) {
         @Nonnull
      ...
      INDIVIDUAL_BLOCK_MASK = new SingleArgumentType<BlockMask>("Block Mask", "Create a block mask using symbols and block names", new String[]{">Grass_Full", "!Fluid_Water", "!^Fluid_Lava", "!#"}) {
         @Nullable
      ...
      BLOCK_MASK = new ProcessedArgumentType<List<BlockMask>, BlockMask>("Block Mask", Message.raw("A list of block masks that combine together"), new ListArgumentType(INDIVIDUAL_BLOCK_MASK), new String[]{"[!Fluid_Water, !^Fluid_Lava]", "[>Grass_Full, !#]"}) {
         public BlockMask processInput(@Nonnull List<BlockMask> masks) {
            return BlockMask.combine((BlockMask[])masks.toArray((x$0) -> new BlockMask[x$0]));
         }
      };
      TICK_RATE = new SingleArgumentType<Integer>("server.commands.parsing.argtype.tickrate.name", "server.commands.parsing.argtype.tickrate.usage", new String[]{"30tps", "33ms", "60", "20tps", "50ms"}) {
         @Nullable
         public Integer parse(@Nonnull String input, @Nonnull ParseResult parseResult) {
      ...
      GAME_MODE = new GameModeArgumentType();
   }
   ...
   public static enum IntegerComparisonOperator {
      ...
      public boolean compare(int left, int right) {
         return (Boolean)this.comparisonFunction.apply(left, right);
      }
      ...
      public static IntegerComparisonOperator getFromStringRepresentation(String stringRepresentation) {
         for(IntegerComparisonOperator value : values()) {
   ...
   public static enum IntegerOperation {
      ...
      public int operate(int previous, int modifier) {
         return (Integer)this.operationFunction.apply(previous, modifier);
      }
