...
public class SpawnBlockCommand extends AbstractWorldCommand {
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      String blockTypeKey = (String)context.get(this.blockArg);
      Vector3d position = ((RelativeDoublePosition)context.get(this.positionArg)).getRelativePosition(context, world, store);
      Vector3f rotation = (Vector3f)this.rotationArg.get(context);
      ...
      TransformComponent transformComponent = (TransformComponent)blockEntityHolder.ensureAndGetComponent(TransformComponent.getComponentType());
      transformComponent.setPosition(position);
      transformComponent.setRotation(rotation);
      UUIDComponent uuidComponent = (UUIDComponent)blockEntityHolder.getComponent(UUIDComponent.getComponentType());
