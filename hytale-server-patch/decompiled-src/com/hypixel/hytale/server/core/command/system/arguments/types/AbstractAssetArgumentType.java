...
public abstract class AbstractAssetArgumentType<DataType extends JsonAssetWithMap<AssetKeyType, M>, M extends AssetMap<AssetKeyType, DataType>, AssetKeyType> extends SingleArgumentType<DataType> {
   ...
   public DataType parse(@Nonnull String input, @Nonnull ParseResult parseResult) {
      ...
      AssetKeyType assetKey = (AssetKeyType)this.getAssetKey(input);
      if (assetKey == null) {
         parseResult.fail(Message.translation("server.commands.notfound").param("type", this.dataTypeClass.getSimpleName()).param("id", input).color(Color.RED), Message.translation("server.general.failed.didYouMean").param("choices", StringUtil.sortByFuzzyDistance(input, assetMap.getAssetMap().keySet(), CommandUtil.RECOMMEND_COUNT).toString()));
         return null;
      } else {
         DataType asset = (DataType)(((AssetMap)assetMap).getAsset(assetKey));
         if (asset == null) {
            parseResult.fail(Message.translation("server.commands.notfound").param("type", this.dataTypeClass.getSimpleName()).param("id", input).color(Color.RED), Message.translation("server.general.failed.didYouMean").param("choices", StringUtil.sortByFuzzyDistance(input, assetMap.getAssetMap().keySet(), CommandUtil.RECOMMEND_COUNT).toString()));
            return null;
         } else {
