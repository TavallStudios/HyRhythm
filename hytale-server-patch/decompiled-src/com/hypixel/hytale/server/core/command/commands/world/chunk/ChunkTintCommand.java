...
public class ChunkTintCommand extends AbstractPlayerCommand {
   ...
   public ChunkTintCommand() {
      ...
      this.radiusArg = (DefaultArg)this.withDefaultArg("radius", "server.commands.chunk.tint.radius.desc", ArgTypes.INTEGER, 5, "server.commands.chunk.tint.radius.default").addValidator(Validators.greaterThan(0));
      this.sigmaArg = (DefaultArg)this.withDefaultArg("sigma", "server.commands.chunk.tint.sigma.desc", ArgTypes.DOUBLE, 1.5, "server.commands.chunk.tint.sigma.default").addValidator(Validators.greaterThan(0.0));
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      int color = (Integer)this.colorArg.get(context);
      ...
      if (chunkRef != null && chunkRef.isValid()) {
         BlockChunk blockChunk = (BlockChunk)chunkStoreStore.getComponent(chunkRef, BlockChunk.getComponentType());
         if (blockChunk != null) {
            for(int x = 0; x < 32; ++x) {
               for(int z = 0; z < 32; ++z) {
         ...
         if (this.blurArg.provided(context)) {
            radius = (Integer)this.radiusArg.get(context);
            sigma = (Double)this.sigmaArg.get(context);
            ...
            ObjectIterator var37 = newTintMap.long2IntEntrySet().iterator();

            while(var37.hasNext()) {
               Long2IntMap.Entry entry = (Long2IntMap.Entry)var37.next();
   ...
   private static int blur(@Nonnull ChunkAccessor<WorldChunk> chunkAccessor, int radius, double[] matrix, int x, int z) {
      ...
      for(int ix = -radius; ix <= radius; ++ix) {
         for(int iz = -radius; iz <= radius; ++iz) {
            ...
            int ax = x + ix;
            int az = z + iz;
            WorldChunk worldChunk = (WorldChunk)chunkAccessor.getChunk(ChunkUtil.indexChunkFromBlock(ax, az));
            if (worldChunk != null) {
   ...
   private static double[] gaussianMatrix(double sigma, int radius) {
      ...
      for(int x = -radius; x <= radius; ++x) {
         for(int y = -radius; y <= radius; ++y) {
            double value = gaussian2d(sigma, (double)x, (double)y);
   ...
   public static class TintChunkPage extends InteractiveCustomUIPage<TintChunkPageEventData> {
      ...
      public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, @Nonnull Store<EntityStore> store) {
         ...
         eventBuilder.addEventBinding(CustomUIEventBindingType.ValueChanged, "#ColorPicker", (new EventData()).append("@Color", "#ColorPicker.Value").append("Submit", ChunkTintCommand.TintChunkPage.TintChunkPageAction.ColorChanged.name()), false);
         eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#ApplyButton", (new EventData()).append("@Color", "#ColorPicker.Value").append("@Radius", "#Radius.Value").append("@BlurEnabled", "#BlurEnabledContainer #CheckBox.Value").append("@Sigma", "#Sigma.Value").append("@HexColor", "#HexColor.Value").append("Submit", ChunkTintCommand.TintChunkPage.TintChunkPageAction.Submit.name()), false);
      }

      public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull TintChunkPageEventData data) {
      ...
      public static class TintChunkPageEventData {
         ...
         static {
            CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(TintChunkPageEventData.class, TintChunkPageEventData::new).addField(new KeyedCodec("@Color", Codec.STRING), (entry, s) -> entry.color = s, (entry) -> entry.color)).addField(new KeyedCodec("@Radius", Codec.INTEGER), (entry, s) -> entry.radius = s, (entry) -> entry.radius)).addField(new KeyedCodec("@Sigma", Codec.INTEGER), (entry, s) -> entry.sigma = s, (entry) -> entry.sigma)).addField(new KeyedCodec("@BlurEnabled", Codec.BOOLEAN), (entry, b) -> entry.isBlurEnabled = b, (entry) -> entry.isBlurEnabled)).addField(new KeyedCodec("@HexColor", Codec.STRING), (entry, s) -> entry.hexColor = s, (entry) -> entry.hexColor)).addField(new KeyedCodec("Submit", new EnumCodec(TintChunkPageAction.class)), (entry, s) -> entry.action = s, (entry) -> entry.action)).build();
         }
      }
   }
}
