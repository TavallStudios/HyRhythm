...
public class ResourceSlotTransaction extends SlotTransaction {
   ...
   public ResourceSlotTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      ...
      return newSlot >= 0 && newSlot < container.getCapacity() ? new ResourceSlotTransaction(this.succeeded(), this.getAction(), newSlot, this.getSlotBefore(), this.getSlotAfter(), this.getOutput(), this.isAllOrNothing(), this.isExactAmount(), this.isFilter(), this.query, this.remainder, this.consumed) : null;
   }

   @Nonnull
   public String toString() {
      String var10000 = String.valueOf(this.query);
      return "ResourceSlotTransaction{query=" + var10000 + ", remainder=" + this.remainder + ", consumed=" + this.consumed + "} " + super.toString();
   }
}
