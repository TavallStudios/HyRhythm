...
public class TagSlotTransaction extends SlotTransaction {
   ...
   public TagSlotTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      ...
      return newSlot >= 0 && newSlot < container.getCapacity() ? new TagSlotTransaction(this.succeeded(), this.getAction(), newSlot, this.getSlotBefore(), this.getSlotAfter(), this.getOutput(), this.isAllOrNothing(), this.isExactAmount(), this.isFilter(), this.query, this.remainder) : null;
   }

   @Nonnull
   public String toString() {
      int var10000 = this.query;
      return "TagSlotTransaction{tag=" + var10000 + ", remainder=" + this.remainder + "} " + super.toString();
   }
}
