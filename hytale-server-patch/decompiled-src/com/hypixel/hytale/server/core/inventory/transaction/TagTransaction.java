...
public class TagTransaction extends ListTransaction<TagSlotTransaction> {
   ...
   public TagTransaction toParent(ItemContainer parent, short start, ItemContainer container) {
      List<TagSlotTransaction> slotTransactions = (List)this.getList().stream().map((transaction) -> transaction.toParent(parent, start, container)).collect(Collectors.toList());
   ...
   public TagTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      List<TagSlotTransaction> slotTransactions = (List)this.getList().stream().map((transactionx) -> transactionx.fromParent(parent, start, container)).filter(Objects::nonNull).collect(Collectors.toList());
   ...
   public String toString() {
      ...
      return "TagTransaction{action=" + var10000 + ", tag=" + this.tagIndex + ", remainder=" + this.remainder + ", allOrNothing=" + this.allOrNothing + ", exactAmount=" + this.exactAmount + ", filter=" + this.filter + "} " + super.toString();
   }
}
