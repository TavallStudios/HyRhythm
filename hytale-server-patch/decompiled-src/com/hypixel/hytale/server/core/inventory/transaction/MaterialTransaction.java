...
public class MaterialTransaction extends ListTransaction<MaterialSlotTransaction> {
   ...
   public MaterialTransaction toParent(ItemContainer parent, short start, ItemContainer container) {
      List<MaterialSlotTransaction> slotTransactions = (List)this.getList().stream().map((transaction) -> transaction.toParent(parent, start, container)).collect(Collectors.toList());
   ...
   public MaterialTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      List<MaterialSlotTransaction> slotTransactions = (List)this.getList().stream().map((transactionx) -> transactionx.fromParent(parent, start, container)).filter(Objects::nonNull).collect(Collectors.toList());
   ...
   public String toString() {
      ...
      return "MaterialTransaction{action=" + var10000 + ", material=" + String.valueOf(this.material) + ", remainder=" + this.remainder + ", allOrNothing=" + this.allOrNothing + ", exactAmount=" + this.exactAmount + ", filter=" + this.filter + "} " + super.toString();
   }
}
