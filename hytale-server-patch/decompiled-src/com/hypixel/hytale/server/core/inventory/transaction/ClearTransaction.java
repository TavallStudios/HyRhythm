...
public class ClearTransaction implements Transaction {
   ...
   public ClearTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      ...
      if (newStart < 0) {
         int from = -newStart;
         return this.items.length + newStart > capacity ? new ClearTransaction(this.succeeded, (short)0, (ItemStack[])Arrays.copyOfRange(this.items, from, from + capacity)) : new ClearTransaction(this.succeeded, (short)0, (ItemStack[])Arrays.copyOfRange(this.items, from, this.items.length));
      } else {
         return this.items.length > capacity ? new ClearTransaction(this.succeeded, newStart, (ItemStack[])Arrays.copyOf(this.items, capacity)) : new ClearTransaction(this.succeeded, newStart, this.items);
      }
   }
