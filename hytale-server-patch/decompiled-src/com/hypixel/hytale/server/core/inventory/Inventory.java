...
public class Inventory implements NetworkSerializable<UpdatePlayerInventory> {
   ...
   private ItemContainer getItemContainerForPickupLocation(@Nonnull PickupLocation pickupLocation) {
      CombinedItemContainer var10000;
      switch (pickupLocation) {
         case Hotbar -> var10000 = this.combinedHotbarStorageBackpack;
         case Storage -> var10000 = this.combinedStorageHotbarBackpack;
         case Backpack -> var10000 = this.combinedBackpackStorageHotbar;
         default -> var10000 = this.combinedHotbarStorageBackpack;
      }
   ...
   public ItemContainer getContainerForItemPickup(@Nonnull Item item, PlayerSettings playerSettings) {
      return this.getContainerForItemPickup(item, playerSettings, (PickupLocation)null);
   }
   ...
   public ItemContainer getContainerForItemPickup(@Nonnull Item item, PlayerSettings playerSettings, @Nullable PickupLocation overridePickupLocation) {
      if (overridePickupLocation != null) {
         ...
         if (item.getUtility().isUsable()) {
            ...
            return blockType.getMaterial() == BlockMaterial.Solid ? this.getItemContainerForPickupLocation(playerSettings.solidBlockItemsPreferredPickupLocation()) : this.getItemContainerForPickupLocation(playerSettings.miscItemsPreferredPickupLocation());
         }
      } else {
   ...
   private void buildCombinedContains() {
      ...
      this.combinedBackpackHotbarStorage = new CombinedItemContainer(new ItemContainer[]{this.backpack, this.hotbar, this.storage});
      this.combinedStorageHotbarBackpack = new CombinedItemContainer(new ItemContainer[]{this.storage, this.hotbar, this.backpack});
      this.combinedHotbarStorageBackpack = new CombinedItemContainer(new ItemContainer[]{this.hotbar, this.storage, this.backpack});
