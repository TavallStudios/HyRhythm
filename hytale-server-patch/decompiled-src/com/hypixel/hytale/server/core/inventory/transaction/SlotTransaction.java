...
public class SlotTransaction implements Transaction {
   ...
   public SlotTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      ...
      return newSlot >= 0 && newSlot < container.getCapacity() ? new SlotTransaction(this.succeeded, this.action, newSlot, this.slotBefore, this.slotAfter, this.output, this.allOrNothing, this.exactAmount, this.filter) : null;
   }

   @Nonnull
   public String toString() {
      boolean var10000 = this.succeeded;
      return "SlotTransaction{succeeded=" + var10000 + ", action=" + String.valueOf(this.action) + ", slot=" + this.slot + ", slotBefore=" + String.valueOf(this.slotBefore) + ", slotAfter=" + String.valueOf(this.slotAfter) + ", allOrNothing=" + this.allOrNothing + ", exactAmount=" + this.exactAmount + ", filter=" + this.filter + "}";
   }

   static {
      FAILED_ADD = new SlotTransaction(false, ActionType.ADD, (short)-1, (ItemStack)null, (ItemStack)null, (ItemStack)null, false, false, false);
   }
}
