...
public class ItemStackTransaction implements Transaction {
   ...
   public ItemStackTransaction toParent(ItemContainer parent, short start, ItemContainer container) {
      List<ItemStackSlotTransaction> slotTransactions = (List)this.slotTransactions.stream().map((transaction) -> transaction.toParent(parent, start, container)).collect(Collectors.toList());
   ...
   public ItemStackTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      List<ItemStackSlotTransaction> slotTransactions = (List)this.slotTransactions.stream().map((transactionx) -> transactionx.fromParent(parent, start, container)).filter(Objects::nonNull).collect(Collectors.toList());
   ...
   public String toString() {
      boolean var10000 = this.succeeded;
      return "ItemStackTransaction{succeeded=" + var10000 + ", action=" + String.valueOf(this.action) + ", query=" + String.valueOf(this.query) + ", remainder=" + String.valueOf(this.remainder) + ", allOrNothing=" + this.allOrNothing + ", filter=" + this.filter + ", slotTransactions=" + String.valueOf(this.slotTransactions) + "}";
   }

   static {
      FAILED_ADD = new ItemStackTransaction(false, ActionType.ADD, (ItemStack)null, (ItemStack)null, false, false, Collections.emptyList());
   }
}
