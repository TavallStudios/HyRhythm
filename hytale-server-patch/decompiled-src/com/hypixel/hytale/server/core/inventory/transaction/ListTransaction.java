...
public class ListTransaction<T extends Transaction> implements Transaction {
   ...
   public ListTransaction<T> toParent(ItemContainer parent, short start, ItemContainer container) {
      List<T> list = (List)this.list.stream().map((transaction) -> transaction.toParent(parent, start, container)).collect(Collectors.toList());
   ...
   public ListTransaction<T> fromParent(ItemContainer parent, short start, ItemContainer container) {
      List<T> list = (List)this.list.stream().map((transactionx) -> transactionx.fromParent(parent, start, container)).filter(Objects::nonNull).collect(Collectors.toList());
      if (list.isEmpty()) {
         return null;
      } else {
   ...
   public String toString() {
      ...
      return "ListTransaction{succeeded=" + var10000 + ", list=" + String.valueOf(this.list) + "}";
   }
}
