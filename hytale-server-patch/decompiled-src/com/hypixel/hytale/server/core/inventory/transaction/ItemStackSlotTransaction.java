...
public class ItemStackSlotTransaction extends SlotTransaction {
   ...
   public ItemStackSlotTransaction toParent(ItemContainer parent, short start, ItemContainer container) {
      ...
      return new ItemStackSlotTransaction(this.succeeded(), this.getAction(), newSlot, this.getSlotBefore(), this.getSlotAfter(), this.getOutput(), this.isAllOrNothing(), this.isExactAmount(), this.isFilter(), this.addToExistingSlot, this.query, this.remainder);
   }
   ...
   public ItemStackSlotTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      ...
      return newSlot >= 0 && newSlot < container.getCapacity() ? new ItemStackSlotTransaction(this.succeeded(), this.getAction(), newSlot, this.getSlotBefore(), this.getSlotAfter(), this.getOutput(), this.isAllOrNothing(), this.isExactAmount(), this.isFilter(), this.addToExistingSlot, this.query, this.remainder) : null;
   }

   @Nonnull
   public String toString() {
      boolean var10000 = this.addToExistingSlot;
      return "ItemStackSlotTransaction{addToExistingSlot=" + var10000 + ", query=" + String.valueOf(this.query) + ", remainder=" + String.valueOf(this.remainder) + "} " + super.toString();
   }
}
