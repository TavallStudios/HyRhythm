...
public class ResourceTransaction extends ListTransaction<ResourceSlotTransaction> {
   ...
   public ResourceTransaction toParent(ItemContainer parent, short start, ItemContainer container) {
      List<ResourceSlotTransaction> slotTransactions = (List)this.getList().stream().map((transaction) -> transaction.toParent(parent, start, container)).collect(Collectors.toList());
   ...
   public ResourceTransaction fromParent(ItemContainer parent, short start, @Nonnull ItemContainer container) {
      List<ResourceSlotTransaction> slotTransactions = (List)this.getList().stream().map((transactionx) -> transactionx.fromParent(parent, start, container)).filter(Objects::nonNull).collect(Collectors.toList());
   ...
   public String toString() {
      ...
      return "ResourceTransaction{action=" + var10000 + ", resource=" + String.valueOf(this.resource) + ", remainder=" + this.remainder + ", consumed=" + this.consumed + ", allOrNothing=" + this.allOrNothing + ", exactAmount=" + this.exactAmount + ", filter=" + this.filter + "} " + super.toString();
   }
}
