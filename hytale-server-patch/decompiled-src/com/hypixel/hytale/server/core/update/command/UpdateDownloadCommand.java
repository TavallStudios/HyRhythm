...
public class UpdateDownloadCommand extends AbstractAsyncCommand {
   ...
   protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext context) {
      ...
      } else if (!UpdateService.isValidUpdateLayout() && !(Boolean)this.forceFlag.get(context)) {
            ...
            return updateService.checkForUpdate(UpdateService.getEffectivePatchline()).thenCompose((manifest) -> {
               if (manifest == null) {
                  ...
                  if (currentVersion != null && currentVersion.equals(manifest.version)) {
                     ...
                     UpdateService.DownloadTask downloadTask = updateService.downloadUpdate(manifest, UpdateService.getStagingDir(), (percent, downloaded, total) -> {
                        if (updateModule != null) {
