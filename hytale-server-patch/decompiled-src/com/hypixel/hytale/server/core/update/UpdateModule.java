...
public class UpdateModule extends JavaPlugin {
   ...
   protected void start() {
      if (!KILL_SWITCH_ENABLED) {
         ...
         if (this.shouldEnableUpdateChecker()) {
            ...
            this.updateCheckTask = this.scheduler.scheduleAtFixedRate(this::performUpdateCheck, 60L, (long)intervalSeconds, TimeUnit.SECONDS);
         }
   ...
   public UpdateService.VersionManifest getLatestKnownVersion() {
      return (UpdateService.VersionManifest)this.latestKnownVersion.get();
   }
   ...
   public void releaseDownloadLock() {
      this.activeDownload.set((Object)null);
      this.activeDownloadThread.set((Object)null);
   ...
   public boolean cancelDownload() {
      CompletableFuture<?> download = (CompletableFuture)this.activeDownload.getAndSet((Object)null);
      Thread thread = (Thread)this.activeDownloadThread.getAndSet((Object)null);
      if (thread != null) {
         thread.interrupt();
      }
         ...
         if (!config.isEnabled()) {
            ...
            if (!"dev".equals(manifestPatchline) || configPatchline != null && !configPatchline.isEmpty()) {
               if (!UpdateService.isValidUpdateLayout()) {
                  LOGGER.at(Level.WARNING).log("Update checker disabled: invalid folder layout. Expected to run from Server/ with Assets.zip and start.sh/bat in parent directory.");
                  return false;
               } else {
                  return true;
               }
   ...
   private void autoDownloadUpdate(@Nonnull UpdateService updateService, @Nonnull UpdateService.VersionManifest manifest) {
      if (UpdateService.getStagedVersion() == null && this.tryAcquireDownloadLock()) {
         UpdateService.DownloadTask downloadTask = updateService.downloadUpdate(manifest, UpdateService.getStagingDir(), (percent, downloaded, total) -> this.updateDownloadProgress(downloaded, total));
      ...
      if (mode != UpdateConfig.AutoApplyMode.DISABLED) {
         ...
         if (universe != null) {
            ...
            if (playerCount == 0) {
               ...
               long applyTime = now + (long)(delayMinutes * 60) * 1000L;
