...
public abstract class AbstractMetaStore<K> implements IMetaStoreImpl<K> {
   ...
   protected <T> T decodeOrNewMetaObject(MetaKey<T> key) {
      return (T)(key instanceof PersistentMetaKey && this.tryDecodeUnknownKey((PersistentMetaKey)key) ? this.get0(key) : this.registry.newMetaObject(key, this.parent));
   }
      ...
      if (!this.notUnknownKeys.add(key.getId())) {
         ...
         if (value != null) {
            ...
            if (codec instanceof DirectDecodeCodec) {
               obj = (T)this.registry.newMetaObject(key, this.parent);
               ((DirectDecodeCodec)codec).decode(value, obj, (ExtraInfo)null);
            } else {
               obj = codec.decode(value, (ExtraInfo)null);
            }
