...
public class MetaRegistry<K> implements IMetaRegistry<K> {
   private final Map<String, MetaRegistryEntry> parameterMapping = new Object2ObjectOpenHashMap();
   private final List<MetaRegistryEntry> suppliers = new ObjectArrayList();
   ...
   public <T> MetaKey<T> registerMetaObject(Function<K, T> function, boolean persistent, String keyName, @Nonnull Codec<T> codec) {
      ...
      try {
         ...
         MetaRegistry<K>.MetaRegistryEntry<T> metaEntry = new MetaRegistryEntry<T>(function, key);
   ...
   public <T> T newMetaObject(@Nonnull MetaKey<T> key, K parent) {
      ...
      Object var3;
      try {
         var3 = ((MetaRegistryEntry)this.suppliers.get(key.getId())).getFunction().apply(parent);
      } finally {
   ...
   public void forEachMetaEntry(@Nonnull IMetaStore<K> store, @Nonnull final IMetaRegistry.MetaEntryConsumer consumer) {
      store.forEachMetaObject(new IMetaStore.MetaEntryConsumer() {
         public <T> void accept(int id, T value) {
            MetaRegistry<K>.MetaRegistryEntry<T> entry = (MetaRegistryEntry)MetaRegistry.this.suppliers.get(id);
   ...
   public PersistentMetaKey<?> getMetaKeyForCodecKey(String codecKey) {
      MetaRegistryEntry entry = (MetaRegistryEntry)this.parameterMapping.get(codecKey);
