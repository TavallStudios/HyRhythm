         ...
         world.execute(() -> {
            ...
            if (playerComponent.getGameMode() != GameMode.Creative) {
               ...
               if (quantity > 0) {
                  ...
                  inventory.getSectionById(packet.inventorySectionId).setItemStackForSlot((short)packet.slotId, (ItemStack)null);
               }
                           ...
                           if (desiredIndex != -1 && desiredIndex != currentIndex) {
                              ...
                              for(short i = 0; i < combinedInventory.getCapacity(); ++i) {
                                 ...
                                 if (!ItemStack.isEmpty(potentialSelector)) {
                                    ...
                                    if (selectorTool != null && (maxSelectorTool == null || maxSelectorTool.getDurability() < potentialSelector.getDurability())) {
   ...
   public void handle(@Nonnull SmartMoveItemStack packet) {
      ...
      if (ref != null && ref.isValid()) {
         ...
         world.execute(() -> {
            ...
            Inventory inventory = playerComponent.getInventory();
            PlayerSettings settings = (PlayerSettings)store.getComponent(ref, PlayerSettings.getComponentType());
            if (settings == null) {
                  ...
                  case TakeAll:
                     ...
                     if (window instanceof ItemContainerWindow) {
                        ...
                        if (window.getType() == WindowType.Processing) {
                           ...
                           if (itemContainer instanceof CombinedItemContainer) {
                              CombinedItemContainer combinedItemContainer = (CombinedItemContainer)itemContainer;
                              if (combinedItemContainer.getContainersSize() >= 3) {
