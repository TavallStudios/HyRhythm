...
public class PacketAdapters {
   ...
   private static <T extends Packet> boolean handle(@Nonnull List<PacketFilter> list, PacketHandler player, T packet) {
      for(int i = 0; i < list.size(); ++i) {
         try {
            if (((PacketFilter)list.get(i)).test(player, packet)) {
               return true;
            }
         } catch (Throwable t) {
            ((HytaleLogger.Api)HytaleLogger.getLogger().at(Level.SEVERE).withCause(t)).log("Failed to test packet %s against %s:", packet, player);
         }
      }
