...
public interface IWorldPacketHandler<T extends Packet> {
   ...
   static <T extends Packet> void registerHandler(@Nonnull IPacketHandler packetHandler, int packetId, @Nonnull IWorldPacketHandler<T> handler) {
      registerHandler(packetHandler, packetId, handler, (Predicate)null);
   }

   static <T extends Packet> void registerHandler(@Nonnull IPacketHandler packetHandler, int packetId, @Nonnull IWorldPacketHandler<T> handler, @Nullable Predicate<PlayerRef> precondition) {
      packetHandler.registerHandler(packetId, (packet) -> {
         ...
         if (ref != null && ref.isValid()) {
            if (precondition == null || precondition.test(playerRef)) {
               ...
               world.execute(() -> {
                  if (ref.isValid()) {
                     handler.handle(packet, playerRef, ref, world, store);
                  }
               });
            }
         }
