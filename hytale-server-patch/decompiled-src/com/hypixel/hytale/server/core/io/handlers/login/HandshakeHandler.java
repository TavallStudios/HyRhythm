...
public abstract class HandshakeHandler extends GenericConnectionPacketHandler {
   ...
   public HandshakeHandler(@Nonnull Channel channel, @Nonnull ProtocolVersion protocolVersion, @Nonnull String language, @Nonnull ClientType clientType, @Nonnull String identityToken, @Nonnull UUID playerUuid, @Nonnull String username, @Nullable byte[] referralData, @Nullable HostAddress referralSource) {
      ...
      this.authState = HandshakeHandler.AuthState.REQUESTING_AUTH_GRANT;
      ...
      if (identityClaims == null) {
         ...
         if (tokenUuid != null && tokenUuid.equals(this.playerUuid)) {
            ...
            if (!identityClaims.hasScope(requiredScope)) {
               LOGGER.at(Level.WARNING).log("Identity token missing required scope for %s from %s (clientType: %s, required: %s, actual: %s)", this.username, NettyUtil.formatRemoteAddress(this.getChannel()), this.clientType, requiredScope, identityClaims.scope);
                     ...
                     channel.eventLoop().execute(() -> {
                        if (channel.isActive()) {
                           if (this.authState != HandshakeHandler.AuthState.REQUESTING_AUTH_GRANT) {
                              ...
                              this.clearTimeout();
                              LOGGER.at(Level.INFO).log("Sending AuthGrant to %s (with server identity: %s)", NettyUtil.formatRemoteAddress(channel), !serverIdentityToken.isEmpty());
                              this.write(new AuthGrant(authGrant, serverIdentityToken));
                              this.authState = HandshakeHandler.AuthState.AWAITING_AUTH_TOKEN;
                              HytaleServerConfig.TimeoutProfile timeouts = HytaleServer.get().getConfig().getConnectionTimeouts();
                              this.continueStage("auth:token", timeouts.getAuthToken(), () -> this.authState != HandshakeHandler.AuthState.AWAITING_AUTH_TOKEN);
                           }
                        }
                     });
                  } else {
   ...
   public void handle(@Nonnull AuthToken packet) {
      ...
      if (this.authState != HandshakeHandler.AuthState.AWAITING_AUTH_TOKEN) {
         ...
         if (accessToken != null && !accessToken.isEmpty()) {
            ...
            LOGGER.at(Level.INFO).log("Received AuthToken from %s, validating JWT (mTLS cert present: %s, server auth grant: %s)", NettyUtil.formatRemoteAddress(channel), clientCert != null, serverAuthGrant != null && !serverAuthGrant.isEmpty());
                  ...
                  if (tokenUsername != null && !tokenUsername.isEmpty()) {
                     if (!tokenUsername.equals(this.username)) {
                        ...
                        if (serverAuthGrant != null && !serverAuthGrant.isEmpty()) {
                           ...
                           HytaleServerConfig.TimeoutProfile timeouts = HytaleServer.get().getConfig().getConnectionTimeouts();
                           this.continueStage("auth:server-exchange", timeouts.getAuthServerExchange(), () -> this.authState != HandshakeHandler.AuthState.EXCHANGING_SERVER_TOKEN);
                  ...
                  channel.eventLoop().execute(() -> {
                     if (channel.isActive()) {
                        if (this.authState != HandshakeHandler.AuthState.EXCHANGING_SERVER_TOKEN) {
                           ...
                           LOGGER.at(Level.INFO).log("Sending ServerAuthToken to %s (with password challenge: %s)", NettyUtil.formatRemoteAddress(channel), passwordChallenge != null);
   ...
   private void completeAuthentication(byte[] passwordChallenge) {
      ...
      this.clearTimeout();
      LOGGER.at(Level.INFO).log("Mutual authentication complete for %s (%s) from %s", this.authenticatedUsername, this.playerUuid, NettyUtil.formatRemoteAddress(this.getChannel()));
