...
public class HytaleServer {
   ...
   public HytaleServer() throws IOException {
      ...
      if (EarlyPluginLoader.hasTransformers()) {
         ...
         if (!optionSet.has(Options.DISABLE_SENTRY)) {
   ...
   private void boot() {
      if (!this.booting.getAndSet(true)) {
         ...
         var10000.log("Booting up HytaleServer - Version: " + var10001 + ", Revision: " + ManifestUtil.getImplementationRevisionId());
         ...
         try {
            ...
            LoadAssetEvent loadAssetEvent = (LoadAssetEvent)get().getEventBus().dispatchFor(LoadAssetEvent.class).dispatch(new LoadAssetEvent(this.bootStart));
            if (this.isShuttingDown()) {
               return;
            }
            ...
            ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(throwable)).log("Failed to boot HytaleServer!");
         ...
         LOGGER.at(Level.INFO).log("%s         Hytale Server Booted! [%s] took %s", "\u001b[0;32m", String.join(", ", tags), FormatUtil.nanosToString(System.nanoTime() - this.bootStart));
         LOGGER.at(Level.INFO).log("\u001b[0;32m===============================================================================================");
   ...
   public void shutdownServer(@Nonnull ShutdownReason reason) {
      ...
      if (this.shutdown.getAndSet(reason) == null) {
         if (reason.getMessage() != null) {
            String var10001 = reason.getMessage();
            this.sendSingleplayerSignal("-=|Shutdown|" + var10001.replace("\n", "\\n"));
         }
   ...
   void shutdown0(@Nonnull ShutdownReason reason) {
      ...
      try {
         ...
         ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(t)).log("Exception while shutting down:");
      }
   ...
   public void sendSingleplayerProgress() {
      ...
      if (this.shutdown.get() != null) {
         double var10001 = MathUtil.round((double)(plugins.size() - this.pluginsProgress) / (double)plugins.size(), 2);
         this.sendSingleplayerSignal("-=|Shutdown Modules|" + var10001 * 100.0);
      } else if (this.pluginManager.getState() == PluginState.SETUP) {
         double var2 = MathUtil.round((double)this.pluginsProgress / (double)plugins.size(), 2);
         this.sendSingleplayerSignal("-=|Setup|" + var2 * 100.0);
      } else if (this.pluginManager.getState() == PluginState.START) {
         double var3 = MathUtil.round((double)this.pluginsProgress / (double)plugins.size(), 2);
         this.sendSingleplayerSignal("-=|Starting|" + var3 * 100.0);
      }
   ...
   public ShutdownReason getShutdownReason() {
      return (ShutdownReason)this.shutdown.get();
   }
   ...
   public void reportSaveProgress(@Nonnull World world, int saved, int total) {
      if (this.isShuttingDown()) {
         double progress = MathUtil.round((double)saved / (double)total, 2) * 100.0;
         if (Constants.SINGLEPLAYER) {
            String var10001 = world.getName();
            this.sendSingleplayerSignal("-=|Saving world " + var10001 + " chunks|" + progress);
   ...
   static {
      METRICS_REGISTRY = (new MetricsRegistry()).register("Time", (server) -> Instant.now(), Codec.INSTANT).register("Boot", (server) -> server.boot, Codec.INSTANT).register("BootStart", (server) -> server.bootStart, Codec.LONG).register("Booting", (server) -> server.booting.get(), Codec.BOOLEAN).register("ShutdownReason", (server) -> {
         ShutdownReason reason = (ShutdownReason)server.shutdown.get();
         return reason == null ? null : reason.toString();
      }, Codec.STRING).register("PluginManager", HytaleServer::getPluginManager, PluginManager.METRICS_REGISTRY).register("Config", HytaleServer::getConfig, HytaleServerConfig.CODEC).register("JVM", JVMMetrics.METRICS_REGISTRY);
