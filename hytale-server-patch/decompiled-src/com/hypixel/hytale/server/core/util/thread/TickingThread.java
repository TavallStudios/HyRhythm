...
public abstract class TickingThread implements Runnable {
   ...
   public TickingThread(String threadName, int tps, boolean daemon) {
      this.needsShutdown = new AtomicBoolean(true);
      this.startedFuture = new CompletableFuture();
      this.threadName = threadName;
      this.daemon = daemon;
      this.tps = tps;
      this.tickStepNanos = 1000000000 / tps;
      this.bufferedTickLengthMetricSet = HistoricMetric.builder((long)this.tickStepNanos, TimeUnit.NANOSECONDS).addPeriod(10L, TimeUnit.SECONDS).addPeriod(1L, TimeUnit.MINUTES).addPeriod(5L, TimeUnit.MINUTES).build();
   }

   public void run() {
      try {
         this.onStart();
         this.startedFuture.complete((Object)null);
         ...
         if (this.possibleFailureCause == null) {
            ...
            ((HytaleLogger.Api)HytaleLogger.getLogger().at(Level.SEVERE).withCause(t)).log("Exception in thread %s potentially caused by %s:", this.thread, this.possibleFailureCause);
         }
      }
