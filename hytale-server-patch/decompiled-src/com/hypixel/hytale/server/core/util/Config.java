...
public class Config<T> {
   ...
   public CompletableFuture<T> load() {
      ...
      } else if (!Files.exists(this.path, new LinkOption[0])) {
         this.config = this.codec.getDefaultValue();
         return CompletableFuture.completedFuture(this.config);
      } else {
         return this.loadingConfig = CompletableFuture.supplyAsync(SneakyThrow.sneakySupplier((ThrowableSupplier)(() -> {
            this.config = (T)RawJsonReader.readSync(this.path, this.codec, HytaleLogger.getLogger());
   ...
   public T get() {
      if (this.config == null && this.loadingConfig == null) {
         ...
         return (T)(this.loadingConfig != null ? this.loadingConfig.join() : this.config);
      }
   }
   ...
   public CompletableFuture<Void> save() {
      if (this.config == null && this.loadingConfig == null) {
         ...
         return this.loadingConfig != null ? CompletableFuture.completedFuture((Object)null) : BsonUtil.writeDocument(this.path, this.codec.encode(this.config, new ExtraInfo()));
      }
   }
}
