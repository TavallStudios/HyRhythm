...
public class EnvironmentChunk implements Component<ChunkStore> {
   ...
   public void setColumn(int x, int z, int environmentId) {
      ...
      do {
         ...
         this.decrementBlockCount(id, (long)count);
      } while(maxY < 2147483647);

      this.createIfNotExist(environmentId);
      this.incrementBlockCount(environmentId, 2147483647);
      column.set(environmentId);
   }
   ...
   private byte[] serialize() {
      ...
      try {
         ...
         ObjectIterator var2 = this.counts.int2LongEntrySet().iterator();

         while(var2.hasNext()) {
            Int2LongMap.Entry entry = (Int2LongMap.Entry)var2.next();
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(EnvironmentChunk.class, EnvironmentChunk::new).addField(new KeyedCodec("Data", Codec.BYTE_ARRAY), EnvironmentChunk::deserialize, EnvironmentChunk::serialize)).build();
   }

   public static class BulkWriter {
      private final ColumnWriter[] columnWriters = new ColumnWriter[1024];

      public BulkWriter() {
         for(int i = 0; i < this.columnWriters.length; ++i) {
            this.columnWriters[i] = new ColumnWriter();
         }

      }

      @Nonnull
      public ColumnWriter getColumnWriter(int x, int z) {
         assert x >= 0 && x < 32;
      ...
      public static class ColumnWriter {
         ...
         public void count(int environmentId, int count) {
            ...
            count = (int)((long)count + currentCount);
            this.columnCounts.put(environmentId, (long)count);
         }
