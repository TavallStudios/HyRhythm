...
public class FluidSection implements Component<ChunkStore> {
      ...
      if (!this.lock.validate(stamp)) {
         ...
         try {
            if (this.levelData != null) {
               byteIndex = index >> 1;
               byte byteValue = this.levelData[byteIndex];
               byte var8 = (byte)(byteValue >> (index & 1) * 4 & 15);
               return var8;
            }
   ...
   private void deserialize(@Nonnull byte[] bytes, ExtraInfo extraInfo) {
      ...
      this.typePalette = (ISectionPalette)type.getConstructor().get();
   ...
   public CompletableFuture<CachedPacket<SetFluids>> getCachedPacket() {
      ...
      CompletableFuture<CachedPacket<SetFluids>> future = ref != null ? (CompletableFuture)ref.get() : null;
      if (future != null) {
         return future;
      } else {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(FluidSection.class, FluidSection::new).versioned()).codecVersion(0)).append(new KeyedCodec("Data", Codec.BYTE_ARRAY), FluidSection::deserialize, FluidSection::serialize).add()).build();
   }
}
