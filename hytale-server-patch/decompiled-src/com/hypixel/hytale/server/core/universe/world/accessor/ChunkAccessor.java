   ...
   default boolean performBlockUpdate(int x, int y, int z, boolean allowPartialLoad) {
      ...
      for(int ix = -1; ix < 2; ++ix) {
         ...
         for(int iz = -1; iz < 2; ++iz) {
            int wz = z + iz;
            WorldChunk worldChunk = allowPartialLoad ? this.getNonTickingChunk(ChunkUtil.indexChunkFromBlock(wx, wz)) : this.getChunkIfInMemory(ChunkUtil.indexChunkFromBlock(wx, wz));
            if (worldChunk == null) {
               success = false;
            } else {
