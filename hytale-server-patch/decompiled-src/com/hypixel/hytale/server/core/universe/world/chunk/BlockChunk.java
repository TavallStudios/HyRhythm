...
public class BlockChunk implements Component<ChunkStore> {
   ...
   public void loadFromHolder(@Nonnull Holder<ChunkStore> holder) {
      ...
      if (column != null) {
         ...
         for(int i = 0; i < sections.length; ++i) {
            ...
            this.chunkSections[i] = this.migratedChunkSections != null ? this.migratedChunkSections[i] : (BlockSection)section.ensureAndGetComponent(BlockSection.getComponentType());
         }
   ...
   public BlockSection getSectionAtIndex(int index) {
      if (index >= 0 && index < this.chunkSections.length) {
         ...
         int var10002 = this.chunkSections.length;
         throw new IllegalArgumentException("Section index must >=0 and <" + var10002 + " but was given " + index);
      }
   }
   ...
   public Int2IntMap blockCounts() {
      ...
      for(BlockSection section : this.chunkSections) {
         ...
         while(var6.hasNext()) {
            Int2ShortMap.Entry entry = (Int2ShortMap.Entry)var6.next();
   ...
   private CompletableFuture<CachedPacket<SetChunkHeightmap>> getCachedHeightmapPacket() {
      ...
      CompletableFuture<CachedPacket<SetChunkHeightmap>> future = ref != null ? (CompletableFuture)ref.get() : null;
      if (future != null) {
         return future;
      } else {
   ...
   private CompletableFuture<CachedPacket<SetChunkTintmap>> getCachedTintsPacket() {
      ...
      CompletableFuture<CachedPacket<SetChunkTintmap>> future = ref != null ? (CompletableFuture)ref.get() : null;
      if (future != null) {
         return future;
      } else {
         future = CompletableFuture.supplyAsync(() -> {
   ...
   private CompletableFuture<CachedPacket<SetChunkEnvironments>> getCachedEnvironmentsPacket() {
      ...
      CompletableFuture<CachedPacket<SetChunkEnvironments>> future = ref != null ? (CompletableFuture)ref.get() : null;
      if (future != null) {
         return future;
      } else {
         future = CompletableFuture.supplyAsync(() -> {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BlockChunk.class, BlockChunk::new).versioned()).codecVersion(3)).append(new KeyedCodec("Data", Codec.BYTE_ARRAY), BlockChunk::deserialize, BlockChunk::serialize).add()).build();
   ...
   public static class LoadBlockChunkPacketSystem extends ChunkStore.LoadFuturePacketDataQuerySystem {
      ...
      public void fetch(int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, Store<ChunkStore> store, CommandBuffer<ChunkStore> commandBuffer, PlayerRef player, @Nonnull List<CompletableFuture<ToClientPacket>> results) {
         BlockChunk component = (BlockChunk)archetypeChunk.getComponent(index, this.componentType);
         ...
         for(int y = 0; y < component.chunkSections.length; ++y) {
            ...
            results.add(section.getCachedChunkPacket(component.getX(), y, component.getZ()).exceptionally((throwable) -> {
               if (throwable != null) {
                  ((HytaleLogger.Api)BlockChunk.LOGGER.at(Level.SEVERE).withCause(throwable)).log("Exception while compressing set chunk (%d, %d):", component.x, component.z);
               }
