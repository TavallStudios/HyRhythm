...
public class ChunkUnloadingSystem extends TickingSystem<ChunkStore> implements RunWhenPausedSystem<ChunkStore> {
   ...
   public void tick(float dt, int systemIndex, @Nonnull Store<ChunkStore> store) {
      Data dataResource = (Data)store.getResource(ChunkStore.UNLOAD_RESOURCE);
   ...
   public static void tryUnload(int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
      ...
      World world = ((ChunkStore)store.getExternalData()).getWorld();
      WorldChunk worldChunkComponent = (WorldChunk)archetypeChunk.getComponent(index, WorldChunk.getComponentType());

      assert worldChunkComponent != null;

      Data dataResource = (Data)commandBuffer.getResource(ChunkStore.UNLOAD_RESOURCE);
      ...
      if (chunkVisibility == ChunkTracker.ChunkVisibility.HOT) {
         ...
         boolean shouldKeepLoaded = worldChunkComponent.shouldKeepLoaded() || keepLoaded != null && isChunkInBox(keepLoaded, worldChunkComponent.getX(), worldChunkComponent.getZ());
   ...
   private static void collectTrackers(@Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      Data dataResource = (Data)chunkStore.getResource(ChunkStore.UNLOAD_RESOURCE);

      for(int index = 0; index < archetypeChunk.size(); ++index) {
         ChunkTracker chunkTracker = (ChunkTracker)archetypeChunk.getComponent(index, ChunkTracker.getComponentType());
