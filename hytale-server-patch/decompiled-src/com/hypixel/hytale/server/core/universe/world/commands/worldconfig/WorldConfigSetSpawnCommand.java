...
public class WorldConfigSetSpawnCommand extends AbstractWorldCommand {
   ...
   public WorldConfigSetSpawnCommand() {
      ...
      this.positionArg = this.withOptionalArg("position", "server.commands.world.config.setspawn.position.desc", ArgTypes.RELATIVE_POSITION);
      this.rotationArg = this.withDefaultArg("rotation", "server.commands.world.config.setspawn.rotation.desc", ArgTypes.ROTATION, Vector3f.FORWARD, "server.commands.world.config.setspawn.rotation.default.desc");
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      ...
      if (this.positionArg.provided(context)) {
         RelativeDoublePosition relativePosition = (RelativeDoublePosition)this.positionArg.get(context);
      ...
      if (this.rotationArg.provided(context)) {
         ...
         Ref<EntityStore> playerRef = context.senderAsPlayerRef();
         if (playerRef != null && playerRef.isValid()) {
            HeadRotation headRotationComponent = (HeadRotation)store.getComponent(playerRef, HeadRotation.getComponentType());
      ...
      context.sendMessage(Message.translation("server.universe.setspawn.info").param("posX", DECIMAL.format(position.getX())).param("posY", DECIMAL.format(position.getY())).param("posZ", DECIMAL.format(position.getZ())).param("rotX", DECIMAL.format((double)rotation.getX())).param("rotY", DECIMAL.format((double)rotation.getY())).param("rotZ", DECIMAL.format((double)rotation.getZ())));
   }
}
