...
public class ChunkSavingSystems {
   ...
   public static CompletableFuture<Void> saveChunksInWorld(@Nonnull Store<ChunkStore> store) {
      HytaleLogger logger = ((ChunkStore)store.getExternalData()).getWorld().getLogger();
      Data data = (Data)store.getResource(ChunkStore.SAVE_RESOURCE);
   ...
   public static void tryQueue(int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store) {
      WorldChunk worldChunkComponent = (WorldChunk)archetypeChunk.getComponent(index, WORLD_CHUNK_COMPONENT_TYPE);
   ...
   public static void saveChunk(@Nonnull Ref<ChunkStore> reference, @Nonnull Data data, boolean report, @Nonnull Store<ChunkStore> store) {
      if (!reference.isValid()) {
         ...
         WorldChunk worldChunkComponent = (WorldChunk)store.getComponent(reference, WORLD_CHUNK_COMPONENT_TYPE);
   ...
   public static class WorldRemoved extends StoreSystem<ChunkStore> {
      ...
      public WorldRemoved() {
         this.dependencies = Set.of(new SystemDependency(Order.AFTER, ChunkStore.ChunkLoaderSaverSetupSystem.class));
      }
   ...
   public static class Ticking extends TickingSystem<ChunkStore> implements RunWhenPausedSystem<ChunkStore> {
      ...
      public void tick(float dt, int systemIndex, @Nonnull Store<ChunkStore> store) {
         Data data = (Data)store.getResource(ChunkStore.SAVE_RESOURCE);
         if (data.isSaving && ((ChunkStore)store.getExternalData()).getWorld().getWorldConfig().canSaveChunks()) {
            ...
            World world = ((ChunkStore)store.getExternalData()).getWorld();
            IChunkSaver saver = ((ChunkStore)store.getExternalData()).getSaver();
            ...
            for(int i = 0; i < parallelSaves; ++i) {
               ...
               WorldChunk chunk = (WorldChunk)store.getComponent(reference, ChunkSavingSystems.WORLD_CHUNK_COMPONENT_TYPE);
               chunk.setSaving(true);
               Holder<ChunkStore> holder = store.copySerializableEntity(reference);
               data.toSaveTotal.getAndIncrement();
               CompletableFuture<Void> savingFuture = CompletableFuture.supplyAsync(() -> saver.saveHolder(chunk.getX(), chunk.getZ(), holder)).thenCompose(Function.identity());
   ...
   public static class Data implements Resource<ChunkStore> {
      ...
      public Ref<ChunkStore> poll() {
         Ref<ChunkStore> reference = (Ref)this.queue.poll();
         if (reference == null) {
            return null;
         } else {
      ...
      public CompletableFuture<Void> waitForSavingChunks() {
         return CompletableFuture.allOf((CompletableFuture[])this.chunkSavingFutures.toArray((x$0) -> new CompletableFuture[x$0]));
      }
   }
}
