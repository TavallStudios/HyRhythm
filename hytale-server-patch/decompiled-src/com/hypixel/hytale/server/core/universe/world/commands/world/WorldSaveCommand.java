...
public class WorldSaveCommand extends AbstractAsyncCommand {
   ...
   public WorldSaveCommand() {
      ...
      this.saveAllFlag = this.withFlagArg("all", "server.commands.world.save.all.desc");
   }

   @Nonnull
   protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext context) {
      if ((Boolean)this.saveAllFlag.get(context)) {
         ...
         return CompletableFuture.runAsync(() -> saveWorld(world), world).thenRun(() -> context.sendMessage(Message.translation("server.commands.world.save.savingDone").param("world", world.getName())));
      }
   }
   ...
   private CompletableFuture<Void> saveAllWorlds(@Nonnull CommandContext context) {
      ...
      CompletableFuture[] completableFutures = (CompletableFuture[])Universe.get().getWorlds().values().stream().map((world) -> CompletableFuture.runAsync(() -> saveWorld(world), world)).toArray((x$0) -> new CompletableFuture[x$0]);
