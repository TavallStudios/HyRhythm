...
public class CustomConnectedBlockPattern extends CustomTemplateConnectedBlockPattern {
   ...
   private static boolean checkPatternRuleAgainstBlockType(@Nonnull CustomTemplateConnectedBlockRuleSet placedRuleset, @Nonnull CustomConnectedBlockTemplateAsset template, @Nonnull String block, @Nonnull ConnectedBlockPatternRule rule, @Nonnull String blockToTest, RotationTuple rotationToCheckUnrotated, int fillerToCheckUnrotated) {
      if (!rule.getFaceTags().getDirections().isEmpty()) {
         BlockType checkingBlockType = (BlockType)BlockType.getAssetMap().getAsset(blockToTest);
         ConnectedBlockRuleSet checkingRuleSet = checkingBlockType.getConnectedBlockRuleSet();
         if (!(checkingRuleSet instanceof CustomTemplateConnectedBlockRuleSet)) {
            return !rule.isInclude();
         }

         CustomTemplateConnectedBlockRuleSet checkingConnectedBlockRuleSet = (CustomTemplateConnectedBlockRuleSet)checkingRuleSet;
         int index = BlockType.getAssetMap().getIndex(blockToTest);
         Set shapeNames = checkingConnectedBlockRuleSet.getShapesForBlockType(index);
         ...
         for(String shapeName : shapeNames) {
            if (template.connectsToOtherMaterials || placedRuleset.getShapeNameToBlockPatternMap().equals(checkingConnectedBlockRuleSet.getShapeNameToBlockPatternMap())) {
               ConnectedBlockShape blockToCheckConnectedBlockShape = (ConnectedBlockShape)checkingTemplateAsset.connectedBlockShapes.get(shapeName);
               ...
               for(Map.Entry<Vector3i, HashSet<String>> ruleFaceTag : ruleFaceTags.entrySet()) {
                  Vector3i adjustedDirectionOfPattern = Rotation.rotate(((Vector3i)ruleFaceTag.getKey()).clone(), Rotation.None.subtract(rotationToCheckUnrotated.yaw()), Rotation.None);

                  for(String faceTag : (HashSet)ruleFaceTag.getValue()) {
      ...
      if (!rule.getShapeBlockTypeKeys().isEmpty()) {
         ...
         for(String shapeName : checkingConnectedBlockRuleSet.getShapesForBlockType(index)) {
            if ((template.connectsToOtherMaterials || placedRuleset.getShapeNameToBlockPatternMap().equals(checkingConnectedBlockRuleSet.getShapeNameToBlockPatternMap())) && rule.getShapeBlockTypeKeys().contains(new BlockPattern.BlockEntry(shapeName, rotationToCheckUnrotated.index(), fillerToCheckUnrotated))) {
               return rule.isInclude();
            }
         }
      }
            ...
            for(int i = 0; i < rotations.size(); ++i) {
               ...
               for(ConnectedBlockPatternRule ruleToMatch : this.rulesToMatch) {
                  ...
                  if (ruleToMatch.getPlacementNormals() != null) {
                     ...
                     boolean patternMatches = checkPatternRuleAgainstBlockType(connectedBlockRuleset, shapeTemplate, blockType.getId(), ruleToMatch, blockToCheckUnrotated, rotationToCheckUnrotated, fillerToCheckUnrotated);
                     if (!patternMatches) {
                        continue label100;
                     }
                  }
               ...
               BlockPattern resultBlockPattern = (BlockPattern)connectedBlockRuleset.getShapeNameToBlockPatternMap().get(shapeName);
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(CustomConnectedBlockPattern.class, CustomConnectedBlockPattern::new).append(new KeyedCodec("TransformRulesToOrientation", Codec.BOOLEAN, false), (o, transformRulesToPlacedOrientation) -> o.transformRulesToOrientation = transformRulesToPlacedOrientation, (o) -> o.transformRulesToOrientation).documentation("If the rules should be transformed to the current orientation of the block.").add()).append(new KeyedCodec("YawToApplyAddReplacedBlockType", Rotation.CODEC, false), (o, yawToApplyAddReplacedBlockType) -> o.yawToApplyAddReplacedBlockType = yawToApplyAddReplacedBlockType, (o) -> o.yawToApplyAddReplacedBlockType).documentation("Apply an additional Yaw to the resulting BlockType represented by this shape. This allows your replacement to be offset from your original placement").add()).append(new KeyedCodec("RequireFaceTagsMatchingRoll", Codec.BOOLEAN, false), (o, requireFaceTagsMatchingRoll) -> o.requireFaceTagsMatchingRoll = requireFaceTagsMatchingRoll, (o) -> o.requireFaceTagsMatchingRoll).documentation("Adds Roll comparison to face tag matching in patterns below").add()).append(new KeyedCodec("AllowedPatternTransformations", PatternRotationDefinition.CODEC, false), (o, patternRotations) -> o.patternRotationDefinition = patternRotations, (o) -> o.patternRotationDefinition).documentation("Will create additional generated patterns that are variants of this pattern, but rotated/mirrored/flipped to achieve different results. A common example of this is the Fence, which should its resulting shape based on the rotation of its pattern (fence corner rotates depending on which two sides have the corner fence shape)").add()).append(new KeyedCodec("RulesToMatch", new ArrayCodec(ConnectedBlockPatternRule.CODEC, (x$0) -> new ConnectedBlockPatternRule[x$0]), true), (o, matchingPatterns) -> o.rulesToMatch = matchingPatterns, (o) -> o.rulesToMatch).documentation("All rules must match in order for the pattern to match").add()).append(new KeyedCodec("OnlyOnPlacement", new BooleanCodec(), false), (o, onlyOnPlacement) -> o.onlyOnPlacement = onlyOnPlacement, (o) -> o.onlyOnPlacement).documentation("If true, this pattern will only be checked when the block is first placed.").add()).append(new KeyedCodec("OnlyOnUpdate", new BooleanCodec(), false), (o, onlyOnUpdate) -> o.onlyOnUpdate = onlyOnUpdate, (o) -> o.onlyOnUpdate).documentation("If true, this pattern will only be checked when the block is updated by neighboring block changes.").add()).build();
      random = new Random();
   }
}
