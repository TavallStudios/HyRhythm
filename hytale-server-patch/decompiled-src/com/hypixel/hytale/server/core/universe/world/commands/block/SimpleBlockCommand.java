...
public abstract class SimpleBlockCommand extends AbstractWorldCommand {
   ...
   public SimpleBlockCommand(@Nonnull String name, @Nonnull String description) {
      ...
      this.coordsArg = this.withRequiredArg("x y z", "server.commands.block.set.arg.coords", ArgTypes.RELATIVE_BLOCK_POSITION);
   }

   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      CommandSender sender = context.sender();
      RelativeIntPosition coords = (RelativeIntPosition)context.get(this.coordsArg);
