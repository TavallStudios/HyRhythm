...
public interface IChunkAccessorSync<WorldChunk extends BlockAccessor> {
   ...
   default BlockType getBlockType(int x, int y, int z) {
      ...
      if (worldChunk == null) {
         ...
         return (BlockType)BlockType.getAssetMap().getAsset(blockId);
      }
   }
   ...
   default BlockPosition getBaseBlock(@Nonnull BlockPosition position) {
      ...
      return filler != 0 ? new BlockPosition(position.x - FillerBlockUtil.unpackX(filler), position.y - FillerBlockUtil.unpackY(filler), position.z - FillerBlockUtil.unpackZ(filler)) : position;
   }
