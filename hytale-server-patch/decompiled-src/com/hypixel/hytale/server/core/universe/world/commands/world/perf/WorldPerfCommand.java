...
public class WorldPerfCommand extends AbstractWorldCommand {
      ...
      if (context.sender() instanceof Player) {
         for(int i = 0; i < periods.length; ++i) {
            ...
            if (showDelta) {
               ...
               msg.insert(Message.translation("server.commands.world.perf.period").param("length", length).param("padding", padding).param("value", value).insert("\n"));
            } else {
               msg.insert(Message.translation("server.commands.world.perf.tpsTime").param("time", length).param("tps", FormatUtil.simpleFormat(min, average, max, (d1) -> tpsFromDelta(d1, (long)tickStepNanos), 2)).insert("\n"));
            }
         }
      } else {
         String tickLimitFormatted = FormatUtil.simpleTimeUnitFormat((long)tickStepNanos, TimeUnit.NANOSECONDS, 3);
         ...
         for(int i = 0; i < periods.length; ++i) {
            ...
            if (showDelta) {
               ...
               msg.insert(Message.translation("server.commands.world.perf.tpsMinMaxMetric").param("time", length).param("min", tpsFromDelta(max, (long)tickStepNanos)).param("avg", tpsFromDelta(average, (long)tickStepNanos)).param("max", tpsFromDelta(min, (long)tickStepNanos)).insert("\n"));
            }

            if (showAll) {
               msg.insert(Message.translation("server.commands.world.perf.deltaMinMaxMetric").param("time", length).param("min", min).param("avg", (long)average).param("max", max).insert("\n"));
            }
         }
      }
   ...
   public static double tpsFromDelta(long delta, long min) {
      ...
      return (double)Math.round(1.0 / (double)adjustedDelta * 1.0E9 * 1000.0) / 1000.0;
   }

   public static double tpsFromDelta(double delta, long min) {
      double adjustedDelta = delta;
      if (delta < (double)min) {
         adjustedDelta = (double)min;
      }

      return (double)Math.round(1.0 / adjustedDelta * 1.0E9 * 1000.0) / 1000.0;
   }
}
