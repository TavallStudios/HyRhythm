...
public class DiskResourceStorageProvider implements IResourceStorageProvider {
   ...
   public <T extends WorldProvider> IResourceStorage getResourceStorage(@Nonnull World world) {
      ...
      if (resolved == null) {
         ...
         return new DiskResourceStorage(resolved);
      }
   }
   ...
   public static void migrateFiles(@Nonnull World world) {
      ...
      if (Files.exists(entityStorePath, new LinkOption[0])) {
         try {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(DiskResourceStorageProvider.class, DiskResourceStorageProvider::new).append(new KeyedCodec("Path", Codec.STRING), (o, s) -> o.path = s, (o) -> o.path).add()).build();
   }
   ...
   public static class DiskResourceStorage implements IResourceStorage {
      ...
      public <T extends Resource<ECS_TYPE>, ECS_TYPE> CompletableFuture<T> load(@Nonnull Store<ECS_TYPE> store, @Nonnull ComponentRegistry.Data<ECS_TYPE> data, @Nonnull ResourceType<ECS_TYPE, T> resourceType) {
         ...
         return codec == null ? CompletableFuture.completedFuture(data.createResource(resourceType)) : CompletableFuture.supplyAsync(SneakyThrow.sneakySupplier((ThrowableSupplier)(() -> {
            ...
            if (attributes.size() == 0L) {
               ...
               try {
                  T resource = (T)((Resource)RawJsonReader.readSync(file, codec, LOGGER));
