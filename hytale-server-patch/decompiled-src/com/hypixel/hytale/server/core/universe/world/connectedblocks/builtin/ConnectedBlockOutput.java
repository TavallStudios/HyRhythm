...
public class ConnectedBlockOutput {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ConnectedBlockOutput.class, ConnectedBlockOutput::new).append(new KeyedCodec("State", Codec.STRING), (output, state) -> output.state = state, (output) -> output.state).documentation("An optional state definition to apply to the base block type").add()).append(new KeyedCodec("Block", Codec.STRING), (output, blockTypeKey) -> output.blockTypeKey = blockTypeKey, (output) -> output.blockTypeKey).documentation("An optional block ID to use instead of the base block type").add()).build();
   }
}
