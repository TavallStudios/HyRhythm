...
public class BlockStateModule extends JavaPlugin {
   ...
   public <T extends BlockState> BlockStateRegistration registerBlockState(@Nonnull Class<T> clazz, @Nonnull String key, Codec<T> codec) {
      return this.registerBlockState(clazz, key, codec, (Class)null, (Codec)null);
   }
   ...
   public <T extends BlockState, D extends StateData> BlockStateRegistration registerBlockState(@Nonnull Class<T> clazz, @Nonnull String key, @Nullable Codec<T> codec, Class<D> dataClass, @Nullable Codec<D> dataCodec) {
      if (this.isDisabled()) {
         ...
         if (codec != null) {
            componentType = this.getChunkStoreRegistry().registerComponent(clazz, key, (BuilderCodec)codec);
         } else {
         ...
         this.getChunkStoreRegistry().registerSystem(new LegacyLateInitBlockStateSystem(componentType), true);
         this.getChunkStoreRegistry().registerSystem(new LegacyBlockStateHolderSystem(componentType), true);
         this.getChunkStoreRegistry().registerSystem(new LegacyBlockStateRefSystem(componentType), true);
         if (TickableBlockState.class.isAssignableFrom(clazz)) {
            this.getChunkStoreRegistry().registerSystem(new LegacyTickingBlockStateSystem(componentType), true);
         }

         if (SendableBlockState.class.isAssignableFrom(clazz)) {
            this.getChunkStoreRegistry().registerSystem(new LegacyLoadPacketBlockStateSystem(componentType), true);
            this.getChunkStoreRegistry().registerSystem(new LegacyUnloadPacketBlockStateSystem(componentType), true);
         }
   ...
   public <T extends BlockState, D extends StateData> void unregisterBlockState(Class<T> clazz, @Nullable Class<D> dataClass) {
      if (!HytaleServer.get().isShuttingDown()) {
         ...
         ChunkStore.REGISTRY.unregisterComponent((ComponentType)this.classToComponentType.remove(clazz));
         if (dataClass != null) {
   ...
   public <T extends BlockState> T createBlockState(Class<T> clazz, WorldChunk chunk, Vector3i pos, BlockType blockType) {
      String id = (String)BlockState.CODEC.getIdFor(clazz);
   ...
   public <T extends BlockState> ComponentType<ChunkStore, T> getComponentType(@Nullable Class<T> entityClass) {
      if (this.isDisabled()) {
         ...
         return entityClass == null ? null : (ComponentType)this.classToComponentType.get(entityClass);
      }
   }
   ...
   public static class LegacyBlockStateHolderSystem<T extends BlockState> extends HolderSystem<ChunkStore> implements DisableProcessingAssert {
      ...
      public void onEntityRemoved(@Nonnull Holder<ChunkStore> holder, @Nonnull RemoveReason reason, @Nonnull Store<ChunkStore> store) {
         T blockState = (T)(holder.getComponent(this.componentType));
         switch (reason) {
            case REMOVE:
   ...
   public static class LegacyBlockStateRefSystem<T extends BlockState> extends RefSystem<ChunkStore> implements DisableProcessingAssert {
      ...
      public void onEntityAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         T blockState = (T)(store.getComponent(ref, this.componentType));
         int index = blockState.getIndex();
         WorldChunk chunk = blockState.getChunk();
         if (chunk == null) {
            Vector3i position = blockState.getBlockPosition();
            int chunkX = MathUtil.floor((double)position.getX()) >> 5;
            int chunkZ = MathUtil.floor((double)position.getZ()) >> 5;
   ...
   public static class LegacyLateInitBlockStateSystem<T extends BlockState> extends EntityTickingSystem<ChunkStore> implements DisableProcessingAssert {
      ...
      public LegacyLateInitBlockStateSystem(ComponentType<ChunkStore, T> componentType) {
         this.componentType = componentType;
         this.query = Query.<ChunkStore>and(componentType, BlockModule.BlockStateInfo.getComponentType());
      }
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         T blockStateComponent = (T)(archetypeChunk.getComponent(index, this.componentType));

         assert blockStateComponent != null;

         BlockModule.BlockStateInfo blockStateInfoComponent = (BlockModule.BlockStateInfo)archetypeChunk.getComponent(index, BlockModule.BlockStateInfo.getComponentType());
         ...
         try {
            if (!blockStateComponent.initialized.get()) {
               ...
               WorldChunk worldChunkComponent = (WorldChunk)chunkStore.getComponent(blockStateInfoComponent.getChunkRef(), WorldChunk.getComponentType());
            ...
            ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(throwable)).log("Exception while re-init BlockState! Removing!! %s", blockStateComponent);
            commandBuffer.removeEntity(archetypeChunk.getReferenceTo(index), RemoveReason.REMOVE);
         }
   ...
   public static class LegacyTickingBlockStateSystem<T extends BlockState> extends EntityTickingSystem<ChunkStore> implements DisableProcessingAssert, MetricSystem<ChunkStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         T blockState = (T)(archetypeChunk.getComponent(index, this.componentType));

         try {
            ((TickableBlockState)blockState).tick(dt, index, archetypeChunk, store, commandBuffer);
         } catch (Throwable throwable) {
            ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(throwable)).log("Exception while ticking BlockState! Removing!! %s", blockState);
            commandBuffer.removeEntity(archetypeChunk.getReferenceTo(index), RemoveReason.REMOVE);
         }
      ...
      static {
         METRICS_REGISTRY = (new MetricsRegistry()).register("ComponentType", (o) -> o.componentType.getTypeClass().toString(), Codec.STRING);
      }
   }
