   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      ...
      CompletableFutureUtil._catch(world.getChunkStore().getChunkSectionReferenceAsync(chunkX, chunkY, chunkZ).thenAcceptAsync((chunk) -> {
         ...
         if (blockSection == null) {
            ...
            Vector3d offset = new Vector3d((double)ChunkUtil.minBlock(chunkX), (double)ChunkUtil.minBlock(chunkY), (double)ChunkUtil.minBlock(chunkZ));
            ...
            for(int idx = 0; idx < 32768; ++idx) {
               ...
               if (blockType != null) {
                  ...
                  Vector3d pos = new Vector3d((double)bx, (double)by, (double)bz);
                  pos.add(0.5, 0.5, 0.5);
                  pos.add(offset);
                  RotationTuple rotation = blockSection.getRotation(idx);
                  if (rotation.index() != 0) {
                     Vector3f colour = new Vector3f();
                     colour.x = (float)rotation.yaw().ordinal() / ((float)Rotation.VALUES.length - 1.0F);
                     colour.y = (float)rotation.pitch().ordinal() / ((float)Rotation.VALUES.length - 1.0F);
                     colour.z = (float)rotation.roll().ordinal() / ((float)Rotation.VALUES.length - 1.0F);
