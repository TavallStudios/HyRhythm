...
public abstract class AbstractCachedAccessor {
   ...
   protected void init(ComponentAccessor<ChunkStore> commandBuffer, int cx, int cy, int cz, int radius) {
      ...
      for(int i = 0; i < this.sectionComponents.length; ++i) {
         ...
         if (sectionComps != null && sectionComps.length >= size3d) {
            Arrays.fill(sectionComps, (Object)null);
         } else {
   ...
   public Ref<ChunkStore> getChunk(int cx, int cz) {
      ...
      if (index >= 0 && index < this.chunks.length) {
         ...
         if (chunk == null) {
            this.chunks[index] = chunk = ((ChunkStore)this.commandBuffer.getExternalData()).getChunkReference(ChunkUtil.indexChunk(cx, cz));
         }

         return chunk;
      } else {
         return ((ChunkStore)this.commandBuffer.getExternalData()).getChunkReference(ChunkUtil.indexChunk(cx, cz));
      }
   }
   ...
   public Ref<ChunkStore> getSection(int cx, int cy, int cz) {
      if (cy >= 0 && cy < 10) {
         ...
         if (index >= 0 && index < this.sections.length) {
            ...
            if (section == null) {
               this.sections[index] = section = ((ChunkStore)this.commandBuffer.getExternalData()).getChunkSectionReference(this.commandBuffer, cx, cy, cz);
            }

            return section;
         } else {
            return ((ChunkStore)this.commandBuffer.getExternalData()).getChunkSectionReference(this.commandBuffer, cx, cy, cz);
         }
      } else {
         return null;
      }
   ...
   protected <T extends Component<ChunkStore>> T getComponentSection(int cx, int cy, int cz, int typeIndex, @Nonnull ComponentType<ChunkStore, T> componentType) {
      ...
      int index = x + z * this.length + y * this.length * this.length;
      if (index >= 0 && index < this.sections.length) {
         T comp = this.sectionComponents[typeIndex][index];
         if (comp == null) {
         ...
         Ref<ChunkStore> section = this.getSection(cx, cy, cz);
         return (T)(section == null ? null : this.commandBuffer.getComponent(section, componentType));
      }
   }
}
