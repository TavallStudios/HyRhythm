   ...
   private void spawnBlocksRow(@Nonnull World world, @Nonnull Vector3d origin, @Nonnull Vector3i direction, @Nonnull List<BlockType> blockTypes) {
      ...
      for(int x = 0; x < blockTypes.size(); x += step) {
         ...
         for(int i = 0; i < step; ++i) {
            BlockType blockType = (BlockType)blockTypes.get(i + x);
            Box boundingBox = ((BlockBoundingBoxes)boundingBoxes.getAsset(blockType.getHitboxTypeIndex())).get(0).getBoundingBox();
            double dimension = Math.ceil(boundingBox.dimension(axis));
            distance += Math.floor(dimension) + 1.0;
            Vector3i blockPos = origin.clone().add(direction.clone().scale(distance)).add(Rotation.Ninety.rotateY(direction, new Vector3i()).scale(x / step * 2)).toVector3i();
