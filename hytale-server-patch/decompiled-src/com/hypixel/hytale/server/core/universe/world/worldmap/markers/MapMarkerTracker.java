...
public class MapMarkerTracker {
   ...
   public void sendMapMarker(MapMarker marker) {
      ...
      MapMarker oldMarker = (MapMarker)this.sentToClientById.get(marker.id);
   ...
   private boolean doesMarkerNeedNetworkUpdate(@Nullable MapMarker oldMarker, MapMarker newMarker) {
      if (oldMarker == null) {
         ...
         double yawDistance = (double)Math.abs(oldMarker.transform.orientation.yaw - newMarker.transform.orientation.yaw);
   ...
   public void copyFrom(@Nonnull MapMarkerTracker other) {
      for(Map.Entry<String, MapMarker> entry : other.sentToClientById.entrySet()) {
         this.sentToClientById.put((String)entry.getKey(), new MapMarker((MapMarker)entry.getValue()));
      }
