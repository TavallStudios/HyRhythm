...
public class EntityChunk implements Component<ChunkStore> {
   ...
   public void addEntityHolder(@Nonnull Holder<EntityStore> holder) {
      this.entityHolders.add((Holder)Objects.requireNonNull(holder));
   ...
   public void addEntityReference(@Nonnull Ref<EntityStore> reference) {
      this.entityReferences.add((Ref)Objects.requireNonNull(reference));
      this.markNeedsSaving();
   }
   ...
   public Holder<EntityStore>[] takeEntityHolders() {
      if (this.entityHolders.isEmpty()) {
         ...
         Holder<EntityStore>[] holders = (Holder[])this.entityHolders.toArray((x$0) -> new Holder[x$0]);
   ...
   public Ref<EntityStore>[] takeEntityReferences() {
      if (this.entityReferences.isEmpty()) {
         ...
         Ref<EntityStore>[] holders = (Ref[])this.entityReferences.toArray((x$0) -> new Ref[x$0]);
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(EntityChunk.class, EntityChunk::new).addField(new KeyedCodec("Entities", new ArrayCodec(new StoredCodec(EntityStore.HOLDER_CODEC_KEY), (x$0) -> new Holder[x$0])), (entityChunk, array) -> {
         ...
         if (entityChunk.entityReferences.isEmpty()) {
            ...
            return index == array.length ? array : (Holder[])Arrays.copyOfRange(array, 0, index);
         }
      })).build();
   }
