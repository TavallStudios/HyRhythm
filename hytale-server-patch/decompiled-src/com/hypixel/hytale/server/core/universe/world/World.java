...
public class World extends TickingThread implements Executor, ExecutorMetricsRegistry.ExecutorMetric, ChunkAccessor<WorldChunk>, IWorldChunks, IMessageReceiver {
   ...
   public void stopIndividualWorld(Map<UUID, PlayerRef> players) {
      ...
      if (defaultWorld != null && !defaultWorld.equals(this)) {
         ...
         if (this.isInThread()) {
            ...
            CompletableFuture.supplyAsync(() -> this.drainPlayersTo(defaultWorld, players.values()), this).thenCompose((v) -> v).join();
         }
      } else {
         String message;
         if (this.getFailureException() == null) {
            ...
            message = "The world you were on has crashed (possibly caused by " + String.valueOf(this.getPossibleFailureCause()) + ")";
         }
   ...
   private void deleteWorldFromDisk() throws IOException {
      ...
      String var10000 = String.valueOf(filename);
      String noCollisionsName = var10000 + "_del" + UUID.randomUUID().toString().substring(0, 8);
   ...
   public List<Player> getPlayers() {
      if (!this.isInThread()) {
         ...
         this.entityStore.getStore().forEachChunk(Player.getComponentType(), (BiConsumer)((archetypeChunk, commandBuffer) -> {
   ...
   public <T extends Entity> T addEntity(T entity, @Nonnull Vector3d position, @Nullable Vector3f rotation, @Nonnull AddReason reason) {
      if (!EntityModule.get().isKnown(entity)) {
         ...
         throw new IllegalArgumentException("Entity already has a valid EntityReference: " + String.valueOf(((Entity)entity).getReference()));
                        ...
                        for(int y = 0; y < 32; ++y) {
                           ...
                           for(int z = 0; z < 32; ++z) {
                              for(int x = 0; x < 32; ++x) {
                                 ...
                                 blockValidator.accept(worldX, worldY, worldZ, blockId, 0, 1.0F, holder, blockPhys != null ? blockPhys.get(x, y, z) : 0, rotation, filler, (Object)null);
                                    ...
                                    <undefinedtype> fetcher = new FillerBlockUtil.FillerFetcher<BlockSection, ChunkStore>() {
                                       public int getBlock(BlockSection blockSection, ChunkStore chunkStore, int x, int y, int z) {
                                          if (x >= 0 && y >= 0 && z >= 0 && x < 32 && y < 32 && z < 32) {
                                             ...
                                             return blocks == null ? -2147483648 : blocks.get(x, y, z);
                                          }
                                       }
                                       ...
                                       public int getFiller(BlockSection blockSection, ChunkStore chunkStore, int x, int y, int z) {
                                          if (x >= 0 && y >= 0 && z >= 0 && x < 32 && y < 32 && z < 32) {
                                             ...
                                             Ref<ChunkStore> ref = (Ref)refFuture.join();
                                             BlockSection blocks = (BlockSection)chunkStore.getStore().getComponent(ref, BlockSection.getComponentType());
                                             return blocks == null ? -2147483648 : blocks.getFiller(x, y, z);
                                          }
                                       }
                                       ...
                                       public int getRotationIndex(BlockSection blockSection, ChunkStore chunkStore, int x, int y, int z) {
                                          if (x >= 0 && y >= 0 && z >= 0 && x < 32 && y < 32 && z < 32) {
                                             ...
                                             Ref<ChunkStore> ref = (Ref)refFuture.join();
                                             BlockSection blocks = (BlockSection)chunkStore.getStore().getComponent(ref, BlockSection.getComponentType());
                                             return blocks == null ? -2147483648 : blocks.getRotationIndex(x, y, z);
                                          }
                                       }
                                    };
