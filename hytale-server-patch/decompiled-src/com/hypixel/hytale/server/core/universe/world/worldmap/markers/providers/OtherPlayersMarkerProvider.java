         ...
         for(PlayerRef otherPlayer : world.getPlayerRefs()) {
            if (!otherPlayer.getUuid().equals(player.getUuid())) {
               ...
               if (collector.isInViewDistance(otherPos) && (playerMapFilter == null || !playerMapFilter.test(otherPlayer))) {
                  ...
                  MapMarker marker = (new MapMarkerBuilder(markerId, "Player.png", otherPlayerTransform)).withCustomName(otherPlayer.getUsername()).withComponent(playerMarker).withComponent(heightDeltaComponent).build();
