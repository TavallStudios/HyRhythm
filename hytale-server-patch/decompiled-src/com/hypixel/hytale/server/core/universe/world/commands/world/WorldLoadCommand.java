...
public class WorldLoadCommand extends CommandBase {
   ...
   protected void executeSync(@Nonnull CommandContext context) {
      ...
      if (Universe.get().getWorld(name) != null) {
         ...
         CompletableFutureUtil._catch(Universe.get().loadWorld(name).thenRun(() -> sender.sendMessage(Message.translation("server.universe.loadWorld.worldCreated").param("worldName", name))).exceptionally((throwable) -> {
