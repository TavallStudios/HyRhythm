...
public class WorldPerfGraphCommand extends AbstractWorldCommand {
   ...
   public WorldPerfGraphCommand() {
      ...
      this.widthArg = this.withDefaultArg("width", "server.commands.world.perf.graph.width.desc", ArgTypes.INTEGER, 100, "server.commands.world.perf.graph.width.default");
      this.heightArg = this.withDefaultArg("height", "server.commands.world.perf.graph.height.desc", ArgTypes.INTEGER, 10, "server.commands.world.perf.graph.height.default");
   }

   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      Integer width = (Integer)this.widthArg.get(context);
      Integer height = (Integer)this.heightArg.get(context);
      ...
      for(int i = 0; i < periods.length; ++i) {
         ...
         StringUtil.generateGraph(sb, width, height, startNano - period, startNano, 0.0, (double)world.getTps(), (value) -> String.valueOf(MathUtil.round(value, 2)), historyTimestamps.length, (ii) -> historyTimestamps[ii], (ii) -> WorldPerfCommand.tpsFromDelta(historyValues[ii], (long)world.getTickStepNanos()));
