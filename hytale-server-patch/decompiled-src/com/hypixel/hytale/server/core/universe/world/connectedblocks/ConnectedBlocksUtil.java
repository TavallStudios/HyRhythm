...
public class ConnectedBlocksUtil {
   ...
   public static void setConnectedBlockAndNotifyNeighbors(int blockTypeId, @Nonnull RotationTuple blockTypeRotation, @Nonnull Vector3i placementNormal, @Nonnull Vector3i blockPosition, @Nonnull WorldChunk worldChunkComponent, @Nonnull BlockChunk blockChunkComponent) {
      ...
      BlockType blockType = (BlockType)BlockType.getAssetMap().getAsset(blockTypeId);
      if (blockType != null) {
         ...
         if (blockType.getConnectedBlockRuleSet() != null && filler == 0) {
            ...
            Optional<ConnectedBlockResult> foundPattern = getDesiredConnectedBlockType(worldChunkComponent.getWorld(), coordinate, blockType, rotationIndex, placementNormal, true);
            if (foundPattern.isPresent() && (!((ConnectedBlockResult)foundPattern.get()).blockTypeKey().equals(blockType.getId()) || ((ConnectedBlockResult)foundPattern.get()).rotationIndex != rotationIndex)) {
               ConnectedBlockResult result = (ConnectedBlockResult)foundPattern.get();
               int id = BlockType.getAssetMap().getIndex(result.blockTypeKey());
               int rotation = result.rotationIndex();
               worldChunkComponent.setBlock(coordinate.x, coordinate.y, coordinate.z, id, (BlockType)BlockType.getAssetMap().getAsset(id), rotation, 0, settings);
            }
         }
   ...
   private static void updateNeighborsWithDepth(@Nonnull WorldChunk worldChunkComponent, @Nonnull Vector3i startCoordinate, @Nonnull Vector3i placementNormal, int settings) {
      ...
      while(!queue.isEmpty()) {
         ...
         Map<Vector3i, ConnectedBlockResult> desiredChanges = new Object2ObjectOpenHashMap();
         ...
         while(true) {
            ...
            for(Map.Entry<Vector3i, ObjectIntPair<String>> additionalEntry : connectedBlockResult.getAdditionalConnectedBlocks().entrySet()) {
               Vector3i offset = (Vector3i)additionalEntry.getKey();
               ...
               BlockType additionalBlock = (BlockType)BlockType.getAssetMap().getAsset(additionalBlockId);
                     ...
                     if (chunk != null) {
                        ...
                        if (blockChunk != null) {
                           ...
                           if (blockSection != null) {
                              ...
                              BlockType neighborBlockType = (BlockType)BlockType.getAssetMap().getAsset(neighborBlockId);
                              if (neighborBlockType != null) {
                                 ...
                                 if (ruleSet != null && !ruleSet.onlyUpdateOnPlacement()) {
                                    ...
                                    Optional<ConnectedBlockResult> output = getDesiredConnectedBlockType(world, coordinate, neighborBlockType, existingRotation, placementNormal, false);
                                    if (output.isPresent() && (!neighborBlockType.getId().equals(((ConnectedBlockResult)output.get()).blockTypeKey()) || ((ConnectedBlockResult)output.get()).rotationIndex != existingRotation)) {
   ...
   public static Optional<ConnectedBlockResult> getDesiredConnectedBlockType(@Nonnull World world, @Nonnull Vector3i coordinate, @Nonnull BlockType currentBlockType, int currentRotation, @Nonnull Vector3i placementNormal, boolean isPlacement) {
      ...
      return ruleSet == null ? Optional.empty() : ruleSet.getConnectedBlockType(world, coordinate, currentBlockType, currentRotation, placementNormal, isPlacement);
   }
   ...
   public static final class ConnectedBlockResult {
      ...
      public int hashCode() {
         return Objects.hash(new Object[]{this.blockTypeKey, this.rotationIndex, this.additionalConnectedBlocks});
      }

      public String toString() {
         String var10000 = this.blockTypeKey;
         return "ConnectedBlockResult[blockTypeKey=" + var10000 + ", rotationIndex=" + this.rotationIndex + ", additionalConnectedBlocks=" + String.valueOf(this.additionalConnectedBlocks) + "]";
      }
   }
}
