...
public abstract class BlockState implements Component<ChunkStore> {
   ...
   public static final CodecMapCodec<BlockState> CODEC = new CodecMapCodec<BlockState>("Type");
   ...
   public Vector3d getCenteredBlockPosition() {
      ...
      return blockCenter.add((double)this.getBlockX(), (double)this.getBlockY(), (double)this.getBlockZ());
   }
   ...
   public Component<ChunkStore> clone() {
      ...
      return (Component)CODEC.decode(document, (ExtraInfo)ExtraInfo.THREAD_LOCAL.get());
   }
   ...
   public Holder<ChunkStore> toHolder() {
      if (this.reference != null && this.reference.isValid() && this.chunk != null) {
         ...
         Holder<ChunkStore> holder = ChunkStore.REGISTRY.newHolder();
         ComponentType<ChunkStore, ? extends BlockState> componentType = BlockStateModule.get().getComponentType(this.getClass());
         if (componentType == null) {
   ...
   public static BlockState getBlockState(@Nullable Ref<ChunkStore> reference, @Nonnull ComponentAccessor<ChunkStore> componentAccessor) {
      if (reference == null) {
         ...
         return componentType == null ? null : (BlockState)componentAccessor.getComponent(reference, componentType);
      }
   }
   ...
   public static BlockState getBlockState(int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk) {
      ...
      return componentType == null ? null : (BlockState)archetypeChunk.getComponent(index, componentType);
   }
   ...
   public static BlockState getBlockState(@Nonnull Holder<ChunkStore> holder) {
      ...
      return componentType == null ? null : (BlockState)holder.getComponent(componentType);
   }
   ...
   private static <C extends Component<ChunkStore>, T extends C> ComponentType<ChunkStore, T> findComponentType(@Nonnull Archetype<ChunkStore> archetype, @Nonnull Class<C> entityClass) {
      for(int i = archetype.getMinIndex(); i < archetype.length(); ++i) {
         ComponentType<ChunkStore, ? extends Component<ChunkStore>> componentType = archetype.get(i);
   ...
   static {
      BASE_CODEC = ((BuilderCodec.Builder)BuilderCodec.abstractBuilder(BlockState.class).addField(new KeyedCodec("Position", Vector3i.CODEC), (entity, o) -> entity.position = o, (entity) -> entity.position != null && !Vector3i.ZERO.equals(entity.position) ? entity.position : null)).build();
