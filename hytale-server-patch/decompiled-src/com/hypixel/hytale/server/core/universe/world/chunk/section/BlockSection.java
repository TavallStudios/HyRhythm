...
public class BlockSection implements Component<ChunkStore> {
   ...
   public boolean setTicking(int blockIdx, boolean ticking) {
      ...
      long var15 = this.chunkSectionLock.writeLock();
      ...
      try {
         ...
         this.chunkSectionLock.unlockWrite(var15);
      }
   ...
   public void preTick(Instant gameTime) {
      TickRequest request;
      while(!this.tickRequests.isEmpty() && (request = (TickRequest)this.tickRequests.first()).requestedGameTime.isBefore(gameTime)) {
   ...
   public void deserialize(ToIntFunction<ByteBuf> keyDeserializer, @Nonnull ByteBuf buf, int version) {
      ...
      for(BlockMigration migration = (BlockMigration)blockMigrationMap.get(blockMigrationVersion); migration != null; migration = (BlockMigration)blockMigrationMap.get(blockMigrationVersion)) {
      ...
      if (version <= 4) {
         ISectionPalette tempSection = (ISectionPalette)typeEnum.getConstructor().get();
         ...
         typesRev.defaultReturnValue(-2147483648);
         tempSection.deserialize((bytebuf) -> {
            ...
            if (index != -2147483648) {
               ...
               if (migratable) {
                  int var10003 = nextTempIndex[0];
                  int var10000 = nextTempIndex[0];
                  nextTempIndex[0] = var10003 - 1;
                  index = var10000;
               } else {
                  index = BlockType.getBlockIdOrUnknown(key, "Unknown BlockType %s", key);
                  needsPhysics[0] |= ((BlockType)BlockType.getAssetMap().getAsset(index)).hasSupport();
               }
      ...
      if (version >= 4) {
         ...
         this.fillerSection = (ISectionPalette)fillerTypeEnum.getConstructor().get();
      ...
      if (version >= 5) {
         ...
         this.rotationSection = (ISectionPalette)rotationTypeEnum.getConstructor().get();
   ...
   public CompletableFuture<CachedPacket<SetChunk>> getCachedChunkPacket(int x, int y, int z) {
      ...
      CompletableFuture<CachedPacket<SetChunk>> future = ref != null ? (CompletableFuture)ref.get() : null;
      if (future != null) {
         return future;
      } else {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BlockSection.class, BlockSection::new).versioned()).codecVersion(6)).append(new KeyedCodec("Data", Codec.BYTE_ARRAY), BlockSection::deserialize, BlockSection::serialize).add()).build();
      TICK_REQUEST_COMPARATOR = Comparator.comparing((t) -> t.requestedGameTime);
   }

   private static record TickRequest(int index, @Nonnull Instant requestedGameTime) {
   }
}
