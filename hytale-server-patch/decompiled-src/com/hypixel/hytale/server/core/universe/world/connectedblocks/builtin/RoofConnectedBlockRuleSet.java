...
public class RoofConnectedBlockRuleSet extends ConnectedBlockRuleSet implements StairLikeConnectedBlockRuleSet {
   ...
   private static StairConnectedBlockRuleSet.StairType getConnectedBlockStairType(World world, Vector3i coordinate, StairLikeConnectedBlockRuleSet currentRuleSet, int blockId, int rotation, int width) {
      ...
      StairConnectedBlockRuleSet.StairConnection backConnection = StairConnectedBlockRuleSet.getCornerConnection(world, currentRuleSet, coordinate, mutablePos, rotation, currentYaw, upsideDown, width);
      ...
      if (resultingStair == StairConnectedBlockRuleSet.StairType.STRAIGHT) {
         ...
         StairConnectedBlockRuleSet.StairConnection resultingConnection = getValleyConnection(world, coordinate, aboveCoordinate, currentRuleSet, currentRotation, mutablePos, false, blockId, rotation, width);
      ...
      if (resultingStair == StairConnectedBlockRuleSet.StairType.STRAIGHT) {
         ...
         StairConnectedBlockRuleSet.StairConnection resultingConnection = getValleyConnection(world, coordinate, belowCoordinate, currentRuleSet, currentRotation, mutablePos, true, blockId, rotation, width);
         if (resultingConnection != null) {
      ...
      if (upsideDown) {
         ...
         switch (resultingStair) {
            case CORNER_LEFT -> var10000 = StairConnectedBlockRuleSet.StairType.CORNER_RIGHT;
            case CORNER_RIGHT -> var10000 = StairConnectedBlockRuleSet.StairType.CORNER_LEFT;
   ...
   private static boolean isValleyConnectionCompatible(RotationTuple rotation, ObjectIntPair<StairConnectedBlockRuleSet.StairType> otherStair, Rotation yawOffset, boolean inverted) {
      ...
      if (otherStair == null) {
         ...
         StairConnectedBlockRuleSet.StairType otherStairType = (StairConnectedBlockRuleSet.StairType)otherStair.first();
         if (stairRotation.pitch() != rotation.pitch()) {
            return false;
         } else if (inverted && otherStairType.isCorner()) {
            return false;
         } else if (!inverted && otherStairType.isInvertedCorner()) {
            return false;
         } else {
   ...
   public Optional<ConnectedBlocksUtil.ConnectedBlockResult> getConnectedBlockType(World world, Vector3i coordinate, BlockType blockType, int rotation, Vector3i placementNormal, boolean isPlacement) {
      ...
      if (chunk == null) {
         ...
         BlockType belowBlockType = (BlockType)BlockType.getAssetMap().getAsset(belowBlockId);
         ...
         if (this.topper != null && stairType == StairConnectedBlockRuleSet.StairType.STRAIGHT) {
            ...
            if (topper) {
               BlockType topperBlockType = (BlockType)BlockType.getAssetMap().getAsset(this.topper.blockTypeKey);
