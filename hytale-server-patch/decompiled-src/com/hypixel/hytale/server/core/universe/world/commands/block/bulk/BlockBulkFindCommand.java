...
public class BlockBulkFindCommand extends AbstractWorldCommand {
   ...
   public BlockBulkFindCommand() {
      ...
      this.chunkXArg = this.withRequiredArg("chunkX", "server.commands.block.find.chunkX.desc", ArgTypes.RELATIVE_INT_COORD);
      this.chunkZArg = this.withRequiredArg("chunkZ", "server.commands.block.find.chunkZ.desc", ArgTypes.RELATIVE_INT_COORD);
      this.blockTypeArg = this.withRequiredArg("block", "server.commands.block.find.block.desc", ArgTypes.BLOCK_TYPE_KEY);
      this.countArg = this.withRequiredArg("count", "server.commands.block.find.count.desc", ArgTypes.INTEGER);
      this.timeoutArg = this.withRequiredArg("timeout", "server.commands.block.find.timeout.desc", ArgTypes.INTEGER);
   }

   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      CommandSender sender = context.sender();
      IntCoord relChunkX = (IntCoord)this.chunkXArg.get(context);
      IntCoord relChunkZ = (IntCoord)this.chunkZArg.get(context);
      int baseChunkX = 0;
      int baseChunkZ = 0;
      if (context.isPlayer()) {
      ...
      int timeout = (Integer)this.timeoutArg.get(context);
      CompletableFuture.runAsync(() -> {
         ...
         while(iterator.hasNext()) {
            ...
            BlockChunk blockChunk = (BlockChunk)chunkComponentStore.getChunkReferenceAsync(key).thenApplyAsync((chunkRef) -> (BlockChunk)chunkComponentStore.getStore().getComponent(chunkRef, BlockChunk.getComponentType()), world).join();
            ...
            for(int sectionIndex = 0; sectionIndex < 10; ++sectionIndex) {
               ...
               if (section.contains(blockId)) {
                  ...
                  section.find(idAsList, temp, (blockIndex) -> {
                     if (found[0] < count) {
                        int var10002 = found[0]++;
                        int x = chunkX << 5 | ChunkUtil.xFromIndex(blockIndex);
                        int y = sectionIndex << 5 | ChunkUtil.yFromIndex(blockIndex);
            ...
            if (diff > TimeUnit.SECONDS.toNanos((long)timeout)) {
