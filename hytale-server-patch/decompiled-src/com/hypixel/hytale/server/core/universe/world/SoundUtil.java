...
public class SoundUtil {
   public static void playItemSoundEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull Item item, @Nonnull ItemSoundEvent itemSoundEvent) {
      ItemSoundSet soundSet = (ItemSoundSet)ItemSoundSet.getAssetMap().getAsset(item.getItemSoundSetIndex());
      if (soundSet != null) {
         String soundEventId = (String)soundSet.getSoundEventIds().get(itemSoundEvent);
   ...
   public static void playSoundEvent3dToPlayer(@Nullable Ref<EntityStore> playerRef, int soundEventIndex, @Nonnull SoundCategory soundCategory, double x, double y, double z, float volumeModifier, float pitchModifier, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (playerRef != null && soundEventIndex != 0) {
         SoundEvent soundEventAsset = (SoundEvent)SoundEvent.getAssetMap().getAsset(soundEventIndex);
