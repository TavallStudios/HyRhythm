...
public class PluginCommand extends AbstractCommandCollection {
   @Nonnull
   private static final SingleArgumentType<PluginIdentifier> PLUGIN_IDENTIFIER_ARG_TYPE = new SingleArgumentType<PluginIdentifier>("server.commands.parsing.argtype.pluginidentifier.name", "server.commands.parsing.argtype.pluginidentifier.usage", new String[0]) {
      @Nonnull
   ...
   public PluginCommand() {
      ...
      this.addAliases(new String[]{"plugins", "pl"});
   ...
   private static class PluginListCommand extends CommandBase {
      ...
      protected void executeSync(@Nonnull CommandContext context) {
         ...
         Set<Message> plugins = (Set)module.getPlugins().stream().map(PluginBase::getIdentifier).map(PluginIdentifier::toString).map(Message::raw).collect(Collectors.toSet());
   ...
   private static class PluginLoadCommand extends CommandBase {
      ...
      public PluginLoadCommand() {
         ...
         this.addAliases(new String[]{"l"});
      }

      protected void executeSync(@Nonnull CommandContext context) {
         PluginManager module = PluginManager.get();
         PluginIdentifier identifier = (PluginIdentifier)this.pluginNameArg.get(context);
         ...
         if (plugin != null && plugin.getState() != PluginState.DISABLED) {
            ...
            switch (plugin.getState()) {
               case NONE -> context.sendMessage(Message.translation("server.commands.plugin.failedToLoadInvalidState").param("id", identifier.toString()));
               case SETUP -> context.sendMessage(Message.translation("server.commands.plugin.failedToLoadSetup").param("id", identifier.toString()));
               case START -> context.sendMessage(Message.translation("server.commands.plugin.failedToLoadStarted").param("id", identifier.toString()));
               case ENABLED -> context.sendMessage(Message.translation("server.commands.plugin.failedToLoadAlreadyEnabled").param("id", identifier.toString()));
               default -> context.sendMessage(Message.translation("server.commands.plugin.failedPluginState").param("state", plugin.getState().toString()));
            }
         } else {
