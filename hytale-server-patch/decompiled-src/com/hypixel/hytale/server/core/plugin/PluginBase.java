...
public abstract class PluginBase implements CommandOwner {
   ...
   public PluginBase(@Nonnull PluginInit init) {
      ...
      String var10001 = String.valueOf(this.getIdentifier());
   ...
   public CompletableFuture<Void> preLoad() {
      if (this.configs.isEmpty()) {
         ...
         for(int i = 0; i < this.configs.size(); ++i) {
            futures[i] = ((Config)this.configs.get(i)).load();
         }
   ...
   public <T, C extends Codec<? extends T>> CodecMapRegistry<T, C> getCodecRegistry(@Nonnull StringCodecMapCodec<T, C> mapCodec) {
      IRegistry registry = (IRegistry)this.codecMapRegistries.computeIfAbsent(mapCodec, (v) -> new CodecMapRegistry(this.shutdownTasks, mapCodec));
   ...
   public <K, T extends JsonAsset<K>> CodecMapRegistry.Assets<T, ?> getCodecRegistry(@Nonnull AssetCodecMapCodec<K, T> mapCodec) {
      IRegistry registry = (IRegistry)this.codecMapRegistries.computeIfAbsent(mapCodec, (v) -> new CodecMapRegistry.Assets(this.shutdownTasks, mapCodec));
      return (CodecMapRegistry.Assets)registry;
   }

   @Nonnull
   public <V> MapKeyMapRegistry<V> getCodecRegistry(@Nonnull MapKeyMapCodec<V> mapCodec) {
      IRegistry registry = (IRegistry)this.codecMapRegistries.computeIfAbsent(mapCodec, (v) -> new MapKeyMapRegistry(this.shutdownTasks, mapCodec));
