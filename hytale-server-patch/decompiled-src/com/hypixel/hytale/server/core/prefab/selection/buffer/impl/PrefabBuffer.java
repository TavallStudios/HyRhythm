...
public class PrefabBuffer {
   ...
   public static class Builder {
      ...
      private Builder() {
         ...
         this.columns = new Int2ObjectOpenHashMap();
         this.childPrefabs = new ObjectArrayList(0);
               ...
               if (blockCount != 0 || entityHolders != null && entityHolders.length != 0) {
                  ...
                  if (blockCount > 0) {
                     ...
                     for(int i = 0; i < blockCount; ++i) {
                        ...
                        int mask = PrefabBuffer.BlockMaskConstants.getBlockMask(blockBytes, fluidBytes, hasChance, offsetBytes, holder, entry.supportValue, entry.rotation, entry.filler);
                        this.buf.writeShort(mask);
                        ByteBufUtil.writeNumber(this.buf, blockBytes, blockId);
                        ByteBufUtil.writeNumber(this.buf, offsetBytes, offset);
                        if (hasChance) {
      ...
      private void handleBlockComponents(int blockRotation, int x, int y, int z, @Nonnull Holder<ChunkStore> holder) {
         ...
         PrefabSpawnerState spawnerState = (PrefabSpawnerState)holder.getComponent(componentType);
      ...
      public PrefabBuffer build() {
         ...
         this.buf.release();
         ChildPrefab[] childPrefabArray = (ChildPrefab[])this.childPrefabs.toArray((x$0) -> new ChildPrefab[x$0]);
   ...
   public static class PrefabBufferAccessor implements IPrefabBuffer {
      ...
      public <T> boolean forEachRaw(@Nonnull IPrefabBuffer.ColumnPredicate<T> columnPredicate, @Nonnull IPrefabBuffer.RawBlockPredicate<T> blockPredicate, @Nonnull IPrefabBuffer.FluidPredicate<T> fluidPredicate, @Nullable IPrefabBuffer.EntityPredicate<T> entityPredicate, @Nullable T t) {
         ...
         while(var6.hasNext()) {
            Int2ObjectMap.Entry<PrefabBufferColumn> entry = (Int2ObjectMap.Entry)var6.next();
            int columnIndex = entry.getIntKey();
            int x = MathUtil.unpackLeft(columnIndex);
            int z = MathUtil.unpackRight(columnIndex);
            PrefabBufferColumn column = (PrefabBufferColumn)entry.getValue();
      ...
      public <T extends PrefabBufferCall> boolean compare(@Nonnull IPrefabBuffer.BlockComparingPrefabPredicate<T> blockComparingIterator, @Nonnull T t, @Nonnull IPrefabBuffer otherPrefab) {
         if (!(otherPrefab instanceof PrefabBufferAccessor secondPrefab)) {
            ...
            while(columnIterator.hasNext()) {
               ...
               if (firstColumnBlockCount != 0 || secondColumnBlockCount != 0) {
                  int firstColumnY = firstColumnBlockCount > 0 ? this.buffer.readInt() : 2147483647;
                  int secondColumnY = secondColumnBlockCount > 0 ? secondPrefab.buffer.readInt() : 2147483647;
                  int firstColumnBlockId = -2147483648;
                  float firstColumnChance = 1.0F;
                  int firstColumnRotation = 0;
                  int firstColumnFiller = 0;
                  Holder<ChunkStore> firstColumnComponents = null;
                  int secondColumnBlockId = -2147483648;
                  ...
                  while(firstColumnBlocksRead < firstColumnBlockCount || secondColumnBlocksRead < secondColumnBlockCount) {
                     ...
                     if (firstColumnBlocksRead < firstColumnBlockCount) {
                        ...
                        firstColumnComponents = PrefabBuffer.BlockMaskConstants.hasComponents(mask) ? (Holder)firstColumn.getBlockComponents().get(firstColumnY) : null;
                        this.buffer.skipBytes(PrefabBuffer.BlockMaskConstants.getFluidBytes(mask));
                     }
                     ...
                     if (secondColumnBlocksRead < secondColumnBlockCount) {
                        ...
                        secondColumnRotation = t.rotation.getRotation(PrefabBuffer.BlockMaskConstants.hasRotation(mask) ? secondPrefab.buffer.readUnsignedByte() : 0);
                        secondColumnFiller = PrefabBuffer.BlockMaskConstants.hasFiller(mask) ? t.rotation.getFiller(secondPrefab.buffer.readUnsignedShort()) : 0;
                        secondColumnComponents = PrefabBuffer.BlockMaskConstants.hasComponents(mask) ? (Holder)secondColumn.getBlockComponents().get(secondColumnY) : null;
                     ...
                     if (firstColumnY == secondColumnY) {
                        ...
                        boolean test = blockComparingIterator.test(x, firstColumnY, z, firstColumnBlockId, firstColumnComponents, firstColumnChance, firstColumnRotation, firstColumnFiller, secondColumnBlockId, secondColumnComponents, secondColumnChance, secondColumnRotation, secondColumnFiller, t);
                        if (!test) {
                           return false;
                        }
                        ...
                        boolean test = blockComparingIterator.test(x, secondColumnY, z, -2147483648, (Holder)null, 1.0F, 0, 0, secondColumnBlockId, secondColumnComponents, secondColumnChance, secondColumnRotation, secondColumnFiller, t);
                        if (!test) {
                           return false;
                        }
                     } else {
                        ...
                        boolean test = blockComparingIterator.test(x, firstColumnY, z, firstColumnBlockId, firstColumnComponents, firstColumnChance, firstColumnRotation, firstColumnFiller, -2147483648, (Holder)null, 1.0F, 0, 0, t);
                        if (!test) {
                           return false;
                        }
                     }
         ...
         if (column == null) {
            ...
            if (blockCount <= 0) {
               ...
               for(int i = 0; i < blockCount; ++i) {
                  int mask = this.buffer.readUnsignedShort();
                  int blockBytes = PrefabBuffer.BlockMaskConstants.getBlockBytes(mask);
                  ByteBufUtil.readNumber(this.buffer, blockBytes);
                  int offsetBytes = PrefabBuffer.BlockMaskConstants.getOffsetBytes(mask);
                  blockY += offsetBytes == 0 ? 1 : ByteBufUtil.readNumber(this.buffer, offsetBytes);
                  if (blockY > y) {
                     return 0;
