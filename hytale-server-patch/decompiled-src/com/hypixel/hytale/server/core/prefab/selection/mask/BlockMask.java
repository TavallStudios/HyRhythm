...
public class BlockMask {
   ...
   private static BlockMask groupFilters(@Nonnull BlockFilter[] inputFilters) {
      if (inputFilters.length == 0) {
         ...
         for(BlockFilter filter : inputFilters) {
            ...
            List<String> list = (List)groups.computeIfAbsent(key, (k) -> new ArrayList());
         ...
         if (groups.size() == inputFilters.length) {
            ...
            BlockFilter.FilterType filterType;
            boolean inverted;
            String[] blocks;
            for(ObjectBidirectionalIterator var14 = groups.int2ObjectEntrySet().iterator(); var14.hasNext(); filters[i++] = new BlockFilter(filterType, blocks, inverted)) {
               Int2ObjectMap.Entry<List<String>> entry = (Int2ObjectMap.Entry)var14.next();
               int key = entry.getIntKey();
               filterType = BlockFilter.FilterType.values()[key >> 1];
               inverted = (key & 1) != 0;
               blocks = (String[])((List)entry.getValue()).toArray(new String[0]);
            }
   ...
   static {
      ...
      CODEC = new FunctionCodec(Codec.STRING, BlockMask::parse, BlockMask::toString);
   }
}
