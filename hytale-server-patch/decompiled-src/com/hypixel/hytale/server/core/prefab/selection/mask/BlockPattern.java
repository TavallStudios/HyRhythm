...
public class BlockPattern {
   ...
   public Integer[] getResolvedKeys() {
      this.resolve();
      return (Integer[])this.resolvedWeightedMap.internalKeys();
   }
   ...
   public void resolve() {
      if (this.resolvedWeightedMap == null) {
         ...
         WeightedMap.Builder<BlockEntry> mapBuilderKey = WeightedMap.<BlockEntry>builder(new BlockEntry[0]);
   ...
   public int nextBlock(Random random) {
      this.resolve();
      return (Integer)this.resolvedWeightedMap.get(random);
   }

   @Nullable
   public BlockEntry nextBlockTypeKey(Random random) {
      this.resolve();
      return (BlockEntry)this.resolvedWeightedMapBtk.get(random);
   }
   ...
   public int firstBlock() {
      this.resolve();
      return this.resolvedWeightedMap.size() > 0 ? ((Integer[])this.resolvedWeightedMap.internalKeys())[0] : 0;
   }
   ...
   private String toString0() {
      if (this.weightedMap.size() == 1) {
         return ((String[])this.weightedMap.internalKeys())[0];
      } else {
   ...
   static {
      CODEC = new FunctionCodec(Codec.STRING, BlockPattern::parse, BlockPattern::toString);
   ...
   public static record BlockEntry(String blockTypeKey, int rotation, int filler) {
      /** @deprecated */
      @Deprecated(
         forRemoval = true
      )
      ...
      static {
         CODEC = new FunctionCodec(Codec.STRING, BlockEntry::decode, BlockEntry::encode);
      }
   }
}
