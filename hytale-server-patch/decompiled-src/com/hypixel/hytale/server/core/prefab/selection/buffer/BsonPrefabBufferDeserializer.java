   ...
   public PrefabBuffer deserialize(Path path, @Nonnull BsonDocument document) {
      ...
      if (version > 8) {
         ...
         if (version < 1) {
            ...
            for(BlockMigration migration = (BlockMigration)blockMigrationMap.get(blockIdVersion); migration != null; migration = (BlockMigration)blockMigrationMap.get(v)) {
            ...
            if (blocksValue != null) {
               ...
               for(BsonValue blockValue : blocksValue.asArray()) {
                  ...
                  Int2ObjectMap<PrefabBufferBlockEntry> column = (Int2ObjectMap)columnMap.get(columnIndex);
                  if (column == null) {
                     columnMap.put(columnIndex, column = new Int2ObjectOpenHashMap());
                  }

                  PrefabBufferBlockEntry existing = (PrefabBufferBlockEntry)column.putIfAbsent(y, blockEntry);
                  if (existing != null) {
            ...
            if (fluidsValue != null) {
               ...
               for(BsonValue fluidValue : fluidsValue.asArray()) {
                  ...
                  PrefabBufferBlockEntry entry = (PrefabBufferBlockEntry)column.computeIfAbsent(y, builder::newBlockEntry);
            ...
            columnMap.int2ObjectEntrySet().fastForEach((entryx) -> {
               int columnIndex = entryx.getIntKey();
               ...
               PrefabBufferBlockEntry[] entries = (PrefabBufferBlockEntry[])columnBlockMap.values().toArray((x$0) -> new PrefabBufferBlockEntry[x$0]);
               Arrays.sort(entries, Comparator.comparingInt((o) -> o.y));
               List<Holder<EntityStore>> entityColumn = (List)entityMap.remove(columnIndex);
               Holder[] entityArray = entityColumn != null && !entityColumn.isEmpty() ? (Holder[])entityColumn.toArray((x$0) -> new Holder[x$0]) : null;
            ...
            entityMap.int2ObjectEntrySet().fastForEach((entryx) -> {
               ...
               Holder[] entityArray = !entityColumn.isEmpty() ? (Holder[])entityColumn.toArray((x$0) -> new Holder[x$0]) : null;
               if (entityArray != null) {
      ...
      if (entitiesValue == null) {
         ...
         for(int size = entitiesArray.size(); i < size; ++i) {
            ...
            try {
               ...
               position.add((double)(-anchor.x), (double)(-anchor.y), (double)(-anchor.z));
               int x = MathUtil.floor(position.getX()) & '\uffff';
               int z = MathUtil.floor(position.getZ()) & '\uffff';
               int columnIndex = MathUtil.packInt(x, z);
               List<Holder<EntityStore>> entityColumn = (List)entityMap.get(columnIndex);
