...
public class ServerFileBrowser {
   ...
   public ServerFileBrowser(@Nonnull FileBrowserConfig config) {
      ...
      if (!config.roots().isEmpty()) {
         this.root = ((FileBrowserConfig.RootEntry)config.roots().get(0)).path();
      } else {
         this.root = Paths.get("");
      }
      ...
      if (data.getSearchQuery() != null) {
         ...
         if ("..".equals(fileName)) {
            ...
            if (this.config.enableDirectoryNav()) {
               ...
               if (targetPath != null && Files.isDirectory(targetPath, new LinkOption[0]) && PathUtil.isChildOf(this.root, targetPath)) {
   ...
   private List<FileListProvider.FileEntry> buildSearchResults() {
      ...
      if (!Files.isDirectory(this.root, new LinkOption[0])) {
         ...
         return (List)var10000.sorted(Comparator.comparingInt(matchScores::getInt).reversed()).limit((long)this.config.maxResults()).map((filex) -> {
            Path relativePath = PathUtil.relativize(this.root, filex);
            String fileName = filex.getFileName().toString();
   ...
   private List<FileListProvider.FileEntry> buildAssetPackSearchResults() {
      List<AssetPackSearchResult> allResults = new ObjectArrayList();
      String currentDirStr = this.currentDir.toString().replace('\\', '/');
      if (currentDirStr.isEmpty()) {
         for(AssetPack pack : AssetModule.get().getAssetPacks()) {
            Path subPath = this.getAssetPackSubPath(pack);
         ...
         if (pack != null) {
            ...
            if (packSubPath != null) {
               ...
               if (searchRoot != null && Files.isDirectory(searchRoot, new LinkOption[0])) {
      ...
      return (List)allResults.stream().limit((long)this.config.maxResults()).map((r) -> new FileListProvider.FileEntry(r.virtualPath(), r.displayName(), r.isTerminal(), r.isTerminal(), r.score())).collect(Collectors.toList());
   }
   ...
   private static record AssetPackSearchResult(@Nonnull String virtualPath, @Nonnull String displayName, int score, boolean isTerminal) {
   }
}
