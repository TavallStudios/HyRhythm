...
public abstract class ChoiceElement {
   ...
   public String toString() {
      String var10000 = this.displayNameKey;
      return "ChoiceElement{displayNameKey='" + var10000 + "', descriptionKey='" + this.descriptionKey + "', interactions=" + Arrays.toString(this.interactions) + ", requirements=" + Arrays.toString(this.requirements) + "}";
   }

   static {
      BASE_CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.abstractBuilder(ChoiceElement.class).append(new KeyedCodec("DisplayNameKey", Codec.STRING), (choiceElement, s) -> choiceElement.displayNameKey = s, (choiceElement) -> choiceElement.displayNameKey).addValidator(Validators.nonEmptyString()).add()).append(new KeyedCodec("DescriptionKey", Codec.STRING), (choiceElement, s) -> choiceElement.descriptionKey = s, (choiceElement) -> choiceElement.descriptionKey).addValidator(Validators.nonEmptyString()).add()).append(new KeyedCodec("Interactions", new ArrayCodec(ChoiceInteraction.CODEC, (x$0) -> new ChoiceInteraction[x$0])), (choiceElement, choiceInteractions) -> choiceElement.interactions = choiceInteractions, (choiceElement) -> choiceElement.interactions).addValidator(Validators.nonEmptyArray()).add()).append(new KeyedCodec("Requirements", new ArrayCodec(ChoiceRequirement.CODEC, (x$0) -> new ChoiceRequirement[x$0])), (choiceElement, choiceRequirements) -> choiceElement.requirements = choiceRequirements, (choiceElement) -> choiceElement.requirements).add()).build();
   }
}
