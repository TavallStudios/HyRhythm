...
public class BlockEntity implements Component<EntityStore> {
   ...
   public SimplePhysicsProvider initPhysics(@Nonnull BoundingBox boundingBox) {
      this.simplePhysicsProvider.initialize((Projectile)Projectile.getAssetMap().getAsset("Projectile"), boundingBox);
   ...
   public BoundingBox createBoundingBoxComponent() {
      if (this.blockTypeKey == null) {
         ...
         if (assetMap == null) {
            ...
            BlockType blockType = (BlockType)assetMap.getAsset(this.blockTypeKey);
            return blockType == null ? null : new BoundingBox(((BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(blockType.getHitboxTypeIndex())).get(0).getBoundingBox());
         }
      }
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(BlockEntity.class, BlockEntity::new).append(new KeyedCodec("BlockTypeKey", Codec.STRING), (blockEntity, newBlockTypeKey, extraInfo) -> {
