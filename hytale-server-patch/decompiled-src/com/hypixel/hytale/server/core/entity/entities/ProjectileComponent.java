...
public class ProjectileComponent implements Component<EntityStore> {
   ...
   private void onProjectileHitEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Vector3d position, @Nonnull Ref<EntityStore> targetRef, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (targetEntity instanceof LivingEntity) {
         Ref<EntityStore> shooterRef = ((EntityStore)componentAccessor.getExternalData()).getRefFromUUID(this.creatorUuid);
         DamageSystems.executeDamage(targetRef, componentAccessor, new Damage(new Damage.ProjectileSource(shooterRef != null ? shooterRef : ref, ref), DamageCause.PROJECTILE, (float)this.projectile.getDamage() * this.brokenDamageModifier));
         this.haveHit = true;
      }
