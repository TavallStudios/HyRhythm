...
public class EntityGroup implements Component<EntityStore> {
   ...
   @Nonnull
   private final List<Ref<EntityStore>> memberList = new ObjectArrayList();
   @Nullable
   ...
   public Ref<EntityStore> getFirst() {
      return !this.memberList.isEmpty() ? (Ref)this.memberList.getFirst() : null;
   }
   ...
   public <T> void forEachMember(@Nonnull TriConsumer<Ref<EntityStore>, Ref<EntityStore>, T> consumer, Ref<EntityStore> sender, T arg) {
      this.forEachMember(consumer, sender, arg, (Ref)null);
   }
   ...
   public <T> void forEachMember(@Nonnull TriConsumer<Ref<EntityStore>, Ref<EntityStore>, T> consumer, Ref<EntityStore> sender, T arg, Ref<EntityStore> excludeReference) {
      for(int i = 0; i < this.memberList.size(); ++i) {
         Ref<EntityStore> member = (Ref)this.memberList.get(i);
   ...
   public <T, V> void forEachMember(@Nonnull QuadConsumer<Ref<EntityStore>, Ref<EntityStore>, T, V> consumer, Ref<EntityStore> sender, T t, V v) {
      this.forEachMember(consumer, sender, t, v, (Ref)null);
   }
   ...
   public <T> void forEachMember(@Nonnull IntTriObjectConsumer<Ref<EntityStore>, Ref<EntityStore>, T> consumer, Ref<EntityStore> sender, T t, int value) {
      this.forEachMember(consumer, sender, t, value, (Ref)null);
   }
   ...
   public String toString() {
      String var10000 = String.valueOf(this.leaderRef);
      return "EntityGroup{leader=" + var10000 + ", memberSet=" + String.valueOf(this.memberSet) + ", memberList=" + String.valueOf(this.memberList) + ", dissolved=" + this.dissolved + "}";
   }
}
