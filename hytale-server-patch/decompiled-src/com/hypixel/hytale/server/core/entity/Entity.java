...
public abstract class Entity implements Component<EntityStore> {
   ...
   public TransformComponent getTransformComponent() {
      if (this.world != null && this.reference != null) {
         if (!this.world.isInThread()) {
            ((HytaleLogger.Api)((HytaleLogger.Api)LOGGER.at(Level.WARNING).atMostEvery(5, TimeUnit.MINUTES)).withCause(new Throwable())).log("getPositionComponent called async");
            return this.transformComponent;
         } else {
            Store<EntityStore> store = this.world.getEntityStore().getStore();
            TransformComponent transformComponent = (TransformComponent)store.getComponent(this.reference, TransformComponent.getComponentType());
   ...
   public String toString() {
      int var10000 = this.networkId;
      return "Entity{id=" + var10000 + ", uuid=" + String.valueOf(this.legacyUuid) + ", reference='" + String.valueOf(this.reference) + "', world=" + (this.world != null ? this.world.getName() : null) + ", displayName='" + this.legacyDisplayName + "', wasRemoved='" + String.valueOf(this.wasRemoved) + "', removedBy='" + (this.removedBy != null ? String.valueOf(this.removedBy) + "\n" + Arrays.toString(this.removedBy.getStackTrace()) : null) + "'}";
   }
   ...
   public void setReference(@Nonnull Ref<EntityStore> reference) {
      if (this.reference != null && this.reference.isValid()) {
         ...
         throw new IllegalArgumentException("Entity already has a valid EntityReference: " + var10002 + " new reference " + String.valueOf(reference));
      } else {
   ...
   public Component<EntityStore> clone() {
      DirectDecodeCodec<Entity> codec = EntityModule.get().<Entity>getCodec(this.getClass());
      Function<World, Entity> constructor = EntityModule.get().getConstructor(this.getClass());
      BsonDocument document = codec.encode(this, (ExtraInfo)ExtraInfo.THREAD_LOCAL.get()).asDocument();
      document.put("EntityType", new BsonString(EntityModule.get().getIdentifier(this.getClass())));
      Entity t = (Entity)constructor.apply((Object)null);
   ...
   public Holder<EntityStore> toHolder() {
      if (this.reference != null && this.reference.isValid() && this.world != null) {
         if (!this.world.isInThread()) {
            return (Holder)CompletableFuture.supplyAsync(this::toHolder, this.world).join();
         } else {
         ...
         if (this instanceof Player) {
            ...
            ComponentType<EntityStore, ? extends Entity> componentType = EntityModule.get().getComponentType(this.getClass());
            holder.addComponent(componentType, this);
         }

         DirectDecodeCodec<? extends Entity> codec = EntityModule.get().<Entity>getCodec(this.getClass());
         if (codec == null) {
   ...
   static {
      MODEL = new KeyedCodec<Model.ModelReference>("Model", Model.ModelReference.CODEC);
      DISPLAY_NAME = new KeyedCodec<String>("DisplayName", Codec.STRING);
      UUID = new KeyedCodec<UUID>("UUID", Codec.UUID_BINARY);
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.abstractBuilder(Entity.class).legacyVersioned()).codecVersion(5)).append(DISPLAY_NAME, (entity, o) -> entity.legacyDisplayName = o, (entity) -> entity.legacyDisplayName).add()).append(UUID, (entity, o) -> entity.legacyUuid = o, (entity) -> entity.legacyUuid).add()).build();
   }
