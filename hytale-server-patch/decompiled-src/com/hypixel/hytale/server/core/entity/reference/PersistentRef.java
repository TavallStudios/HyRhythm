...
public class PersistentRef {
   ...
   public void setEntity(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      this.uuid = ((UUIDComponent)componentAccessor.getComponent(ref, UUIDComponent.getComponentType())).getUuid();
      this.reference = ref;
   }
   ...
   public Ref<EntityStore> getEntity(@Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (!this.isValid()) {
         ...
         this.reference = ((EntityStore)componentAccessor.getExternalData()).getRefFromUUID(this.uuid);
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(PersistentRef.class, PersistentRef::new).append(new KeyedCodec("UUID", Codec.UUID_BINARY), (instance, value) -> instance.uuid = value, (instance) -> instance.uuid).add()).build();
   }
}
