...
public class InteractionManager implements Component<EntityStore> {
   ...
   public InteractionManager(@Nonnull LivingEntity entity, @Nullable PlayerRef playerRef, @Nonnull IInteractionSimulationHandler simulationHandler) {
      this.unmodifiableChains = Int2ObjectMaps.unmodifiable(this.chains);
      this.cooldownHandler = new CooldownHandler();
      this.tempSyncDataList = new ObjectArrayList();
      ...
      this.syncPackets = new ObjectArrayList();
      this.currentTime = 1L;
      this.chainStartQueue = new ObjectArrayList();
   ...
   private long getOperationTimeoutThreshold() {
      if (this.playerRef != null) {
         ...
         World world = ((EntityStore)this.commandBuffer.getExternalData()).getWorld();
         return (long)(world.getTickStepNanos() / 1000000 * 10);
      }
   }
   ...
   private boolean waitingForClient(@Nonnull Ref<EntityStore> ref) {
      ...
      Player playerComponent = (Player)this.commandBuffer.getComponent(ref, Player.getComponentType());
   ...
   public void tick(@Nonnull Ref<EntityStore> ref, @Nonnull CommandBuffer<EntityStore> commandBuffer, float dt) {
      this.currentTime += (long)((EntityStore)commandBuffer.getExternalData()).getWorld().getTickStepNanos();
      this.commandBuffer = commandBuffer;
      this.clearAllGlobalTimeShift(dt);
      this.cooldownHandler.tick(dt);
      ObjectListIterator var4 = this.chainStartQueue.iterator();

      while(var4.hasNext()) {
         InteractionChain interactionChain = (InteractionChain)var4.next();
   ...
   private boolean tryConsumePacketQueue(@Nonnull Ref<EntityStore> ref, @Nonnull Deque<SyncInteractionChain> packetQueue) {
      ...
      label116:
      while(it.hasNext()) {
         SyncInteractionChain packet = (SyncInteractionChain)it.next();
         if (packet.desync) {
         ...
         if (chain.getServerState() != InteractionState.NotFinished) {
            if (chain.requiresClient() && chain.getClientState() == InteractionState.NotFinished) {
               if (!this.waitingForClient(ref)) {
                  ...
                  TimeResource timeResource = (TimeResource)this.commandBuffer.getResource(TimeResource.getResourceType());
            ...
            } catch (ChainCancelledException e) {
            ...
            if (rootInteractionId == null) {
               ...
               if (rootInteraction == null) {
                  ...
                  if (packet.activeHotbarSlot != entityInventory.getActiveHotbarSlot()) {
                     HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
                     if (ctx.isEnabled()) {
                        ctx.log("Active slot miss match: %d, %d != %d, %s, %s, %s", index, entityInventory.getActiveHotbarSlot(), packet.activeHotbarSlot, serverItemInHandId, packet.itemInHandId, type);
                     }
                     ...
                     HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
                     if (ctx.isEnabled()) {
                        ctx.log("Active slot miss match: %d, %d != %d, %s, %s, %s", index, entityInventory.getActiveUtilitySlot(), packet.activeUtilitySlot, serverItemInHandId, packet.itemInHandId, type);
                     }
                     ...
                     HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
                     if (ctx.isEnabled()) {
                        ctx.log("ItemInHand miss match: %d, %s, %s, %s", index, serverItemInHandId, packet.itemInHandId, type);
                     }
                     ...
                     HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
                     if (ctx.isEnabled()) {
                        ctx.log("UtilityItem miss match: %d, %s, %s, %s", index, serverUtilityItemId, packet.utilityItemId, type);
                     }
                     ...
                     InteractionChain chain = this.initChain(packet.data, type, context, rootInteraction, (Runnable)null, true);
                     ...
                     if (packet.data.blockPosition != null) {
                        ...
                        if (!packet.data.blockPosition.equals(targetBlock)) {
                           WorldChunk otherChunk = world.getChunkIfInMemory(ChunkUtil.indexChunkFromBlock(packet.data.blockPosition.x, packet.data.blockPosition.z));
                           if (otherChunk == null) {
                              HytaleLogger.Api ctx = LOGGER.at(Level.FINE);
                              if (ctx.isEnabled()) {
                     ...
                     if (packet.data.targetSlot != -2147483648) {
                     ...
                     if (packet.data.hitLocation != null) {
                        ...
                        context.getMetaStore().putMetaObject(Interaction.HIT_LOCATION, new Vector4d((double)hit.x, (double)hit.y, (double)hit.z, 1.0));
                     }
            ...
            if (syncData != null) {
               ...
               if (!chainSyncStorage.isSyncDataOutOfOrder(index)) {
                  ...
                  if (interaction != null && chainSyncStorage instanceof InteractionChain) {
                     InteractionChain interactionChain = (InteractionChain)chainSyncStorage;
   ...
   public boolean canRun(@Nonnull InteractionType type, short equipSlot, @Nonnull RootInteraction rootInteraction) {
      return applyRules((InteractionChainData)null, type, equipSlot, rootInteraction, this.chains, (List)null);
   }
   ...
   private static boolean applyRules(@Nullable InteractionChainData data, @Nonnull InteractionType type, int heldItemSlot, @Nullable RootInteraction rootInteraction, @Nonnull Map<?, InteractionChain> chains, @Nullable List<InteractionChain> chainsToCancel) {
      if (!chains.isEmpty() && rootInteraction != null) {
         for(InteractionChain chain : chains.values()) {
            if ((chain.getForkedChainId() == null || chain.isPredicted()) && (data == null || Objects.equals(chain.getChainData().proxyId, data.proxyId)) && (type != InteractionType.Equipped || chain.getType() != InteractionType.Equipped || chain.getContext().getHeldItemSlot() == heldItemSlot)) {
               if (chain.getServerState() == InteractionState.NotFinished) {
   ...
   public InteractionChain initChain(@Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull RootInteraction rootInteraction, boolean forceRemoteSync) {
      return this.initChain(type, context, rootInteraction, -1, (BlockPosition)null, forceRemoteSync);
   }
   ...
   public InteractionChain initChain(@Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull RootInteraction rootInteraction, int entityId, @Nullable BlockPosition blockPosition, boolean forceRemoteSync) {
      InteractionChainData data = new InteractionChainData(entityId, UUIDUtil.EMPTY_UUID, (Vector3f)null, (String)null, blockPosition, -2147483648, (Vector3f)null);
      return this.initChain(data, type, context, rootInteraction, (Runnable)null, forceRemoteSync);
   }
   ...
   private boolean isOnCooldown(@Nonnull Ref<EntityStore> ref, @Nonnull InteractionType type, @Nonnull RootInteraction root, boolean remote) {
      ...
      RootInteractionSettings settings = (RootInteractionSettings)root.getSettings().get(gameMode);
      if (settings != null) {
      ...
      if (this.timeShiftsDirty) {
         ...
         for(int i = 0; i < this.globalTimeShift.length; ++i) {
            if (!this.globalTimeShiftDirty[i]) {
               ...
               clearFlag = false;
               float[] var10000 = this.globalTimeShift;
               var10000[i] += dt;
            }
         }
   ...
   public <T> T forEachInteraction(@Nonnull TriFunction<InteractionChain, Interaction, T, T> func, @Nonnull T val) {
      return (T)forEachInteraction(this.chains, func, val);
   }
      ...
      if (chains.isEmpty()) {
         ...
         for(InteractionChain chain : chains.values()) {
            ...
            if (operation != null) {
               ...
               if (operation instanceof Interaction) {
                  Interaction interaction = (Interaction)operation;
   ...
   public void walkChain(@Nonnull Ref<EntityStore> ref, @Nonnull Collector collector, @Nonnull InteractionType type, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      this.walkChain(ref, collector, type, (RootInteraction)null, componentAccessor);
   }
   ...
   public static void walkChain(@Nonnull Collector collector, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nullable RootInteraction rootInteraction) {
      if (rootInteraction == null) {
         String rootInteractionId = context.getRootInteractionId(type);
         if (rootInteractionId == null) {
            String var5 = String.valueOf(type);
            throw new IllegalArgumentException("No interaction ID found for " + var5 + ", " + String.valueOf(context));
         }
      ...
      if (rootInteraction == null) {
         ...
         throw new IllegalArgumentException("No interactions are defined for " + var10002 + ", " + String.valueOf(context));
      } else {
         collector.start();
         collector.into(context, (Interaction)null);
   ...
   public static boolean walkInteraction(@Nonnull Collector collector, @Nonnull InteractionContext context, @Nonnull CollectorTag tag, @Nullable String id) {
      if (id == null) {
         ...
         Interaction interaction = (Interaction)Interaction.getAssetMap().getAsset(id);
         if (interaction == null) {
