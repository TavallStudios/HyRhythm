...
public class InteractionEntry {
   ...
   public InteractionEntry(int index, int counter, int rootInteraction) {
      this.index = index;
      this.metaStore = new DynamicMetaStore<Interaction>((Object)null, Interaction.META_REGISTRY);
   ...
   public boolean setClientState(@Nullable InteractionSyncData clientState) {
      if (clientState != null && (clientState.operationCounter != this.serverState.operationCounter || clientState.rootInteraction != this.serverState.rootInteraction)) {
         ...
         if (ctx.isEnabled()) {
            RootInteraction root = (RootInteraction)RootInteraction.getAssetMap().getAsset(this.serverState.rootInteraction);
            Operation op = root.getOperation(this.serverState.operationCounter);
            Operation innerOp = op.getInnerOperation();
            String info;
            if (innerOp instanceof Interaction) {
               Interaction interaction = (Interaction)innerOp;
               String var10000 = interaction.getId();
               info = var10000 + " (" + interaction.getClass().getSimpleName() + ")";
            } else {
               String var8 = String.valueOf(op);
