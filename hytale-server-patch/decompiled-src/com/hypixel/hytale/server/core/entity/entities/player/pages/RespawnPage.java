...
public class RespawnPage extends InteractiveCustomUIPage<RespawnPageEventData> {
   ...
   private static ItemStack[] combineSimilarItemStacks(@Nullable ItemStack[] itemsLostOnDeath) {
      if (itemsLostOnDeath == null) {
         ...
         return (ItemStack[])singleItemStacks.toArray((x$0) -> new ItemStack[x$0]);
      }
   }
   ...
   public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull RespawnPageEventData data) {
      if ("Respawn".equals(data.action)) {
         ...
         CompletableFutureUtil._catch(DeathComponent.respawn(store, ref).thenCompose((v) -> {
            ...
            }, ((EntityStore)currentRef.getStore().getExternalData()).getWorld()) : CompletableFuture.completedFuture((Object)null);
         }));
      }
   }
   ...
   public static class RespawnPageEventData {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(RespawnPageEventData.class, RespawnPageEventData::new).addField(new KeyedCodec("Action", Codec.STRING), (entry, s) -> entry.action = s, (entry) -> entry.action)).build();
      }
   }
}
