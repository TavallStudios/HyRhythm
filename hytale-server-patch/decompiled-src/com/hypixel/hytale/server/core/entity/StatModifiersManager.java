...
public class StatModifiersManager {
   ...
   public void recalculateEntityStatModifiers(@Nonnull Ref<EntityStore> ref, @Nonnull EntityStatMap statMap, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (this.recalculate.getAndSet(false)) {
         ...
         Entity var6 = EntityUtils.getEntity(ref, componentAccessor);
      ...
      if (effectControllerComponent == null) {
         ...
         effectControllerComponent.getActiveEffects().forEach((k, v) -> {
            if (v.isInfinite() || !(v.getRemainingDuration() <= 0.0F)) {
               ...
               EntityEffect effect = (EntityEffect)EntityEffect.getAssetMap().getAsset(index);
               if (effect != null && effect.getStatModifiers() != null) {
                  Iterator i$ = effect.getStatModifiers().int2ObjectEntrySet().iterator();

                  while(i$.hasNext()) {
                     Int2ObjectMap.Entry<StaticModifier[]> entry = (Int2ObjectMap.Entry)i$.next();
                     ...
                     for(StaticModifier modifier : (StaticModifier[])entry.getValue()) {
                        ...
                        Object2FloatMap<StaticModifier.CalculationType> statModifierToApply = (Object2FloatMap)statModifiers.computeIfAbsent(entityStatType, (x) -> new Object2FloatOpenHashMap());
   ...
   private static void applyEffectModifiers(@Nonnull EntityStatMap statMap, @Nonnull Int2ObjectMap<Object2FloatMap<StaticModifier.CalculationType>> statModifiers) {
      for(int i = 0; i < statMap.size(); ++i) {
         ...
         if (statModifiersForEntityStat == null) {
            ...
            ObjectIterator var8 = statModifiersForEntityStat.object2FloatEntrySet().iterator();

            while(var8.hasNext()) {
               Object2FloatMap.Entry<StaticModifier.CalculationType> entry = (Object2FloatMap.Entry)var8.next();
               StaticModifier.CalculationType calculationType = (StaticModifier.CalculationType)entry.getKey();
   ...
   private static void computeStatModifiers(double brokenPenalty, @Nonnull Int2ObjectMap<Object2FloatMap<StaticModifier.CalculationType>> statModifiers, @Nonnull ItemStack itemInHand, @Nonnull Int2ObjectMap<StaticModifier[]> itemStatModifiers) {
      ...
      ObjectIterator var6 = itemStatModifiers.int2ObjectEntrySet().iterator();

      while(var6.hasNext()) {
         Int2ObjectMap.Entry<StaticModifier[]> entry = (Int2ObjectMap.Entry)var6.next();
         int entityStatType = entry.getIntKey();
   ...
   private static void addItemStatModifiers(@Nullable ItemStack itemStack, @Nonnull EntityStatMap entityStatMap, @Nonnull String prefix, @Nonnull Function<Item, Int2ObjectMap<StaticModifier[]>> toStatModifiers) {
      if (ItemStack.isEmpty(itemStack)) {
         clearAllStatModifiers(EntityStatMap.Predictable.SELF, entityStatMap, prefix, (Int2ObjectMap)null);
      } else {
         Int2ObjectMap<StaticModifier[]> itemStatModifiers = (Int2ObjectMap)toStatModifiers.apply(itemStack.getItem());
         if (itemStatModifiers == null) {
            clearAllStatModifiers(EntityStatMap.Predictable.SELF, entityStatMap, prefix, (Int2ObjectMap)null);
         } else {
            ObjectIterator var5 = itemStatModifiers.int2ObjectEntrySet().iterator();

            while(var5.hasNext()) {
               Int2ObjectMap.Entry<StaticModifier[]> entry = (Int2ObjectMap.Entry)var5.next();
               int offset = 0;
               ...
               for(StaticModifier modifier : (StaticModifier[])entry.getValue()) {
                  ...
                  if (existing instanceof StaticModifier) {
                     StaticModifier existingStatic = (StaticModifier)existing;
