...
public class UniqueItemUsagesComponent implements Component<EntityStore> {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(UniqueItemUsagesComponent.class, UniqueItemUsagesComponent::new).append(new KeyedCodec("UniqueItemUsed", new ArrayCodec(Codec.STRING, (x$0) -> new String[x$0])), (playerMemories, usages) -> {
         if (usages != null) {
            Collections.addAll(playerMemories.usedUniqueItems, usages);
         }
      }, (playerMemories) -> (String[])playerMemories.usedUniqueItems.toArray((x$0) -> new String[x$0])).add()).build();
   }
}
