...
public class ExplosionUtils {
   ...
   private static void processTargetBlocks(@Nonnull Vector3d position, @Nonnull ExplosionConfig config, @Nullable Ref<EntityStore> ignoreRef, @Nonnull Set<Ref<EntityStore>> targetRefs, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull ComponentAccessor<ChunkStore> chunkStore) {
      ...
      if (config.damageEntities) {
         double var10002 = (double)config.entityDamageRadius;
         Objects.requireNonNull(potentialTargets);
         Selector.selectNearbyEntities(commandBuffer, position, var10002, potentialTargets::add, (e) -> ignoreRef == null || !e.equals(ignoreRef));
      }
      ...
      if (config.damageBlocks || !potentialTargets.isEmpty()) {
         ...
         BlockSphereUtil.forEachBlock(posX, posY, posZ, explosionBlockRadius, (Object)null, (x, y, z, aVoid) -> {
         ...
         for(Vector3i targetBlock : targetBlocks) {
            ...
            if (!(distance <= 0.0) && !Double.isNaN(distance)) {
               ...
               Vector3i targetBlockPos = TargetUtil.getTargetBlock(world, (id, fluidId) -> isValidTargetBlock(id, config.damageBlocks), position.x, position.y, position.z, direction.x, direction.y, direction.z, distance);
               if (targetBlockPos == null) {
                  ...
                  float damageScale = calculateBlockDamageScale(damageDistance, (float)explosionBlockRadius, config.blockDamageFalloff);
                  ...
                  if (chunkReference != null) {
                     boolean canDamageBlock = distance <= (double)config.blockDamageRadius;
                     if (!config.damageBlocks || canDamageBlock && !BlockHarvestUtils.performBlockDamage(targetBlockPos, (ItemStack)null, itemTool, damageScale, setBlockSettings, chunkReference, commandBuffer, chunkStore)) {
   ...
   private static float calculateBlockDamageScale(float distance, float radius, float fallOff) {
      if (distance >= radius) {
         ...
         return 1.0F - (float)Math.pow((double)normalizedDistance, (double)fallOff);
      }
   }
   ...
   private static void processTargetEntity(@Nonnull ExplosionConfig config, @Nonnull Ref<EntityStore> targetRef, @Nonnull Vector3d position, @Nonnull Damage.Source damageSource, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      Velocity targetVelocityComponent = (Velocity)commandBuffer.getComponent(targetRef, Velocity.getComponentType());
      ...
      float damage = (float)((double)explosionDamage * Math.pow(1.0 - distance / (double)entityDamageRadius, (double)explosionFalloff));
      if (damage > 0.0F) {
      ...
      if (knockbackConfig != null) {
         ...
         KnockbackComponent knockbackComponent = (KnockbackComponent)commandBuffer.getComponent(targetRef, knockbackComponentType);
