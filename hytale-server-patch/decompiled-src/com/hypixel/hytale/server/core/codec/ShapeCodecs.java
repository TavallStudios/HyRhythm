...
public class ShapeCodecs {
   ...
   static {
      BOX = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(Box.class, Box::new).addField(new KeyedCodec("Min", Vector3d.CODEC), (shape, min) -> shape.min.assign(min), (shape) -> shape.min)).addField(new KeyedCodec("Max", Vector3d.CODEC), (shape, max) -> shape.max.assign(max), (shape) -> shape.max)).build();
      ...
      CYLINDER = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(Cylinder.class, Cylinder::new).addField(new KeyedCodec("Height", Codec.DOUBLE), (shape, height) -> shape.height = height, (shape) -> shape.height)).addField(new KeyedCodec("RadiusX", Codec.DOUBLE), (shape, radiusX) -> shape.radiusX = radiusX, (shape) -> shape.radiusX)).addField(new KeyedCodec("RadiusZ", Codec.DOUBLE), (shape, radiusZ) -> shape.radiusZ = radiusZ, (shape) -> shape.radiusZ)).addField(new KeyedCodec("Radius", Codec.DOUBLE), Cylinder::assign, (shape) -> null)).build();
      ORIGIN_SHAPE = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(OriginShape.class, OriginShape::new).addField(new KeyedCodec("Origin", Vector3d.CODEC), (shape, origin) -> shape.origin.assign(origin), (shape) -> shape.origin)).addField(new KeyedCodec("Shape", SHAPE), (shape, childShape) -> shape.shape = (S)childShape, (shape) -> shape.shape)).build();
      SHAPE.register((String)"Box", Box.class, BOX);
      SHAPE.register((String)"Ellipsoid", Ellipsoid.class, ELLIPSOID);
      SHAPE.register((String)"Cylinder", Cylinder.class, CYLINDER);
      SHAPE.register((String)"OriginShape", OriginShape.class, ORIGIN_SHAPE);
   }
}
