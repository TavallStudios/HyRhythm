...
public class BlockPhysics implements Component<ChunkStore> {
   ...
   public int get(int index) {
      ...
      try {
         if (this.supportData != null) {
            var4 = BitUtil.getNibble(this.supportData, index);
            return var4;
         }
   ...
   public static void clear(@Nonnull Store<ChunkStore> store, @Nonnull Ref<ChunkStore> section, int x, int y, int z) {
      BlockPhysics blockPhysics = (BlockPhysics)store.getComponent(section, getComponentType());
   ...
   public static void clear(@Nonnull Holder<ChunkStore> section, int x, int y, int z) {
      BlockPhysics blockPhysics = (BlockPhysics)section.getComponent(getComponentType());
      if (blockPhysics != null) {
         blockPhysics.set(ChunkUtil.indexBlock(x, y, z), 0);
      }
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BlockPhysics.class, BlockPhysics::new).versioned()).codecVersion(0)).append(new KeyedCodec("Data", Codec.BYTE_ARRAY), BlockPhysics::deserialize, BlockPhysics::serialize).add()).build();
   }
}
