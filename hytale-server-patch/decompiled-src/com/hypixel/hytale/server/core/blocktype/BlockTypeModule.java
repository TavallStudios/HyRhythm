...
public class BlockTypeModule extends JavaPlugin {
         ...
         if (column != null) {
            ...
            if (sections != null) {
               for(int sectionIndex = 0; sectionIndex < 10; ++sectionIndex) {
                  BlockSection section = (BlockSection)sections[sectionIndex].ensureAndGetComponent(BlockSection.getComponentType());
      ...
      if (event.isNewlyGenerated()) {
         ...
         if (column != null) {
            ...
            if (sections != null) {
               ...
               Arrays.fill(tempBlocks, (Object)null);
   ...
   private static void onChunksectionPreLoadProcess(@Nonnull WorldChunk chunk, @Nonnull BlockSection section, int sectionIndex, @Nonnull BlockType[] blocks) {
      ...
      for(int y = 0; y < 32; ++y) {
         ...
         for(int x = 0; x < 32; ++x) {
            ...
            for(int z = 0; z < 32; ++z) {
               BlockType blockType = getBlockType(blockTypeAssetMap, blocks, section, finalX, finalY, z, true);
               if (blockType != null) {
                  ...
                  if (filler != 0) {
                     ...
                     if (blockX >= 0 && blockX < 32) {
                        ...
                        if (blockY >= 0 && blockY < 320) {
                           int blockZ = z - FillerBlockUtil.unpackZ(filler);
                           if (blockZ >= 0 && blockZ < 32) {
                              ...
                              if (originBlockType != null) {
                                 ...
                                 if (blockType.isUnknown() || !blockTypeKey.equals(originBlockType.getId())) {
                                    chunk.breakBlock(finalX, finalY, z, 157);
                                 }
                              }
                           }
                        }
   ...
   private static BlockType getOriginBlockType(@Nonnull BlockTypeAssetMap<String, BlockType> blockTypeAssetMap, @Nonnull ChunkAccessor<?> accessor, @Nonnull BlockAccessor section, int originX, int originY, int originZ) {
      if (originX >= 0 && originX < 32 && originY >= 0 && originY < 320 && originZ >= 0 && originZ < 32) {
         ...
         if (fillerOriginChunk != null) {
            ...
            get().getLogger().at(Level.WARNING).log("Blocking chunk load when trying to get origin block for filler! Origin: %s, %s, %s", originX, originY, originZ);
   ...
   private static class MigrateLegacySections extends ChunkColumnMigrationSystem {
      ...
      private MigrateLegacySections() {
         this.DEPENDENCIES = Set.of(new SystemDependency(Order.BEFORE, LegacyModule.MigrateLegacySections.class), new SystemDependency(Order.AFTER, ChunkSystems.OnNewChunk.class), RootDependency.first());
      }
