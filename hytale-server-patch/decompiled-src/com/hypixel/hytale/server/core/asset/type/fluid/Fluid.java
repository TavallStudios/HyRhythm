...
public class Fluid implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, Fluid>>, NetworkSerializable<com.hypixel.hytale.protocol.Fluid> {
   ...
   public Fluid() {
      this.drawType = FluidDrawType.Liquid;
      this.opacity = Opacity.Solid;
      this.requiresAlphaBlending = true;
      this.fluidFXId = "Empty";
      this.fluidFXIndex = 0;
      this.ticker = DefaultFluidTicker.INSTANCE;
      this.blockSoundSetId = "EMPTY";
   ...
   public static String convertLegacyName(@Nonnull String fluidName, byte level) {
      String var10000;
      switch (fluidName) {
         case "Fluid_Water" -> var10000 = level == 0 ? "Water_Source" : "Water";
         case "Fluid_Water_Test" -> var10000 = "Water_Finite";
         case "Fluid_Lava" -> var10000 = level == 0 ? "Lava_Source" : "Lava";
         case "Fluid_Tar" -> var10000 = level == 0 ? "Tar_Source" : "Tar";
         case "Fluid_Slime" -> var10000 = level == 0 ? "Slime_Source" : "Slime";
         case "Fluid_Poison" -> var10000 = level == 0 ? "Poison_Source" : "Poison";
   ...
   public static ConversionResult convertBlockToFluid(@Nonnull String blockTypeStr) {
      ...
      if (fluidPos != -1) {
         ...
         if (fluidNameEnd != -1) {
            ...
            String var19 = blockTypeStr.substring(0, fluidPos);
         ...
         if (fluidLevelStart != -1) {
            ...
            if (fluidLevelEnd != -1) {
               ...
               String var20 = blockTypeStr.substring(0, fluidLevelStart);
         ...
         return new ConversionResult(blockTypeStr, fluidId, fluidLevel);
         ...
         if (fluidLevelStart != -1) {
            ...
            if (fluidLevelEnd != -1) {
               fluidLevelStr = blockTypeStr.substring(fluidLevelStart + "|FluidLevel=".length(), fluidLevelEnd);
               String var10000 = blockTypeStr.substring(0, fluidLevelStart);
         ...
         fluidLevel = fluidLevel == 0 ? (byte)((Fluid)getAssetMap().getAsset(fluidId)).getMaxFluidLevel() : fluidLevel;
         return new ConversionResult((String)null, fluidId, fluidLevel);
      } else {
         return null;
      }
   }
   ...
   static {
      ...
      EMPTY = new Fluid("Empty") {
         {
            this.processConfig();
         }
      };
      UNKNOWN = new Fluid("Unknown") {
         {
            this.unknown = true;
            this.processConfig();
         }
