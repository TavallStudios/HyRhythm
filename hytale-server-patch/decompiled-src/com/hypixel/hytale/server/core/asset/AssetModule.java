...
public class AssetModule extends JavaPlugin {
   ...
   protected void setup() {
      ...
      for(ObjectBooleanPair<AssetPack> p : this.pendingAssetPacks) {
         if (this.getAssetPack(((AssetPack)p.left()).getName()) != null) {
            ...
            this.getLogger().at(Level.WARNING).log("Asset pack with name '%s' already exists, skipping registration from path: %s", ((AssetPack)p.left()).getName(), ((AssetPack)p.left()).getRoot());
         } else {
            ...
            if (!pack.getName().equals("Hytale:Hytale")) {
               ...
               if (targetServerVersion == null || !targetServerVersion.equals(serverVersion)) {
                  ...
                  if (targetServerVersion != null && !"*".equals(targetServerVersion)) {
                     this.getLogger().at(Level.WARNING).log("Plugin '%s' targets a different server version %s. You may encounter issues, please check for plugin updates.", pack.getName(), serverVersion);
                  } else {
   ...
   private void loadAndRegisterPack(Path packPath, boolean isExternal) {
      ...
      ModConfig modConfig = (ModConfig)serverConfig.getModConfig().get(packIdentifier);
      boolean enabled;
