...
public class BlockType implements JsonAssetWithMap<String, BlockTypeAssetMap<String, BlockType>>, NetworkSerializable<com.hypixel.hytale.protocol.BlockType> {
   ...
   public static BlockType fromString(@Nonnull String input) {
      return (BlockType)getAssetMap().getAsset(input);
   }
   ...
   public static BlockTypeAssetMap<String, BlockType> getAssetMap() {
      return (BlockTypeAssetMap)getAssetStore().getAssetMap();
   }

   public BlockType() {
      this.cubeShadingMode = ShadingMode.Standard;
      this.customModelScale = 1.0F;
      this.drawType = DrawType.Cube;
      this.material = BlockMaterial.Empty;
      this.opacity = Opacity.Solid;
      this.blockSupportsRequiredFor = BlockSupportsRequiredForType.All;
      this.randomRotation = RandomRotation.None;
      this.variantRotation = VariantRotation.None;
      this.flipType = BlockFlipType.SYMMETRIC;
      this.rotationYawPlacementOffset = Rotation.None;
      this.hitboxType = "Full";
      this.hitboxTypeIndex = 0;
      this.interactionHitboxTypeIndex = -2147483648;
      this.movementSettings = new BlockMovementSettings();
      this.flags = new BlockFlags(false, true);
      this.allowsMultipleUsers = true;
      this.supportDropType = SupportDropType.BREAK;
      this.interactions = Collections.emptyMap();
      this.aliases = EMPTY_ALIAS_LIST;
   }
   ...
   public com.hypixel.hytale.protocol.BlockType toPacket() {
      com.hypixel.hytale.protocol.BlockType cached = this.cachedPacket == null ? null : (com.hypixel.hytale.protocol.BlockType)this.cachedPacket.get();
      if (cached != null) {
         return cached;
      } else {
         ...
         switch (this.blockSupportsRequiredFor) {
            case Any -> var10001 = com.hypixel.hytale.protocol.BlockSupportsRequiredForType.Any;
            case All -> var10001 = com.hypixel.hytale.protocol.BlockSupportsRequiredForType.All;
         ...
         if (this.textures != null && this.textures.length > 0) {
            ...
            for(int i = 0; i < this.textures.length; ++i) {
               texturePackets[i] = this.textures[i].toPacket((float)totalWeight);
            }
         ...
         if (this.customModelTexture != null && this.customModelTexture.length > 0) {
            ...
            for(int i = 0; i < this.customModelTexture.length; ++i) {
               texturePackets[i] = this.customModelTexture[i].toPacket((float)totalWeight);
            }
         ...
         if (this.transitionToTag != null) {
            ...
            packet.transitionToTag = -2147483648;
         }
   ...
   public Item getItem() {
      if (this.data == null) {
         ...
         return itemKey == null ? null : (Item)Item.getAssetMap().getAsset(itemKey);
      }
   }
   ...
   public BlockType getBlockForState(@Nonnull String state) {
      ...
      return key == null ? null : (BlockType)getAssetMap().getAsset(key);
   }
   ...
   public Map<BlockFace, RequiredBlockFaceSupport[]> getSupport(int rotationIndex) {
      if (this.support != null && rotationIndex != 0) {
         ...
         if (rotatedSupportArray == null) {
            ...
            for(Map.Entry<BlockFace, RequiredBlockFaceSupport[]> entry : this.support.entrySet()) {
               ...
               for(RequiredBlockFaceSupport requiredBlockFaceSupport : requiredBlockFaceSupports) {
                  if (requiredBlockFaceSupport.isRotated()) {
                     ...
                     ((List)rotatedSupport.computeIfAbsent(rotatedBlockFace, (k) -> new ObjectArrayList())).add(rotatedRequiredBlockFaceSupport);
                  } else {
                     ((List)rotatedSupport.computeIfAbsent(blockFace, (k) -> new ObjectArrayList())).add(requiredBlockFaceSupport);
                  }
               }
            }
            ...
            for(Map.Entry<BlockFace, List<RequiredBlockFaceSupport>> entry : rotatedSupport.entrySet()) {
               rotatedSupportArray.put((BlockFace)entry.getKey(), (RequiredBlockFaceSupport[])((List)entry.getValue()).toArray((x$0) -> new RequiredBlockFaceSupport[x$0]));
            }
            ...
            if (this.isFullySupportive()) {
               ...
               for(Map.Entry<BlockFace, BlockFaceSupport[]> entry : this.supporting.entrySet()) {
                  ...
                  for(BlockFaceSupport blockFaceSupport : blockFaceSupports) {
                     ...
                     ((List)rotatedSupporting.computeIfAbsent(rotatedBlockFace, (k) -> new ObjectArrayList())).add(rotatedBlockFaceSupport);
                  }
               }

               rotatedSupportingArray = new EnumMap(BlockFace.class);

               for(Map.Entry<BlockFace, List<BlockFaceSupport>> entry : rotatedSupporting.entrySet()) {
                  rotatedSupportingArray.put((BlockFace)entry.getKey(), (BlockFaceSupport[])((List)entry.getValue()).toArray((x$0) -> new BlockFaceSupport[x$0]));
               }
            }
   ...
   protected void processConfig() {
      ...
      if (this.hitboxTypeIndex == -2147483648) {
      ...
      if (this.interactionHitboxType != null) {
         this.interactionHitboxTypeIndex = this.interactionHitboxType.equals("Full") ? 0 : BlockBoundingBoxes.getAssetMap().getIndex(this.interactionHitboxType);
         if (this.interactionHitboxTypeIndex == -2147483648) {
            HytaleLogger.getLogger().at(Level.WARNING).log("Unknown interaction hitbox '%s' for block '%s', using collision hitbox", this.interactionHitboxType, this.getId());
      ...
      if (this.blockSoundSetIndex == -2147483648) {
   ...
   public void getBlockCenter(int rotationIndex, @Nonnull Vector3d outCenter) {
      BlockBoundingBoxes hitboxAsset = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(this.hitboxTypeIndex);
      if (hitboxAsset == null) {
   ...
   public static int getBlockIdOrUnknown(@Nonnull BlockTypeAssetMap<String, BlockType> assetMap, @Nonnull String blockTypeKey, String message, Object... params) {
      ...
      if (blockId == -2147483648) {
         ...
         AssetRegistry.getAssetStore(BlockType.class).loadAssets("Hytale:Hytale", Collections.singletonList(getUnknownFor(blockTypeKey)), AssetUpdateQuery.DEFAULT_NO_REBUILD);
   ...
   static {
      ...
      }, (blockType) -> null).metadata(new UIEditorSectionStart("Tint")).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("TintUp", ProtocolCodecs.COLOR_ARRAY), (blockType, o) -> blockType.tintUp = o, (blockType) -> blockType.tintUp, (blockType, parent) -> blockType.tintUp = parent.tintUp).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("TintDown", ProtocolCodecs.COLOR_ARRAY), (blockType, o) -> blockType.tintDown = o, (blockType) -> blockType.tintDown, (blockType, parent) -> blockType.tintDown = parent.tintDown).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("TintNorth", ProtocolCodecs.COLOR_ARRAY), (blockType, o) -> blockType.tintNorth = o, (blockType) -> blockType.tintNorth, (blockType, parent) -> blockType.tintNorth = parent.tintNorth).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("TintSouth", ProtocolCodecs.COLOR_ARRAY), (blockType, o) -> blockType.tintSouth = o, (blockType) -> blockType.tintSouth, (blockType, parent) -> blockType.tintSouth = parent.tintSouth).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("TintWest", ProtocolCodecs.COLOR_ARRAY), (blockType, o) -> blockType.tintWest = o, (blockType) -> blockType.tintWest, (blockType, parent) -> blockType.tintWest = parent.tintWest).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("TintEast", ProtocolCodecs.COLOR_ARRAY), (blockType, o) -> blockType.tintEast = o, (blockType) -> blockType.tintEast, (blockType, parent) -> blockType.tintEast = parent.tintEast).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).append(new KeyedCodec("BiomeTint", Codec.INTEGER), (blockType, o) -> {
      ...
      }, (blockType) -> null).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("BiomeTintUp", Codec.INTEGER), (blockType, o) -> blockType.biomeTintUp = o, (blockType) -> blockType.biomeTintUp, (blockType, parent) -> blockType.biomeTintUp = parent.biomeTintUp).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("BiomeTintDown", Codec.INTEGER), (blockType, o) -> blockType.biomeTintDown = o, (blockType) -> blockType.biomeTintDown, (blockType, parent) -> blockType.biomeTintDown = parent.biomeTintDown).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("BiomeTintNorth", Codec.INTEGER), (blockType, o) -> blockType.biomeTintNorth = o, (blockType) -> blockType.biomeTintNorth, (blockType, parent) -> blockType.biomeTintNorth = parent.biomeTintNorth).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("BiomeTintSouth", Codec.INTEGER), (blockType, o) -> blockType.biomeTintSouth = o, (blockType) -> blockType.biomeTintSouth, (blockType, parent) -> blockType.biomeTintSouth = parent.biomeTintSouth).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("BiomeTintWest", Codec.INTEGER), (blockType, o) -> blockType.biomeTintWest = o, (blockType) -> blockType.biomeTintWest, (blockType, parent) -> blockType.biomeTintWest = parent.biomeTintWest).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("BiomeTintEast", Codec.INTEGER), (blockType, o) -> blockType.biomeTintEast = o, (blockType) -> blockType.biomeTintEast, (blockType, parent) -> blockType.biomeTintEast = parent.biomeTintEast).metadata(new UIRebuildCaches(new UIRebuildCaches.ClientCache[]{UIRebuildCaches.ClientCache.MAP_GEOMETRY})).add()).appendInherited(new KeyedCodec("State", StateData.CODEC), (blockType, s) -> {
      ...
      EMPTY_ALIAS_LIST = new String[0];
      StateData.addDefinitions();
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(BlockType::getAssetStore));
      UNKNOWN_CUSTOM_MODEL_TEXTURE = new ModelTexture[]{new ModelTexture("BlockTextures/Unknown.png", 1.0F)};
      UNKNOWN_BLOCK_TEXTURES = new BlockTextures[]{new BlockTextures("BlockTextures/Unknown.png", "BlockTextures/Unknown.png", "BlockTextures/Unknown.png", "BlockTextures/Unknown.png", "BlockTextures/Unknown.png", "BlockTextures/Unknown.png", 1.0F)};
