...
public class Model implements NetworkSerializable<com.hypixel.hytale.protocol.Model> {
   ...
   public Model(String modelAssetId, float scale, Map<String, String> randomAttachmentIds, ModelAttachment[] attachments, @Nullable Box boundingBox, String model, String texture, String gradientSet, String gradientId, float eyeHeight, float crouchOffset, float sittingOffset, float sleepingOffset, Map<String, ModelAsset.AnimationSet> animationSetMap, CameraSettings camera, ColorLight light, ModelParticle[] particles, ModelTrail[] trails, PhysicsValues physicsValues, Map<String, DetailBox[]> detailBoxes, Phobia phobia, String phobiaModelAssetId) {
      ...
      this.crouchBoundingBox = boundingBox == null ? null : new Box(boundingBox.min.clone(), boundingBox.max.clone().add(0.0, (double)crouchOffset, 0.0));
      this.sittingBoundingBox = boundingBox == null ? null : new Box(boundingBox.min.clone(), boundingBox.max.clone().add(0.0, (double)sittingOffset, 0.0));
      this.sleepingBoundingBox = boundingBox == null ? null : new Box(boundingBox.min.clone(), boundingBox.max.clone().add(0.0, (double)sleepingOffset, 0.0));
      this.phobia = phobia;
   ...
   public com.hypixel.hytale.protocol.Model toPacket() {
      com.hypixel.hytale.protocol.Model cached = this.cachedPacket == null ? null : (com.hypixel.hytale.protocol.Model)this.cachedPacket.get();
      if (cached != null) {
         return cached;
      } else {
         ...
         if (this.detailBoxes != null) {
            ...
            for(Map.Entry<String, DetailBox[]> entry : this.detailBoxes.entrySet()) {
               map.put((String)entry.getKey(), (com.hypixel.hytale.protocol.DetailBox[])Arrays.stream((DetailBox[])entry.getValue()).map(NetworkSerializable::toPacket).toArray((x$0) -> new com.hypixel.hytale.protocol.DetailBox[x$0]));
            }
         }

         if (this.phobia != Phobia.None && this.phobiaModelAssetId != null) {
            ModelAsset modelAsset = (ModelAsset)ModelAsset.getAssetMap().getAsset(this.phobiaModelAssetId);
            if (modelAsset != null) {
   ...
   public ModelReference toReference() {
      return "Player".equals(this.modelAssetId) ? Model.ModelReference.DEFAULT_PLAYER_MODEL : new ModelReference(this.modelAssetId, this.scale, this.randomAttachmentIds, this.animationSetMap == null);
   }
   ...
   public boolean equals(@Nullable Object o) {
      if (this == o) {
         ...
         if (Float.compare(model1.scale, this.scale) != 0) {
            ...
            return false;
         } else if (!Arrays.equals(this.attachments, model1.attachments)) {
            return false;
            ...
            return false;
         } else if (!Arrays.equals(this.particles, model1.particles)) {
            return false;
         } else {
            return !Arrays.equals(this.trails, model1.trails) ? false : Objects.equals(this.physicsValues, model1.physicsValues);
         }
      } else {
         return false;
      }
   ...
   public String toString() {
      ...
      return "Model{modelAssetId='" + var10000 + "', scale=" + this.scale + ", randomAttachmentIds=" + String.valueOf(this.randomAttachmentIds) + ", attachments=" + Arrays.toString(this.attachments) + ", boundingBox=" + String.valueOf(this.boundingBox) + ", crouchBoundingBox=" + String.valueOf(this.crouchBoundingBox) + ", sittingBoundingBox=" + String.valueOf(this.sittingBoundingBox) + ", sleepingBoundingBox=" + String.valueOf(this.sleepingBoundingBox) + ", model='" + this.model + "', texture='" + this.texture + "', gradientSet='" + this.gradientSet + "', gradientId='" + this.gradientId + "', eyeHeight=" + this.eyeHeight + ", crouchOffset=" + this.crouchOffset + ", sittingOffset=" + this.sittingOffset + ", sleepingOffset=" + this.sleepingOffset + ", animationSetMap=" + String.valueOf(this.animationSetMap) + ", camera=" + String.valueOf(this.camera) + ", light=" + String.valueOf(this.light) + ", particles=" + Arrays.toString(this.particles) + ", trails=" + Arrays.toString(this.trails) + ", physicsValues=" + String.valueOf(this.physicsValues) + ", detailBoxes=" + String.valueOf(this.detailBoxes) + ", phobia=" + String.valueOf(this.phobia) + ", phobiaModelAssetId='" + this.phobiaModelAssetId + "'}";
   }
   ...
   public static Model createStaticScaledModel(@Nonnull ModelAsset modelAsset, float scale) {
      return createScaledModel(modelAsset, scale, modelAsset.generateRandomAttachmentIds(), (Box)null, true);
   }

   @Nonnull
   public static Model createUnitScaleModel(@Nonnull ModelAsset modelAsset) {
      return createScaledModel(modelAsset, 1.0F, (Map)null);
   }

   @Nonnull
   public static Model createUnitScaleModel(@Nonnull ModelAsset modelAsset, @Nullable Box boundingBox) {
      return createScaledModel(modelAsset, 1.0F, (Map)null, boundingBox);
   }
   ...
   public static Model createScaledModel(@Nonnull ModelAsset modelAsset, float scale, @Nullable Map<String, String> randomAttachmentIds) {
      return createScaledModel(modelAsset, scale, randomAttachmentIds, (Box)null, false);
   }
   ...
   public static Model createScaledModel(@Nonnull ModelAsset modelAsset, float scale, @Nullable Map<String, String> randomAttachmentIds, @Nullable Box overrideBoundingBox, boolean staticModel) {
      ...
      if (scale <= 0.0F) {
         ...
         if (scale != 1.0F) {
            ...
            if (detailBoxes != null) {
               ...
               for(Map.Entry<String, DetailBox[]> entry : detailBoxes.entrySet()) {
                  scaledDetailBoxes.put((String)entry.getKey(), (DetailBox[])Arrays.stream((DetailBox[])entry.getValue()).map((v) -> v.scaled(scale)).toArray((x$0) -> new DetailBox[x$0]));
               }
         ...
         return new Model(modelAsset.getId(), scale, randomAttachmentIds, attachments, boundingBox, modelAsset.getModel(), modelAsset.getTexture(), modelAsset.getGradientSet(), modelAsset.getGradientId(), eyeHeight, crouchOffset, sittingOffset, sleepingOffset, animationSetMap, camera, modelAsset.getLight(), particles, trails, physicsValues, detailBoxes, modelAsset.getPhobia(), modelAsset.getPhobiaModelAssetId());
      }
   }
   ...
   public static class ModelReference {
      ...
      public Model toModel() {
         if (this.modelAssetId == null) {
            ...
            ModelAsset modelAsset = (ModelAsset)ModelAsset.getAssetMap().getAsset(this.modelAssetId);
            if (modelAsset == null) {
               modelAsset = ModelAsset.DEBUG;
            }

            return Model.createScaledModel(modelAsset, this.scale, this.randomAttachmentIds, (Box)null, this.staticModel);
         }
      }
