...
public class EqualizerEffect implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, EqualizerEffect>>, NetworkSerializable<com.hypixel.hytale.protocol.EqualizerEffect> {
   ...
   public com.hypixel.hytale.protocol.EqualizerEffect toPacket() {
      com.hypixel.hytale.protocol.EqualizerEffect cached = this.cachedPacket == null ? null : (com.hypixel.hytale.protocol.EqualizerEffect)this.cachedPacket.get();
      if (cached != null) {
         return cached;
      } else {
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(EqualizerEffect.class, EqualizerEffect::new, Codec.STRING, (t, k) -> t.id = k, (t) -> t.id, (asset, data) -> asset.data = data, (asset) -> asset.data).documentation("An asset used to define a 4-band equalizer audio effect.")).metadata(new UIEditorPreview(UIEditorPreview.PreviewType.EQUALIZER_EFFECT))).appendInherited(new KeyedCodec("LowGain", Codec.FLOAT), (eq, f) -> eq.lowGain = AudioUtil.decibelsToLinearGain(f), (eq) -> AudioUtil.linearGainToDecibels(eq.lowGain), (eq, parent) -> eq.lowGain = parent.lowGain).metadata(new UIEditor(new UIEditor.FormattedNumber((Double)null, " dB", (Integer)null))).addValidator(Validators.range(-18.0F, 18.0F)).documentation("Low band gain in decibels.").add()).appendInherited(new KeyedCodec("LowCutOff", Codec.FLOAT), (eq, f) -> eq.lowCutOff = f, (eq) -> eq.lowCutOff, (eq, parent) -> eq.lowCutOff = parent.lowCutOff).addValidator(Validators.range(50.0F, 800.0F)).documentation("Low band cutoff frequency in Hz.").add()).appendInherited(new KeyedCodec("LowMidGain", Codec.FLOAT), (eq, f) -> eq.lowMidGain = AudioUtil.decibelsToLinearGain(f), (eq) -> AudioUtil.linearGainToDecibels(eq.lowMidGain), (eq, parent) -> eq.lowMidGain = parent.lowMidGain).metadata(new UIEditor(new UIEditor.FormattedNumber((Double)null, " dB", (Integer)null))).addValidator(Validators.range(-18.0F, 18.0F)).documentation("Low-mid band gain in decibels.").add()).appendInherited(new KeyedCodec("LowMidCenter", Codec.FLOAT), (eq, f) -> eq.lowMidCenter = f, (eq) -> eq.lowMidCenter, (eq, parent) -> eq.lowMidCenter = parent.lowMidCenter).addValidator(Validators.range(200.0F, 3000.0F)).documentation("Low-mid band center frequency in Hz.").add()).appendInherited(new KeyedCodec("LowMidWidth", Codec.FLOAT), (eq, f) -> eq.lowMidWidth = f, (eq) -> eq.lowMidWidth, (eq, parent) -> eq.lowMidWidth = parent.lowMidWidth).addValidator(Validators.range(0.01F, 1.0F)).documentation("Low-mid band width.").add()).appendInherited(new KeyedCodec("HighMidGain", Codec.FLOAT), (eq, f) -> eq.highMidGain = AudioUtil.decibelsToLinearGain(f), (eq) -> AudioUtil.linearGainToDecibels(eq.highMidGain), (eq, parent) -> eq.highMidGain = parent.highMidGain).metadata(new UIEditor(new UIEditor.FormattedNumber((Double)null, " dB", (Integer)null))).addValidator(Validators.range(-18.0F, 18.0F)).documentation("High-mid band gain in decibels.").add()).appendInherited(new KeyedCodec("HighMidCenter", Codec.FLOAT), (eq, f) -> eq.highMidCenter = f, (eq) -> eq.highMidCenter, (eq, parent) -> eq.highMidCenter = parent.highMidCenter).addValidator(Validators.range(1000.0F, 8000.0F)).documentation("High-mid band center frequency in Hz.").add()).appendInherited(new KeyedCodec("HighMidWidth", Codec.FLOAT), (eq, f) -> eq.highMidWidth = f, (eq) -> eq.highMidWidth, (eq, parent) -> eq.highMidWidth = parent.highMidWidth).addValidator(Validators.range(0.01F, 1.0F)).documentation("High-mid band width.").add()).appendInherited(new KeyedCodec("HighGain", Codec.FLOAT), (eq, f) -> eq.highGain = AudioUtil.decibelsToLinearGain(f), (eq) -> AudioUtil.linearGainToDecibels(eq.highGain), (eq, parent) -> eq.highGain = parent.highGain).metadata(new UIEditor(new UIEditor.FormattedNumber((Double)null, " dB", (Integer)null))).addValidator(Validators.range(-18.0F, 18.0F)).documentation("High band gain in decibels.").add()).appendInherited(new KeyedCodec("HighCutOff", Codec.FLOAT), (eq, f) -> eq.highCutOff = f, (eq) -> eq.highCutOff, (eq, parent) -> eq.highCutOff = parent.highCutOff).addValidator(Validators.range(4000.0F, 16000.0F)).documentation("High band cutoff frequency in Hz.").add()).build();
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(EqualizerEffect::getAssetStore));
   }
}
