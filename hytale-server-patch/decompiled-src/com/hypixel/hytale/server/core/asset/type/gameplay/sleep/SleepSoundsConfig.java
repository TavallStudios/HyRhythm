...
public class SleepSoundsConfig {
   ...
   public long getNotificationCooldownSeconds() {
      return (long)this.notificationCooldownSeconds;
   }

   public long getNotificationLoopCooldownMs() {
      return (long)(this.notificationCooldownSeconds * 1000);
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SleepSoundsConfig.class, SleepSoundsConfig::new).append(new KeyedCodec("Success", Codec.STRING), (config, o) -> config.success = o, (config) -> config.success).addValidator(SoundEvent.VALIDATOR_CACHE.getValidator()).add()).append(new KeyedCodec("Fail", Codec.STRING), (config, o) -> config.fail = o, (config) -> config.fail).addValidator(SoundEvent.VALIDATOR_CACHE.getValidator()).add()).append(new KeyedCodec("Notification", Codec.STRING), (config, o) -> config.notification = o, (config) -> config.notification).addValidator(SoundEvent.VALIDATOR_CACHE.getValidator()).add()).append(new KeyedCodec("NotificationLoop", Codec.STRING), (config, o) -> config.notificationLoop = o, (config) -> config.notificationLoop).addValidator(SoundEvent.VALIDATOR_CACHE.getValidator()).add()).append(new KeyedCodec("NotificationCooldownSeconds", Codec.INTEGER), (config, o) -> config.notificationCooldownSeconds = o, (config) -> config.notificationCooldownSeconds).add()).append(new KeyedCodec("NotificationLoopEnabled", Codec.BOOLEAN), (config, o) -> config.notificationLoopEnabled = o, (config) -> config.notificationLoopEnabled).add()).build();
   }
}
