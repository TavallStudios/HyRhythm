...
public class CraftingRecipe implements JsonAssetWithMap<String, DefaultAssetMap<String, CraftingRecipe>> {
   ...
   public CraftingRecipe(MaterialQuantity[] input, MaterialQuantity primaryOutput, MaterialQuantity[] outputs, int outputQuantity, BenchRequirement[] benchRequirement, float timeSeconds, boolean knowledgeRequired, int requiredMemoriesLevel) {
      this.outputs = EMPTY_OUTPUT;
      this.primaryOutputQuantity = 1;
      this.requiredMemoriesLevel = 1;
      this.input = input;
   ...
   public com.hypixel.hytale.protocol.CraftingRecipe toPacket(String id) {
      ...
      if (this.input != null && this.input.length > 0) {
         packet.inputs = (com.hypixel.hytale.protocol.MaterialQuantity[])ArrayUtil.copyAndMutate(this.input, MaterialQuantity::toPacket, (x$0) -> new com.hypixel.hytale.protocol.MaterialQuantity[x$0]);
      }

      packet.primaryOutput = this.primaryOutput.toPacket();
      if (this.outputs != null && this.outputs.length > 0) {
         packet.outputs = (com.hypixel.hytale.protocol.MaterialQuantity[])ArrayUtil.copyAndMutate(this.outputs, MaterialQuantity::toPacket, (x$0) -> new com.hypixel.hytale.protocol.MaterialQuantity[x$0]);
      }
   ...
   public String toString() {
      ...
      return "CraftingRecipe{input=" + var10000 + ", extraOutputs=" + Arrays.toString(this.outputs) + ", primaryOutput=" + String.valueOf(this.primaryOutput) + ", outputQuantity=" + this.primaryOutputQuantity + ", benchRequirement=" + Arrays.toString(this.benchRequirement) + ", timeSeconds=" + this.timeSeconds + ", knowledgeRequired=" + this.knowledgeRequired + ", requiredMemoriesLevel=" + this.requiredMemoriesLevel + "}";
   }
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(CraftingRecipe.class, CraftingRecipe::new, Codec.STRING, (recipe, blockTypeKey) -> recipe.id = blockTypeKey, (recipe) -> recipe.id, (asset, data) -> asset.data = data, (asset) -> asset.data).append(new KeyedCodec("Input", new ArrayCodec(MaterialQuantity.CODEC, (x$0) -> new MaterialQuantity[x$0])), (craftingRecipe, objects) -> craftingRecipe.input = objects, (craftingRecipe) -> craftingRecipe.input).addValidator(Validators.nonNull()).add()).append(new KeyedCodec("Output", new ArrayCodec(MaterialQuantity.CODEC, (x$0) -> new MaterialQuantity[x$0])), (craftingRecipe, objects) -> craftingRecipe.outputs = objects, (craftingRecipe) -> craftingRecipe.outputs).add()).append(new KeyedCodec("PrimaryOutput", MaterialQuantity.CODEC), (craftingRecipe, objects) -> craftingRecipe.primaryOutput = objects, (craftingRecipe) -> craftingRecipe.primaryOutput).add()).append(new KeyedCodec("OutputQuantity", Codec.INTEGER), (craftingRecipe, quantity) -> craftingRecipe.primaryOutputQuantity = quantity, (craftingRecipe) -> craftingRecipe.primaryOutputQuantity).add()).append(new KeyedCodec("BenchRequirement", new ArrayCodec(((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BenchRequirement.class, BenchRequirement::new).append(new KeyedCodec("Type", new EnumCodec(BenchType.class)), (benchRequirement, benchType) -> benchRequirement.type = benchType, (benchRequirement) -> benchRequirement.type).addValidator(Validators.nonNull()).add()).append(new KeyedCodec("Id", Codec.STRING), (benchRequirement, s) -> benchRequirement.id = s, (benchRequirement) -> benchRequirement.id).addValidator(Validators.nonNull()).add()).append(new KeyedCodec("Categories", Codec.STRING_ARRAY), (benchRequirement, s) -> benchRequirement.categories = s, (benchRequirement) -> benchRequirement.categories).add()).appendInherited(new KeyedCodec("RequiredTierLevel", Codec.INTEGER), (benchRequirement, s) -> benchRequirement.requiredTierLevel = s, (benchRequirement) -> benchRequirement.requiredTierLevel, (benchRequirement, parent) -> benchRequirement.requiredTierLevel = parent.requiredTierLevel).add()).build(), (x$0) -> new BenchRequirement[x$0])), (craftingRecipe, objects) -> craftingRecipe.benchRequirement = objects, (craftingRecipe) -> craftingRecipe.benchRequirement).add()).append(new KeyedCodec("TimeSeconds", Codec.DOUBLE), (craftingRecipe, d) -> craftingRecipe.timeSeconds = d.floatValue(), (craftingRecipe) -> (double)craftingRecipe.timeSeconds).addValidator(Validators.min(0.0)).add()).append(new KeyedCodec("KnowledgeRequired", Codec.BOOLEAN), (craftingRecipe, b) -> craftingRecipe.knowledgeRequired = b, (craftingRecipe) -> craftingRecipe.knowledgeRequired).add()).append(new KeyedCodec("RequiredMemoriesLevel", Codec.INTEGER), (craftingRecipe, integer) -> craftingRecipe.requiredMemoriesLevel = integer, (craftingRecipe) -> craftingRecipe.requiredMemoriesLevel).documentation("The level of Memories starts from 1, meaning a recipe with a RequiredMemoriesLevel set at 1 will always be available to players.").addValidator(Validators.greaterThanOrEqual(1)).add()).validator((craftingRecipe, results) -> {
      ...
      EMPTY_OUTPUT = new MaterialQuantity[0];
   }
}
