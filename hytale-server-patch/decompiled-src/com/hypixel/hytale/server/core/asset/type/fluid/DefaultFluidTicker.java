...
public class DefaultFluidTicker extends FluidTicker {
            ...
            if (isSolid(blockBelow) || fluidBelowId != 0 && fluidBelowId != spreadFluidId && fluidBelowId == fluidId) {
               if (fluidBelowId == 0 || fluidBelowId != spreadFluidId) {
                  ...
                  if (spreadFluidId != fluidId) {
                     childFillLevel = ((Fluid)Fluid.getAssetMap().getAsset(spreadFluidId)).getMaxFluidLevel() - 1;
                  }
                        ...
                        if (!this.blocksFluidFrom(sourceBlock, sourceRotationIndex, -x, -z, sourceFiller)) {
                           ...
                           if (!this.blocksFluidFrom(block, rotationIndex, x, z, destFiller)) {
                              ...
                              if (otherFluidId != 0 && otherFluidId != spreadFluidId) {
                                 FluidCollisionConfig config = (FluidCollisionConfig)this.getCollisionMap().get(otherFluidId);
   ...
   public Int2ObjectMap<FluidCollisionConfig> getCollisionMap() {
      if (this.collisionMap == null) {
         Int2ObjectOpenHashMap<FluidCollisionConfig> collisionMap = new Int2ObjectOpenHashMap(this.rawCollisionMap.size());

         for(Map.Entry<String, FluidCollisionConfig> entry : this.rawCollisionMap.entrySet()) {
            int index = Fluid.getAssetMap().getIndex((String)entry.getKey());
            if (index != -2147483648) {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(DefaultFluidTicker.class, DefaultFluidTicker::new, BASE_CODEC).appendInherited(new KeyedCodec("SpreadFluid", Codec.STRING), (ticker, o) -> ticker.spreadFluid = o, (ticker) -> ticker.spreadFluid, (ticker, parent) -> ticker.spreadFluid = parent.spreadFluid).addValidator(Fluid.VALIDATOR_CACHE.getValidator().late()).add()).appendInherited(new KeyedCodec("Collisions", new MapCodec(DefaultFluidTicker.FluidCollisionConfig.CODEC, HashMap::new)), (ticker, o) -> ticker.rawCollisionMap = MapUtil.<String, FluidCollisionConfig>combineUnmodifiable(ticker.rawCollisionMap, o), (ticker) -> ticker.rawCollisionMap, (ticker, parent) -> ticker.rawCollisionMap = parent.rawCollisionMap).documentation("Defines what happens when this fluid tries to spread into another fluid").add()).build();
      INSTANCE = new DefaultFluidTicker();
   }
