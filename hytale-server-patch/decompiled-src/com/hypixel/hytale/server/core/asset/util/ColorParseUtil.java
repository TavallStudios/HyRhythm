...
public class ColorParseUtil {
   ...
   public static ColorAlpha readColorAlpha(@Nonnull RawJsonReader reader) throws IOException {
      ...
      ColorAlpha var10000;
      switch (reader.peek()) {
         case 35 -> var10000 = readHexStringToColorAlpha(reader);
         case 114 -> var10000 = readRgbaStringToColorAlpha(reader);
   ...
   public static Color readColor(@Nonnull RawJsonReader reader) throws IOException {
      ...
      Color var10000;
      switch (reader.peek()) {
         case 35 -> var10000 = readHexStringToColor(reader);
         case 114 -> var10000 = readRgbStringToColor(reader);
         default -> var10000 = null;
      }
   ...
   public static Color readHexStringToColor(@Nonnull RawJsonReader reader) throws IOException {
      ...
      return new Color((byte)(rgba >> 24 & 255), (byte)(rgba >> 16 & 255), (byte)(rgba >> 8 & 255));
   }
   ...
   public static ColorAlpha readHexStringToColorAlpha(@Nonnull RawJsonReader reader) throws IOException {
      int rgba = readHexAlphaStringToRGBAInt(reader);
      return new ColorAlpha((byte)(rgba & 255), (byte)(rgba >> 24 & 255), (byte)(rgba >> 16 & 255), (byte)(rgba >> 8 & 255));
   }
   ...
   public static int readHexAlphaStringToRGBAInt(@Nonnull RawJsonReader reader) throws IOException {
      ...
      try {
         ...
         int blue = value >> 4 & 15;
         int alpha = value & 15;
         var13 = red << 28 | red << 24 | green << 20 | green << 16 | blue << 12 | blue << 8 | alpha << 4 | alpha;
      } finally {
         reader.unmark();
         reader.consumeWhiteSpace();
      }
   ...
   public static ColorAlpha readRgbaDecimalStringToColor(@Nonnull RawJsonReader reader, boolean readStart) throws IOException {
      ...
      reader.expect(',');
      reader.consumeWhiteSpace();
      byte alpha = (byte)((int)MathUtil.clamp(reader.readFloatValue() * 255.0F, 0.0F, 255.0F));
      reader.consumeWhiteSpace();
      reader.expect(')');
      reader.consumeWhiteSpace();
   ...
   public static ColorAlpha rgbaDecimalStringToColor(String color) {
      ...
      if (color.startsWith("rgba(") && color.charAt(color.length() - 1) == ')') {
         ...
         if (channelLength != 4) {
            ...
            byte alpha = (byte)((int)MathUtil.clamp(Float.parseFloat(channels[3]) * 255.0F, 0.0F, 255.0F));
            return new ColorAlpha(alpha, red, green, blue);
         }
      } else {
   ...
   public static ColorAlpha readRgbaHexStringToColor(@Nonnull RawJsonReader reader, boolean readStart) throws IOException {
      ...
      long val = (long)readHexAlphaStringToRGBAInt(reader);
      reader.consumeWhiteSpace();
      reader.expect(',');
      reader.consumeWhiteSpace();
      byte alpha = (byte)((int)MathUtil.clamp(reader.readFloatValue() * 255.0F, 0.0F, 255.0F));
      ...
      reader.expect(')');
      reader.consumeWhiteSpace();
      return new ColorAlpha(alpha, (byte)((int)(val >> 24 & 255L)), (byte)((int)(val >> 16 & 255L)), (byte)((int)(val >> 8 & 255L)));
   }
   ...
   public static ColorAlpha rgbaHexStringToColor(String color) {
      ...
      if (color.startsWith("rgba(") && color.charAt(color.length() - 1) == ')') {
         ...
         if (channelLength != 2) {
            ...
            long val = (long)hexAlphaStringToRGBAInt(channels[0].trim());
            byte alpha = (byte)((int)MathUtil.clamp(Float.parseFloat(channels[1]) * 255.0F, 0.0F, 255.0F));
            return new ColorAlpha(alpha, (byte)((int)(val >> 24 & 255L)), (byte)((int)(val >> 16 & 255L)), (byte)((int)(val >> 8 & 255L)));
         }
      } else {
         throw new IllegalArgumentException("Color must start with 'rgba(' and end with ')'");
   ...
   public static int colorToARGBInt(@Nullable Color color) {
      return color == null ? -1 : -16777216 | (color.red & 255) << 16 | (color.green & 255) << 8 | color.blue & 255;
   }
   ...
   public static String toHexString(int rgb) {
      ...
      String var10000 = "0".repeat(6 - hexString.length());
      return "#" + var10000 + hexString;
   }
   ...
   public static String toHexAlphaString(int rgba) {
      ...
      String var10000 = "0".repeat(8 - hexString.length());
      return "#" + var10000 + hexString;
   }
}
