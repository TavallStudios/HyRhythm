...
public abstract class FluidTicker {
   ...
   protected static final int SPREAD_NO_PATH = 2147483647;
   ...
   public BlockTickStrategy tick(@Nonnull CommandBuffer<ChunkStore> commandBuffer, @Nonnull CachedAccessor cachedAccessor, @Nonnull FluidSection fluidSection, @Nonnull BlockSection blockSection, @Nonnull Fluid fluid, int fluidId, int worldX, int worldY, int worldZ) {
      ...
      if (!this.canOccupySolidBlocks() && isFullySolid((BlockType)BlockType.getAssetMap().getAsset(block))) {
         ...
         long hash = HashUtil.rehash((long)worldX, (long)worldY, (long)worldZ, 4030921250L);
         long tick = ((ChunkStore)commandBuffer.getExternalData()).getWorld().getTick();
         int flowRateLimitTicks = Math.round(this.flowRate * (float)world.getTps());
         return (hash + tick) % (long)flowRateLimitTicks != 0L ? BlockTickStrategy.CONTINUE : this.process(world, tick, cachedAccessor, fluidSection, blockSection, fluid, fluidId, worldX, worldY, worldZ);
      }
   }
   ...
   public BlockTickStrategy process(World world, long tick, @Nonnull Accessor accessor, @Nonnull FluidSection fluidSection, @Nonnull BlockSection blockSection, @Nonnull Fluid fluid, int fluidId, int worldX, int worldY, int worldZ) {
      ...
      switch (this.isAlive(accessor, fluidSection, blockSection, fluid, fluidId, fluidLevel, worldX, worldY, worldZ).ordinal()) {
         case 0:
   ...
   protected AliveStatus isAlive(@Nonnull Accessor accessor, @Nonnull FluidSection fluidSection, @Nonnull BlockSection blockSection, Fluid fluid, int fluidId, byte fluidLevel, int worldX, int worldY, int worldZ) {
      if (!this.canDemote) {
         ...
         FluidSection aboveFluidSection = ChunkUtil.chunkCoordinate(worldY + 1) == fluidSection.getY() ? fluidSection : accessor.getFluidSectionByBlock(worldX, worldY + 1, worldZ);
            ...
            if (fluidAbove != fluidId && fluidAbove != supportId) {
               ...
               for(Vector2i offset : ORTO_OFFSETS) {
                  ...
                  if (otherFluidSection != null && otherBlockSection != null) {
                     ...
                     if (supportId != -2147483648 && otherFluid == supportId) {
                        ...
                        if (sourceBlock != null) {
                           ...
                           if (!this.blocksFluidFrom(sourceBlock, sourceRotation, x, z, sourceFiller) && !this.blocksFluidFrom(thisBlock, thisRotation, -x, -z, thisFiller)) {
                              return FluidTicker.AliveStatus.ALIVE;
                           }
                        }
                     } else {
               ...
               if (chunkNotLoaded) {
                  ...
                  return FluidTicker.AliveStatus.DEMOTE;
               }
            } else {
               return FluidTicker.AliveStatus.ALIVE;
            }
      ...
      for(int y = -1; y <= 1; ++y) {
         for(int z = -1; z <= 1; ++z) {
            for(int x = -1; x <= 1; ++x) {
               ...
               int by = worldY + y;
               int bz = worldZ + z;
               BlockSection chunk = ChunkUtil.isSameChunkSection(worldX, worldY, worldZ, bx, by, bz) ? blockSection : accessor.getBlockSectionByBlock(bx, by, bz);
               if (chunk != null) {
   ...
   protected int getSpreadOffsets(@Nonnull BlockTypeAssetMap<String, BlockType> blockMap, @Nonnull Accessor accessor, @Nonnull FluidSection fluidSection, BlockSection blockSection, int worldX, int worldY, int worldZ, @Nonnull Vector2i[] offsetArray, int fluidId, int maxDropDistance) {
      int shortestDistanceToDrop = 2147483647;
      int offsets = 0;
      if (worldY <= 0) {
         return offsets;
      } else {
         for(int i = 0; i < offsetArray.length; ++i) {
            ...
            if (distance <= shortestDistanceToDrop && distance != 2147483647) {
               offsets |= 1 << i;
            }
         }
   ...
   protected int distanceToDrop(@Nonnull BlockTypeAssetMap<String, BlockType> blockMap, @Nonnull Accessor accessor, @Nonnull FluidSection fluidSection, BlockSection blockSection, int worldX, int worldY, int worldZ, @Nonnull Vector2i offset, int fluidId, int maxDropDistance) {
      ...
      for(int i = 1; i < maxDropDistance; ++i) {
         ...
         if (otherFluidId != 0 && (otherFluidId != fluidId || otherFluidId == supportedById || supportedById == -2147483648) || otherFluidId == 0 && isSolid(block)) {
            break;
         }
   ...
   public boolean blocksFluidFrom(@Nonnull BlockType blockType, int rotationIndex, int offsetX, int offsetZ, int filler) {
      if (blockType.getMaterial() != BlockMaterial.Solid) {
         ...
         BlockBoundingBoxes hitboxAsset = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(hitboxIndex);
         if (hitboxAsset == null) {
            return true;
         } else {
   ...
   public static class CachedAccessor extends AbstractCachedAccessor implements Accessor {
      ...
      public BlockSection getBlockSection(int cx, int cy, int cz) {
         return (BlockSection)this.getComponentSection(cx, cy, cz, 1, BlockSection.getComponentType());
      }
