...
public class Environment implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, Environment>>, NetworkSerializable<WorldEnvironment> {
   ...
   public WorldEnvironment toPacket() {
      WorldEnvironment cached = this.cachedPacket == null ? null : (WorldEnvironment)this.cachedPacket.get();
      if (cached != null) {
         return cached;
      } else {
         ...
         if (!this.fluidParticles.isEmpty()) {
            ...
            for(Map.Entry<String, FluidParticle> entry : this.fluidParticles.entrySet()) {
               int index = FluidFX.getAssetMap().getIndex((String)entry.getKey());
               if (index != -2147483648) {
   ...
   public String toString() {
      ...
      return "Environment{id='" + var10000 + "', waterTint='" + String.valueOf(this.waterTint) + "', fluidParticles='" + String.valueOf(this.fluidParticles) + "', weatherForecasts=" + String.valueOf(this.weatherForecasts) + ", spawnDensity=" + this.spawnDensity + ", packet=" + String.valueOf(this.cachedPacket) + "}";
   }
   ...
   static {
      ...
      MAX_KEY_HOUR = HOURS_PER_DAY - 1;
      HOURS = new Integer[HOURS_PER_DAY];
