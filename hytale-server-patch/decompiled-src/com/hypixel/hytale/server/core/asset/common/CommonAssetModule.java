...
public class CommonAssetModule extends JavaPlugin {
   ...
   private final CachedSupplier<Asset[]> assets = SupplierUtil.<Asset[]>cache(() -> (Asset[])CommonAssetRegistry.getAllAssets().stream().map(List::getLast).map(CommonAssetRegistry.PackAsset::asset).map(CommonAsset::toPacket).toArray((x$0) -> new Asset[x$0]));
   ...
   private void readCommonAssetsIndexCache(@Nonnull AssetPack pack) {
      ...
      if (Files.isRegularFile(assetCacheFile, new LinkOption[0])) {
         ...
         CompletableFuture.allOf((CompletableFuture[])futures.toArray((x$0) -> new CompletableFuture[x$0])).join();
   ...
   private void walkFileTree(@Nonnull final AssetPack pack) throws IOException {
      ...
      if (Files.exists(commonPath, new LinkOption[0])) {
         ...
         Files.walkFileTree(commonPath, FileUtil.DEFAULT_WALK_TREE_OPTIONS_SET, 2147483647, new SimpleFileVisitor<Path>() {
            @Nonnull
   ...
   private void reloadAsset(@Nonnull List<CompletableFuture<Void>> addedOrUpdatedAssets, String pack, @Nonnull Path file, @Nonnull String name) {
      ...
      addedOrUpdatedAssets.add(CompletableFuture.supplyAsync(SneakyThrow.sneakySupplier((ThrowableSupplier)(() -> Files.readAllBytes(file)))).thenAcceptAsync((bytes) -> this.addCommonAsset(pack, new FileCommonAsset(file, name, bytes))).exceptionally((throwable) -> {
   ...
   public void sendRemoveAssets(@Nonnull List<CommonAssetRegistry.PackAsset> assets, boolean forceRebuild) {
      ...
      if (assets.size() < 5) {
         ...
         for(CommonAssetRegistry.PackAsset asset : assets) {
            ...
            Message assetName = Message.raw(var10000 + ":" + asset.asset().getName()).color("#FF3874");
            packets[i++] = new Notification(message.getFormattedMessage(), assetName.getFormattedMessage(), "Icons/AssetNotifications/Trash.png", (ItemWithAllMetadata)null, NotificationStyle.Default);
         }
   ...
   private class CommonAssetMonitorHandler implements AssetMonitorHandler {
      ...
      public void accept(Map<Path, EventKind> map) {
         ...
         if (!createdOrModifiedFilesToLoad.isEmpty()) {
            ...
            CompletableFuture.allOf((CompletableFuture[])addedOrUpdatedAssets.toArray((x$0) -> new CompletableFuture[x$0])).thenAccept((v) -> Universe.get().broadcastPacketNoCache(new RequestCommonAssetsRebuild()));
         } else {
            Universe.get().broadcastPacketNoCache(new RequestCommonAssetsRebuild());
         }
