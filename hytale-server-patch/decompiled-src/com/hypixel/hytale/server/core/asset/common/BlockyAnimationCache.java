...
public class BlockyAnimationCache {
   ...
   public static CompletableFuture<BlockyAnimation> get(@Nonnull CommonAsset asset) {
      BlockyAnimation animationData = (BlockyAnimation)animations.get(asset.getName());
   ...
   public static BlockyAnimation getNow(String name) {
      ...
      if (animationData != null) {
         ...
         CommonAsset asset = CommonAssetRegistry.getByName(name);
         return asset == null ? null : (BlockyAnimation)get0(asset).join();
      }
   }

   public static BlockyAnimation getNow(@Nonnull CommonAsset asset) {
      BlockyAnimation animationData = (BlockyAnimation)animations.get(asset.getName());
      return animationData != null ? animationData : (BlockyAnimation)get0(asset).join();
   }

   @Nonnull
   private static CompletableFuture<BlockyAnimation> get0(@Nonnull CommonAsset asset) {
      String name = asset.getName();
      return CompletableFutureUtil.<BlockyAnimation>_catch(asset.getBlob().thenApply((bytes) -> {
         String str = new String(bytes, StandardCharsets.UTF_8);
         RawJsonReader reader = RawJsonReader.fromJsonString(str);

         try {
            ExtraInfo extraInfo = (ExtraInfo)ExtraInfo.THREAD_LOCAL.get();
            BlockyAnimation newAnimationData = BlockyAnimationCache.BlockyAnimation.CODEC.decodeJson(reader, extraInfo);
   ...
   public static class BlockyAnimation {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(BlockyAnimation.class, BlockyAnimation::new).addField(new KeyedCodec("duration", Codec.INTEGER, true, true), (blockyAnimation, i) -> blockyAnimation.duration = i, (blockyAnimation) -> blockyAnimation.duration)).build();
      }
   }
}
