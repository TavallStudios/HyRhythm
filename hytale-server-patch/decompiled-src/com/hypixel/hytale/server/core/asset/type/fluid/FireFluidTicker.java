...
public class FireFluidTicker extends FluidTicker {
   ...
   private boolean tryBurn(@Nonnull World world, @Nonnull FluidTicker.Accessor accessor, @Nonnull FluidSection fluidSection, BlockSection blockSection, @Nonnull FlammabilityConfig config, int blockX, int blockY, int blockZ) {
      ...
      if (resultingBlockIndex != -2147483648) {
   ...
   private int getSpreadFluidId(int fluidId) {
      if (this.spreadFluidId == 0) {
         if (this.spreadFluid != null) {
            ...
            this.spreadFluidId = -2147483648;
         }
      }

      return this.spreadFluidId == -2147483648 ? fluidId : this.spreadFluidId;
   }

   @Nonnull
   public List<FlammabilityConfig> getSortedFlammabilityConfigs() {
      if (this.sortedFlammabilityConfigs == null) {
         List<FlammabilityConfig> configs = new ArrayList();
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(FireFluidTicker.class, FireFluidTicker::new, BASE_CODEC).appendInherited(new KeyedCodec("SpreadFluid", Codec.STRING), (ticker, o) -> ticker.spreadFluid = o, (ticker) -> ticker.spreadFluid, (ticker, parent) -> ticker.spreadFluid = parent.spreadFluid).addValidator(Fluid.VALIDATOR_CACHE.getValidator().late()).add()).appendInherited(new KeyedCodec("Flammability", new ArrayCodec(FireFluidTicker.FlammabilityConfig.CODEC, (x$0) -> new FlammabilityConfig[x$0])), (ticker, o) -> {
         List<FlammabilityConfig> combined = new ArrayList();
         ...
         ticker.rawFlammabilityConfigs = (FlammabilityConfig[])combined.toArray(new FlammabilityConfig[0]);
      }, (ticker) -> ticker.rawFlammabilityConfigs, (ticker, parent) -> ticker.rawFlammabilityConfigs = parent.rawFlammabilityConfigs).documentation("Defines flammability configs per tag pattern").add()).build();
      INSTANCE = new FireFluidTicker();
      OFFSETS = new Vector3i[]{new Vector3i(0, -1, 0), new Vector3i(0, 1, 0), new Vector3i(0, 0, -1), new Vector3i(0, 0, 1), new Vector3i(-1, 0, 0), new Vector3i(1, 0, 0)};
   }

   public static class FlammabilityConfig {
      public static final BuilderCodec<FlammabilityConfig> CODEC;
      ...
      private int resultingBlockIndex = -2147483648;
      private String soundEvent;
      private int soundEventIndex = -2147483648;
      ...
      public int getResultingBlockIndex() {
         if (this.resultingBlockIndex == -2147483648 && this.resultingBlock != null) {
      ...
      public int getSoundEventIndex() {
         if (this.soundEventIndex == -2147483648 && this.soundEvent != null) {
