...
public class ItemCategory implements JsonAssetWithMap<String, DefaultAssetMap<String, ItemCategory>>, NetworkSerializable<com.hypixel.hytale.protocol.ItemCategory> {
   ...
   public com.hypixel.hytale.protocol.ItemCategory toPacket() {
      com.hypixel.hytale.protocol.ItemCategory cached = this.cachedPacket == null ? null : (com.hypixel.hytale.protocol.ItemCategory)this.cachedPacket.get();
      if (cached != null) {
         return cached;
      } else {
         ...
         if (this.children != null && this.children.length > 0) {
            packet.children = (com.hypixel.hytale.protocol.ItemCategory[])ArrayUtil.copyAndMutate(this.children, ItemCategory::toPacket, (x$0) -> new com.hypixel.hytale.protocol.ItemCategory[x$0]);
         }
   ...
   public String toString() {
      ...
      return "ItemCategory{id='" + var10000 + "', name='" + this.name + "', icon='" + this.icon + "', order=" + this.order + ", infoDisplayMode='" + String.valueOf(this.infoDisplayMode) + "', children=" + Arrays.toString(this.children) + "}";
   }

   static {
      CODEC_BUILDER = (AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(ItemCategory.class, ItemCategory::new, Codec.STRING, (itemCategory, k) -> itemCategory.id = k, (itemCategory) -> itemCategory.id, (asset, data) -> asset.data = data, (asset) -> asset.data).addField(new KeyedCodec("Id", Codec.STRING), (itemCategory, s) -> itemCategory.id = s, (itemCategory) -> itemCategory.id)).addField(new KeyedCodec("Name", Codec.STRING), (itemCategory, s) -> itemCategory.name = s, (itemCategory) -> itemCategory.name)).append(new KeyedCodec("Icon", Codec.STRING), (itemCategory, s) -> itemCategory.icon = s, (itemCategory) -> itemCategory.icon).addValidator(CommonAssetValidator.ICON_ITEM_CATEGORIES).add()).append(new KeyedCodec("InfoDisplayMode", new EnumCodec(ItemGridInfoDisplayMode.class), false, true), (itemCategory, s) -> itemCategory.infoDisplayMode = s, (itemCategory) -> itemCategory.infoDisplayMode).addValidator(Validators.nonNull()).add()).addField(new KeyedCodec("Order", Codec.INTEGER), (itemCategory, s) -> itemCategory.order = s, (itemCategory) -> itemCategory.order)).afterDecode((itemCategory) -> {
      ...
      CODEC_BUILDER.addField(new KeyedCodec("Children", new ArrayCodec(CODEC, (x$0) -> new ItemCategory[x$0])), (itemCategory, l) -> itemCategory.children = l, (itemCategory) -> itemCategory.children);
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(ItemCategory::getAssetStore));
   }
}
