...
public class SoftBlockDropType implements NetworkSerializable<SoftBlock> {
   ...
   public SoftBlockDropType withoutDrops() {
      return new SoftBlockDropType((String)null, (String)null, this.isWeaponBreakable);
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SoftBlockDropType.class, SoftBlockDropType::new).append(new KeyedCodec("ItemId", Codec.STRING), (softBlock, o) -> softBlock.itemId = o, (softBlock) -> softBlock.itemId).addValidatorLate(() -> Item.VALIDATOR_CACHE.getValidator().late()).add()).append(new KeyedCodec("DropList", new ContainedAssetCodec(ItemDropList.class, ItemDropList.CODEC)), (softBlock, o) -> softBlock.dropListId = o, (softBlock) -> softBlock.dropListId).addValidatorLate(() -> ItemDropList.VALIDATOR_CACHE.getValidator().late()).add()).addField(new KeyedCodec("IsWeaponBreakable", Codec.BOOLEAN), (blockGathering, o) -> blockGathering.isWeaponBreakable = o, (blockGathering) -> blockGathering.isWeaponBreakable)).build();
   }
}
