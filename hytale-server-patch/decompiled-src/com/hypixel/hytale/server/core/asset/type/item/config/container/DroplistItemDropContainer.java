...
public class DroplistItemDropContainer extends ItemDropContainer {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(DroplistItemDropContainer.class, DroplistItemDropContainer::new, ItemDropContainer.DEFAULT_CODEC).append(new KeyedCodec("DroplistId", Codec.STRING), (droplistItemDropContainer, s) -> droplistItemDropContainer.droplistId = s, (droplistItemDropContainer) -> droplistItemDropContainer.droplistId).addValidator(Validators.nonNull()).addValidatorLate(() -> ItemDropList.VALIDATOR_CACHE.getValidator().late()).add()).build();
   }
}
