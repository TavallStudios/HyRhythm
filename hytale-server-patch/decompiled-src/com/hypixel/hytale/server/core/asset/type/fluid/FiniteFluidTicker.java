...
public class FiniteFluidTicker extends FluidTicker {
   ...
   protected BlockTickStrategy spread(World world, long tick, @Nonnull FluidTicker.Accessor accessor, @Nonnull FluidSection fluidSection, BlockSection blockSection, @Nonnull Fluid fluid, int fluidId, byte fluidLevel, int worldX, int worldY, int worldZ) {
      if (worldY == 0) {
         ...
         if (belowFluidSection != null && belowBlockSection != null) {
            ...
            return this.spreadDownwards(accessor, fluidSection, blockSection, belowFluidSection, belowBlockSection, worldX, worldY, worldZ, fluid, fluidId, fluidLevel, bottomFluidId, bottomFluidLevel) ? BlockTickStrategy.CONTINUE : this.spreadSideways(tick, accessor, fluidSection, blockSection, worldX, worldY, worldZ, fluid, fluidId, fluidLevel);
         } else {
            return BlockTickStrategy.SLEEP;
         }
      }
   ...
   private boolean spreadDownwards(@Nonnull FluidTicker.Accessor accessor, @Nonnull FluidSection fluidSection, BlockSection blockSection, @Nonnull FluidSection belowFluidSection, @Nonnull BlockSection belowBlockSection, int worldX, int worldY, int worldZ, @Nonnull Fluid fluid, int fluidId, byte fluidLevel, int bottomFluidId, byte bottomFluidLevel) {
      if (fluidId != bottomFluidId && bottomFluidId != 0) {
         return false;
      } else if (isSolid((BlockType)BlockType.getAssetMap().getAsset(belowBlockSection.get(worldX, worldY - 1, worldZ)))) {
         return false;
      } else {
   ...
   private BlockTickStrategy spreadSideways(long tick, @Nonnull FluidTicker.Accessor accessor, @Nonnull FluidSection fluidSection, BlockSection blockSection, int worldX, int worldY, int worldZ, @Nonnull Fluid fluid, int fluidId, byte fluidLevel) {
      if (fluidLevel == 1) {
         ...
         long hash = HashUtil.rehash((long)worldX, (long)worldY, (long)worldZ, 4032035379L);
         ...
         for(int idx = 0; idx < offsetsList.size(); ++idx) {
            ...
            for(int i = 0; i < offsetArray.length && newLevel != 1; ++i) {
               if (!spreadDownhill || (offsets & 1 << i) != 0) {
                  ...
                  SpreadOutcome spreadOutcome = this.spreadToOffset(accessor, fluidSection, blockSection, offset, worldX, worldY, worldZ, fluid, fluidId, fluidLevel);
         ...
         if (newLevel == fluidLevel) {
            ...
            return !this.drainFromTopBlock(accessor, fluidSection, blockSection, worldX, worldY, worldZ, fluid, fluidId, (byte)(fluidLevel - newLevel)) ? BlockTickStrategy.WAIT_FOR_ADJACENT_CHUNK_LOAD : BlockTickStrategy.CONTINUE;
         }
      }
   }
   ...
   private int getTopY(@Nonnull FluidTicker.Accessor accessor, @Nonnull FluidSection fluidSection, int worldX, int worldY, int worldZ, @Nonnull Fluid fluid, int fluidId) {
      int topY = worldY;
      FluidSection aboveFluidSection = fluidSection.getY() != ChunkUtil.chunkCoordinate(worldY + 1) ? accessor.getFluidSectionByBlock(worldX, worldY + 1, worldZ) : fluidSection;
   ...
   private static boolean isOffsetConnected(@Nonnull FluidTicker.Accessor accessor, BlockSection blockSection, @Nonnull Vector2i offset, int worldX, int worldY, int worldZ) {
      ...
      if (x != 0 && z != 0) {
         ...
         BlockSection section2 = ChunkUtil.isSameChunkSection(worldX, worldY, worldZ, worldX, worldY, worldZ + z) ? blockSection : accessor.getBlockSection(worldX, worldY, worldZ + z);
         if (section1 != null && section2 != null) {
            int block1 = section1.get(worldX + x, worldY, worldZ);
            int block2 = section2.get(worldX, worldY, worldZ + z);
            return block1 == 0 || block2 == 0 || !isSolid((BlockType)BlockType.getAssetMap().getAsset(block1)) || !isSolid((BlockType)BlockType.getAssetMap().getAsset(block2));
         } else {
            return false;
         }
      } else {
   ...
   static {
      CODEC = BuilderCodec.builder(FiniteFluidTicker.class, FiniteFluidTicker::new, FluidTicker.BASE_CODEC).build();
      DIAG_OFFSETS = new Vector2i[]{new Vector2i(-1, -1), new Vector2i(1, 1), new Vector2i(1, -1), new Vector2i(-1, 1)};
      OFFSETS_LISTS = new ObjectArrayList();
      ...
      for(int i = 0; i < 16; ++i) {
         ...
         for(Vector2i[] offset : offsets) {
            ...
            offsetLists.add((Vector2i[])offsetArray.toArray((x$0) -> new Vector2i[x$0]));
         }
