...
public abstract class FarmingStageData {
   ...
   public void apply(@Nonnull ComponentAccessor<ChunkStore> commandBuffer, @Nonnull Ref<ChunkStore> sectionRef, @Nonnull Ref<ChunkStore> blockRef, int x, int y, int z, @Nullable FarmingStageData previousStage) {
      ...
      if (chunkSectionComponent != null) {
         ...
         SoundUtil.playSoundEvent3d(this.soundEventIndex, SoundCategory.SFX, (double)worldX, (double)worldY, (double)worldZ, entityStore);
   ...
   public String toString() {
      String var10000 = String.valueOf(this.duration);
      return "FarmingStageData{duration=" + var10000 + ", soundEventId='" + this.soundEventId + "'}";
   }

   static {
      BASE_CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.abstractBuilder(FarmingStageData.class).append(new KeyedCodec("Duration", ProtocolCodecs.RANGEF), (farmingStage, duration) -> farmingStage.duration = duration, (farmingStage) -> farmingStage.duration).add()).append(new KeyedCodec("SoundEventId", Codec.STRING), (farmingStage, sound) -> farmingStage.soundEventId = sound, (farmingStage) -> farmingStage.soundEventId).addValidator(SoundEvent.VALIDATOR_CACHE.getValidator()).addValidator(SoundEventValidators.MONO).addValidator(SoundEventValidators.ONESHOT).add()).afterDecode((farmingStage) -> {
