...
public abstract class TagPattern implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, TagPattern>>, NetworkSerializable<com.hypixel.hytale.protocol.TagPattern> {
   ...
   static {
      CODEC = new AssetCodecMapCodec<String, TagPattern>("Op", Codec.STRING, (t, k) -> t.id = k, (t) -> t.id, (t, data) -> t.data = data, (t) -> t.data);
      BASE_CODEC = BuilderCodec.abstractBuilder(TagPattern.class).build();
      CHILD_ASSET_CODEC = new ContainedAssetCodec(TagPattern.class, CODEC);
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(TagPattern::getAssetStore));
   }
}
