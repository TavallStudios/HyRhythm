...
public class CameraAxis implements NetworkSerializable<com.hypixel.hytale.protocol.CameraAxis> {
   ...
   public String toString() {
      String var10000 = String.valueOf(this.angleRange);
      return "CameraAxis{angleRange=" + var10000 + ", targetNodes=" + Arrays.toString(this.targetNodes) + "}";
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(CameraAxis.class, CameraAxis::new).append(new KeyedCodec("AngleRange", ProtocolCodecs.RANGEF), (cameraAxis, s) -> cameraAxis.angleRange = s, (cameraAxis) -> cameraAxis.angleRange).add()).append(new KeyedCodec("TargetNodes", new ArrayCodec(new EnumCodec(CameraNode.class), (x$0) -> new CameraNode[x$0])), (cameraAxis, s) -> cameraAxis.targetNodes = s, (cameraAxis) -> cameraAxis.targetNodes).addValidator(Validators.nonNull()).add()).build();
      STATIC_HEAD = new CameraAxis(new Rangef(0.0F, 0.0F), new CameraNode[]{CameraNode.Head});
   }
}
