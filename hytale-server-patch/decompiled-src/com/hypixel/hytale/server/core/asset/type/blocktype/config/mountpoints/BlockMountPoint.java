...
public class BlockMountPoint {
   ...
   public Vector3f computeRotationEuler(@Nonnull int rotationIndex) {
      ...
      Vector3f rotation = new Vector3f((float)rotationTuple.pitch().getRadians(), (float)rotationTuple.yaw().getRadians(), (float)rotationTuple.roll().getRadians());
      rotation.addYaw(3.1415927F);
      rotation.addYaw((float)Math.toRadians((double)this.yawOffSetDegrees));
      return rotation;
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BlockMountPoint.class, BlockMountPoint::new).appendInherited(new KeyedCodec("Offset", Vector3f.CODEC), (seat, i) -> seat.offset = i, (seat) -> seat.offset, (seat, p) -> seat.offset = p.offset).documentation("Relative offset from the block center (the point at .5,.5,.5 in world). Forward on a chair is 0,0,0.3").add()).appendInherited(new KeyedCodec("Yaw", Codec.DOUBLE), (seat, o) -> seat.yawOffSetDegrees = o.floatValue(), (seat) -> (double)seat.yawOffSetDegrees, (seat, p) -> seat.yawOffSetDegrees = p.yawOffSetDegrees).documentation("Offset for the model sitting on this seat in DEGREES").add()).build();
      EMPTY_ARRAY = new BlockMountPoint[0];
   }
}
