...
public abstract class ResponseCurve implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, ResponseCurve>> {
   ...
   static {
      CODEC = new AssetCodecMapCodec<String, ResponseCurve>(Codec.STRING, (t, k) -> t.id = k, (t) -> t.id, (t, data) -> t.extraData = data, (t) -> t.extraData);
      BASE_CODEC = ((BuilderCodec.Builder)BuilderCodec.abstractBuilder(ResponseCurve.class).afterDecode((responseCurve) -> responseCurve.reference = new WeakReference(responseCurve))).build();
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(ResponseCurve::getAssetStore));
   ...
   public static class Reference {
      ...
      public ResponseCurve get() {
         ResponseCurve responseCurve = (ResponseCurve)this.reference.get();
         if (responseCurve == null) {
            responseCurve = (ResponseCurve)ResponseCurve.getAssetMap().getAsset(this.index);
            this.reference = responseCurve.getReference();
         }
