...
public class ItemRepairPageSupplier implements OpenCustomUIInteraction.CustomPageSupplier {
   ...
   public CustomUIPage tryCreate(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor, @Nonnull PlayerRef playerRef, @Nonnull InteractionContext context) {
      ...
      return itemContext == null ? null : new ItemRepairPage(playerRef, playerComponent.getInventory().getCombinedArmorHotbarUtilityStorage(), this.repairPenalty, itemContext);
   }

   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(ItemRepairPageSupplier.class, ItemRepairPageSupplier::new).appendInherited(new KeyedCodec("RepairPenalty", Codec.DOUBLE), (data, o) -> data.repairPenalty = o, (data) -> data.repairPenalty, (data, parent) -> data.repairPenalty = parent.repairPenalty).add()).build();
   }
}
