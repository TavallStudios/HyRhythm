...
public interface Selector {
   static void selectNearbyBlocks(@Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> attackerRef, double range, @Nonnull TriIntConsumer consumer) {
      ...
      if (!null.$assertionsDisabled && transformComponent == null) {
         throw new AssertionError();
      } else {
         ModelComponent modelComponent = (ModelComponent)commandBuffer.getComponent(attackerRef, ModelComponent.getComponentType());
         if (!null.$assertionsDisabled && modelComponent == null) {
            throw new AssertionError();
         } else {
            Vector3d position = transformComponent.getPosition();
            Model model = modelComponent.getModel();
            selectNearbyBlocks(position.x, position.y + (double)model.getEyeHeight(attackerRef, commandBuffer), position.z, range, consumer);
         }
      }
   }
   ...
   static void selectNearbyEntities(@Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> attacker, double range, @Nonnull Consumer<Ref<EntityStore>> consumer, @Nonnull Predicate<Ref<EntityStore>> filter) {
      ...
      if (!null.$assertionsDisabled && transformComponent == null) {
         ...
         if (!null.$assertionsDisabled && modelComponent == null) {
            ...
            Model model = modelComponent.getModel();
            Vector3d position = attackerPosition.clone().add(0.0, (double)model.getEyeHeight(attacker, commandBuffer), 0.0);
   ...
   static void selectNearbyEntities(@Nonnull ComponentAccessor<EntityStore> componentAccessor, @Nonnull Vector3d position, double range, @Nonnull Consumer<Ref<EntityStore>> consumer, @Nullable Predicate<Ref<EntityStore>> filter) {
      ...
      SpatialResource<Ref<EntityStore>, EntityStore> itemSpatialResource = (SpatialResource)componentAccessor.getResource(EntityModule.get().getItemSpatialResourceType());
      itemSpatialResource.getSpatialStructure().collect(position, range, results);
      ObjectListIterator var10 = results.iterator();

      while(var10.hasNext()) {
         Ref<EntityStore> ref = (Ref)var10.next();
