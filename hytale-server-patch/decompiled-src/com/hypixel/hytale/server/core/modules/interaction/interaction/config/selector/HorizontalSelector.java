...
public class HorizontalSelector extends SelectorType {
   ...
   public static enum Direction {
      ...
      @Nonnull
      public static final EnumCodec<Direction> CODEC = (new EnumCodec<Direction>(Direction.class)).documentKey(TO_LEFT, "A arc that starts at the right and moves towards the left.").documentKey(TO_RIGHT, "A arc that starts at the left and moves towards the right.");
   ...
   private class RuntimeSelector implements Selector {
      ...
      public void tick(@Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> attacker, float time, float runTime) {
         ModelComponent modelComponent = (ModelComponent)commandBuffer.getComponent(attacker, ModelComponent.getComponentType());
         ...
         HeadRotation headRotationComponent = (HeadRotation)commandBuffer.getComponent(attacker, HeadRotation.getComponentType());
         ...
         this.viewProvider.setPosition(posX, posY, posZ).setDirection((double)headRotationComponent.getRotation().getYaw(), (double)headRotationComponent.getRotation().getPitch());
                  ...
                  if (worldChunk == null) {
                     ...
                     if (blockType == null) {
                        ...
                        if (blockType.getMaterial() == BlockMaterial.Solid) {
                           ...
                           BlockBoundingBoxes blockHitboxes = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(hitboxTypeIndex);
         ...
         if (SelectInteraction.SHOW_VISUAL_DEBUG) {
            ...
            matrix.identity().translate(posX, posY, posZ).rotateAxis((double)(-headRotationComponent.getRotation().getYaw()), 0.0, 1.0, 0.0, tmp).rotateAxis((double)(-headRotationComponent.getRotation().getPitch()), 1.0, 0.0, 0.0, tmp);
            Vector3f color = new Vector3f((float)HashUtil.random((long)attacker.getIndex(), (long)this.hashCode(), 10L), (float)HashUtil.random((long)attacker.getIndex(), (long)this.hashCode(), 11L), (float)HashUtil.random((long)attacker.getIndex(), (long)this.hashCode(), 12L));
            DebugUtils.addFrustum(((EntityStore)commandBuffer.getExternalData()).getWorld(), matrix, this.projectionProvider.getMatrix(), color, 5.0F, true);
         }
      ...
      public void selectTargetEntities(@Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> attacker, @Nonnull BiConsumer<Ref<EntityStore>, Vector4d> consumer, Predicate<Ref<EntityStore>> filter) {
         Selector.selectNearbyEntities(commandBuffer, attacker, HorizontalSelector.this.endDistance + 3.0, (entity) -> {
            BoundingBox boundingBoxComponent = (BoundingBox)commandBuffer.getComponent(entity, BoundingBox.getComponentType());
         ...
         Selector.selectNearbyBlocks(commandBuffer, attacker, HorizontalSelector.this.startDistance + HorizontalSelector.this.endDistance, (x, y, z) -> {
            ...
            if (chunk != null) {
               ...
               if (blockType != null && blockType != BlockType.EMPTY) {
                  ...
                  int hitboxTypeIndex = blockType.getHitboxTypeIndex();
                  BlockBoundingBoxes boundingBoxAsset = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(hitboxTypeIndex);
                  if (boundingBoxAsset != null) {
                     Box[] hitboxes = boundingBoxAsset.get(rotation).getDetailBoxes();

                     for(int i = 0; i < hitboxes.length; ++i) {
                        Box hitbox = hitboxes[i];
                        this.modelMatrix.identity().translate((double)x, (double)y, (double)z).translate(hitbox.getMin()).scale(hitbox.width(), hitbox.height(), hitbox.depth());
                        if (this.executor.test(HitDetectionExecutor.CUBE_QUADS, this.modelMatrix)) {
