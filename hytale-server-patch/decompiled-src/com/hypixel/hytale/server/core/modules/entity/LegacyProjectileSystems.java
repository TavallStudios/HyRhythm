...
public class LegacyProjectileSystems {
   ...
   public static class OnAddRefSystem extends RefSystem<EntityStore> {
      ...
      public void onEntityAdded(@Nonnull Ref<EntityStore> ref, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ProjectileComponent projectileComponent = (ProjectileComponent)commandBuffer.getComponent(ref, PROJECTILE_COMPONENT_TYPE);
   ...
   public static class OnAddHolderSystem extends HolderSystem<EntityStore> {
      ...
      public void onEntityAdd(@Nonnull Holder<EntityStore> holder, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store) {
         ...
         ProjectileComponent projectileComponent = (ProjectileComponent)holder.getComponent(PROJECTILE_COMPONENT_TYPE);

         assert projectileComponent != null;

         projectileComponent.initialize();
         ModelAsset modelAsset = (ModelAsset)ModelAsset.getAssetMap().getAsset(projectileComponent.getAppearance());
   ...
   public static class TickingSystem extends EntityTickingSystem<EntityStore> implements DisableProcessingAssert {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         ProjectileComponent projectileComponent = (ProjectileComponent)archetypeChunk.getComponent(index, this.projectileComponentType);
         ...
         try {
            ...
            projectileComponent.getSimplePhysicsProvider().tick((double)dt, velocityComponent, world, transformComponent, ref, commandBuffer);
         } catch (Throwable throwable) {
            ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(throwable)).log("Exception while ticking entity! Removing!! %s", projectileComponent);
            commandBuffer.removeEntity(ref, RemoveReason.REMOVE);
         }
