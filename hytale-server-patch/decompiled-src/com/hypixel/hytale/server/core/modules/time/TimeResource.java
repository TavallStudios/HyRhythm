...
public class TimeResource implements Resource<EntityStore> {
   ...
   public TimeResource(@Nonnull Instant now) {
      this.timeDilationModifier = 1.0F;
      this.now = now;
   }
   ...
   public String toString() {
      return "TimeResource{now=" + String.valueOf(this.now) + "}";
   }

   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(TimeResource.class, TimeResource::new).append(new KeyedCodec("Now", Codec.INSTANT), (o, now) -> o.now = now, (o) -> o.now).documentation("Now. The current instant of time.").add()).build();
   }
}
