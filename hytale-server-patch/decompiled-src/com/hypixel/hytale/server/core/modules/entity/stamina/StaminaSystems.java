...
public class StaminaSystems {
   public static class SprintStaminaEffectSystem extends EntityTickingSystem<EntityStore> {
      ...
      public SprintStaminaEffectSystem() {
         this.query = Query.<EntityStore>and(this.playerComponentType, this.entityStatMapComponentType, this.movementStatesComponentType);
         this.dependencies = Set.of(new SystemDependency(Order.BEFORE, MovementStatesSystems.TickingSystem.class), new SystemDependency(Order.BEFORE, EntityStatsModule.PlayerRegenerateStatsSystem.class));
      }
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         if (!movementStates.getMovementStates().sprinting && movementStates.getSentMovementStates().sprinting) {
            SprintStaminaRegenDelay regenDelay = (SprintStaminaRegenDelay)store.getResource(this.sprintRegenDelayResourceType);
      ...
      protected boolean updateResource(@Nonnull Store<EntityStore> store) {
         SprintStaminaRegenDelay resource = (SprintStaminaRegenDelay)store.getResource(this.sprintRegenDelayResourceType);
         if (resource.validate()) {
            return resource.hasDelay();
         } else {
            GameplayConfig gameplayConfig = ((EntityStore)store.getExternalData()).getWorld().getGameplayConfig();
            StaminaGameplayConfig staminaConfig = (StaminaGameplayConfig)gameplayConfig.getPluginConfig().get(StaminaGameplayConfig.class);
            if (staminaConfig != null && staminaConfig.getSprintRegenDelay().getIndex() != -2147483648) {
