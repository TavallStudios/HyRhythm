...
public class UIComponentList implements Component<EntityStore> {
   ...
   public void update() {
      ...
      if (oldLength <= assetCount) {
         ...
         for(int index = oldLength; index < assetCount; this.componentIds[index] = index++) {
         }
      }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(UIComponentList.class, UIComponentList::new).append(new KeyedCodec("Components", Codec.STRING_ARRAY), (list, v) -> list.components = v, (list) -> list.components).add()).afterDecode((list) -> {
