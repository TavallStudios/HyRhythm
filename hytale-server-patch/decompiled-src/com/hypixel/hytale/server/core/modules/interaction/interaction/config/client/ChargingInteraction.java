...
public class ChargingInteraction extends Interaction {
   ...
   protected void tick0(boolean firstRun, float time, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
      ...
      if (context.getClientState().state == InteractionState.Failed && context.hasLabels()) {
         ...
         if (clientData.forkCounts != null && this.forks != null) {
            Object2IntMap<InteractionType> serverForkCounts = (Object2IntMap)context.getInstanceStore().getMetaObject(FORK_COUNTS);
            InteractionChain forked = (InteractionChain)context.getInstanceStore().getMetaObject(FORKED_CHAIN);
            ...
            for(Map.Entry<InteractionType, Integer> e : clientData.forkCounts.entrySet()) {
               ...
               String forkInteraction = (String)this.forks.get(e.getKey());
               if (forked == null && serverCount < (Integer)e.getValue() && forkInteraction != null) {
                  InteractionContext forkContext = context.duplicate();
                  forked = context.fork((InteractionType)e.getKey(), forkContext, RootInteraction.getRootInteractionOrUnknown(forkInteraction), true);
                  context.getInstanceStore().putMetaObject(FORKED_CHAIN, forked);
                  ++serverCount;
                  serverForkCounts.put((InteractionType)e.getKey(), serverCount);
               }

               matches &= serverCount == (Integer)e.getValue();
            }
         ...
         if (clientData.chargeValue == -1.0F) {
            ...
            if (this.next != null) {
               ...
               damageDataComponent.setLastChargeTime(((TimeResource)commandBuffer.getResource(TimeResource.getResourceType())).getNow());
            }
         }
      }
   }
   ...
   public boolean walk(@Nonnull Collector collector, @Nonnull InteractionContext context) {
      if (this.next != null) {
         ObjectIterator var3 = this.next.float2ObjectEntrySet().iterator();

         while(var3.hasNext()) {
            Float2ObjectMap.Entry<String> entry = (Float2ObjectMap.Entry)var3.next();
            if (InteractionManager.walkInteraction(collector, context, ChargingInteraction.ChargingTag.of(entry.getFloatKey()), (String)entry.getValue())) {
               return true;
            }
         }
      }
   ...
   protected void configurePacket(com.hypixel.hytale.protocol.Interaction packet) {
      ...
      if (this.next != null) {
         ...
         ObjectIterator var4 = this.next.float2ObjectEntrySet().iterator();

         while(var4.hasNext()) {
            Float2ObjectMap.Entry<String> e = (Float2ObjectMap.Entry)var4.next();
            chargedNext.put(e.getFloatKey(), Interaction.getInteractionIdOrUnknown((String)e.getValue()));
         }
      ...
      if (this.forks != null) {
         ...
         for(Map.Entry<InteractionType, String> e : this.forks.entrySet()) {
            intForks.put((InteractionType)e.getKey(), RootInteraction.getRootInteractionIdOrUnknown((String)e.getValue()));
         }
