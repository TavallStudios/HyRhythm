...
public class EntitySystems {
   ...
   public static class OnLoadFromExternal extends HolderSystem<EntityStore> {
      ...
      public OnLoadFromExternal(@Nonnull ComponentType<EntityStore, FromPrefab> fromPrefab, @Nonnull ComponentType<EntityStore, FromWorldGen> fromWorldGen, @Nonnull SystemGroup<EntityStore> group) {
         this.query = Query.<EntityStore>and(Query.or(fromPrefab, fromWorldGen), UUIDComponent.getComponentType());
         this.group = group;
         this.dependencies = Set.of(new SystemDependency(Order.BEFORE, EntityStore.UUIDSystem.class), new SystemDependency(Order.AFTER, EntityModule.LegacyUUIDSystem.class));
      }
   ...
   public static class UnloadEntityFromChunk extends RefSystem<EntityStore> {
      ...
      public void onEntityRemove(@Nonnull Ref<EntityStore> ref, @Nonnull RemoveReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         if (chunkRef != null && chunkRef.isValid()) {
            ...
            EntityChunk entityChunkComponent = (EntityChunk)chunkComponentStore.getComponent(chunkRef, EntityChunk.getComponentType());

            assert entityChunkComponent != null;

            switch (reason) {
               case REMOVE -> entityChunkComponent.removeEntityReference(ref);
               case UNLOAD -> entityChunkComponent.unloadEntityReference(ref);
            }
   ...
   public static class DynamicLightTracker extends EntityTickingSystem<EntityStore> {
      ...
      public DynamicLightTracker(@Nonnull ComponentType<EntityStore, EntityTrackerSystems.Visible> componentType) {
         ...
         this.query = Query.<EntityStore>and(componentType, this.dynamicLightType);
      }
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         EntityTrackerSystems.Visible visibleComponent = (EntityTrackerSystems.Visible)archetypeChunk.getComponent(index, this.componentType);

         assert visibleComponent != null;

         DynamicLight dynamicLightComponent = (DynamicLight)archetypeChunk.getComponent(index, this.dynamicLightType);
   ...
   public static class NewSpawnEntityTrackerUpdate extends EntityTickingSystem<EntityStore> {
      ...
      public NewSpawnEntityTrackerUpdate() {
         this.query = Query.<EntityStore>and(this.visibleComponentType, this.newSpawnComponentType);
      }
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         NewSpawnComponent newSpawnComponent = (NewSpawnComponent)archetypeChunk.getComponent(index, this.newSpawnComponentType);
         ...
         if (!visibleComponent.newlyVisibleTo.isEmpty()) {
            ...
            for(Map.Entry<Ref<EntityStore>, EntityTrackerSystems.EntityViewer> entry : visibleComponent.newlyVisibleTo.entrySet()) {
