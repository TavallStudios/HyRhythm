...
public class CombatTextUIComponentScaleAnimationEvent extends CombatTextUIComponentAnimationEvent {
   ...
   public String toString() {
      float var10000 = this.startScale;
      return "CombatTextUIComponentConfig{startScale=" + var10000 + ", endScale=" + this.endScale + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(CombatTextUIComponentScaleAnimationEvent.class, CombatTextUIComponentScaleAnimationEvent::new, CombatTextUIComponentAnimationEvent.ABSTRACT_CODEC).appendInherited(new KeyedCodec("StartScale", Codec.FLOAT), (event, f) -> event.startScale = f, (event) -> event.startScale, (parent, event) -> event.startScale = parent.startScale).documentation("The scale that should be applied to text instances before the animation event begins.").addValidator(Validators.nonNull()).addValidator(Validators.range(0.0F, 1.0F)).add()).appendInherited(new KeyedCodec("EndScale", Codec.FLOAT), (event, f) -> event.endScale = f, (event) -> event.endScale, (parent, event) -> event.endScale = parent.endScale).documentation("The scale that should be applied to text instances by the end of the animation.").addValidator(Validators.nonNull()).addValidator(Validators.range(0.0F, 1.0F)).add()).build();
   }
}
