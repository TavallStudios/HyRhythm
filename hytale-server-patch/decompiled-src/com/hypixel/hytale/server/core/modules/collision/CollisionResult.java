...
public class CollisionResult implements BoxBlockIterator.BoxIterationConsumer {
   public static final Comparator<BlockCollisionData> BLOCK_COLLISION_DATA_COMPARATOR = Comparator.comparingDouble((a) -> a.collisionStart).thenComparingDouble((a) -> a.collisionEnd);
   @Nonnull
   ...
   public CollisionResult(boolean enableSlides, boolean enableCharacters) {
      ...
      this.characterCollisions = new CollisionDataArray<CharacterCollisionData>(CharacterCollisionData::new, (Consumer)null, characterCollisionDataFreePool);
   ...
   public int defaultTriggerBlocksProcessing(@Nonnull InteractionManager manager, @Nonnull Entity entity, @Nonnull Ref<EntityStore> ref, boolean executeTriggers, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      for(int size = triggerBlocks.size(); i < size; ++i) {
         ...
         if (executeTriggers && entity instanceof LivingEntity) {
            ...
            if (chunk != null) {
               ...
               Fluid fluidType = (Fluid)Fluid.getAssetMap().getAsset(chunk.getFluidId(triggerCollision.x, triggerCollision.y, triggerCollision.z));
               String interactionsEnter = (String)blockType.getInteractions().get(InteractionType.CollisionEnter);
               if (interactionsEnter == null) {
                  interactionsEnter = (String)fluidType.getInteractions().get(InteractionType.CollisionEnter);
               }

               String interactions = (String)blockType.getInteractions().get(InteractionType.Collision);
               if (interactions == null) {
                  interactions = (String)fluidType.getInteractions().get(InteractionType.Collision);
               }
      ...
      if (executeTriggers && entity instanceof LivingEntity && !this.lastTriggers.isEmpty()) {
         ...
         while(var25.hasNext()) {
            ...
            int x = BlockUtil.unpackX(old);
            int y = BlockUtil.unpackY(old);
            int z = BlockUtil.unpackZ(old);
            WorldChunk chunk = world.getChunkIfInMemory(ChunkUtil.indexChunkFromBlock(x, z));
            if (chunk != null) {
               BlockType blockType = chunk.getBlockType(x, y, z);
               Fluid fluidType = (Fluid)Fluid.getAssetMap().getAsset(chunk.getFluidId(x, y, z));
               String interactions = (String)blockType.getInteractions().get(InteractionType.CollisionLeave);
               if (interactions == null) {
                  interactions = (String)fluidType.getInteractions().get(InteractionType.CollisionLeave);
               }

               if (interactions != null) {
                  this.doCollisionInteraction(manager, InteractionType.CollisionLeave, ref, interactions, new BlockPosition(x, y, z), componentAccessor);
               }
            }
         }
      }
   ...
   public boolean accept(long x, long y, long z) {
      if (this.collisionConfig.canCollide((int)x, (int)y, (int)z)) {
         x += (long)this.collisionConfig.getBoundingBoxOffsetX();
         y += (long)this.collisionConfig.getBoundingBoxOffsetY();
         z += (long)this.collisionConfig.getBoundingBoxOffsetZ();
