...
public class SingleplayerModule extends JavaPlugin {
      ...
      if (!Constants.SINGLEPLAYER) {
         ...
         if (access != Access.Private) {
            ...
            for(Channel channel : serverManager.getListeners()) {
               SocketAddress var7 = channel.localAddress();
               if (var7 instanceof InetSocketAddress) {
                  InetSocketAddress inetSocketAddress = (InetSocketAddress)var7;
         ...
         IEventDispatcher<SingleplayerRequestAccessEvent, SingleplayerRequestAccessEvent> dispatchFor = HytaleServer.get().getEventBus().dispatchFor(SingleplayerRequestAccessEvent.class);
   ...
   public static UUID getUuid() {
      return (UUID)Options.getOptionSet().valueOf(Options.OWNER_UUID);
   }

   public static String getUsername() {
      return (String)Options.getOptionSet().valueOf(Options.OWNER_NAME);
   }
