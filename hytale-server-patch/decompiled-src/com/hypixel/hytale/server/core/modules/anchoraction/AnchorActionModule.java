...
public class AnchorActionModule extends JavaPlugin {
   ...
   public void register(@Nonnull String action, @Nonnull WorldThreadAnchorActionHandler handler) {
      this.register(action, (AnchorActionHandler)((playerRef, data) -> {
   ...
   public boolean tryHandle(@Nonnull PlayerRef playerRef, @Nonnull String rawData) {
      ...
      try {
         ...
         if (!data.has("action")) {
            ...
            AnchorActionHandler handler = (AnchorActionHandler)this.handlers.get(action);
            if (handler == null) {
               return false;
            } else {
         ...
         ((HytaleLogger.Api)((HytaleLogger.Api)LOGGER.atWarning()).withCause(e)).log("Failed to handle anchor action '%s' for player %s", action, playerRef.getUuid());
         return false;
      }
   }
