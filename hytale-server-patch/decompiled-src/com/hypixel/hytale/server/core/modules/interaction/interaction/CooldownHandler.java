...
public final class CooldownHandler implements Tickable {
   ...
   public Cooldown getCooldown(@Nonnull String id, float maxTime, @Nonnull float[] chargeTimes, boolean force, boolean interruptRecharge) {
      return force ? (Cooldown)this.cooldowns.computeIfAbsent(id, (k) -> new Cooldown(maxTime, chargeTimes, interruptRecharge)) : (Cooldown)this.cooldowns.get(id);
   }

   @Nullable
   public Cooldown getCooldown(@Nonnull String id) {
      return (Cooldown)this.cooldowns.get(id);
   }
