...
public class StatsConditionWithModifierInteraction extends StatsConditionBaseInteraction {
   ...
   protected boolean canAfford(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      EntityStatMap entityStatMapComponent = (EntityStatMap)componentAccessor.getComponent(ref, EntityStatMap.getComponentType());
      if (entityStatMapComponent == null) {
         return false;
      } else if (this.costs == null) {
         return false;
      } else {
         ObjectIterator var4 = this.costs.int2FloatEntrySet().iterator();

         while(var4.hasNext()) {
            Int2FloatMap.Entry cost = (Int2FloatMap.Entry)var4.next();
            ...
            Entity var10 = EntityUtils.getEntity(ref, componentAccessor);
   ...
   private float calculateDiscount(@Nullable Inventory inventory, int statIndex, float baseCost) {
      ...
      modifiedCost *= Math.max(0.0F, 1.0F - multiplierModifier);
      return modifiedCost;
   }

   @Nonnull
   public String toString() {
      String var10000 = String.valueOf(this.interactionModifierId);
      return "StatsConditionWithModifierInteraction{interactionModifierId=" + var10000 + "}" + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(StatsConditionWithModifierInteraction.class, StatsConditionWithModifierInteraction::new, StatsConditionBaseInteraction.CODEC).documentation("Interaction that is successful if the given stat conditions match.")).append(new KeyedCodec("InteractionModifierId", new EnumCodec(ItemArmor.InteractionModifierId.class)), (changeStatWithModifierInteraction, s) -> changeStatWithModifierInteraction.interactionModifierId = s, (changeStatWithModifierInteraction) -> changeStatWithModifierInteraction.interactionModifierId).addValidator(Validators.nonNull()).add()).build();
   }
}
