...
public class RepulsionSystems {
   public static class PlayerSetup extends HolderSystem<EntityStore> {
      ...
      public PlayerSetup(ComponentType<EntityStore, Repulsion> componentType, ComponentType<EntityStore, Player> playerComponentType) {
         ...
         this.query = Query.<EntityStore>and(playerComponentType, Query.not(componentType));
      }
      ...
      public void onEntityAdd(@Nonnull Holder<EntityStore> holder, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store) {
         ...
         if (repulsionConfigIndex == -1) {
            ...
            RepulsionConfig repulsion = (RepulsionConfig)RepulsionConfig.getAssetMap().getAsset(repulsionConfigIndex);
            if (holder.getComponent(this.componentType) != null) {
               holder.removeComponent(this.componentType);
            }
   ...
   public static class EntityTrackerUpdate extends EntityTickingSystem<EntityStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         Repulsion repulsion = (Repulsion)archetypeChunk.getComponent(index, this.componentType);
   ...
   public static class EntityTrackerRemove extends RefChangeSystem<EntityStore, Repulsion> {
      ...
      public EntityTrackerRemove(ComponentType<EntityStore, EntityTrackerSystems.Visible> visibleComponentType, ComponentType<EntityStore, Repulsion> componentType) {
         this.visibleComponentType = visibleComponentType;
         this.componentType = componentType;
         this.query = Query.<EntityStore>and(visibleComponentType, componentType);
      }
      ...
      public void onComponentAdded(@Nonnull Ref<EntityStore> ref, @Nonnull Repulsion component, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      }

      public void onComponentSet(@Nonnull Ref<EntityStore> ref, Repulsion oldComponent, @Nonnull Repulsion newComponent, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      }

      public void onComponentRemoved(@Nonnull Ref<EntityStore> ref, @Nonnull Repulsion component, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
   ...
   public static class RepulsionTicker extends EntityTickingSystem<EntityStore> implements IVelocityModifyingSystem {
      ...
      public RepulsionTicker(ComponentType<EntityStore, Repulsion> repulsionComponentType, ComponentType<EntityStore, TransformComponent> transformComponentComponentType, ResourceType<EntityStore, SpatialResource<Ref<EntityStore>, EntityStore>> spatialComponent) {
         ...
         this.query = Query.<EntityStore>and(repulsionComponentType, transformComponentComponentType);
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         Repulsion repulsionComponent = (Repulsion)archetypeChunk.getComponent(index, this.repulsionComponentType);
         ...
         if (repulsionConfigIndex != -1) {
            ...
            TransformComponent transformComponent = (TransformComponent)archetypeChunk.getComponent(index, this.transformComponentComponentType);

            assert transformComponent != null;

            Vector2d position = new Vector2d(transformComponent.getPosition().x, transformComponent.getPosition().z);
            SpatialResource<Ref<EntityStore>, EntityStore> spatialResource = (SpatialResource)store.getResource(this.spatialComponent);
            List<Ref<EntityStore>> results = new ObjectArrayList();
            spatialResource.getSpatialStructure().ordered(transformComponent.getPosition(), (double)radius, results);

            for(Ref<EntityStore> entityRef : results) {
               TransformComponent entityTransformComponent = (TransformComponent)commandBuffer.getComponent(entityRef, this.transformComponentComponentType);
               if (entityTransformComponent != null) {
                  ...
                  if (!entityPosition.equals(position)) {
                     ...
                     if (!(distance < 0.1)) {
                        ...
                        double force = Math.max((double)repulsion.minForce, (double)maxForce * fraction);
                        force *= (double)flip;
                        Vector2d push = entityPosition.subtract(position);
                        push.normalize();
                        push.scale(force);
                        Velocity entityVelocityComponent = (Velocity)commandBuffer.getComponent(entityRef, Velocity.getComponentType());
                        if (entityVelocityComponent != null) {
                           Vector3d addedVelocity = new Vector3d((double)((float)push.x), 0.0, (double)((float)push.y));
                           entityVelocityComponent.addInstruction(addedVelocity, (VelocityConfig)null, ChangeVelocityType.Add);
                        }
                     }
                  }
               }
