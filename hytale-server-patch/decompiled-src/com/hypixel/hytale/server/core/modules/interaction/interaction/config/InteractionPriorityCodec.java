...
public class InteractionPriorityCodec implements Codec<InteractionPriority> {
   ...
   public InteractionPriority decode(@Nonnull BsonValue bsonValue, ExtraInfo extraInfo) {
      if (bsonValue.isInt32()) {
         ...
         throw new CodecException("Expected integer or object for InteractionPriority, got: " + String.valueOf(bsonValue.getBsonType()));
      }
   }
   ...
   public BsonValue encode(@Nonnull InteractionPriority priority, ExtraInfo extraInfo) {
      ...
      if (values != null && !values.isEmpty()) {
         return (BsonValue)(values.size() == 1 && values.containsKey(PrioritySlot.Default) ? new BsonInt32((Integer)values.get(PrioritySlot.Default)) : MAP_CODEC.encode(values, extraInfo));
      } else {
   ...
   public InteractionPriority decodeJson(@Nonnull RawJsonReader reader, ExtraInfo extraInfo) throws IOException {
      ...
      if (peek == 123) {
         ...
         throw new CodecException("Expected integer or object for InteractionPriority, got: " + (char)peek, reader, extraInfo, (Throwable)null);
      } else {
   ...
   static {
      MAP_CODEC = new EnumMapCodec<PrioritySlot, Integer>(PrioritySlot.class, Codec.INTEGER, () -> new EnumMap(PrioritySlot.class), false);
   }
}
