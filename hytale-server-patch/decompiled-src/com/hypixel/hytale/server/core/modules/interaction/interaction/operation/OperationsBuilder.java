...
public class OperationsBuilder {
   ...
   public Label createUnresolvedLabel() {
      return new Label(-2147483648);
   }

   public void resolveLabel(@Nonnull Label label) {
      if (label.index != -2147483648) {
   ...
   public void addOperation(@Nonnull Operation operation, Label... labels) {
      this.operationList.add(new LabelOperation(operation, labels));
   }

   @Nonnull
   public Operation[] build() {
      return (Operation[])this.operationList.toArray((x$0) -> new Operation[x$0]);
   }

   private static record LabelOperation(Operation inner, Label[] labels) implements Operation, Operation.NestedOperation {
      ...
      public String toString() {
         String var10000 = String.valueOf(this.inner);
         return "LabelOperation{inner=" + var10000 + ", labels=" + Arrays.toString(this.labels) + "}";
      }
   }
}
