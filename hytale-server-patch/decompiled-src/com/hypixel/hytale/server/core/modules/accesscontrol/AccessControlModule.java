...
public class AccessControlModule extends JavaPlugin {
   ...
   protected void setup() {
      ...
      this.getEventRegistry().register(PlayerSetupConnectEvent.class, (event) -> {
         ...
         Optional<String> disconnectReason = (Optional)completableFuture.join();
         if (disconnectReason.isPresent()) {
            event.setReason((String)disconnectReason.get());
   ...
   public void registerBanParser(String type, BanParser banParser) {
      BanParser currentParser = (BanParser)this.parsers.get(type);
   ...
   public Ban parseBan(String type, JsonObject object) {
      BanParser parser = (BanParser)this.parsers.get(type);
      if (parser == null) {
   ...
   private CompletableFuture<Optional<String>> getDisconnectReason(UUID uuid) {
      return (CompletableFuture)this.providerRegistry.stream().map((p) -> p.getDisconnectReason(uuid)).reduce(CompletableFuture.completedFuture(Optional.empty()), (a, b) -> a.thenCombine(b, (aMessage, bMessage) -> {
            if (aMessage.isPresent()) {
               return aMessage;
            } else {
               return bMessage.isPresent() ? bMessage : Optional.empty();
            }
         }));
   }
}
