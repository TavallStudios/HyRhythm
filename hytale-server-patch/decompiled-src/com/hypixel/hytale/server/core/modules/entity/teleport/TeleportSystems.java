...
public class TeleportSystems {
   public static class MoveSystem extends RefChangeSystem<EntityStore, Teleport> {
      ...
      public MoveSystem() {
         this.query = Query.<EntityStore>and(this.teleportComponentType, this.transformComponentType, Query.not(PlayerRef.getComponentType()));
      }
   ...
   public static class PlayerMoveSystem extends RefChangeSystem<EntityStore, Teleport> {
      ...
      public PlayerMoveSystem() {
         this.query = Query.<EntityStore>and(this.teleportComponentType, this.playerRefComponentType, this.transformComponentType, this.playerComponentType);
      }
      ...
      private void teleportToWorld(@Nonnull Ref<EntityStore> ref, @Nonnull Teleport teleport, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull World targetWorld) {
         PlayerRef playerRefComponent = (PlayerRef)commandBuffer.getComponent(ref, this.playerRefComponentType);

         assert playerRefComponent != null;

         commandBuffer.removeComponent(ref, this.teleportComponentType);
         TransformComponent transformComponent = (TransformComponent)commandBuffer.getComponent(ref, this.transformComponentType);
         String originWorldName = ((EntityStore)commandBuffer.getExternalData()).getWorld().getName();
         ...
         commandBuffer.run((s) -> {
            TeleportRecord teleportRecord = (TeleportRecord)s.ensureAndGetComponent(ref, TeleportRecord.getComponentType());
      ...
      private void teleportToPosition(@Nonnull Ref<EntityStore> ref, @Nonnull Teleport teleport, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         TeleportRecord teleportHistory = (TeleportRecord)commandBuffer.ensureAndGetComponent(ref, TeleportRecord.getComponentType());
