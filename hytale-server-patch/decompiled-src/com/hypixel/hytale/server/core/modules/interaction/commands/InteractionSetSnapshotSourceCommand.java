...
public class InteractionSetSnapshotSourceCommand extends CommandBase {
   @Nonnull
   private static final EnumArgumentType<SelectInteraction.SnapshotSource> SNAPSHOT_SOURCE_ARG_TYPE = new EnumArgumentType<SelectInteraction.SnapshotSource>("server.commands.parsing.argtype.snapshotsource.name", SelectInteraction.SnapshotSource.class);
   @Nonnull
   ...
   public InteractionSetSnapshotSourceCommand() {
      ...
      this.snapshotSourceArg = this.withRequiredArg("snapshotSource", "server.commands.interaction.snapshotSource.set.snapshotSource.desc", SNAPSHOT_SOURCE_ARG_TYPE);
   }

   protected void executeSync(@Nonnull CommandContext context) {
      SelectInteraction.SnapshotSource source = (SelectInteraction.SnapshotSource)this.snapshotSourceArg.get(context);
