...
public class BlockConditionInteraction extends SimpleBlockInteraction {
   ...
   private void doInteraction(@Nonnull InteractionContext context, @Nonnull World world, @Nonnull Vector3i targetBlock, @Nonnull BlockFace face) {
      ...
      if (chunk != null) {
         ...
         if (itemType == null) {
            ...
            boolean ok = false;
            BlockMatcher[] var10 = this.matchers;
            ...
            while(var12 < var11) {
               label69: {
                  label77: {
                     BlockMatcher matcher = var10[var12];
                     if (matcher.face != BlockFace.None) {
                        ...
                        if (!matcher.staticFace) {
                           ...
                           com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockFace newFace = com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockFace.rotate(com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockFace.fromProtocolFace(transformedFace), yaw, pitch);
   ...
   public String toString() {
      String var10000 = Arrays.toString(this.matchers);
      return "BlockConditionInteraction{matchers=" + var10000 + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BlockConditionInteraction.class, BlockConditionInteraction::new, SimpleBlockInteraction.CODEC).documentation("Tests the target block and executes `Next` if it matches all the conditions, otherwise `Failed` is run.")).appendInherited(new KeyedCodec("Matchers", new ArrayCodec(BlockConditionInteraction.BlockMatcher.CODEC, (x$0) -> new BlockMatcher[x$0])), (o, i) -> o.matchers = i, (o) -> o.matchers, (o, p) -> o.matchers = p.matchers).documentation("The matchers to test the block against.").add()).build();
   }
   ...
   public static class BlockIdMatcher implements NetworkSerializable<com.hypixel.hytale.protocol.BlockIdMatcher> {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BlockIdMatcher.class, BlockIdMatcher::new).appendInherited(new KeyedCodec("Id", Codec.STRING), (blockIdMatcher, s) -> blockIdMatcher.id = s, (blockIdMatcher) -> blockIdMatcher.id, (blockIdMatcher, parent) -> blockIdMatcher.id = parent.id).addValidatorLate(() -> BlockType.VALIDATOR_CACHE.getValidator().late()).documentation("Match against a specific block id.").add()).appendInherited(new KeyedCodec("State", Codec.STRING), (blockIdMatcher, s) -> blockIdMatcher.state = s, (blockIdMatcher) -> blockIdMatcher.state, (blockIdMatcher, parent) -> blockIdMatcher.state = parent.state).documentation("Match against specific block state.").add()).appendInherited(new KeyedCodec("Tag", Codec.STRING), (blockIdMatcher, s) -> blockIdMatcher.tag = s, (blockIdMatcher) -> blockIdMatcher.tag, (blockIdMatcher, parent) -> blockIdMatcher.tag = parent.tag).documentation("Match against specific block tag.").add()).afterDecode((blockIdMatcher) -> {
