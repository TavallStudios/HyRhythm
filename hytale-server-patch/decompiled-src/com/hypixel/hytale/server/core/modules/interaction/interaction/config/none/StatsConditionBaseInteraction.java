...
public abstract class StatsConditionBaseInteraction extends SimpleInstantInteraction {
   ...
   public String toString() {
      String var10000 = String.valueOf(this.rawCosts);
      return "StatsConditionBaseInteraction{rawCosts=" + var10000 + ", costs=" + String.valueOf(this.costs) + ", lessThan=" + this.lessThan + ", lenient=" + this.lenient + ", valueType=" + String.valueOf(this.valueType) + "}" + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.abstractBuilder(StatsConditionBaseInteraction.class, SimpleInstantInteraction.CODEC).appendInherited(new KeyedCodec("Costs", new Object2FloatMapCodec(Codec.STRING, Object2FloatOpenHashMap::new)), (i, s) -> i.rawCosts = s, (i) -> i.rawCosts, (i, parent) -> i.rawCosts = parent.rawCosts).addValidator(Validators.nonNull()).addValidator(EntityStatType.VALIDATOR_CACHE.getMapKeyValidator()).add()).appendInherited(new KeyedCodec("LessThan", Codec.BOOLEAN), (statsConditionInteraction, aBoolean) -> statsConditionInteraction.lessThan = aBoolean, (statsConditionInteraction) -> statsConditionInteraction.lessThan, (statsConditionInteraction, parent) -> statsConditionInteraction.lessThan = parent.lessThan).add()).appendInherited(new KeyedCodec("Lenient", Codec.BOOLEAN), (statsConditionInteraction, aBoolean) -> statsConditionInteraction.lenient = aBoolean, (statsConditionInteraction) -> statsConditionInteraction.lenient, (statsConditionInteraction, parent) -> statsConditionInteraction.lenient = parent.lenient).documentation("Specifies that the interaction can run even if the stat cost is not met, providing that the value is greater than zero.").add()).appendInherited(new KeyedCodec("ValueType", new EnumCodec(ValueType.class)), (statsConditionInteraction, valueType) -> statsConditionInteraction.valueType = valueType, (statsConditionInteraction) -> statsConditionInteraction.valueType, (statsConditionInteraction, parent) -> statsConditionInteraction.valueType = parent.valueType).documentation("Enum to specify if the Costs must be considered as absolute values or percent. Default value is Absolute. When using ValueType.Absolute, '100' matches the max value.").add()).afterDecode((c) -> c.costs = EntityStatsModule.resolveEntityStats(c.rawCosts))).build();
   }
}
