...
public class DamageModule extends JavaPlugin {
   public static final PluginManifest MANIFEST = PluginManifest.corePlugin(DamageModule.class).depends(EntityModule.class).depends(EntityStatsModule.class).depends(EntityUIModule.class).build();
   ...
   public static class OrderGatherFilter implements ISystem<EntityStore> {
      ...
      public OrderGatherFilter() {
         this.dependencies = Set.of(new SystemGroupDependency(Order.AFTER, DamageModule.get().getGatherDamageGroup()), new SystemGroupDependency(Order.BEFORE, DamageModule.get().getFilterDamageGroup()));
      }
