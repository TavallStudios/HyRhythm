...
public class CollisionDataArray<T> {
   ...
   public T alloc() {
      ...
      if (this.freeList.isEmpty()) {
         result = (T)this.supplier.get();
      } else {
         int last = this.freeList.size() - 1;
         result = (T)this.freeList.get(last);
   ...
   @Nullable
   public T getFirst() {
      return (T)(this.head < this.array.size() ? this.array.get(this.head) : null);
   }
   ...
   public T get(int i) {
      return (T)this.array.get(this.head + i);
   }
}
