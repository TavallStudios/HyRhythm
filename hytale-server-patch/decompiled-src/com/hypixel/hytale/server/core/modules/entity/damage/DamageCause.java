...
public class DamageCause implements JsonAssetWithMap<String, IndexedLookupTableAssetMap<String, DamageCause>> {
   ...
   public static AssetStore<String, DamageCause, IndexedLookupTableAssetMap<String, DamageCause>> getAssetStore() {
      if (ASSET_STORE == null) {
         ASSET_STORE = AssetRegistry.<String, DamageCause, IndexedLookupTableAssetMap<String, DamageCause>>getAssetStore(DamageCause.class);
      }
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(DamageCause.class, DamageCause::new, Codec.STRING, (damageCause, s) -> damageCause.id = s, (damageCause) -> damageCause.id, (asset, data) -> asset.data = data, (asset) -> asset.data).append(new KeyedCodec("Inherits", Codec.STRING), (builder, name) -> builder.inherits = name, (builder) -> builder.inherits).add()).append(new KeyedCodec("DurabilityLoss", Codec.BOOLEAN), (builder, name) -> builder.durabilityLoss = name, (builder) -> builder.durabilityLoss).add()).append(new KeyedCodec("StaminaLoss", Codec.BOOLEAN), (builder, name) -> builder.staminaLoss = name, (builder) -> builder.staminaLoss).add()).append(new KeyedCodec("BypassResistances", Codec.BOOLEAN), (builder, name) -> builder.bypassResistances = name, (builder) -> builder.bypassResistances).add()).append(new KeyedCodec("DamageTextColor", Codec.STRING), (builder, name) -> builder.damageTextColor = name, (builder) -> builder.damageTextColor).add()).append(new KeyedCodec("AnimationId", Codec.STRING), (builder, name) -> builder.animationId = name, (builder) -> builder.animationId).add()).append(new KeyedCodec("DeathAnimationId", Codec.STRING), (builder, name) -> builder.deathAnimationId = name, (builder) -> builder.deathAnimationId).add()).build();
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(DamageCause::getAssetStore));
      CHILD_ASSET_CODEC = new ContainedAssetCodec(DamageCause.class, CODEC);
   }
}
