...
public class ChangeBlockInteraction extends SimpleBlockInteraction {
   ...
   protected void interactWithBlock(@Nonnull World world, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nullable ItemStack itemInHand, @Nonnull Vector3i targetBlock, @Nonnull CooldownHandler cooldownHandler) {
      if (this.requireNotBroken && itemInHand != null && itemInHand.isBroken()) {
         ...
         if (to != -2147483648) {
            BlockType toBlockType = (BlockType)BlockType.getAssetMap().getAsset(to);
            ...
            if (this.soundEventIndex != 0) {
               ...
               Vector3d pos = new Vector3d((double)x + 0.5, (double)y + 0.5, (double)z + 0.5);
   ...
   private Int2IntMap getChangeMapIds() {
      if (this.changeMapIds == null) {
         ...
         ids.defaultReturnValue(-2147483648);
   ...
   public String toString() {
      String var10000 = String.valueOf(this.blockTypeKeys);
      return "ChangeBlockInteraction{blockTypeKeys=" + var10000 + ", changeMapIds=" + String.valueOf(this.changeMapIds) + ", soundEventId='" + this.soundEventId + "', soundEventIndex=" + this.soundEventIndex + ", requireNotBroken=" + this.requireNotBroken + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ChangeBlockInteraction.class, ChangeBlockInteraction::new, SimpleBlockInteraction.CODEC).documentation("Changes the target block to another block based on the block types provided.")).appendInherited(new KeyedCodec("Changes", new MapCodec(Codec.STRING, HashMap::new)), (interaction, changeMap) -> interaction.blockTypeKeys = changeMap, (interaction) -> interaction.blockTypeKeys, (o, p) -> o.blockTypeKeys = p.blockTypeKeys).documentation("A map of the target block to the new block.\n\nWhen the interaction runs it will look for the block that was interacted with in this map and if found it will replace it with specified block").addValidator(BlockType.VALIDATOR_CACHE.getMapKeyValidator().late()).addValidator(BlockType.VALIDATOR_CACHE.getMapValueValidator().late()).add()).appendInherited(new KeyedCodec("WorldSoundEventId", Codec.STRING), (interaction, s) -> interaction.soundEventId = s, (interaction) -> interaction.soundEventId, (interaction, parent) -> interaction.soundEventId = parent.soundEventId).documentation("Sound event to play at the block location on block change.").addValidator(SoundEvent.VALIDATOR_CACHE.getValidator()).addValidator(SoundEventValidators.MONO).add()).appendInherited(new KeyedCodec("RequireNotBroken", Codec.BOOLEAN), (interaction, s) -> interaction.requireNotBroken = s, (interaction) -> interaction.requireNotBroken, (interaction, parent) -> interaction.requireNotBroken = parent.requireNotBroken).documentation("If true, the interaction will fail if the held item is broken (durability = 0).").add()).afterDecode(ChangeBlockInteraction::processConfig)).build();
   }
}
