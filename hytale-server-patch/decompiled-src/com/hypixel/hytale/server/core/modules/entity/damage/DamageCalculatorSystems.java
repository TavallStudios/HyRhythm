...
public class DamageCalculatorSystems {
   ...
   public static Damage[] queueDamageCalculator(@Nonnull World world, @Nonnull Object2FloatMap<DamageCause> relativeDamage, @Nonnull Ref<EntityStore> ref, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Damage.Source source, @Nullable ItemStack itemInHand) {
      ...
      Damage damage;
      for(ObjectIterator var8 = relativeDamage.keySet().iterator(); var8.hasNext(); results[offset++] = damage) {
         DamageCause damageCause = (DamageCause)var8.next();
         ...
         damage = new Damage(source, damageCause, damageAmount);
   ...
   public static class SequenceModifier extends DamageEventSystem {
      ...
      public SequenceModifier() {
         this.dependencies = Set.of(new SystemGroupDependency(Order.AFTER, DamageModule.get().getGatherDamageGroup()), new SystemGroupDependency(Order.AFTER, DamageModule.get().getFilterDamageGroup()), new SystemDependency(Order.BEFORE, DamageSystems.ApplyDamage.class));
      }
      ...
      public void handle(int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Damage damage) {
         if (!(damage.getAmount() <= 0.0F)) {
            DamageSequence damageSequence = (DamageSequence)damage.getIfPresentMetaObject(DamageCalculatorSystems.DAMAGE_SEQUENCE);
            if (damageSequence != null) {
               ...
               if (entityStatsOnHit != null) {
                  ...
                  if (var10 instanceof Damage.EntitySource) {
                     ...
                     EntityStatMap sourceEntityStatMapComponent = (EntityStatMap)commandBuffer.getComponent(sourceRef, EntityStatMap.getComponentType());
   ...
   public static class Sequence {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(Sequence.class, Sequence::new).append(new KeyedCodec("Hits", Codec.INTEGER), (entityEffect, x) -> entityEffect.hits = x, (entityEffect) -> entityEffect.hits).add()).build();
      }
   }
}
