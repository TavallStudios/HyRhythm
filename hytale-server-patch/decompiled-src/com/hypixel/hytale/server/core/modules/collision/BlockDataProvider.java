...
public class BlockDataProvider extends BlockData {
   ...
   protected final int INVALID_CHUNK_SECTION_INDEX = -2147483648;
   @Nullable
   ...
   public void read(int x, int y, int z) {
      ...
      if (this.blockId != newBlockId || this.fluidId != fluidId) {
         if (newBlockId == 0 && fluidId == 0) {
            ...
            this.blockType = (BlockType)BlockType.getAssetMap().getAsset(newBlockId);
            if (this.blockType.isUnknown()) {
               ...
               this.fillHeight = fluidId != 0 ? (double)fluidLevel / (double)fluid.getMaxFluidLevel() : 0.0;
   ...
   protected int readFluidId(int x, int y, int z) {
      ...
      if (this.chunkSectionRef == null) {
         ...
         FluidSection fluidSection = (FluidSection)this.world.getChunkStore().getStore().getComponent(this.chunkSectionRef, FluidSection.getComponentType());
