...
public class MigrationModule extends JavaPlugin {
   ...
   public void runMigrations() {
      ...
      Map<String, Path> migrationMap = (Map)Options.getOptionSet().valueOf(Options.MIGRATIONS);
      List<Migration> migrations = new ObjectArrayList();
      migrationMap.forEach((s, path) -> {
         Function<Path, Migration> migrationCtor = (Function)this.migrationCtors.get(s);
         if (migrationCtor != null) {
            migrations.add((Migration)migrationCtor.apply(path));
         }
      });
