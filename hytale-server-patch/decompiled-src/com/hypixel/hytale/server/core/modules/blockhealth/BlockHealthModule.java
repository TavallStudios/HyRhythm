...
public class BlockHealthModule extends JavaPlugin {
            ...
            if (!blockHealthMap.isEmpty()) {
               ...
               for(PlayerRef playerRef : allPlayers) {
                  ...
                  if (playerReference != null && playerReference.isValid()) {
                     ChunkTracker chunkTrackerComponent = (ChunkTracker)entityStore.getComponent(playerReference, ChunkTracker.getComponentType());
               ...
               while(iterator.hasNext()) {
                  Map.Entry<Vector3i, BlockHealth> entry = (Map.Entry)iterator.next();
                  Vector3i position = (Vector3i)entry.getKey();
                  BlockHealth blockHealth = (BlockHealth)entry.getValue();
   ...
   private static class BlockHealthPacketSystem extends ChunkStore.LoadPacketDataQuerySystem {
      ...
      public void fetch(int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, Store<ChunkStore> store, CommandBuffer<ChunkStore> commandBuffer, PlayerRef player, @Nonnull List<ToClientPacket> results) {
         BlockHealthChunk blockHealthChunkComponent = (BlockHealthChunk)archetypeChunk.getComponent(index, this.blockHealthCunkComponentType);
