...
public class LangFileParser {
   ...
   private static String literal(@Nonnull String value) {
      ...
      return literal.length() > 1 && literal.charAt(0) == '"' && literal.charAt(literal.length() - 1) == '"' ? literal.substring(1, literal.length() - 1) : literal;
   }
         ...
         if (!line.isEmpty() && line.charAt(0) != '#') {
            if (currKey == null) {
               ...
               if (isMultiline) {
                  ...
                  String existing = (String)translations.put(key, escape(currValue));
                  if (existing != null) {
               ...
               if (!isMultiline) {
                  String existing = (String)translations.put(currKey, escape(currValue));
                  if (existing != null) {
                     throw new TranslationParseException("Duplicate key in line", lineNumber, line);
                  }
