...
public class RepeatInteraction extends SimpleInteraction {
   ...
   protected void tick0(boolean firstRun, float time, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
      ...
      InteractionChain chain = (InteractionChain)instanceStore.getMetaObject(FORKED_CHAIN);
      if (chain != null) {
         switch (chain.getServerState()) {
            case NotFinished:
               ...
               return;
            case Finished:
               if (this.repeat != -1 && (Integer)instanceStore.getMetaObject(REMAINING_REPEATS) <= 0) {
      ...
      context.getState().state = InteractionState.NotFinished;
      if (this.repeat != -1) {
         instanceStore.putMetaObject(REMAINING_REPEATS, (Integer)instanceStore.getMetaObject(REMAINING_REPEATS) - 1);
      }
   ...
   protected void simulateTick0(boolean firstRun, float time, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
      InteractionChain chain = (InteractionChain)context.getInstanceStore().getMetaObject(FORKED_CHAIN);
      DynamicMetaStore<Interaction> instanceStore = context.getInstanceStore();
      if (chain != null) {
         switch (chain.getServerState()) {
            case NotFinished:
   ...
   public boolean walk(@Nonnull Collector collector, @Nonnull InteractionContext context) {
      if (this.forkInteractions != null && InteractionManager.walkInteractions(collector, context, TAG_FORK, RootInteraction.getRootInteractionOrUnknown(this.forkInteractions).getInteractionIds())) {
         return true;
      } else if (this.next != null && InteractionManager.walkInteraction(collector, context, TAG_NEXT, this.next)) {
         return true;
      } else {
