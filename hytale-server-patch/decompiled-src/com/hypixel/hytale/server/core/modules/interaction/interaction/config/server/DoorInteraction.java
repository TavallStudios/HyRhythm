...
public class DoorInteraction extends SimpleBlockInteraction {
   ...
   protected void interactWithBlock(@Nonnull World world, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nullable ItemStack itemInHand, @Nonnull Vector3i targetBlock, @Nonnull CooldownHandler cooldownHandler) {
      ...
      if (chunk != null) {
         ...
         DoorState doorState = DoorInteraction.DoorState.fromBlockState(blockState);
         ...
         if (checkResult == null) {
            ...
            DoorState stateDoubleDoor = getOppositeDoorState(doorState);
            ...
            if (interactionBlockState != null) {
               ...
               BlockBoundingBoxes blockBoundingBoxes = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(hitboxTypeIndex);
   ...
   private boolean checkForDoubleDoor(@Nonnull World world, @Nonnull Vector3i blockPosition, @Nonnull BlockType blockType, int rotation, @Nonnull DoorState fromState, @Nonnull DoorState doorStateToCheck) {
      ...
      if (doorToOpen == null) {
         ...
         DoorState stateForDoubleDoor = otherDoorIsHorizontal ? fromState : getOppositeDoorState(fromState);
   ...
   private static boolean isHorizontalDoor(@Nonnull BlockType blockType) {
      String rootInteractionId = (String)blockType.getInteractions().get(InteractionType.Use);
   ...
   private DoorState checkDoor(@Nonnull ChunkAccessor<WorldChunk> chunkAccessor, @Nonnull Vector3i blockPosition, @Nonnull BlockType blockType, int rotation, @Nonnull DoorState oldDoorState, @Nonnull DoorState newDoorState) {
      DoorInfo doubleDoor = getDoubleDoor(chunkAccessor, blockPosition, blockType, rotation, oldDoorState);
      DoorState newOppositeDoorState = getOppositeDoorState(newDoorState);
      ...
      if (canOpenDoor(chunkAccessor, blockPosition, newDoorInteractionState)) {
         if (!this.horizontal && doubleDoor != null && !canOpenDoor(chunkAccessor, doubleDoor.blockPosition, newOppositeDoorInteractionState)) {
            if (canOpenDoor(chunkAccessor, blockPosition, newOppositeDoorInteractionState) && canOpenDoor(chunkAccessor, doubleDoor.blockPosition, newDoorInteractionState)) {
   ...
   private static BlockType activateDoor(@Nonnull World world, @Nonnull BlockType blockType, @Nonnull Vector3i blockPosition, @Nonnull DoorState fromState, @Nonnull DoorState doorState) {
      ...
      BlockBoundingBoxes oldHitbox = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(blockType.getHitboxTypeIndex());
      ...
      if (currentBlockType == null) {
         ...
         if (newBlockType != null) {
            BlockBoundingBoxes newHitbox = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(newBlockType.getHitboxTypeIndex());
      ...
      if (blockTypeItem == null) {
         ...
         if (blockTypeItemAsset == null) {
            ...
            if (blockBoundingBoxes == null) {
               ...
               DoorInfo matchingDoor = getDoorAtPosition(chunkAccessor, blockPosition.x, blockPosition.y, blockPosition.z, rotationToCheck.flip());
   ...
   private static boolean canOpenDoor(@Nonnull ChunkAccessor<WorldChunk> chunkAccessor, @Nonnull Vector3i blockPosition, @Nonnull String state) {
      ...
      if (chunk == null) {
         ...
         BlockType originalBlockType = (BlockType)BlockType.getAssetMap().getAsset(blockId);
   ...
   private static boolean isInFrontOfDoor(@Nonnull Vector3i blockPosition, @Nullable Rotation doorRotationYaw, @Nonnull Vector3d playerPosition) {
      ...
      Vector3d doorRotationVector = new Vector3d((double)TrigMathUtil.sin(doorRotationRad), 0.0, (double)TrigMathUtil.cos(doorRotationRad));
   ...
   private static DoorState getOppositeDoorState(@Nonnull DoorState doorState) {
      return doorState == DoorInteraction.DoorState.OPENED_OUT ? DoorInteraction.DoorState.OPENED_IN : (doorState == DoorInteraction.DoorState.OPENED_IN ? DoorInteraction.DoorState.OPENED_OUT : DoorInteraction.DoorState.CLOSED);
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(DoorInteraction.class, DoorInteraction::new, SimpleBlockInteraction.CODEC).documentation("Opens/Closes a door")).appendInherited(new KeyedCodec("Horizontal", Codec.BOOLEAN), (t, i) -> t.horizontal = i, (t) -> t.horizontal, (t, parent) -> t.horizontal = parent.horizontal).documentation("Whether the door is horizontal (e.g. gates) or vertical (e.g. regular doors).").add()).build();
   }
   ...
   private static enum DoorState {
      ...
      public static DoorState fromBlockState(@Nullable String state) {
         if (state == null) {
            ...
            DoorState var10000;
            switch (state) {
               case "OpenDoorOut" -> var10000 = OPENED_IN;
               case "OpenDoorIn" -> var10000 = OPENED_OUT;
