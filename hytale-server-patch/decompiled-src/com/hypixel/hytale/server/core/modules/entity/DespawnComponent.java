...
public class DespawnComponent implements Component<EntityStore> {
   ...
   public static DespawnComponent despawnInSeconds(@Nonnull TimeResource time, int seconds) {
      return new DespawnComponent(time.getNow().plus(Duration.ofSeconds((long)seconds)));
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(DespawnComponent.class, DespawnComponent::new).append(new KeyedCodec("Despawn", Codec.INSTANT), (despawnComponent, instant) -> despawnComponent.timeToDespawnAt = instant, (despawnComponent) -> despawnComponent.timeToDespawnAt).add()).build();
   }
}
