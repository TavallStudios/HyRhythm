...
public class EntityUIModule extends JavaPlugin {
   ...
   protected void setup() {
      AssetRegistry.register(((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)HytaleAssetStore.builder(EntityUIComponent.class, new IndexedLookupTableAssetMap((x$0) -> new EntityUIComponent[x$0])).setPath("Entity/UI")).setCodec(EntityUIComponent.CODEC)).setKeyFunction(EntityUIComponent::getId)).setPacketGenerator(new EntityUIComponentPacketGenerator()).setReplaceOnRemove(EntityUIComponent::getUnknownFor)).loadsAfter(new Class[]{EntityStatType.class})).build());
