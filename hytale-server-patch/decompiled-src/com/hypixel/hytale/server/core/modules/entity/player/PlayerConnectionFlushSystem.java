...
public class PlayerConnectionFlushSystem extends EntityTickingSystem<EntityStore> implements RunWhenPausedSystem<EntityStore> {
   ...
   public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ((PlayerRef)archetypeChunk.getComponent(index, this.componentType)).getPacketHandler().tryFlush();
   }

   static {
      DEPENDENCIES = Set.of(new SystemGroupDependency(Order.AFTER, EntityStore.SEND_PACKET_GROUP), new SystemDependency(Order.AFTER, PlayerPingSystem.class, OrderPriority.CLOSEST), RootDependency.last());
   }
}
