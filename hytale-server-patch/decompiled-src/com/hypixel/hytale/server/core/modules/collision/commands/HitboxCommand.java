...
public class HitboxCommand extends AbstractCommandCollection {
   ...
   private static class HitboxGetCommand extends CommandBase {
      ...
      protected void executeSync(@Nonnull CommandContext context) {
         String name = (String)this.hitboxArg.get(context);
         BlockBoundingBoxes boundingBox = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(name);
         if (boundingBox != null) {
            ...
            if (details.length > 0) {
               ...
               Set<Message> detailMessages = (Set)Arrays.stream(details).map(HitboxCommand::formatBox).collect(Collectors.toSet());
   ...
   private static class HitboxExtentsCommand extends CommandBase {
      ...
      protected void executeSync(@Nonnull CommandContext context) {
         ...
         double threshold = this.thresholdArg.provided(context) ? (Double)this.thresholdArg.get(context) : 0.5;

         for(BlockType blockType : blockTypeAssetMap.getAssetMap().values()) {
            Box boundingBox = ((BlockBoundingBoxes)boundingBoxAssetMap.getAsset(blockType.getHitboxTypeIndex())).get(0).getBoundingBox();
            ...
            if (width - (double)blockWidth > threshold) {
               ++blockWidth;
            }

            if (height - (double)blockHeight > threshold) {
               ++blockHeight;
            }

            if (depth - (double)blockDepth > threshold) {
               ++blockDepth;
            }
