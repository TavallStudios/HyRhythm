...
public class EntityStatValue {
   ...
   public Modifier getModifier(String key) {
      return this.modifiers == null ? null : (Modifier)this.modifiers.get(key);
   }
   ...
   protected Modifier putModifier(String key, Modifier modifier) {
      ...
      Modifier oldModifier = (Modifier)this.modifiers.put(key, modifier);
      this.computeModifiers((EntityStatType)EntityStatType.getAssetMap().getAsset(this.index));
      return oldModifier;
   }
   ...
   protected Modifier removeModifier(String key) {
      if (this.modifiers == null) {
         ...
         Modifier modifier = (Modifier)this.modifiers.remove(key);
         if (modifier != null) {
            this.computeModifiers((EntityStatType)EntityStatType.getAssetMap().getAsset(this.index));
         }
         ...
         for(Modifier.ModifierTarget target : Modifier.ModifierTarget.VALUES) {
            ...
            for(Modifier modifier : this.modifiers.values()) {
               if (modifier instanceof StaticModifier) {
                  StaticModifier staticModifier = (StaticModifier)modifier;
   ...
   private void applyModifier(@Nonnull Modifier modifier) {
      switch (modifier.getTarget()) {
         case MIN -> this.min = modifier.apply(this.min);
         case MAX -> this.max = modifier.apply(this.max);
      }
   ...
   public String toString() {
      ...
      return "EntityStatValue{id='" + var10000 + "', index=" + this.index + ", value=" + this.value + ", min=" + this.min + ", max=" + this.max + ", regeneratingValues=" + Arrays.toString(this.regeneratingValues) + ", modifiers=" + String.valueOf(this.modifiers) + "}";
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(EntityStatValue.class, EntityStatValue::new).addField(new KeyedCodec("Id", Codec.STRING), (regenerating, value) -> regenerating.id = value, (regenerating) -> regenerating.id)).addField(new KeyedCodec("Value", Codec.FLOAT), (regenerating, value) -> regenerating.value = value, (regenerating) -> regenerating.value)).addField(new KeyedCodec("Modifiers", new MapCodec(Modifier.CODEC, HashMap::new, false)), (regenerating, value) -> regenerating.modifiers = value, (regenerating) -> regenerating.modifiers != null && !regenerating.modifiers.isEmpty() ? regenerating.modifiers : null)).build();
   }
}
