...
public class CheckUniqueItemUsageInteraction extends SimpleInstantInteraction {
   ...
   protected void firstRun(@Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
      ...
      if (playerRefComponent == null) {
         ...
         UniqueItemUsagesComponent uniqueItemUsagesComponent = (UniqueItemUsagesComponent)commandBuffer.getComponent(ref, UniqueItemUsagesComponent.getComponentType());

         assert uniqueItemUsagesComponent != null;

         if (uniqueItemUsagesComponent.hasUsedUniqueItem(context.getHeldItem().getItemId())) {
            context.getState().state = InteractionState.Failed;
            NotificationUtil.sendNotification(playerRefComponent.getPacketHandler(), Message.translation("server.commands.checkUniqueItemUsage.uniqueItemAlreadyUsed"));
         } else {
   ...
   static {
      CODEC = BuilderCodec.builder(CheckUniqueItemUsageInteraction.class, CheckUniqueItemUsageInteraction::new, SimpleInstantInteraction.CODEC).build();
   }
}
