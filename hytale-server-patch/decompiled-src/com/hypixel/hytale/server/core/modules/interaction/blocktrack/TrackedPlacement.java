...
public class TrackedPlacement implements Component<ChunkStore> {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(TrackedPlacement.class, TrackedPlacement::new).append(new KeyedCodec("BlockName", Codec.STRING), (o, v) -> o.blockName = v, (o) -> o.blockName).add()).build();
   }
      ...
      public void onEntityAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         if (reason == AddReason.SPAWN) {
            ...
            if (chunkRef != null && chunkRef.isValid()) {
               ...
               int blockId = blockChunk.getBlock(ChunkUtil.xFromBlockInColumn(blockInfo.getIndex()), ChunkUtil.yFromBlockInColumn(blockInfo.getIndex()), ChunkUtil.zFromBlockInColumn(blockInfo.getIndex()));
               BlockType blockType = (BlockType)BlockType.getAssetMap().getAsset(blockId);
               if (blockType != null) {
                  BlockCounter counter = (BlockCounter)commandBuffer.getResource(BLOCK_COUNTER_RESOURCE_TYPE);
                  String blockName = blockType.getId();
                  counter.trackBlock(blockName);
                  TrackedPlacement tracked = (TrackedPlacement)commandBuffer.getComponent(ref, COMPONENT_TYPE);
