...
public class BlockModule extends JavaPlugin {
      ...
      if (event.isNewlyGenerated()) {
         ...
         if (chunkColumnComponent != null) {
            ...
            if (sectionHolders != null) {
               BlockComponentChunk blockComponentModule = (BlockComponentChunk)holder.getComponent(BlockComponentChunk.getComponentType());
               if (blockComponentModule != null) {
                  for(int sectionIndex = 0; sectionIndex < 10; ++sectionIndex) {
                     BlockSection section = (BlockSection)sectionHolders[sectionIndex].ensureAndGetComponent(BlockSection.getComponentType());
   ...
   public static <T extends Component<ChunkStore>> T getComponent(ComponentType<ChunkStore, T> componentType, World world, int x, int y, int z) {
      ...
      if (chunkRef != null && chunkRef.isValid()) {
         ...
         if (blockComponentChunk == null) {
            ...
            int blockIndex = ChunkUtil.indexBlockInColumn(x, y, z);
            Ref<ChunkStore> blockRef = blockComponentChunk.getEntityReference(blockIndex);
            return (T)(blockRef != null && blockRef.isValid() ? chunkStore.getComponent(blockRef, componentType) : null);
         }
      } else {
         return null;
      }
   ...
   public static class BlockStateInfo implements Component<ChunkStore> {
      ...
      public void markNeedsSaving(ComponentAccessor<ChunkStore> accessor) {
         if (this.chunkRef.isValid()) {
            BlockComponentChunk blockComponentChunk = (BlockComponentChunk)accessor.getComponent(this.chunkRef, BlockComponentChunk.getComponentType());
            if (blockComponentChunk != null) {
   ...
   public static class BlockStateInfoRefSystem extends RefSystem<ChunkStore> {
      ...
      public void onEntityAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         BlockStateInfo blockStateInfoComponent = (BlockStateInfo)commandBuffer.getComponent(ref, this.blockStateInfoComponentType);
         ...
         if (chunkRef.isValid()) {
            ...
            if (blockComponentChunk != null) {
               switch (reason) {
                  case SPAWN -> blockComponentChunk.addEntityReference(blockStateInfoComponent.getIndex(), ref);
                  case LOAD -> blockComponentChunk.loadEntityReference(blockStateInfoComponent.getIndex(), ref);
               }
      ...
      public void onEntityRemove(@Nonnull Ref<ChunkStore> ref, @Nonnull RemoveReason reason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         ...
         if (chunkRef.isValid()) {
            ...
            if (blockComponentChunk != null) {
               switch (reason) {
                  case REMOVE -> blockComponentChunk.removeEntityReference(blockStateInfoComponent.getIndex(), ref);
                  case UNLOAD -> blockComponentChunk.unloadEntityReference(blockStateInfoComponent.getIndex(), ref);
               }
   ...
   public static class MigrateLaunchPad extends MigrationSystem {
      public void onEntityAdd(@Nonnull Holder<ChunkStore> holder, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store) {
         UnknownComponents<ChunkStore> unknownComponents = (UnknownComponents)holder.getComponent(ChunkStore.REGISTRY.getUnknownComponentType());

         assert unknownComponents != null;

         LaunchPad launchPadComponent = (LaunchPad)unknownComponents.removeComponent("launchPad", LaunchPad.CODEC);
