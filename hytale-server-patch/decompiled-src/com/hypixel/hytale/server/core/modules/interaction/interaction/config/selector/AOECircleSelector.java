...
public class AOECircleSelector extends SelectorType {
   ...
   public Vector3d selectTargetPosition(@Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> attackerRef) {
      TransformComponent transformComponent = (TransformComponent)commandBuffer.getComponent(attackerRef, TransformComponent.getComponentType());

      assert transformComponent != null;

      Vector3d position = transformComponent.getPosition();
      if (this.offset.x != 0.0 || this.offset.y != 0.0 || this.offset.z != 0.0) {
         HeadRotation headRotationComponent = (HeadRotation)commandBuffer.getComponent(attackerRef, HeadRotation.getComponentType());
   ...
   private class RuntimeSelector implements Selector {
      public void tick(@Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> ref, float time, float runTime) {
         if (SelectInteraction.SHOW_VISUAL_DEBUG) {
            ...
            DebugUtils.addSphere(((EntityStore)commandBuffer.getExternalData()).getWorld(), position, color, (double)(AOECircleSelector.this.range * 2.0F), 5.0F);
         }
      ...
      public void selectTargetEntities(@Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> attacker, @Nonnull BiConsumer<Ref<EntityStore>, Vector4d> consumer, Predicate<Ref<EntityStore>> filter) {
         ...
         Selector.selectNearbyEntities(commandBuffer, position, (double)AOECircleSelector.this.range, (entity) -> consumer.accept(entity, Vector4d.newPosition(position)), filter);
      }
