...
public class InteractionRules implements NetworkSerializable<com.hypixel.hytale.protocol.InteractionRules> {
   ...
   protected int blockedByBypassIndex = -2147483648;
   @Nullable
   protected String blockingBypass;
   protected int blockingBypassIndex = -2147483648;
   @Nullable
   protected String interruptedByBypass;
   protected int interruptedByBypassIndex = -2147483648;
   @Nullable
   protected String interruptingBypass;
   protected int interruptingBypassIndex = -2147483648;
   ...
   public boolean validateInterrupts(@Nonnull InteractionType type, @Nonnull Int2ObjectMap<IntSet> selfTags, @Nonnull InteractionType otherType, @Nonnull Int2ObjectMap<IntSet> otherTags, @Nonnull InteractionRules otherRules) {
      if (otherRules.interruptedBy == null || !otherRules.interruptedBy.contains(type) || otherRules.interruptedByBypassIndex != -2147483648 && selfTags.containsKey(otherRules.interruptedByBypassIndex)) {
         return this.interrupting != null && this.interrupting.contains(otherType) && (this.interruptingBypassIndex == -2147483648 || !otherTags.containsKey(this.interruptingBypassIndex));
      } else {
         return true;
      }
   }
   ...
   public boolean validateBlocked(@Nonnull InteractionType type, @Nonnull Int2ObjectMap<IntSet> selfTags, @Nonnull InteractionType otherType, @Nonnull Int2ObjectMap<IntSet> otherTags, @Nonnull InteractionRules otherRules) {
      Set<InteractionType> blockedBy = this.blockedBy != null ? this.blockedBy : (Set)InteractionTypeUtils.DEFAULT_INTERACTION_BLOCKED_BY.get(type);
      if (!blockedBy.contains(otherType) || this.blockedByBypassIndex != -2147483648 && otherTags.containsKey(this.blockedByBypassIndex)) {
         return otherRules.blocking.contains(type) && (otherRules.blockingBypassIndex == -2147483648 || !selfTags.containsKey(otherRules.blockingBypassIndex));
      } else {
         return true;
      }
   }
   ...
   public com.hypixel.hytale.protocol.InteractionRules toPacket() {
      ...
      packet.blockedBy = this.blockedBy == null ? null : (InteractionType[])this.blockedBy.toArray((x$0) -> new InteractionType[x$0]);
      packet.blocking = this.blocking.isEmpty() ? null : (InteractionType[])this.blocking.toArray((x$0) -> new InteractionType[x$0]);
      packet.interruptedBy = this.interruptedBy == null ? null : (InteractionType[])this.interruptedBy.toArray((x$0) -> new InteractionType[x$0]);
      packet.interrupting = this.interrupting == null ? null : (InteractionType[])this.interrupting.toArray((x$0) -> new InteractionType[x$0]);
   ...
   static {
      ...
      DEFAULT_RULES = new InteractionRules();
   }
}
