...
public class BlockSetCommand extends CommandBase {
   ...
   protected void executeSync(@Nonnull CommandContext context) {
      if (!this.blockSetArg.provided(context)) {
         Set<Message> blockSetKeys = (Set)BlockSet.getAssetMap().getAssetMap().keySet().stream().map(Message::raw).collect(Collectors.toSet());
         context.sendMessage(MessageFormat.list((Message)null, blockSetKeys));
      } else {
         String blockSetName = (String)this.blockSetArg.get(context);
         ...
         if (index == -2147483648) {
            ...
            IntSet set = (IntSet)this.blockSetModule.getBlockSets().get(index);
            if (set == null) {
               context.sendMessage(Message.translation("server.modules.blockset.setNotFound").param("name", blockSetName));
            } else {
               List<Message> names = new ObjectArrayList();
               set.forEach((i) -> names.add(Message.raw(((BlockType)BlockType.getAssetMap().getAsset(i)).getId().toString())));
               names.sort((Comparator)null);
               context.sendMessage(MessageFormat.list((Message)null, names));
            }
         }
      }
   }
