...
public class CollisionModuleConfig {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(CollisionModuleConfig.class, CollisionModuleConfig::new).addField(new KeyedCodec("ExtentMax", Codec.DOUBLE), (config, d) -> config.extentMax = d, (config) -> config.extentMax)).addField(new KeyedCodec("DumpInvalidBlocks", Codec.BOOLEAN), (config, b) -> config.dumpInvalidBlocks = b, (config) -> config.dumpInvalidBlocks)).addField(new KeyedCodec("MinimumThickness", Codec.DOUBLE), (config, d) -> config.minimumThickness = d, (config) -> config.minimumThickness)).build();
   }
}
