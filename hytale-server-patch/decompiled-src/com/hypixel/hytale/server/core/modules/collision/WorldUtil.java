   ...
   public static int getFluidIdAtPosition(@Nonnull ComponentAccessor<ChunkStore> chunkStore, @Nonnull ChunkColumn chunkColumnComponent, int x, int y, int z) {
      if (y >= 0 && y < 320) {
         ...
         if (sectionRef != null && sectionRef.isValid()) {
            FluidSection fluidSectionComponent = (FluidSection)chunkStore.getComponent(sectionRef, FluidSection.getComponentType());
               ...
               if (sectionRef != null && sectionRef.isValid()) {
                  ...
                  if (fluidSectionComponent != null) {
                     ...
                     if (fluidId != 0) {
                        ...
                        if (fluid != null) {
                           double yTest = y - (double)blockY;
                           if (yTest > (double)fluidSectionComponent.getFluidLevel(blockX, blockY, blockZ) / (double)fluid.getMaxFluidLevel()) {
                              fluidId = 0;
                           }
                        }
                     }
                  ...
                  if (blockType != null && !blockType.isUnknown()) {
                     ...
                     if (blockTypeAsset == null) {
                        ...
                        if (filler != 0 && blockTypeAsset.getMaterial() == BlockMaterial.Solid) {
                           ...
                           if (rotatedBoxes.containsPosition(x - (double)blockX + (double)fillerX, relativeY + (double)fillerY, z - (double)blockZ + (double)fillerZ)) {
