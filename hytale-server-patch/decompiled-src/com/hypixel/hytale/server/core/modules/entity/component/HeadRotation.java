...
public class HeadRotation implements Component<EntityStore> {
   ...
   private static Vector3d getDirection(float pitch, float yaw, @Nonnull Vector3d result) {
      if (Float.isNaN(pitch)) {
         ...
         return result.assign((double)yaw, (double)pitch);
      }
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(HeadRotation.class, HeadRotation::new).append(new KeyedCodec("Rotation", Vector3f.ROTATION), (o, i) -> o.rotation.assign(i), (o) -> o.rotation).add()).build();
   }
}
