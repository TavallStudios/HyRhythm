...
public class ItemComponent implements Component<EntityStore> {
   ...
   public float computeLifetimeSeconds(@Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      ItemEntityConfig defaultConfig = ((EntityStore)componentAccessor.getExternalData()).getWorld().getGameplayConfig().getItemEntityConfig();
   ...
   public static Holder<EntityStore>[] generateItemDrops(@Nonnull ComponentAccessor<EntityStore> accessor, @Nonnull List<ItemStack> itemStacks, @Nonnull Vector3d position, @Nonnull Vector3f rotation) {
      if (itemStacks.size() == 1) {
         Holder<EntityStore> itemEntityHolder = generateItemDrop(accessor, (ItemStack)itemStacks.getFirst(), position, rotation, 0.0F, 3.25F, 0.0F);
         return itemEntityHolder == null ? Holder.emptyArray() : new Holder[]{itemEntityHolder};
      } else {
         float randomAngleOffset = ThreadLocalRandom.current().nextFloat() * 6.2831855F;
         ...
         }).filter(Objects::nonNull).toArray((x$0) -> new Holder[x$0]);
      }
   }
   ...
   public static Holder<EntityStore> generateItemDrop(@Nonnull ComponentAccessor<EntityStore> accessor, @Nullable ItemStack itemStack, @Nonnull Vector3d position, @Nonnull Vector3f rotation, float velocityX, float velocityY, float velocityZ) {
      if (itemStack != null && !itemStack.isEmpty() && itemStack.isValid()) {
         ...
         ((Velocity)holder.ensureAndGetComponent(Velocity.getComponentType())).set((double)velocityX, (double)velocityY, (double)velocityZ);
   ...
   public static Holder<EntityStore> generatePickedUpItem(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor, @Nonnull Ref<EntityStore> targetRef, @Nonnull Vector3d targetPosition) {
      if (!ref.isValid()) {
         ...
         TransformComponent itemTransformComponent = (TransformComponent)componentAccessor.getComponent(ref, TransformComponent.getComponentType());
         if (itemTransformComponent == null) {
            ...
            ItemComponent itemItemComponent = (ItemComponent)componentAccessor.getComponent(ref, getComponentType());
            if (itemItemComponent == null) {
               ...
               holder.ensureComponent(Intangible.getComponentType());
               holder.addComponent(NetworkId.getComponentType(), new NetworkId(((EntityStore)ref.getStore().getExternalData()).takeNextNetworkId()));
   ...
   public static Holder<EntityStore> generatePickedUpItem(@Nonnull ItemStack itemStack, @Nonnull Vector3d position, @Nonnull ComponentAccessor<EntityStore> componentAccessor, @Nonnull Ref<EntityStore> targetRef) {
      ...
      holder.ensureComponent(PreventItemMerging.getComponentType());
      holder.ensureComponent(Intangible.getComponentType());
      holder.addComponent(NetworkId.getComponentType(), new NetworkId(((EntityStore)componentAccessor.getExternalData()).takeNextNetworkId()));
      holder.ensureComponent(EntityStore.REGISTRY.getNonSerializedComponentType());
      return holder;
   }
   ...
   public static ItemStack addToItemContainer(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> itemRef, @Nonnull ItemContainer itemContainer) {
      if (!itemRef.isValid()) {
         ...
         ItemComponent itemComponent = (ItemComponent)store.getComponent(itemRef, getComponentType());
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ItemComponent.class, ItemComponent::new).append(new KeyedCodec("Item", ItemStack.CODEC), (item, itemStack, extraInfo) -> {
         item.itemStack = itemStack;
         if (extraInfo instanceof BlockMigrationExtraInfo) {
            String newItemId = (String)((BlockMigrationExtraInfo)extraInfo).getBlockMigration().apply(itemStack.getItemId());
      ...
      }, (item, extraInfo) -> item.itemStack).add()).append(new KeyedCodec("StackDelay", Codec.FLOAT), (item, v) -> item.mergeDelay = v, (item) -> item.mergeDelay).add()).append(new KeyedCodec("PickupDelay", Codec.FLOAT), (item, v) -> item.pickupDelay = v, (item) -> item.pickupDelay).add()).append(new KeyedCodec("PickupThrottle", Codec.FLOAT), (item, v) -> item.pickupThrottle = v, (item) -> item.pickupThrottle).add()).append(new KeyedCodec("RemovedByPlayerPickup", Codec.BOOLEAN), (item, v) -> item.removedByPlayerPickup = v, (item) -> item.removedByPlayerPickup).add()).build();
   }
}
