...
public class InteractionModule extends JavaPlugin {
   ...
   @Nonnull
   public static final EnumCodec<InteractionType> INTERACTION_TYPE_CODEC = new EnumCodec<InteractionType>(InteractionType.class);
   @Nonnull
   ...
   protected void setup() {
      ...
      AssetRegistry.register(((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)HytaleAssetStore.builder(UnarmedInteractions.class, new DefaultAssetMap()).setPath("Item/Unarmed/Interactions")).setCodec(UnarmedInteractions.CODEC)).setKeyFunction(UnarmedInteractions::getId)).loadsAfter(new Class[]{RootInteraction.class})).setPacketGenerator(new UnarmedInteractionsPacketGenerator()).build());
      AssetRegistry.register(((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)HytaleAssetStore.builder(Interaction.class, new IndexedLookupTableAssetMap((x$0) -> new Interaction[x$0])).setPath("Item/Interactions")).setCodec(Interaction.CODEC)).setKeyFunction(Interaction::getId)).setReplaceOnRemove((id) -> new SendMessageInteraction(id, "Missing interaction: " + id))).setIsUnknown(Interaction::isUnknown)).setPacketGenerator(new InteractionPacketGenerator()).loadsAfter(new Class[]{EntityStatType.class, EntityEffect.class, Trail.class, ItemPlayerAnimations.class, SoundEvent.class, ParticleSystem.class, ModelAsset.class, HitboxCollisionConfig.class})).preLoadAssets(List.of(ChangeActiveSlotInteraction.DEFAULT_INTERACTION))).build());
      AssetRegistry.register(((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)HytaleAssetStore.builder(RootInteraction.class, new IndexedLookupTableAssetMap((x$0) -> new RootInteraction[x$0])).setPath("Item/RootInteractions")).setCodec(RootInteraction.CODEC)).setKeyFunction(RootInteraction::getId)).setReplaceOnRemove((x$0) -> new RootInteraction(x$0, new String[0]))).setPacketGenerator(new RootInteractionPacketGenerator()).loadsAfter(new Class[]{Interaction.class})).loadsBefore(new Class[]{BlockType.class, Item.class})).preLoadAssets(List.of(ChangeActiveSlotInteraction.DEFAULT_ROOT))).build());
      ...
      this.placedByComponentType = this.getChunkStoreRegistry().registerComponent(PlacedByInteractionComponent.class, "PlacedByInteraction", PlacedByInteractionComponent.CODEC);
      ...
      OpenCustomUIInteraction.registerBlockEntityCustomPage(this, LaunchPad.LaunchPadSettingsPage.class, "LaunchPad", (playerRef, ref) -> ref.getStore().getArchetype(ref).contains(LaunchPad.getComponentType()) ? new LaunchPad.LaunchPadSettingsPage(playerRef, ref, CustomPageLifetime.CanDismissOrCloseThroughInteraction) : null);
      OpenCustomUIInteraction.PAGE_CODEC.register((String)"ItemRepair", ItemRepairPageSupplier.class, ItemRepairPageSupplier.CODEC);
      SelectorType.CODEC.register((String)"Horizontal", HorizontalSelector.class, HorizontalSelector.CODEC);
      SelectorType.CODEC.register((String)"Stab", StabSelector.class, StabSelector.CODEC);
      SelectorType.CODEC.register((String)"AOECircle", AOECircleSelector.class, AOECircleSelector.CODEC);
      SelectorType.CODEC.register((String)"AOECylinder", AOECylinderSelector.class, AOECylinderSelector.CODEC);
      SelectorType.CODEC.register((String)"Raycast", RaycastSelector.class, RaycastSelector.CODEC);
      Knockback.CODEC.register((String)"Directional", DirectionalKnockback.class, DirectionalKnockback.CODEC);
      Knockback.CODEC.register((String)"Point", PointKnockback.class, PointKnockback.CODEC);
      Knockback.CODEC.register((String)"Force", ForceKnockback.class, ForceKnockback.CODEC);
      eventRegistry.register((Class)LoadedAssetsEvent.class, RootInteraction.class, InteractionModule::handledLoadedRootInteractions);
      eventRegistry.register((Class)LoadedAssetsEvent.class, Interaction.class, InteractionModule::handledLoadedInteractions);
      eventRegistry.register((Class)RemovedAssetsEvent.class, Interaction.class, InteractionModule::handledRemovedInteractions);
      ...
      this.getCodecRegistry(Interaction.CODEC).register("PlacementCountCondition", PlacementCountConditionInteraction.class, PlacementCountConditionInteraction.CODEC);
   }
   ...
   private static void handledLoadedInteractions(@Nonnull LoadedAssetsEvent<String, Interaction, ?> event) {
      for(Map.Entry<String, RootInteraction> entry : RootInteraction.getAssetMap().getAssetMap().entrySet()) {
         ((RootInteraction)entry.getValue()).build(event.getLoadedAssets().keySet());
      }
   ...
   private static void handledRemovedInteractions(@Nonnull RemovedAssetsEvent<String, Interaction, ?> event) {
      for(Map.Entry<String, RootInteraction> entry : RootInteraction.getAssetMap().getAssetMap().entrySet()) {
         ((RootInteraction)entry.getValue()).build(event.getRemovedAssets());
      }
      ...
      if (!this.isDisabled()) {
         ...
         if (activeHotbarSlot != packet.activeSlot) {
            ...
            if (ref.isValid()) {
               ...
               CameraManager cameraManagerComponent = (CameraManager)componentAccessor.getComponent(ref, CameraManager.getComponentType());

               assert cameraManagerComponent != null;

               if (packet.mouseButton != null) {
                  IEventDispatcher<PlayerMouseButtonEvent, PlayerMouseButtonEvent> dispatcher = HytaleServer.get().getEventBus().dispatchFor(PlayerMouseButtonEvent.class);
                  ...
                  IEventDispatcher<PlayerMouseMotionEvent, PlayerMouseMotionEvent> dispatcher = HytaleServer.get().getEventBus().dispatchFor(PlayerMouseMotionEvent.class);
                  if (dispatcher.hasListener()) {
                     dispatcher.dispatch(new PlayerMouseMotionEvent(ref, playerComponent, packet.clientTimestamp, item, targetBlock, targetEntity, packet.screenPoint, packet.mouseMotion));
                  }
               }

               cameraManagerComponent.setLastScreenPoint(new Vector2d((double)packet.screenPoint.x, (double)packet.screenPoint.y));
   ...
   static {
      INTERACTION_TYPE_SET_CODEC = new SetCodec<InteractionType, EnumSet<InteractionType>>(INTERACTION_TYPE_CODEC, () -> EnumSet.noneOf(InteractionType.class), true);
   }
}
