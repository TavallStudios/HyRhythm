...
public class EntityUIComponentPacketGenerator extends AssetPacketGenerator<String, EntityUIComponent, IndexedLookupTableAssetMap<String, EntityUIComponent>> {
   @Nonnull
   public ToClientPacket generateInitPacket(@Nonnull IndexedLookupTableAssetMap<String, EntityUIComponent> assetMap, @Nonnull Map<String, EntityUIComponent> assets) {
      ...
      for(Map.Entry<String, EntityUIComponent> entry : assets.entrySet()) {
         configs.put(assetMap.getIndex((String)entry.getKey()), ((EntityUIComponent)entry.getValue()).toPacket());
      }
   ...
   @Nonnull
   public ToClientPacket generateUpdatePacket(@Nonnull IndexedLookupTableAssetMap<String, EntityUIComponent> assetMap, @Nonnull Map<String, EntityUIComponent> loadedAssets, @Nonnull AssetUpdateQuery query) {
      ...
      for(Map.Entry<String, EntityUIComponent> entry : loadedAssets.entrySet()) {
         components.put(assetMap.getIndex((String)entry.getKey()), ((EntityUIComponent)entry.getValue()).toPacket());
      }
   ...
   @Nonnull
   public ToClientPacket generateRemovePacket(@Nonnull IndexedLookupTableAssetMap<String, EntityUIComponent> assetMap, @Nonnull Set<String> removed, @Nonnull AssetUpdateQuery query) {
      Int2ObjectMap<com.hypixel.hytale.protocol.EntityUIComponent> configs = new Int2ObjectOpenHashMap();
