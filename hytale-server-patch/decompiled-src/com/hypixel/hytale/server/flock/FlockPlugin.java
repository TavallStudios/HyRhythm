...
public class FlockPlugin extends JavaPlugin {
   ...
   private final Int2ObjectConcurrentHashMap<Map<UUID, UUID>> prefabFlockRemappings = new Int2ObjectConcurrentHashMap<Map<UUID, UUID>>();
   ...
   public UUID getPrefabRemappedFlockReference(int prefabId, UUID oldId) {
      return (UUID)((Map)this.prefabFlockRemappings.get(prefabId)).computeIfAbsent(oldId, (s) -> UUID.randomUUID());
   }
   ...
   public static Ref<EntityStore> trySpawnFlock(@Nonnull Ref<EntityStore> npcRef, @Nonnull NPCEntity npc, @Nonnull Store<EntityStore> store, int roleIndex, @Nonnull Vector3d position, Vector3f rotation, @Nullable FlockAsset flockDefinition, TriConsumer<NPCEntity, Ref<EntityStore>, Store<EntityStore>> postSpawn) {
      ...
      return trySpawnFlock(npcRef, npc, roleIndex, position, rotation, flockSize, flockDefinition, (TriConsumer)null, postSpawn, store);
   }

   @Nullable
   public static Ref<EntityStore> trySpawnFlock(@Nonnull Ref<EntityStore> npcRef, @Nonnull NPCEntity npc, @Nonnull Store<EntityStore> store, int roleIndex, @Nonnull Vector3d position, Vector3f rotation, int flockSize, TriConsumer<NPCEntity, Ref<EntityStore>, Store<EntityStore>> postSpawn) {
      return trySpawnFlock(npcRef, npc, roleIndex, position, rotation, flockSize, (FlockAsset)null, (TriConsumer)null, postSpawn, store);
   }
   ...
   public static Ref<EntityStore> trySpawnFlock(@Nonnull Ref<EntityStore> npcRef, @Nonnull NPCEntity npc, int roleIndex, @Nonnull Vector3d position, Vector3f rotation, int flockSize, FlockAsset flockDefinition, TriConsumer<NPCEntity, Holder<EntityStore>, Store<EntityStore>> preAddToWorld, @Nullable TriConsumer<NPCEntity, Ref<EntityStore>, Store<EntityStore>> postSpawn, @Nonnull Store<EntityStore> store) {
      if (flockSize > 1 && npcRef.isValid()) {
         ...
         FlockMembership membershipComponent = (FlockMembership)store.getComponent(npcRef, FlockMembership.getComponentType());
         Ref<EntityStore> flockReference = membershipComponent != null ? membershipComponent.getFlockRef() : createFlock(store, flockDefinition, role.getFlockAllowedRoles());
         if (membershipComponent == null) {
            FlockMembershipSystems.join(npcRef, flockReference, store);
         }

         BoundingBox boundingBoxComponent = (BoundingBox)store.getComponent(npcRef, BoundingBox.getComponentType());

         assert boundingBoxComponent != null;

         TransformComponent transformComponent = (TransformComponent)store.getComponent(npcRef, TransformComponent.getComponentType());
         ...
         double yaw = (double)bodyRotation.getYaw();
         ...
         for(int i = 1; i < flockSize; ++i) {
            ...
            Pair<Ref<EntityStore>, NPCEntity> memberPair = NPCPlugin.get().spawnEntity(store, memberRoleIndex, position, rotation, (Model)null, preAddToWorld, postSpawn);
            Ref<EntityStore> memberRef = (Ref)memberPair.first();
            if (memberRef != null && memberRef.isValid()) {
               BoundingBox memberBoundingBoxComponent = (BoundingBox)store.getComponent(memberRef, BoundingBox.getComponentType());

               assert memberBoundingBoxComponent != null;

               TransformComponent memberTransformComponent = (TransformComponent)store.getComponent(memberRef, TransformComponent.getComponentType());

               assert memberTransformComponent != null;

               HeadRotation memberHeadRotationComponent = (HeadRotation)store.getComponent(memberRef, HeadRotation.getComponentType());

               assert memberHeadRotationComponent != null;

               double offsetY = (double)y - memberBoundingBoxComponent.getBoundingBox().min.y;
               memberTransformComponent.getRotation().setYaw((float)(yaw + (double)RandomExtra.randomRange(0.7853982F, 0.7853982F)));
   ...
   public static Flock getFlock(@Nonnull ComponentAccessor<EntityStore> componentAccessor, @Nonnull Ref<EntityStore> reference) {
      FlockMembership flockMembershipComponent = (FlockMembership)componentAccessor.getComponent(reference, FlockMembership.getComponentType());
      if (flockMembershipComponent == null) {
         return null;
      } else {
         Ref<EntityStore> membershipRef = flockMembershipComponent.getFlockRef();
         return membershipRef != null && membershipRef.isValid() ? (Flock)componentAccessor.getComponent(membershipRef, Flock.getComponentType()) : null;
      }
   }

   @Nonnull
   public static Ref<EntityStore> createFlock(@Nonnull Store<EntityStore> store, @Nonnull Role role) {
      return createFlock(store, (FlockAsset)null, role.getFlockAllowedRoles());
   }
   ...
   public static Ref<EntityStore> getFlockReference(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      FlockMembership flockMembershipComponent = (FlockMembership)componentAccessor.getComponent(ref, FlockMembership.getComponentType());
   ...
   public static boolean isFlockMember(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store) {
      FlockMembership flockMembershipComponent = (FlockMembership)store.getComponent(ref, FlockMembership.getComponentType());
