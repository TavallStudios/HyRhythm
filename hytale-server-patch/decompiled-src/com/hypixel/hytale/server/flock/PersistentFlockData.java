...
public class PersistentFlockData implements Component<EntityStore> {
   ...
   private int maxGrowSize = 2147483647;
   ...
   public PersistentFlockData(@Nullable FlockAsset flockDefinition, @Nonnull String[] allowedRoles) {
      this.flockAllowedRoles = ArrayUtil.EMPTY_STRING_ARRAY;
      this.flockAllowedRoles = allowedRoles;
      Arrays.sort(this.flockAllowedRoles);
      if (flockDefinition != null) {
         ...
         if (blockedRoles.length > 0 && this.flockAllowedRoles.length > 0) {
            ...
            this.flockAllowedRoles = (String[])combinedList.toArray((x$0) -> new String[x$0]);
         }
      }
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(PersistentFlockData.class, PersistentFlockData::new).append(new KeyedCodec("MaxGrowSize", Codec.INTEGER), (flock, i) -> flock.maxGrowSize = i, (flock) -> flock.maxGrowSize).add()).append(new KeyedCodec("AllowedRoles", Codec.STRING_ARRAY), (flock, o) -> flock.flockAllowedRoles = o, (flock) -> flock.flockAllowedRoles).add()).append(new KeyedCodec("Size", Codec.INTEGER), (flock, i) -> flock.size = i, (flock) -> flock.size).add()).build();
   }
}
