...
public class FlockMembershipSystems {
   public static boolean canJoinFlock(@Nonnull Ref<EntityStore> reference, @Nonnull Ref<EntityStore> flockReference, @Nonnull Store<EntityStore> store) {
      Flock flockComponent = (Flock)store.getComponent(flockReference, Flock.getComponentType());
      ...
      if (flockData == null) {
         ...
         if (entityGroupComponent.size() >= flockData.getMaxGrowSize()) {
            ...
            NPCEntity npcComponent = (NPCEntity)store.getComponent(reference, NPCEntity.getComponentType());
   ...
   public static void join(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> flockRef, @Nonnull Store<EntityStore> store) {
      ...
      UUIDComponent uuidComponent = (UUIDComponent)store.getComponent(flockRef, UUIDComponent.getComponentType());
   ...
   private static void registerFlockDebugListener(@Nonnull Ref<EntityStore> ref, @Nonnull FlockMembership membership, @Nonnull Flock flock, @Nonnull ComponentAccessor<EntityStore> accessor) {
      NPCEntity npcComponent = (NPCEntity)accessor.getComponent(ref, NPCEntity.getComponentType());
      if (npcComponent != null) {
   ...
   private static void unregisterFlockDebugListener(@Nonnull Ref<EntityStore> ref, @Nonnull FlockMembership membership, @Nonnull Flock flock, @Nonnull ComponentAccessor<EntityStore> accessor) {
      NPCEntity npcComponent = (NPCEntity)accessor.getComponent(ref, NPCEntity.getComponentType());
      if (npcComponent != null) {
         Role role = npcComponent.getRole();
         if (role != null) {
   ...
   public static class EntityRef extends RefSystem<EntityStore> {
      ...
      private void joinOrCreateFlock(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store) {
         FlockMembership flockMembershipComponent = (FlockMembership)store.getComponent(ref, this.flockMembershipComponentType);

         assert flockMembershipComponent != null;

         UUID flockId = flockMembershipComponent.getFlockId();
         Ref<EntityStore> flockReference = ((EntityStore)store.getExternalData()).getRefFromUUID(flockId);
      ...
      public void onEntityRemove(@Nonnull Ref<EntityStore> ref, @Nonnull RemoveReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         if (flockRef != null && flockRef.isValid()) {
            ...
            if (reason != RemoveReason.REMOVE && !store.getArchetype(ref).contains(Player.getComponentType())) {
               if (reason == RemoveReason.UNLOAD) {
                  ...
                  if (!entityGroupComponent.isDissolved() && membership.getMembershipType().isActingAsLeader()) {
                     ...
                     if (interimLeader != null) {
                        FlockMembership interimLeaderMembership = (FlockMembership)store.getComponent(interimLeader, this.flockMembershipComponentType);
               ...
               FlockMembership flockMembershipComponent = (FlockMembership)store.getComponent(newLeader, this.flockMembershipComponentType);
   ...
   public static class RefChange extends RefChangeSystem<EntityStore, FlockMembership> {
      ...
      private static void setNewLeader(@Nonnull UUID flockId, @Nonnull EntityGroup entityGroup, @Nonnull Flock flock, @Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         if (oldLeader == null || !oldLeader.equals(ref)) {
            entityGroup.setLeaderRef(ref);
            if (oldLeader != null) {
               FlockMembership oldLeaderComponent = (FlockMembership)store.getComponent(oldLeader, FlockMembership.getComponentType());
               if (oldLeaderComponent != null) {
                  oldLeaderComponent.setMembershipType(FlockMembership.Type.MEMBER);
               }
            ...
            flockMembershipComponent.setMembershipType(FlockMembership.Type.LEADER);
            NPCEntity newLeaderNpcComponent = (NPCEntity)store.getComponent(ref, NPCEntity.getComponentType());
   ...
   public static class FilterPlayerFlockDamageSystem extends DamageEventSystem {
      @Nonnull
      private final Query<EntityStore> query = Query.<EntityStore>and(Player.getComponentType(), FlockMembership.getComponentType());
      ...
      public void handle(int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Damage damage) {
         FlockMembership flockMembership = (FlockMembership)archetypeChunk.getComponent(index, FlockMembership.getComponentType());
         ...
         if (var8 instanceof Damage.EntitySource) {
            ...
            if (flockRef != null && flockRef.isValid()) {
               ...
               if (sourceRef.isValid()) {
                  EntityGroup group = (EntityGroup)store.getComponent(flockRef, EntityGroup.getComponentType());
      ...
      public void handle(int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Damage damage) {
         ...
         if (var7 instanceof Damage.EntitySource entitySource) {
            ...
            if (damageSourceRef.isValid()) {
               FlockMembership flockMembershipComponent = (FlockMembership)commandBuffer.getComponent(damageSourceRef, FlockMembership.getComponentType());
               if (flockMembershipComponent != null) {
                  ...
                  if (flockReference != null && flockReference.isValid()) {
                     ...
                     flockComponent.getNextDamageData().onInflictedDamage(entityRef, (double)damage.getAmount());
                     if (flockMembershipComponent.getMembershipType().isActingAsLeader()) {
                        flockComponent.getNextLeaderDamageData().onInflictedDamage(entityRef, (double)damage.getAmount());
                     }
   ...
   public static class NPCAddedFromWorldGen extends HolderSystem<EntityStore> {
      ...
      public NPCAddedFromWorldGen() {
         this.query = Query.<EntityStore>and(this.npcComponentType, this.fromWorldGenComponentType, this.flockMembershipComponentType);
      }
