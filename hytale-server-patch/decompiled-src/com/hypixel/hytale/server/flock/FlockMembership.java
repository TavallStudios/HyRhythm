...
public class FlockMembership implements Component<EntityStore>, DebugSupport.DebugFlagsChangeListener {
   ...
   public void onDebugFlagsChanged(EnumSet<RoleDebugFlags> newFlags) {
      ...
      if (isVisFlock != this.wasVisFlock) {
         ...
         if (this.flockRef != null && this.flockRef.isValid()) {
            Flock flock = (Flock)this.flockRef.getStore().getComponent(this.flockRef, Flock.getComponentType());
            if (flock != null) {
               if (isVisFlock) {
                  flock.incrementVisFlockMemberCount();
               } else {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(FlockMembership.class, FlockMembership::new).legacyVersioned()).codecVersion(5)).append(new KeyedCodec("FlockId", Codec.UUID_BINARY), (membership, uuid) -> membership.flockId = uuid, (membership) -> membership.flockId).setVersionRange(5, 5).add()).append(new KeyedCodec("Type", new EnumCodec(Type.class, EnumCodec.EnumStyle.LEGACY)), (membership, type) -> membership.membershipType = type, (membership) -> membership.membershipType).add()).build();
   }
