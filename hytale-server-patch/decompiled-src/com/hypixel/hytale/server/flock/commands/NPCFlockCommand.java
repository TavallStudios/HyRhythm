...
public class NPCFlockCommand extends AbstractCommandCollection {
   ...
   public static int forNpcEntitiesInViewCone(@Nonnull Ref<EntityStore> playerReference, @Nonnull Store<EntityStore> store, @Nonnull BiPredicate<Ref<EntityStore>, NPCEntity> predicate) {
      ...
      TransformComponent transformComponent = (TransformComponent)store.getComponent(playerReference, transformComponentType);

      assert transformComponent != null;

      Vector3d position = transformComponent.getPosition();
      HeadRotation headRotationComponent = (HeadRotation)store.getComponent(playerReference, HeadRotation.getComponentType());
      ...
      SpatialResource<Ref<EntityStore>, EntityStore> spatialResource = (SpatialResource)store.getResource(NPCPlugin.get().getNpcSpatialResource());
      ...
      while(var19.hasNext()) {
         Ref<EntityStore> targetRef = (Ref)var19.next();
         NPCEntity targetNpcComponent = (NPCEntity)store.getComponent(targetRef, npcComponentType);

         assert targetNpcComponent != null;

         TransformComponent entityTransformComponent = (TransformComponent)store.getComponent(targetRef, transformComponentType);
   ...
   public static boolean anyEntityInViewCone(@Nonnull Ref<EntityStore> playerReference, @Nonnull Store<EntityStore> store, @Nonnull Predicate<Ref<EntityStore>> predicate) {
      ...
      while(var17.hasNext()) {
         Ref<EntityStore> entityRef = (Ref)var17.next();
         TransformComponent entityTransformComponent = (TransformComponent)store.getComponent(entityRef, transformComponentType);
