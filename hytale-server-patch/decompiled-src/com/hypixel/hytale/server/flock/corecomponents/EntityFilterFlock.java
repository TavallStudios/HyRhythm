...
public class EntityFilterFlock extends EntityFilterBase {
   ...
   public boolean matchesEntity(@Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> targetRef, @Nonnull Role role, @Nonnull Store<EntityStore> store) {
      FlockMembership membership = (FlockMembership)store.getComponent(targetRef, FLOCK_MEMBERSHIP_COMPONENT_TYPE);
      ...
      if (membership != null) {
         ...
         if (flockReference != null && flockReference.isValid()) {
            group = (EntityGroup)store.getComponent(flockReference, ENTITY_GROUP_COMPONENT_TYPE);
         }
      }
      ...
      if (this.size == null || group == null || group.size() >= this.size[0] && group.size() <= this.size[1]) {
         if (!this.checkCanJoin || membership != null && FlockMembershipSystems.canJoinFlock(targetRef, membership.getFlockRef(), store)) {
            ...
            boolean var10000;
            switch (this.flockPlayerMembership) {
               case Member -> var10000 = leaderIsPlayer;
               case NotMember -> var10000 = !leaderIsPlayer;
