...
public class ActionFlockBeacon extends ActionBase {
   ...
   public boolean canExecute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      if (!super.canExecute(ref, role, sensorInfo, dt, store)) {
         ...
         return store.getArchetype(ref).contains(FLOCK_MEMBERSHIP_COMPONENT_TYPE) && (this.sendTargetSlot == -2147483648 || role.getMarkedEntitySupport().hasMarkedEntityInSlot(this.sendTargetSlot));
      }
   }

   public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      super.execute(ref, role, sensorInfo, dt, store);
      FlockMembership flockMembership = (FlockMembership)store.getComponent(ref, FLOCK_MEMBERSHIP_COMPONENT_TYPE);
      ...
      if (flockReference != null && flockReference.isValid()) {
         EntityGroup entityGroup = (EntityGroup)store.getComponent(flockReference, ENTITY_GROUP_COMPONENT_TYPE);
         if (entityGroup == null) {
            return true;
         } else {
            ...
            if (this.sendToLeaderOnly) {
               ...
               entityGroup.forEachMember((TriConsumer)((flockMember, entity, _target) -> this.sendNPCMessage(flockMember, _target, store)), ref, targetRef, this.sendToSelf ? null : ref);
            }
   ...
   protected void sendNPCMessage(@Nonnull Ref<EntityStore> ref, @Nullable Ref<EntityStore> targetRef, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      BeaconSupport beaconSupport = (BeaconSupport)componentAccessor.getComponent(ref, BEACON_SUPPORT_COMPONENT_TYPE);
