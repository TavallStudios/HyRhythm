...
public class BuilderActionFlockBeacon extends BuilderActionBase {
   ...
   public BuilderActionFlockBeacon readConfig(@Nonnull JsonElement data) {
      this.requireString(data, "Message", this.message, StringNotEmptyValidator.get(), BuilderDescriptorState.Stable, "Message to send to targets", (String)null);
      this.getString(data, "SendTargetSlot", (b) -> this.sendTargetSlot = b, (String)null, StringNullOrNotEmptyValidator.get(), BuilderDescriptorState.Stable, "The marked target slot to send. If omitted, sends own position", (String)null);
      this.getDouble(data, "ExpirationTime", (d) -> this.expirationTime = d, 1.0, DoubleOrValidator.greaterEqual0OrMinus1(), BuilderDescriptorState.Stable, "The number of seconds that the message should last. -1 represents infinite time.", "The number of seconds that the message should last and be acknowledged by the receiving NPC. -1 represents infinite time.");
      this.getBoolean(data, "SendToSelf", (b) -> this.sendToSelf = b, true, BuilderDescriptorState.Stable, "Send the message to self", (String)null);
      this.getBoolean(data, "SendToLeaderOnly", (b) -> this.sendToLeaderOnly = b, false, BuilderDescriptorState.Stable, "Only send the message to the leader of the flock", (String)null);
      return this;
   }
   ...
   public int getSendTargetSlot(@Nonnull BuilderSupport support) {
      return this.sendTargetSlot == null ? -2147483648 : support.getTargetSlot(this.sendTargetSlot);
   }
