...
public class BodyMotionFlock extends BodyMotionBase {
   ...
   public boolean computeSteering(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, @Nullable InfoProvider sensorInfo, double dt, @Nonnull Steering desiredSteering, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      FlockMembership flockMembership = (FlockMembership)componentAccessor.getComponent(ref, FLOCK_MEMBERSHIP_COMPONENT_TYPE);
      ...
      if (flockReference != null && flockReference.isValid()) {
         EntityGroup entityGroup = (EntityGroup)componentAccessor.getComponent(flockReference, ENTITY_GROUP_COMPONENT_TYPE);
         ...
         if (!leaderRef.isValid()) {
            ...
            TransformComponent leaderTransformComponent = (TransformComponent)componentAccessor.getComponent(leaderRef, TRANSFORM_COMPONENT_TYPE);
            ...
            if (sumOfVelocities.squaredLength() > 1.0E-4) {
               ...
               TransformComponent parentEntityTransformComponent = (TransformComponent)componentAccessor.getComponent(ref, TRANSFORM_COMPONENT_TYPE);
