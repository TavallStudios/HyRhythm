...
public class SensorFlockLeader extends SensorBase {
   ...
   public boolean matches(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, double dt, @Nonnull Store<EntityStore> store) {
      if (!super.matches(ref, role, dt, store)) {
         ...
         FlockMembership membership = (FlockMembership)store.getComponent(ref, FlockMembership.getComponentType());
         if (membership == null) {
            this.positionProvider.clear();
            return false;
         } else {
            ...
            if (flockReference != null && flockReference.isValid()) {
               group = (EntityGroup)store.getComponent(flockReference, EntityGroup.getComponentType());
            }
