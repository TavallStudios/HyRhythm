...
public class StoredFlock {
   ...
   public void storeNPCs(@Nonnull List<Ref<EntityStore>> refs, @Nonnull Store<EntityStore> store) {
      ...
      for(int i = 0; i < refs.size(); ++i) {
         Ref<EntityStore> ref = (Ref)refs.get(i);
         if (ref.isValid()) {
      ...
      this.members = (Holder[])members.toArray((x$0) -> new Holder[x$0]);
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(StoredFlock.class, StoredFlock::new).append(new KeyedCodec("Members", new ArrayCodec(new StoredCodec(EntityStore.HOLDER_CODEC_KEY), (x$0) -> new Holder[x$0])), (flock, array) -> flock.members = array, (flock) -> flock.members).add()).build();
   }
}
