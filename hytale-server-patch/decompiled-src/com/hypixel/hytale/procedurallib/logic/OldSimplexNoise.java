...
public class OldSimplexNoise implements NoiseFunction {
   ...
   public double get(int seed, int offsetSeed, double x, double y, double z) {
      ...
      if (inSum <= 1.0) {
         ...
         double attn2 = 2.0 - dx3 * dx3 - dy2 * dy2 - dz2 * dz2;
         if (attn2 > 0.0) {
            attn2 *= attn2;
            value += attn2 * attn2 * extrapolate(offsetSeed, xsb + 1, ysb + 0, zsb + 1, dx3, dy2, dz2);
         }

         double dx1 = dx0 - 0.0 - 0.6666666666666666;
         double attn1 = 2.0 - dx1 * dx1 - dy3 * dy3 - dz2 * dz2;
         if (attn1 > 0.0) {
            attn1 *= attn1;
            value += attn1 * attn1 * extrapolate(offsetSeed, xsb + 0, ysb + 1, zsb + 1, dx1, dy3, dz2);
         }
         ...
         if (attn0 > 0.0) {
            attn0 *= attn0;
            value += attn0 * attn0 * extrapolate(offsetSeed, xsb + 1, ysb + 1, zsb + 1, dx0, dy0, dz0);
         }
      } else {
         double p1 = xins + yins;
         byte aPoint;
