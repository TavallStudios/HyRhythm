...
public class BranchNoiseJsonLoader<T extends SeedResource> extends AbstractCellJitterJsonLoader<T, BranchNoise> {
   ...
   protected CellType loadParentCellType() {
      return (CellType)this.loadEnum("ParentType", CellType::valueOf, BranchNoiseJsonLoader.Constant.DEFAULT_PARENT_CELL_TYPE);
   }

   protected CellType loadLineCellType() {
      return (CellType)this.loadEnum("LineType", CellType::valueOf, BranchNoiseJsonLoader.Constant.DEFAULT_LINE_CELL_TYPE);
   }
   ...
   protected PointEvaluator loadParentEvaluator() {
      ...
      DistanceCalculationMode distanceMode = (DistanceCalculationMode)this.loadEnum("ParentDistanceMode", DistanceCalculationMode::valueOf, BranchNoiseJsonLoader.Constant.DEFAULT_PARENT_DISTANCE_CAL_MODE);
      return PointEvaluator.of(distanceMode.getFunction(), (IDoubleCondition)null, (IDoubleRange)null, jitter);
   }
   ...
   protected DistanceNoise.Distance2Function loadParentDistance2Function() {
      return ((DistanceNoise.Distance2Mode)this.loadEnum("ParentDistanceType", DistanceNoise.Distance2Mode::valueOf, BranchNoiseJsonLoader.Constant.DEFAULT_PARENT_DISTANCE_TYPE)).getFunction();
   }

   protected NoiseFormulaProperty.NoiseFormula.Formula loadParentFormula() {
      return ((NoiseFormulaProperty.NoiseFormula)this.loadEnum("ParentFormula", NoiseFormulaProperty.NoiseFormula::valueOf, BranchNoiseJsonLoader.Constant.DEFAULT_PARENT_FORMULA)).getFormula();
   }
   ...
   protected PointEvaluator loadLineEvaluator(@Nonnull CellDistanceFunction parentFunction, @Nonnull CellPointFunction linePointFunction, double lineScale) {
      ...
      CellJitter jitter = CellJitter.of(jitterX, jitterY, 1.0);
      BranchEvaluator.Direction direction = (BranchEvaluator.Direction)this.loadEnum("LineDirection", BranchEvaluator.Direction::valueOf, BranchNoiseJsonLoader.Constant.DEFAULT_LINE_DIRECTION);
   ...
   protected <E extends Enum<E>> E loadEnum(String key, @Nonnull Function<String, E> valueOf, E def) {
      return (E)(!this.has(key) ? def : (Enum)valueOf.apply(this.get(key).getAsString()));
   }
   ...
   protected static CellPointFunction getCellPointFunction(@Nonnull CellType cellType) {
      ...
      switch (cellType) {
         case SQUARE -> var10000 = GridCellDistanceFunction.POINT_FUNCTION;
         case HEX -> var10000 = HexCellDistanceFunction.POINT_FUNCTION;
         default -> throw new MatchException((String)null, (Throwable)null);
      }
