...
public class SyncEventBusRegistry<KeyType, EventType extends IEvent<KeyType>> extends EventBusRegistry<KeyType, EventType, SyncEventConsumerMap<EventType>> {
   ...
   public SyncEventBusRegistry(HytaleLogger logger, Class<EventType> eventClass) {
      super(logger, eventClass, new SyncEventConsumerMap((SyncEventBusRegistry)null), new SyncEventConsumerMap((SyncEventBusRegistry)null));
   ...
   public EventRegistration<KeyType, EventType> register(short priority, @Nullable KeyType key, @Nonnull Consumer<EventType> consumer) {
      if (this.shutdown) {
         ...
         SyncEventConsumerMap<EventType> eventMap = (SyncEventConsumerMap)this.map.computeIfAbsent(k, (o) -> new SyncEventConsumerMap(this));
   ...
   private void unregister(@Nullable KeyType key, @Nonnull SyncEventConsumer<EventType> consumer) {
      if (this.shutdown) {
         ...
         KeyType k = (KeyType)(key != null ? key : NULL);
         SyncEventConsumerMap<EventType> eventMap = (SyncEventConsumerMap)this.map.get(k);
   ...
   private void unregisterGlobal(@Nonnull SyncEventConsumer<EventType> consumer) {
      if (this.shutdown) {
         throw new IllegalArgumentException("EventRegistry is shutdown!");
      } else if (!((SyncEventConsumerMap)this.global).remove(consumer)) {
         throw new IllegalArgumentException(String.valueOf(consumer));
      }
   }
   ...
   private void unregisterUnhandled(@Nonnull SyncEventConsumer<EventType> consumer) {
      if (this.shutdown) {
         throw new IllegalArgumentException("EventRegistry is shutdown!");
      } else if (!((SyncEventConsumerMap)this.unhandled).remove(consumer)) {
         throw new IllegalArgumentException(String.valueOf(consumer));
      }
   }
   ...
   public IEventDispatcher<EventType, EventType> dispatchFor(@Nullable KeyType key) {
      if (this.shutdown) {
         ...
         if (eventMap != null && !eventMap.isEmpty()) {
            ...
            return ((SyncEventConsumerMap)this.global).isEmpty() && ((SyncEventConsumerMap)this.unhandled).isEmpty() ? NO_OP : this.globalDispatcher;
         }
      }
   }
   ...
   private boolean dispatchEventMap(EventType event, @Nonnull SyncEventConsumerMap<EventType> eventMap, String s) {
      ...
      for(short priority : eventMap.getPriorities()) {
         ...
         if (consumers != null) {
            for(SyncEventConsumer<EventType> consumer : consumers) {
               try {
   ...
   protected static class SyncEventConsumer<EventType extends IEvent> extends EventBusRegistry.EventConsumer {
      ...
      public String toString() {
         String var10000 = String.valueOf(this.consumer);
         return "SyncEventConsumer{consumer=" + var10000 + ", timedConsumer=" + String.valueOf(this.timedConsumer) + "} " + super.toString();
      }
   }

   protected static class SyncEventConsumerMap<EventType extends IEvent> extends EventBusRegistry.EventConsumerMap<EventType, SyncEventConsumer<EventType>, EventType> {
