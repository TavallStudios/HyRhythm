...
public class ChangeModelPage extends InteractiveCustomUIPage<PageEventData> {
   ...
   private void buildModelList(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder) {
      ...
      if (!this.searchQuery.isEmpty()) {
         ...
         this.models = (List)var10001.sorted(Comparator.comparingInt(map::getInt).reversed()).limit(20L).collect(Collectors.toList());
      } else {
         this.models = (List)roleTemplateNames.stream().sorted().sorted(String::compareTo).collect(Collectors.toList());
      }

      int i = 0;

      for(int bound = this.models.size(); i < bound; ++i) {
         String id = (String)this.models.get(i);
      ...
      if (!this.models.isEmpty()) {
         if (!this.models.contains(this.selectedModel)) {
            this.selectModel(ref, store, (String)this.models.getFirst(), commandBuilder);
   ...
   private Model getModel(float scale) {
      ModelAsset modelAsset = (ModelAsset)ModelAsset.getAssetMap().getAsset(this.selectedModel);
   ...
   public static class PageEventData {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(PageEventData.class, PageEventData::new).append(new KeyedCodec("Model", Codec.STRING), (entry, s) -> entry.model = s, (entry) -> entry.model).add()).append(new KeyedCodec("Type", Codec.STRING), (entry, s) -> entry.type = s, (entry) -> entry.type).add()).append(new KeyedCodec("@SearchQuery", Codec.STRING), (entry, s) -> entry.searchQuery = s, (entry) -> entry.searchQuery).add()).append(new KeyedCodec("@Scale", Codec.FLOAT), (entry, s) -> entry.scale = s, (entry) -> entry.scale).add()).build();
      }
   }
}
