...
public class ReturnToHubButtonSystem extends RefSystem<EntityStore> {
   public void onEntityAdded(@Nonnull Ref<EntityStore> ref, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      if (!isInChildWorld.isEmpty()) {
         ...
         boolean disabled = !(Boolean)isInChildWorld.get();
   ...
   private Optional<Boolean> getCreativeHubWorldStatus(@Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Ref<EntityStore> ref) {
      CreativeHubEntityConfig hubEntityConfig = (CreativeHubEntityConfig)commandBuffer.getComponent(ref, CreativeHubEntityConfig.getComponentType());
