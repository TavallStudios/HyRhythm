...
public final class ReturnToHubButtonUI {
   ...
   public static void register() {
      AnchorActionModule.get().register("returnToHub", (AnchorActionModule.WorldThreadAnchorActionHandler)((playerRef, ref, store, data) -> executeReturnToHub(playerRef, ref, store)));
   }
   ...
   public static void clear(@Nonnull PlayerRef playerRef) {
      playerRef.getPacketHandler().writeNoCache(new UpdateAnchorUI("MapServerContent", true, (CustomUICommand[])null, (CustomUIEventBinding[])null));
   }
      ...
      if (parentWorld != null) {
         ...
         if (hubConfig != null && hubConfig.getStartupInstance() != null) {
            ...
            if (!world.equals(currentHub)) {
               ...
               InstancesPlugin.teleportPlayerToInstance(ref, store, hubInstance, (Transform)null);
            }
         }
      }
   }
   ...
   private static World findParentHubWorld(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref) {
      CreativeHubEntityConfig hubEntityConfig = (CreativeHubEntityConfig)store.getComponent(ref, CreativeHubEntityConfig.getComponentType());
