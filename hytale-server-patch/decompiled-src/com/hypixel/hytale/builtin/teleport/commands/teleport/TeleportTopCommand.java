...
public class TeleportTopCommand extends AbstractPlayerCommand {
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      ...
      if (worldChunk == null) {
         ...
         ((TeleportHistory)store.ensureAndGetComponent(ref, TeleportHistory.getComponentType())).append(world, position.clone(), headRotation.clone(), "Underground");
         store.addComponent(ref, Teleport.getComponentType(), Teleport.createForPlayer(new Vector3d(position.getX(), (double)(height + 2), position.getZ()), Vector3f.NaN));
