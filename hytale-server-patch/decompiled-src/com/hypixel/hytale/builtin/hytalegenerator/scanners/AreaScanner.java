...
public class AreaScanner extends Scanner {
      ...
      if (resultCap >= 0 && range >= 0) {
         ...
         for(int x = -range; x <= range; ++x) {
            for(int z = -range; z <= range; ++z) {
               if (scanShape != AreaScanner.ScanShape.CIRCLE || !(Calculator.distance((double)x, (double)z, 0.0, 0.0) > (double)range)) {
   ...
   public static enum ScanShape {
      ...
      @Nonnull
      public static final Codec<ScanShape> CODEC = new EnumCodec<ScanShape>(ScanShape.class, EnumCodec.EnumStyle.LEGACY);
   }
}
