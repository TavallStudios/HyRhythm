...
public class ColumnRandomScanner extends Scanner {
   ...
   public ColumnRandomScanner(int minY, int maxY, int resultsCap, int seed, @Nonnull Strategy strategy, boolean isRelativeToPosition, double baseHeight) {
      if (resultsCap < 0) {
   ...
   public List<Vector3i> scan(@Nonnull Scanner.Context context) {
      ...
      switch (this.strategy.ordinal()) {
         case 0 -> var10000 = this.scanDartThrow(context);
         case 1 -> var10000 = this.scanPickValid(context);
         ...
         if (validPositions.isEmpty()) {
            ...
            for(int i = 0; i < this.resultsCap; ++i) {
               ...
               if (!usedIndices.contains(pickedIndex)) {
                  ...
                  outPositions.add((Vector3i)validPositions.get(pickedIndex));
               }
            }
   ...
   private List<Vector3i> scanDartThrow(@Nonnull Scanner.Context context) {
      if (this.resultsCap == 0) {
         ...
         int scanMinY = this.isRelativeToPosition ? Math.max(context.position.y + this.minY, context.materialSpace.minY()) : Math.max(this.minY, context.materialSpace.minY());
