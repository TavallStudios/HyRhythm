...
public class FastNoiseLite {
   ...
   public float getNoise(double x, double y) {
      x *= (double)this.mFrequency;
      y *= (double)this.mFrequency;
      switch (this.mNoiseType.ordinal()) {
         case 0:
         case 1:
         ...
         default:
            ...
            switch (this.mFractalType.ordinal()) {
               case 1 -> var10000 = this.GenFractalFBm(x, y);
               case 2 -> var10000 = this.GenFractalRidged(x, y);
               case 3 -> var10000 = this.GenFractalPingPong(x, y);
               default -> var10000 = this.GenNoiseSingle(this.mSeed, x, y);
            }
   ...
   public float getNoise(double x, double y, double z) {
      ...
      float var10000;
      switch (this.mFractalType.ordinal()) {
         case 1 -> var10000 = this.GenFractalFBm(x, y, z);
         case 2 -> var10000 = this.GenFractalRidged(x, y, z);
         case 3 -> var10000 = this.GenFractalPingPong(x, y, z);
         default -> var10000 = this.GenNoiseSingle(this.mSeed, x, y, z);
      }

      return var10000;
   }

   public void DomainWarp(@Nonnull Vector2 coord) {
      switch (this.mFractalType.ordinal()) {
         case 4 -> this.DomainWarpFractalProgressive(coord);
         case 5 -> this.DomainWarpFractalIndependent(coord);
         default -> this.DomainWarpSingle(coord);
      }

   }

   public void DomainWarp(@Nonnull Vector3 coord) {
      switch (this.mFractalType.ordinal()) {
         case 4 -> this.DomainWarpFractalProgressive(coord);
         case 5 -> this.DomainWarpFractalIndependent(coord);
         default -> this.DomainWarpSingle(coord);
   ...
   private float GenNoiseSingle(int seed, double x, double y) {
      float var10000;
      switch (this.mNoiseType.ordinal()) {
         case 0 -> var10000 = this.SingleSimplex(seed, x, y);
         case 1 -> var10000 = this.SingleOpenSimplex2S(seed, x, y);
         case 2 -> var10000 = this.SingleCellular(seed, x, y);
         case 3 -> var10000 = this.SinglePerlin(seed, x, y);
         case 4 -> var10000 = this.SingleValueCubic(seed, x, y);
         case 5 -> var10000 = this.SingleValue(seed, x, y);
   ...
   private float GenNoiseSingle(int seed, double x, double y, double z) {
      float var10000;
      switch (this.mNoiseType.ordinal()) {
         case 0 -> var10000 = this.SingleOpenSimplex2(seed, x, y, z);
         case 1 -> var10000 = this.SingleOpenSimplex2S(seed, x, y, z);
         case 2 -> var10000 = this.SingleCellular(seed, x, y, z);
         case 3 -> var10000 = this.SinglePerlin(seed, x, y, z);
         case 4 -> var10000 = this.SingleValueCubic(seed, x, y, z);
         case 5 -> var10000 = this.SingleValue(seed, x, y, z);
         default -> throw new MatchException((String)null, (Throwable)null);
      }
   ...
   private void UpdateWarpTransformType3D() {
      switch (this.mRotationType3D.ordinal()) {
         ...
         case 2:
            ...
            break;
         default:
            switch (this.mDomainWarpType.ordinal()) {
               case 0:
               case 1:
                  this.mWarpTransformType3D = FastNoiseLite.TransformType3D.DefaultOpenSimplex2;
                  break;
   ...
   private float GenFractalFBm(double x, double y) {
      ...
      for(int i = 0; i < this.mOctaves; ++i) {
         ...
         x *= (double)this.mLacunarity;
         y *= (double)this.mLacunarity;
         amp *= this.mGain;
      }
   ...
   private float SingleOpenSimplex2(int seed, double x, double y, double z) {
      ...
      int j = fastRound(y);
      int k = fastRound(z);
      float x0 = (float)(x - (double)i);
      float y0 = (float)(y - (double)j);
      float z0 = (float)(z - (double)k);
   ...
   public Vector3d pointFor(int seed, double jitter, double x, double y, double z) {
      ...
      int idx = hash & 1020;
      double vecX = (double)randVecs3D[idx] * jitter + (double)xr;
      double vecY = (double)randVecs3D[idx | 1] * jitter + (double)yr;
      double vecZ = (double)randVecs3D[idx | 2] * jitter + (double)zr;
   ...
   public float SingleCellular(int seed, double x, double y) {
      ...
      float var10000;
      switch (this.mCellularReturnType.ordinal()) {
         case 0 -> var10000 = (float)closestHash * 4.656613E-10F;
         case 1 -> var10000 = distance0 - 1.0F;
         case 2 -> var10000 = distance1 - 1.0F;
         case 3 -> var10000 = (distance1 + distance0) * 0.5F - 1.0F;
         case 4 -> var10000 = distance1 - distance0 - 1.0F;
         case 5 -> var10000 = distance1 * distance0 * 0.5F - 1.0F;
         case 6 -> var10000 = distance0 / distance1 - 1.0F;
         default -> throw new MatchException((String)null, (Throwable)null);
      }
   ...
   private float SinglePerlin(int seed, double x, double y) {
      int x0 = FastFloor(x);
      int y0 = FastFloor(y);
      float xd0 = (float)(x - (double)x0);
      float yd0 = (float)(y - (double)y0);
      float xd1 = xd0 - 1.0F;
      float yd1 = yd0 - 1.0F;
   ...
   private float SingleValueCubic(int seed, double x, double y) {
      int x1 = FastFloor(x);
      int y1 = FastFloor(y);
      float xs = (float)(x - (double)x1);
      float ys = (float)(y - (double)y1);
      x1 *= 501125321;
      y1 *= 1136930381;
      int x0 = x1 - 501125321;
   ...
   private float SingleValue(int seed, double x, double y) {
      int x0 = FastFloor(x);
      int y0 = FastFloor(y);
      float xs = InterpHermite((float)(x - (double)x0));
      float ys = InterpHermite((float)(y - (double)y0));
      x0 *= 501125321;
      y0 *= 1136930381;
      int x1 = x0 + 501125321;
      int y1 = y0 + 1136930381;
   ...
   private float SingleValue(int seed, double x, double y, double z) {
      ...
      float xs = InterpHermite((float)(x - (double)x0));
      float ys = InterpHermite((float)(y - (double)y0));
      float zs = InterpHermite((float)(z - (double)z0));
      x0 *= 501125321;
      y0 *= 1136930381;
      z0 *= 1720413743;
      int x1 = x0 + 501125321;
   ...
   private void DoSingleDomainWarp(int seed, float amp, float freq, double x, double y, @Nonnull Vector2 coord) {
      switch (this.mDomainWarpType.ordinal()) {
         case 0 -> this.SingleDomainWarpSimplexGradient(seed, amp * 38.283688F, freq, x, y, coord, false);
         case 1 -> this.SingleDomainWarpSimplexGradient(seed, amp * 16.0F, freq, x, y, coord, true);
         case 2 -> this.SingleDomainWarpBasicGrid(seed, amp, freq, x, y, coord);
      }

   }

   private void DoSingleDomainWarp(int seed, float amp, float freq, double x, double y, double z, @Nonnull Vector3 coord) {
      switch (this.mDomainWarpType.ordinal()) {
         case 0 -> this.SingleDomainWarpOpenSimplex2Gradient(seed, amp * 32.694283F, freq, x, y, z, coord, false);
         case 1 -> this.SingleDomainWarpOpenSimplex2Gradient(seed, amp * 7.716049F, freq, x, y, z, coord, true);
         case 2 -> this.SingleDomainWarpBasicGrid(seed, amp, freq, x, y, z, coord);
      }

   }

   private void DomainWarpSingle(@Nonnull Vector2 coord) {
      int seed = this.mSeed;
   ...
   public void DomainWarpSingle(@Nonnull Vector3 coord) {
      int seed = this.mSeed;
      float amp = this.mDomainWarpAmp * this.mFractalBounding;
      float freq = this.mDomainWarpFreq;
      double xs = coord.x;
   ...
   public void DomainWarpFractalProgressive(@Nonnull Vector2 coord) {
      int seed = this.mSeed;
      float amp = this.mDomainWarpAmp * this.mFractalBounding;
      float freq = this.mDomainWarpFreq;
   ...
   public void DomainWarpFractalProgressive(@Nonnull Vector3 coord) {
      int seed = this.mSeed;
      float amp = this.mDomainWarpAmp * this.mFractalBounding;
      float freq = this.mDomainWarpFreq;
   ...
   private void DomainWarpFractalIndependent(@Nonnull Vector2 coord) {
      double xs = coord.x;
      double ys = coord.y;
      switch (this.mDomainWarpType.ordinal()) {
         case 0:
   ...
   private void DomainWarpFractalIndependent(@Nonnull Vector3 coord) {
      double xs = coord.x;
      double ys = coord.y;
      double zs = coord.z;
      switch (this.mWarpTransformType3D.ordinal()) {
   ...
   private void SingleDomainWarpBasicGrid(int seed, float warpAmp, float frequency, double x, double y, @Nonnull Vector2 coord) {
      double xf = x * (double)frequency;
      double yf = y * (double)frequency;
      int x0 = FastFloor(xf);
      int y0 = FastFloor(yf);
      float xs = InterpHermite((float)(xf - (double)x0));
      float ys = InterpHermite((float)(yf - (double)y0));
      x0 *= 501125321;
      y0 *= 1136930381;
      int x1 = x0 + 501125321;
      int y1 = y0 + 1136930381;
      ...
      coord.x += (double)(Lerp(lx0x, lx1x, ys) * warpAmp);
      coord.y += (double)(Lerp(ly0x, ly1x, ys) * warpAmp);
   }

   private void SingleDomainWarpBasicGrid(int seed, float warpAmp, float frequency, double x, double y, double z, @Nonnull Vector3 coord) {
      double xf = x * (double)frequency;
      double yf = y * (double)frequency;
      ...
      float xs = InterpHermite((float)(xf - (double)x0));
      float ys = InterpHermite((float)(yf - (double)y0));
      float zs = InterpHermite((float)(zf - (double)z0));
      x0 *= 501125321;
      y0 *= 1136930381;
      z0 *= 1720413743;
      int x1 = x0 + 501125321;
      ...
      coord.x += (double)(Lerp(lx0y, Lerp(lx0x, lx1x, ys), zs) * warpAmp);
      coord.y += (double)(Lerp(ly0y, Lerp(ly0x, ly1x, ys), zs) * warpAmp);
      coord.z += (double)(Lerp(lz0y, Lerp(lz0x, lz1x, ys), zs) * warpAmp);
   }

   private void SingleDomainWarpSimplexGradient(int seed, float warpAmp, float frequency, double x, double y, @Nonnull Vector2 coord, boolean outGradOnly) {
      float SQRT3 = 1.7320508F;
      float G2 = 0.21132487F;
      x *= (double)frequency;
      y *= (double)frequency;
      ...
      coord.x += (double)(vx * warpAmp);
      coord.y += (double)(vy * warpAmp);
   }

   private void SingleDomainWarpOpenSimplex2Gradient(int seed, float warpAmp, float frequency, double x, double y, double z, @Nonnull Vector3 coord, boolean outGradOnly) {
      x *= (double)frequency;
      y *= (double)frequency;
      z *= (double)frequency;
      int i = fastRound(x);
   ...
   public static enum CellularReturnType {
      ...
      @Nonnull
      public static final Codec<CellularReturnType> CODEC = new EnumCodec<CellularReturnType>(CellularReturnType.class);
   }
