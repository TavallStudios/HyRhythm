...
public class BiomeFrontierAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, BiomeFrontierAsset>> {
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(BiomeFrontierAsset.class, BiomeFrontierAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("InterpolationRadius", Codec.INTEGER, true), (t, k) -> t.interpolationRadius = k, (t) -> t.interpolationRadius).add()).build();
   }
}
