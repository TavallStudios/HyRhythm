...
public class BasicWorldStructureAsset extends WorldStructureAsset {
   ...
   public WorldStructure build(@Nonnull WorldStructureAsset.Argument argument) {
      ...
      if (defaultBiomeAsset == null) {
         ...
         Registry<Biome> biomeRegistry = new Registry<Biome>();
         int defaultBiomeId = biomeRegistry.getIdOrRegister(defaultBiome);
         SimpleNoiseCarta<Integer> carta = new SimpleNoiseCarta<Integer>(noise, defaultBiomeId);
         ...
         for(BiomeRangeAsset asset : this.biomeRangeAssets) {
            ...
            if (biomeAsset == null) {
               ...
               if (biomeAssetToBiomeType.containsKey(biomeAsset)) {
                  biome = (Biome)biomeAssetToBiomeType.get(biomeAsset);
               } else {
         ...
         PositionProvider spawnPositions = this.spawnPositionsAsset.build(new PositionProviderAsset.Argument(argument.parentSeed, referenceBundle, argument.workerId));
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BasicWorldStructureAsset.class, BasicWorldStructureAsset::new, WorldStructureAsset.ABSTRACT_CODEC).append(new KeyedCodec("Biomes", new ArrayCodec(BiomeRangeAsset.CODEC, (x$0) -> new BiomeRangeAsset[x$0]), true), (asset, value) -> asset.biomeRangeAssets = value, (asset) -> asset.biomeRangeAssets).add()).append(new KeyedCodec("Density", DensityAsset.CODEC, true), (asset, value) -> asset.densityAsset = value, (asset) -> asset.densityAsset).add()).append(new KeyedCodec("DefaultBiome", new ContainedAssetCodec(BiomeAsset.class, BiomeAsset.CODEC), true), (asset, value) -> asset.defaultBiomeId = value, (asset) -> asset.defaultBiomeId).addValidatorLate(() -> BiomeAsset.VALIDATOR_CACHE.getValidator().late()).add()).append(new KeyedCodec("DefaultTransitionDistance", Codec.INTEGER, true), (asset, value) -> asset.biomeTransitionDistance = value, (asset) -> asset.biomeTransitionDistance).addValidator(Validators.greaterThan(0)).add()).append(new KeyedCodec("MaxBiomeEdgeDistance", Codec.INTEGER, true), (asset, value) -> asset.maxBiomeEdgeDistance = value, (asset) -> asset.maxBiomeEdgeDistance).addValidator(Validators.greaterThanOrEqual(0)).add()).append(new KeyedCodec("Framework", new ArrayCodec(FrameworkAsset.CODEC, (x$0) -> new FrameworkAsset[x$0]), false), (asset, value) -> asset.frameworkAssets = value, (asset) -> asset.frameworkAssets).add()).append(new KeyedCodec("SpawnPositions", PositionProviderAsset.CODEC, false), (asset, value) -> asset.spawnPositionsAsset = value, (asset) -> asset.spawnPositionsAsset).add()).build();
   }
}
