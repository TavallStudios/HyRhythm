...
public class RangeIntAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, RangeIntAsset>> {
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(RangeIntAsset.class, RangeIntAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("MinInclusive", Codec.INTEGER, true), (t, value) -> t.minInclusive = value, (t) -> t.minInclusive).add()).append(new KeyedCodec("MaxExclusive", Codec.INTEGER, true), (t, value) -> t.maxExclusive = value, (t) -> t.maxExclusive).add()).build();
   }
}
