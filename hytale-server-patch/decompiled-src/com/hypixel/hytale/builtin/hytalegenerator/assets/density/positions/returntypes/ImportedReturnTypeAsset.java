...
public class ImportedReturnTypeAsset extends ReturnTypeAsset {
   @Nonnull
   public static final BuilderCodec<ImportedReturnTypeAsset> CODEC;
   ...
   public ReturnType build(@Nonnull SeedBox parentSeed, @Nonnull ReferenceBundle referenceBundle, @Nonnull WorkerIndexer.Id workerId) {
      ...
      if (asset == null) {
         ...
         return new ReturnType() {
            public double get(double distance0, double distance1, @Nonnull Vector3d samplePoint, @Nullable Vector3d closestPoint0, Vector3d closestPoint1, @Nullable Density.Context context) {
               return 0.0;
            }
         };
      } else {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(ImportedReturnTypeAsset.class, ImportedReturnTypeAsset::new, ReturnTypeAsset.ABSTRACT_CODEC).append(new KeyedCodec("Name", Codec.STRING, true), (t, k) -> t.importedAssetName = k, (k) -> k.importedAssetName).add()).build();
   }
}
