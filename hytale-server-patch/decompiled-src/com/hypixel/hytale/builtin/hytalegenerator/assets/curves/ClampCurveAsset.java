...
public class ClampCurveAsset extends CurveAsset {
   ...
   public Double2DoubleFunction build() {
      ...
      if (this.curveAsset == null) {
         ...
         return (in) -> {
            ...
            value = Calculator.clamp(this.wallA, value, this.wallB);
            return value;
         };
      }
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ClampCurveAsset.class, ClampCurveAsset::new, CurveAsset.ABSTRACT_CODEC).append(new KeyedCodec("Curve", CurveAsset.CODEC, false), (t, k) -> t.curveAsset = k, (k) -> k.curveAsset).add()).append(new KeyedCodec("WallA", Codec.DOUBLE, false), (t, k) -> t.wallA = k, (k) -> k.wallA).add()).append(new KeyedCodec("WallB", Codec.DOUBLE, false), (t, k) -> t.wallB = k, (k) -> k.wallB).add()).build();
   }
}
