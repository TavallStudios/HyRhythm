...
public abstract class MaterialProviderAsset implements Cleanable, JsonAssetWithMap<String, DefaultAssetMap<String, MaterialProviderAsset>> {
   ...
   public abstract MaterialProvider<Material> build(@Nonnull Argument var1);
   ...
   public static MaterialProviderAsset getExportedAsset(@Nonnull String name) {
      return (MaterialProviderAsset)exportedNodes.get(name);
   }
   ...
   public static Argument argumentFrom(@Nonnull DensityAsset.Argument argument, @Nonnull MaterialCache materialCache) {
      return new Argument(argument.parentSeed, materialCache, argument.referenceBundle, argument.workerId);
   }
   ...
   static {
      CODEC = new AssetCodecMapCodec<String, MaterialProviderAsset>(Codec.STRING, (t, k) -> t.id = k, (t) -> t.id, (t, data) -> t.data = data, (t) -> t.data);
      exportedNodes = new ConcurrentHashMap();
      CHILD_ASSET_CODEC = new ContainedAssetCodec(MaterialProviderAsset.class, CODEC);
      CHILD_ASSET_CODEC_ARRAY = new ArrayCodec<String[]>(CHILD_ASSET_CODEC, (x$0) -> new String[x$0]);
      ABSTRACT_CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.abstractBuilder(MaterialProviderAsset.class).append(new KeyedCodec("Skip", Codec.BOOLEAN, false), (t, k) -> t.skip = k, (t) -> t.skip).add()).append(new KeyedCodec("ExportAs", Codec.STRING, false), (t, k) -> t.exportName = k, (t) -> t.exportName).add()).afterDecode((asset) -> {
