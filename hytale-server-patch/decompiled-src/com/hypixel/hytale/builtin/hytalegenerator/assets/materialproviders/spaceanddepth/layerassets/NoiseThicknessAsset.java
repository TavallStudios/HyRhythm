...
public class NoiseThicknessAsset extends LayerAsset {
   ...
   public SpaceAndDepthMaterialProvider.Layer<Material> build(@Nonnull MaterialProviderAsset.Argument argument) {
      ...
      return new NoiseThickness<Material>(functionTree, materialProvider);
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(NoiseThicknessAsset.class, NoiseThicknessAsset::new, LayerAsset.ABSTRACT_CODEC).append(new KeyedCodec("ThicknessFunctionXZ", DensityAsset.CODEC, true), (asset, k) -> asset.densityAsset = k, (asset) -> asset.densityAsset).add()).append(new KeyedCodec("Material", MaterialProviderAsset.CODEC, true), (t, k) -> t.materialProviderAsset = k, (k) -> k.materialProviderAsset).add()).build();
   }
}
