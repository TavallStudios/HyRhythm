...
public class BlockMaskAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, BlockMaskAsset>>, Cleanable {
   @Nonnull
   private static final Map<String, Exported> exportedNodes = new HashMap();
   @Nonnull
   ...
   public BlockMask build(@Nonnull MaterialCache materialCache) {
      if (this.importName != null && !this.importName.isEmpty()) {
         Exported importedAssetEntry = (Exported)exportedNodes.get(this.importName);
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(BlockMaskAsset.class, BlockMaskAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("DontPlace", MaterialSetAsset.CODEC, false), (t, k) -> t.dontPlaceMaterialSetAsset = k, (t) -> t.dontPlaceMaterialSetAsset).add()).append(new KeyedCodec("DontReplace", MaterialSetAsset.CODEC, false), (t, k) -> t.dontReplaceMaterialSetAsset = k, (t) -> t.dontReplaceMaterialSetAsset).add()).append(new KeyedCodec("Advanced", new ArrayCodec(BlockMaskEntryAsset.CODEC, (x$0) -> new BlockMaskEntryAsset[x$0]), false), (t, k) -> t.blockMaskEntries = k, (t) -> t.blockMaskEntries).add()).append(new KeyedCodec("ExportAs", Codec.STRING, false), (t, k) -> t.exportName = k, (t) -> t.exportName).add()).append(new KeyedCodec("Import", Codec.STRING, false), (t, k) -> t.importName = k, (t) -> t.importName).add()).afterDecode((asset) -> {
