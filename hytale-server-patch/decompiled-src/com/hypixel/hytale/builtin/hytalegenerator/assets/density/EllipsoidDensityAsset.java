...
public class EllipsoidDensityAsset extends DensityAsset {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(EllipsoidDensityAsset.class, EllipsoidDensityAsset::new, DensityAsset.ABSTRACT_CODEC).append(new KeyedCodec("Curve", CurveAsset.CODEC, true), (t, k) -> t.densityCurveAsset = k, (k) -> k.densityCurveAsset).add()).append(new KeyedCodec("Scale", Vector3d.CODEC, false), (t, k) -> t.scaleVector = k, (k) -> k.scaleVector).addValidator((LegacyValidator)((v, r) -> {
         if (v.x == 0.0 || v.y == 0.0 || v.z == 0.0) {
            r.fail("scale vector contains 0.0");
         }

      })).add()).append(new KeyedCodec("NewYAxis", Vector3d.CODEC, false), (t, k) -> {
         if (k.length() != 0.0) {
            t.newYAxis = k;
         }

      }, (k) -> k.newYAxis).add()).append(new KeyedCodec("Spin", Codec.DOUBLE, false), (t, k) -> t.spinAngle = k, (k) -> k.spinAngle).add()).build();
   }
}
