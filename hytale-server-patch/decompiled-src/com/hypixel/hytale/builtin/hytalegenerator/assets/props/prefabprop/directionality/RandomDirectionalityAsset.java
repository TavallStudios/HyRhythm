...
public class RandomDirectionalityAsset extends DirectionalityAsset {
   ...
   public Directionality build(@Nonnull DirectionalityAsset.Argument argument) {
      return new RandomDirectionality(this.patternAsset.build(PatternAsset.argumentFrom(argument)), (Integer)argument.parentSeed.child(this.seed).createSupplier().get());
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(RandomDirectionalityAsset.class, RandomDirectionalityAsset::new, DirectionalityAsset.ABSTRACT_CODEC).append(new KeyedCodec("Seed", Codec.STRING, true), (asset, v) -> asset.seed = v, (asset) -> asset.seed).add()).append(new KeyedCodec("Pattern", PatternAsset.CODEC, true), (asset, v) -> asset.patternAsset = v, (asset) -> asset.patternAsset).add()).build();
   }
}
