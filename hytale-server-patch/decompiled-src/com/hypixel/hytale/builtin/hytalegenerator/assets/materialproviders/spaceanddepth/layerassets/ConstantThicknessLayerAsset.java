...
public class ConstantThicknessLayerAsset extends LayerAsset {
   ...
   public SpaceAndDepthMaterialProvider.Layer<Material> build(@Nonnull MaterialProviderAsset.Argument argument) {
      return new ConstantThicknessLayer<Material>(this.thickness, this.materialProviderAsset.build(argument));
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ConstantThicknessLayerAsset.class, ConstantThicknessLayerAsset::new, LayerAsset.ABSTRACT_CODEC).append(new KeyedCodec("Thickness", Codec.INTEGER, true), (t, k) -> t.thickness = k, (k) -> k.thickness).addValidator(Validators.greaterThanOrEqual(0)).add()).append(new KeyedCodec("Material", MaterialProviderAsset.CODEC, true), (t, k) -> t.materialProviderAsset = k, (k) -> k.materialProviderAsset).add()).build();
   }
}
