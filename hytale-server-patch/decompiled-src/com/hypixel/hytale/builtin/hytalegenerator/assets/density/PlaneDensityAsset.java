...
public class PlaneDensityAsset extends DensityAsset {
   ...
   public Density build(@Nonnull DensityAsset.Argument argument) {
      return (Density)(!this.isSkipped() && this.distanceCurveAsset != null ? new PlaneDensity(this.distanceCurveAsset.build(), this.planeNormal, this.isAnchored) : new ConstantValueDensity(0.0));
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(PlaneDensityAsset.class, PlaneDensityAsset::new, DensityAsset.ABSTRACT_CODEC).append(new KeyedCodec("Curve", CurveAsset.CODEC, true), (t, k) -> t.distanceCurveAsset = k, (k) -> k.distanceCurveAsset).add()).append(new KeyedCodec("IsAnchored", Codec.BOOLEAN, false), (t, k) -> t.isAnchored = k, (k) -> k.isAnchored).add()).append(new KeyedCodec("PlaneNormal", Vector3d.CODEC, false), (t, k) -> t.planeNormal = k, (k) -> k.planeNormal).addValidator((LegacyValidator)((v, r) -> {
         if (v.length() == 0.0) {
