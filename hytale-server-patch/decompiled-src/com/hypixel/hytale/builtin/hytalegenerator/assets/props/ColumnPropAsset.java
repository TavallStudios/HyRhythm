...
public class ColumnPropAsset extends PropAsset {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ColumnPropAsset.class, ColumnPropAsset::new, PropAsset.ABSTRACT_CODEC).append(new KeyedCodec("ColumnBlocks", new ArrayCodec(ColumnPropAsset.ColumnBlock.CODEC, (x$0) -> new ColumnBlock[x$0]), true), (asset, v) -> asset.columnBlocks = v, (asset) -> asset.columnBlocks).add()).append(new KeyedCodec("BlockMask", BlockMaskAsset.CODEC, false), (asset, v) -> asset.blockMaskAsset = v, (asset) -> asset.blockMaskAsset).add()).append(new KeyedCodec("Directionality", DirectionalityAsset.CODEC, true), (asset, v) -> asset.directionalityAsset = v, (asset) -> asset.directionalityAsset).add()).append(new KeyedCodec("Scanner", ScannerAsset.CODEC, true), (asset, v) -> asset.scannerAsset = v, (asset) -> asset.scannerAsset).add()).build();
   }

   public static class ColumnBlock implements JsonAssetWithMap<String, DefaultAssetMap<String, ColumnBlock>> {
      @Nonnull
      ...
      static {
         CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(ColumnBlock.class, ColumnBlock::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("Y", Codec.INTEGER, true), (t, y) -> t.y = y, (t) -> t.y).add()).append(new KeyedCodec("Material", MaterialAsset.CODEC, true), (asset, value) -> asset.materialAsset = value, (asset) -> asset.materialAsset).add()).build();
      }
   }
}
