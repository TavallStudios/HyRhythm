...
public class WallPatternAsset extends PatternAsset {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(WallPatternAsset.class, WallPatternAsset::new, PatternAsset.ABSTRACT_CODEC).append(new KeyedCodec("Wall", PatternAsset.CODEC, true), (t, k) -> t.wall = k, (k) -> k.wall).add()).append(new KeyedCodec("Origin", PatternAsset.CODEC, true), (t, k) -> t.origin = k, (k) -> k.origin).add()).append(new KeyedCodec("RequireAllDirections", Codec.BOOLEAN, false), (t, k) -> t.matchAll = k, (k) -> k.matchAll).add()).append(new KeyedCodec("Directions", new ArrayCodec(WallPattern.WallDirection.CODEC, (x$0) -> new WallPattern.WallDirection[x$0]), true), (t, k) -> t.directions = k, (k) -> k.directions).add()).build();
   }
}
