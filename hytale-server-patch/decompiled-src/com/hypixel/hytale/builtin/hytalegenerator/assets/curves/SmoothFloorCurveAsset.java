...
public class SmoothFloorCurveAsset extends CurveAsset {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SmoothFloorCurveAsset.class, SmoothFloorCurveAsset::new, CurveAsset.ABSTRACT_CODEC).append(new KeyedCodec("Curve", CurveAsset.CODEC, true), (t, k) -> t.curveAsset = k, (k) -> k.curveAsset).add()).append(new KeyedCodec("Range", Codec.DOUBLE, true), (t, k) -> t.range = k, (k) -> k.range).addValidator(Validators.greaterThanOrEqual(0.0)).add()).append(new KeyedCodec("Floor", Codec.DOUBLE, true), (t, k) -> t.limit = k, (k) -> k.limit).add()).build();
   }
}
