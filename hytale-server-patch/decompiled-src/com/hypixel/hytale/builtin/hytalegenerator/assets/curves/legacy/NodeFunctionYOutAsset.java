...
public class NodeFunctionYOutAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, NodeFunctionYOutAsset>>, Cleanable {
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(NodeFunctionYOutAsset.class, NodeFunctionYOutAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("Points", new ArrayCodec(PointYOutAsset.CODEC, (x$0) -> new PointYOutAsset[x$0]), true), (t, k) -> t.nodes = k, (t) -> t.nodes).addValidator((LegacyValidator)((v, r) -> {
