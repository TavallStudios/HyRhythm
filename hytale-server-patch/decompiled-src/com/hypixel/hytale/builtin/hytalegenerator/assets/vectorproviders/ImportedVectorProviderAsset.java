...
public class ImportedVectorProviderAsset extends VectorProviderAsset {
   ...
   public VectorProvider build(@Nonnull VectorProviderAsset.Argument argument) {
      if (this.isSkipped()) {
         ...
         if (exported == null) {
            ...
            VectorProvider builtInstance = (VectorProvider)exported.threadInstances.get(argument.workerId);
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(ImportedVectorProviderAsset.class, ImportedVectorProviderAsset::new, VectorProviderAsset.ABSTRACT_CODEC).append(new KeyedCodec("Name", Codec.STRING, true), (t, k) -> t.importedNodeName = k, (k) -> k.importedNodeName).add()).build();
   }
}
