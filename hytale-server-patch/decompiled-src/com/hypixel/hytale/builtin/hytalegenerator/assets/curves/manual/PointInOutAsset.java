...
public class PointInOutAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, PointInOutAsset>> {
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(PointInOutAsset.class, PointInOutAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("In", Codec.DOUBLE, true), (t, y) -> t.y = y, (t) -> t.y).add()).append(new KeyedCodec("Out", Codec.DOUBLE, true), (t, out) -> t.out = out, (t) -> t.out).add()).build();
   }
}
