...
public class MaterialSetAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, MaterialSetAsset>>, Cleanable {
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(MaterialSetAsset.class, MaterialSetAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("Inclusive", Codec.BOOLEAN, false), (t, k) -> t.inclusive = k, (t) -> t.inclusive).add()).append(new KeyedCodec("Materials", new ArrayCodec(MaterialAsset.CODEC, (x$0) -> new MaterialAsset[x$0]), true), (asset, value) -> asset.materialAssets = value, (asset) -> asset.materialAssets).add()).build();
   }
}
