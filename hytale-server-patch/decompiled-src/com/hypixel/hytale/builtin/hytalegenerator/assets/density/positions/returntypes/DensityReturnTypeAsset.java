...
public class DensityReturnTypeAsset extends ReturnTypeAsset {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(DensityReturnTypeAsset.class, DensityReturnTypeAsset::new, ReturnTypeAsset.ABSTRACT_CODEC).append(new KeyedCodec("ChoiceDensity", DensityAsset.CODEC, true), (t, k) -> t.choiceDensityAsset = k, (t) -> t.choiceDensityAsset).add()).append(new KeyedCodec("Delimiters", new ArrayCodec(DensityReturnTypeAsset.DelimiterAsset.CODEC, (x$0) -> new DelimiterAsset[x$0]), true), (t, k) -> t.delimiterAssets = k, (t) -> t.delimiterAssets).add()).append(new KeyedCodec("DefaultValue", Codec.DOUBLE, false), (t, k) -> t.defaultValue = k, (t) -> t.defaultValue).add()).build();
   }
   ...
   public static class DelimiterAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, FieldFunctionMaterialProviderAsset.DelimiterAsset>> {
      ...
      static {
         CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(DelimiterAsset.class, DelimiterAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("From", Codec.DOUBLE, true), (t, y) -> t.from = y, (t) -> t.from).add()).append(new KeyedCodec("To", Codec.DOUBLE, true), (t, out) -> t.to = out, (t) -> t.to).add()).append(new KeyedCodec("Density", DensityAsset.CODEC, true), (t, out) -> t.densityAsset = out, (t) -> t.densityAsset).add()).build();
      }
   }
}
