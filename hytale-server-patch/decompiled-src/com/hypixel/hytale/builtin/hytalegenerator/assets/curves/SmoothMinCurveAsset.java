...
public class SmoothMinCurveAsset extends CurveAsset {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SmoothMinCurveAsset.class, SmoothMinCurveAsset::new, CurveAsset.ABSTRACT_CODEC).append(new KeyedCodec("CurveA", CurveAsset.CODEC, true), (t, k) -> t.curveAAsset = k, (k) -> k.curveAAsset).add()).append(new KeyedCodec("CurveB", CurveAsset.CODEC, true), (t, k) -> t.curveBAsset = k, (k) -> k.curveBAsset).add()).append(new KeyedCodec("Range", Codec.DOUBLE, true), (t, k) -> t.range = k, (k) -> k.range).addValidator(Validators.greaterThanOrEqual(0.0)).add()).build();
   }
}
