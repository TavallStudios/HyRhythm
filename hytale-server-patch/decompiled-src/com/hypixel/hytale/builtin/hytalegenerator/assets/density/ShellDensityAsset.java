...
public class ShellDensityAsset extends DensityAsset {
   ...
   public Density build(@Nonnull DensityAsset.Argument argument) {
      return (Density)(!this.isSkipped() && this.angleCurveAsset != null && this.distanceCurveAsset != null ? new ShellDensity(this.angleCurveAsset.build(), this.distanceCurveAsset.build(), this.axis, this.isMirrored) : new ConstantValueDensity(0.0));
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ShellDensityAsset.class, ShellDensityAsset::new, DensityAsset.ABSTRACT_CODEC).append(new KeyedCodec("Axis", Vector3d.CODEC, true), (t, k) -> t.axis = k, (k) -> k.axis).add()).append(new KeyedCodec("Mirror", Codec.BOOLEAN, false), (t, k) -> t.isMirrored = k, (k) -> k.isMirrored).add()).append(new KeyedCodec("AngleCurve", CurveAsset.CODEC, true), (t, k) -> t.angleCurveAsset = k, (k) -> k.angleCurveAsset).add()).append(new KeyedCodec("DistanceCurve", CurveAsset.CODEC, true), (t, k) -> t.distanceCurveAsset = k, (k) -> k.distanceCurveAsset).add()).build();
   }
}
