...
public class PointYOutAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, PointYOutAsset>> {
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(PointYOutAsset.class, PointYOutAsset::new, Codec.STRING, (asset, id) -> asset.id = id, (config) -> config.id, (config, data) -> config.data = data, (config) -> config.data).append(new KeyedCodec("Y", Codec.DOUBLE, true), (t, y) -> t.y = y, (t) -> t.y).add()).append(new KeyedCodec("Out", Codec.DOUBLE, true), (t, out) -> t.out = out, (t) -> t.out).add()).build();
   }
}
