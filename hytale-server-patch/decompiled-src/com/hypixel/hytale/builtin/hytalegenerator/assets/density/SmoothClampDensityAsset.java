...
public class SmoothClampDensityAsset extends DensityAsset {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SmoothClampDensityAsset.class, SmoothClampDensityAsset::new, DensityAsset.ABSTRACT_CODEC).append(new KeyedCodec("WallA", Codec.DOUBLE, true), (t, k) -> t.wallA = k, (k) -> k.wallA).add()).append(new KeyedCodec("WallB", Codec.DOUBLE, true), (t, k) -> t.wallB = k, (k) -> k.wallB).add()).append(new KeyedCodec("Range", Codec.DOUBLE, true), (t, k) -> t.range = k, (k) -> k.range).addValidator(Validators.greaterThanOrEqual(0.0)).add()).build();
   }
}
