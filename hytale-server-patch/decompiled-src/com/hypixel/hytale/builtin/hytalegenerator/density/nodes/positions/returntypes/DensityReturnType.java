...
public class DensityReturnType extends ReturnType {
   ...
   public DensityReturnType(@Nonnull Density choiceDensity, @Nonnull Map<Range, Density> densityDelimiters, boolean calculateDistanceFromWall, double defaultValue) {
      ...
      for(Map.Entry<Range, Density> entry : densityDelimiters.entrySet()) {
         this.delimiters[i][0] = (double)((Range)entry.getKey()).getMin();
         this.delimiters[i][1] = (double)((Range)entry.getKey()).getMax();
         this.sampleDensities[i] = (Density)entry.getValue();
         ++i;
      }
   ...
   public double get(double distance0, double distance1, @Nonnull Vector3d samplePoint, @Nullable Vector3d closestPoint0, @Nullable Vector3d closestPoint1, @Nullable Density.Context context) {
      ...
      if (closestPoint0 != null && this.calculateDistanceFromWall) {
         ...
         if (closestPoint1 == null) {
            ...
            double var10000 = distance1 / this.maxDistance;
   ...
   private static class Entry {
      ...
      public Entry(int size) {
         if (size < 0) {
            ...
            this.map = new Object2DoubleAVLTreeMap(new Vector3dComparator());
      ...
      public void put(Vector3d k, double v) {
         if (this.keyHistory.size() == this.size) {
            Vector3d oldKey = (Vector3d)this.keyHistory.removeLast();
