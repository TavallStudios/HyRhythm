         ...
         for(itPosition.z = min.z; itPosition.z <= max.z; ++itPosition.z) {
            for(itPosition.y = min.y; itPosition.y <= max.y; ++itPosition.y) {
               if (densitySpace.isInsideSpace(itPosition.x, itPosition.y, itPosition.z)) {
                  childContext.position.x = (double)itPosition.x;
                  childContext.position.y = (double)itPosition.y;
                  childContext.position.z = (double)itPosition.z;
      ...
      for(itPosition.x = min.x; itPosition.x <= max.x; ++itPosition.x) {
         for(itPosition.z = min.z; itPosition.z <= max.z; ++itPosition.z) {
            ...
            for(itPosition.y = top; itPosition.y >= bottom; --itPosition.y) {
               ...
               boolean density = (Boolean)densitySpace.getContent(itPosition.x, itPosition.y, itPosition.z);
               if (itPosition.y == top) {
                  if (density) {
                     depthIntoFloor[i] = 1;
                  } else {
                  ...
                  depthIntoFloor[i] = 0;
                  if ((Boolean)densitySpace.getContent(itPosition.x, itPosition.y + 1, itPosition.z)) {
                     spaceAboveFloor[i] = 0;
                  } else {
            ...
            for(itPosition.y = bottom; itPosition.y < top; ++itPosition.y) {
               ...
               if (itPosition.y == bottom) {
                  ...
                  spaceBelowCeiling[i] = 2147483647;
               } else if (density) {
