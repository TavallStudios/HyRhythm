...
public class PrefabProp extends Prop {
   ...
   private PrefabBuffer pickPrefab(@Nonnull Random rand) {
      ...
      return (PrefabBuffer)list.get(randomIndex);
   }
   ...
   private void place(@Nonnull RotatedPosition position, @Nonnull VoxelSpace<Material> materialSpace, @Nonnull EntityContainer entityBuffer, @Nonnull WorkerIndexer.Id id) {
         ...
         if (this.moldingDirection == MoldingDirection.DOWN || this.moldingDirection == MoldingDirection.UP) {
            ...
            for(pointer.x = moldingOffsets.minX(); pointer.x < moldingOffsets.maxX(); ++pointer.x) {
               for(pointer.z = moldingOffsets.minZ(); pointer.z < moldingOffsets.maxZ(); ++pointer.z) {
                  ...
                  Integer offset = scanResult.isEmpty() ? null : ((Vector3i)scanResult.getFirst()).y - position.y;
      ...
      try {
         ...
         prefabAccess.forEach(IPrefabBuffer.iterateAllColumns(), (x, yx, z, blockId, holder, support, rotation, filler, call, fluidId, fluidLevel) -> {
            ...
            if (materialSpace.isInsideSpace(worldX, worldY, worldZ)) {
               ...
               if (this.materialMask.canPlace(materialHash)) {
                  if (this.moldingDirection == MoldingDirection.DOWN || this.moldingDirection == MoldingDirection.UP) {
                     Integer offset = null;
                     if (moldingOffsets.isInsideSpace(worldX, 0, worldZ)) {
                        offset = moldingOffsets.getContent(worldX, 0, worldZ);
                     }
            ...
            if (this.loadEntities) {
               if (entityWrappers != null) {
                  for(int i = 0; i < entityWrappers.length; ++i) {
                     TransformComponent transformComp = (TransformComponent)entityWrappers[i].getComponent(TransformComponent.getComponentType());
                     if (transformComp != null) {
                        ...
                        if (entityBuffer.isInsideBuffer((int)entityWorldPosition.x, (int)entityWorldPosition.y, (int)entityWorldPosition.z)) {
                           ...
                           transformComp = (TransformComponent)entityClone.getComponent(TransformComponent.getComponentType());
                           if (transformComp != null) {
      ...
      for(int i = 0; i < this.childProps.size(); ++i) {
         PrefabProp prop = (PrefabProp)this.childProps.get(i);
         RotatedPosition childPosition = ((RotatedPosition)this.childPositions.get(i)).getRelativeTo(position);
