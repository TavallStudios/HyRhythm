...
public class ViewportCommand extends AbstractPlayerCommand {
   ...
   public ViewportCommand(@Nonnull AssetManager assetManager) {
      ...
      this.radiusArg = this.withOptionalArg("radius", "server.commands.viewport.radius.desc", ArgTypes.INTEGER);
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      ...
      if ((Boolean)context.get(this.deleteFlag)) {
         playerRef.sendMessage(Message.translation("server.commands.viewport.removed"));
      } else {
         Integer radius = (Integer)context.get(this.radiusArg) << 5;
         Bounds3i viewportBounds_voxelGrid;
         if (radius != null) {
            Vector3d playerPosition_voxelGrid = ((TransformComponent)store.getComponent(ref, TransformComponent.getComponentType())).getPosition();
            Vector3i min_voxelGrid = playerPosition_voxelGrid.clone().subtract((double)radius).toVector3i();
            Vector3i max_voxelGrid = playerPosition_voxelGrid.clone().add((double)radius).toVector3i().add(Vector3i.ALL_ONES);
