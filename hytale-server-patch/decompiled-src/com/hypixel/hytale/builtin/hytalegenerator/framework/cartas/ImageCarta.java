...
public class ImageCarta<R> extends TriCarta<R> {
   ...
   public R apply(int x, int y, int z, @Nonnull WorkerIndexer.Id tHreadId) {
      ...
      int sampleX = Calculator.toNearestInt((Double)this.functionX.apply((double)x, (double)y, (double)z) * (double)this.width);
      sampleX = sampleX < 0 ? 0 : Math.min(sampleX, this.width - 1);
      int sampleY = Calculator.toNearestInt((Double)this.functionY.apply((double)x, (double)y, (double)z) * (double)this.height);
      sampleY = sampleY < 0 ? 0 : Math.min(sampleY, this.height - 1);
      int rgb = this.rgbArray[sampleX + sampleY * this.width];
      return (R)(!this.rgbToTerrainMap.containsKey(rgb) ? null : this.rgbToTerrainMap.get(rgb));
   }
   ...
   public String toString() {
      String var10000 = Arrays.toString(this.rgbArray);
      return "ImageCarta{rgbArray=" + var10000 + ", width=" + this.width + ", height=" + this.height + ", functionX=" + String.valueOf(this.functionX) + ", functionY=" + String.valueOf(this.functionY) + ", rgbToTerrainMap=" + String.valueOf(this.rgbToTerrainMap) + ", allPossibleValues=" + String.valueOf(this.allPossibleValues) + "}";
   }
   ...
   public static class Builder<R> {
      ...
      public ImageCarta<R> build() {
         if (this.bufferedImageCheck && this.noiseCheck) {
            ImageCarta<R> instance = new ImageCarta<R>();
