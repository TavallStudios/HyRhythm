...
public class TieredList<E> {
   ...
   public void add(@Nonnull E element, int tier) {
      if (element == null) {
         ...
         ((ArrayList)this.elements.get(tier)).add(element);
      }
   }
   ...
   public E peek() {
      for(int tier = 0; tier < this.tiers; ++tier) {
         List<E> tierElements = (List)this.elements.get(tier);
         if (!tierElements.isEmpty()) {
            return (E)tierElements.getFirst();
         }
      }
   ...
   public E remove() {
      for(int tier = 0; tier < this.tiers; ++tier) {
         List<E> tierElements = (List)this.elements.get(tier);
         if (!tierElements.isEmpty()) {
            return (E)tierElements.removeFirst();
         }
      }
   ...
   public int size(int tier) {
      return !this.tierExists(tier) ? 0 : ((ArrayList)this.elements.get(tier)).size();
   }
   ...
   public TieredList<E> forEach(int tier, @Nonnull Consumer<? super E> consumer) {
      if (!this.tierExists(tier)) {
         ...
         ((ArrayList)this.elements.get(tier)).forEach(consumer);
         return this;
      }
   }
   ...
   public TieredList<E> removeEach(int tier, @Nonnull Consumer<? super E> consumer) {
      if (!this.tierExists(tier)) {
         ...
         for(E e : (List)this.elements.get(tier)) {
            consumer.accept(e);
         }
   ...
   public Iterator<E> iterator(int tier) {
      if (!this.tierExists(tier)) {
         ...
         return ((ArrayList)this.elements.get(tier)).iterator();
      }
   }
   ...
   public String toString() {
      String var10000 = String.valueOf(this.elements);
      return "TieredList{elements=" + var10000 + ", tiers=" + this.tiers + ", sortedTierList=" + String.valueOf(this.sortedTierList) + "}";
   }
}
