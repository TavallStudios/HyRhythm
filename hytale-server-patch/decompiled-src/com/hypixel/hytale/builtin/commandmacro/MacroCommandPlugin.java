...
public class MacroCommandPlugin extends JavaPlugin {
   ...
   protected void setup() {
      instance = this;
      AssetRegistry.register(((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)HytaleAssetStore.builder(MacroCommandBuilder.class, new DefaultAssetMap()).setPath("MacroCommands")).setKeyFunction(MacroCommandBuilder::getId)).setCodec(MacroCommandBuilder.CODEC)).build());
      this.getEventRegistry().register((Class)LoadedAssetsEvent.class, MacroCommandBuilder.class, this::loadCommandMacroAsset);
   ...
   public void loadCommandMacroAsset(@Nonnull LoadedAssetsEvent<String, MacroCommandBuilder, DefaultAssetMap<String, MacroCommandBuilder>> event) {
      for(MacroCommandBuilder value : event.getLoadedAssets().values()) {
         if (this.macroCommandRegistrations.containsKey(value.getName())) {
            ((CommandRegistration)this.macroCommandRegistrations.get(value.getName())).unregister();
         }
