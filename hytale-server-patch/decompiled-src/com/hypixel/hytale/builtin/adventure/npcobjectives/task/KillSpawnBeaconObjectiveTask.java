...
public class KillSpawnBeaconObjectiveTask extends KillObjectiveTask {
   ...
   protected TransactionRecord[] setup0(@Nonnull Objective objective, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      ...
      return (TransactionRecord[])ArrayUtil.append(transactionRecords, transaction);
   }
   ...
   private TransactionRecord[] setupSpawnBeacons(@Nonnull Objective objective, @Nonnull World world, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (position == null) {
         return TransactionRecord.appendFailedTransaction((TransactionRecord[])null, new WorldTransactionRecord(), "No valid position found for the objective.");
      } else {
         ...
         for(int i = 0; i < spawnBeaconConfigs.length; ++i) {
            ...
            if (spawnPosition == null) {
               ...
               UUIDComponent spawnBeaconUuidComponent = (UUIDComponent)componentAccessor.getComponent((Ref)spawnBeaconPair.first(), UUIDComponent.getComponentType());
