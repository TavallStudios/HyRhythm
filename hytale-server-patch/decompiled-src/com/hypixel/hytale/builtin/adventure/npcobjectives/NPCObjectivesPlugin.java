...
public class NPCObjectivesPlugin extends JavaPlugin {
   ...
   public static boolean hasTask(@Nonnull UUID playerUUID, @Nonnull UUID npcId, @Nonnull String taskId) {
      ...
      if (entityObjectives == null) {
         return false;
      } else {
         return entityObjectives.get(taskId) != null;
      }
   }
   ...
   public static String updateTaskCompletion(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull UUID npcId, @Nonnull String taskId) {
      ...
      if (uuidComponent == null) {
         ...
         if (entityObjectiveUUIDs == null) {
            ...
            Set<UUID> objectiveUUIDsForTaskId = (Set)entityObjectiveUUIDs.get(taskId);
                     ...
                     if (currentTasks != null) {
                        for(ObjectiveTask task : currentTasks) {
                           if (task instanceof UseEntityObjectiveTask) {
                              UseEntityObjectiveTask useEntityTask = (UseEntityObjectiveTask)task;
   ...
   public static void startObjective(@Nonnull Ref<EntityStore> playerRef, @Nonnull String taskId, @Nonnull Store<EntityStore> store) {
      ...
      if (uuidComponent != null) {
         ...
         ObjectivePlugin.get().startObjective(taskId, Set.of(uuidComponent.getUuid()), world.getWorldConfig().getUuid(), (UUID)null, store);
      }
   }
