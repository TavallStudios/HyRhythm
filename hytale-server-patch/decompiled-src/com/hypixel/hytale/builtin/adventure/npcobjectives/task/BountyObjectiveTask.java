...
public class BountyObjectiveTask extends ObjectiveTask implements KillTask {
      ...
      if (this.serializedTransactionRecords != null) {
         ...
         if (objectivePosition == null) {
            ...
            if (spawnPosition == null) {
               ...
               Pair<Ref<EntityStore>, INonPlayerCharacter> npcPair = NPCPlugin.get().spawnNPC(store, npcId, (String)null, spawnPosition.toVector3d(), Vector3f.ZERO);
               if (npcPair == null) {
                  return null;
               } else {
                  Ref<EntityStore> npcReference = (Ref)npcPair.first();
                  UUIDComponent npcUuidComponent = (UUIDComponent)store.getComponent(npcReference, UUIDComponent.getComponentType());
                  if (npcUuidComponent == null) {
                     ...
                     ObjectiveTaskMarker marker = new ObjectiveTaskMarker(getBountyMarkerIDFromUUID(npcUuid), new Transform(spawnPosition), "Home.png", Message.translation("server.objectives.bounty.marker"));
                     this.addMarker(marker);
                     KillTaskTransaction transaction = new KillTaskTransaction(this, objective, store);
                     ((KillTrackerResource)store.getResource(KillTrackerResource.getResourceType())).watch(transaction);
   ...
   public static String getBountyMarkerIDFromUUID(@Nonnull UUID uuid) {
      return "Bounty_" + String.valueOf(uuid);
   }
   ...
   public String toString() {
      ...
      return "BountyObjectiveTask{completed=" + var10000 + ", entityUuid=" + String.valueOf(this.entityUuid) + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BountyObjectiveTask.class, BountyObjectiveTask::new, ObjectiveTask.BASE_CODEC).append(new KeyedCodec("Completed", Codec.BOOLEAN), (bountyObjectiveTask, aBoolean) -> bountyObjectiveTask.completed = aBoolean, (bountyObjectiveTask) -> bountyObjectiveTask.completed).add()).append(new KeyedCodec("EntityUUID", Codec.UUID_BINARY), (bountyObjectiveTask, uuid) -> bountyObjectiveTask.entityUuid = uuid, (bountyObjectiveTask) -> bountyObjectiveTask.entityUuid).add()).build();
   }
}
