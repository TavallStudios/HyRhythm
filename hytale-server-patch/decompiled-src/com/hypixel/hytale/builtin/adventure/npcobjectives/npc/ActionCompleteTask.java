...
public class ActionCompleteTask extends ActionPlayAnimation {
   ...
   public boolean execute(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, InfoProvider sensorInfo, double dt, @Nonnull Store<EntityStore> store) {
      UUIDComponent parentUuidComponent = (UUIDComponent)store.getComponent(ref, UUIDComponent.getComponentType());
      if (parentUuidComponent == null) {
         ...
         if (targetPlayerReference == null) {
            ...
            PlayerRef targetPlayerRefComponent = (PlayerRef)store.getComponent(targetPlayerReference, PlayerRef.getComponentType());
            if (targetPlayerRefComponent == null) {
               ...
               if (activeTasks != null) {
                  for(int i = 0; i < activeTasks.size(); ++i) {
                     animation = NPCObjectivesPlugin.updateTaskCompletion(store, targetPlayerReference, targetPlayerRefComponent, parentUuidComponent.getUuid(), (String)activeTasks.get(i));
                  }
               }
