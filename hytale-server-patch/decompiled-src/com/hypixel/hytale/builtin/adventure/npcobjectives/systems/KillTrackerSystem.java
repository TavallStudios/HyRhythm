...
public class KillTrackerSystem extends DeathSystems.OnDeathSystem {
   ...
   public KillTrackerSystem(@Nonnull ComponentType<EntityStore, NPCEntity> npcEntityComponentType, @Nonnull ResourceType<EntityStore, KillTrackerResource> killTrackerResourceType) {
   ...
   public void onComponentAdded(@Nonnull Ref<EntityStore> ref, @Nonnull DeathComponent component, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      NPCEntity npcEntityComponent = (NPCEntity)store.getComponent(ref, this.npcEntityComponentType);

      assert npcEntityComponent != null;

      KillTrackerResource killTrackerResource = (KillTrackerResource)store.getResource(this.killTrackerResourceType);
      ...
      if (deathInfo != null) {
         ...
         for(int i = size - 1; i >= 0; --i) {
            KillTaskTransaction entry = (KillTaskTransaction)killTasks.get(i);
