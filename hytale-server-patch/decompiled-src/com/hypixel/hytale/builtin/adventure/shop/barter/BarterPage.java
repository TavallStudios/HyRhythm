...
public class BarterPage extends InteractiveCustomUIPage<BarterEventData> {
   ...
   public BarterPage(@Nonnull PlayerRef playerRef, @Nonnull String shopId) {
      ...
      this.shopAsset = (BarterShopAsset)BarterShopAsset.getAssetMap().getAsset(shopId);
   }
   ...
   public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, @Nonnull Store<EntityStore> store) {
      if (this.shopAsset != null) {
         ...
         Player playerComponent = playerEntityRef != null ? (Player)store.getComponent(playerEntityRef, Player.getComponentType()) : null;
         ...
         for(int i = 0; i < trades.length; ++i) {
            ...
            if (trade.getInput().length > 0) {
               ...
               commandBuilder.set(selector + " #HaveNeedLabel.Text", Message.translation("server.barter.customUI.barterPage.quantityStock").param("count", playerHas));
   ...
   public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull BarterEventData data) {
      if (this.shopAsset != null) {
                        ...
                        if (playerComponent != null) {
                           ...
                           if (maxQuantity > 0) {
                              ...
                              if (remainder != null && !remainder.isEmpty()) {
                                 ...
                                 if (addedQty > 0) {
                                    playerComponent.notifyPickupItem(playerEntityRef, outputStack.withQuantity(addedQty), (Vector3d)null, store);
                                 }

                                 ItemUtils.dropItem(playerEntityRef, remainder, store);
                              } else {
                                 playerComponent.notifyPickupItem(playerEntityRef, outputStack, (Vector3d)null, store);
                              }
         ...
         if (interval == null) {
            ...
            if (nextRefresh == null) {
               ...
               if (!remaining.isNegative() && !remaining.isZero()) {
                  long currentDayNumber = gameTime.getEpochSecond() / (long)WorldTimeResource.SECONDS_PER_DAY;
                  long refreshDayNumber = nextRefresh.getEpochSecond() / (long)WorldTimeResource.SECONDS_PER_DAY;
                  ...
                  if (daysUntilRefresh <= 0L) {
                     ...
                     return daysUntilRefresh == 1L ? Message.translation("server.barter.customUI.barterPage.restocksTomorrow").param("restockTime", timeString) : Message.translation("server.barter.customUI.barterPage.restocksInDays").param("days", (int)daysUntilRefresh);
                  }
               } else {
                  return null;
               }
   ...
   public static class BarterEventData {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(BarterEventData.class, BarterEventData::new).append(new KeyedCodec("TradeIndex", Codec.STRING), (data, s) -> data.tradeIndex = Integer.parseInt(s), (data) -> String.valueOf(data.tradeIndex)).add()).append(new KeyedCodec("Quantity", Codec.STRING), (data, s) -> data.quantity = Integer.parseInt(s), (data) -> String.valueOf(data.quantity)).add()).append(new KeyedCodec("ShiftHeld", Codec.BOOLEAN), (data, b) -> {
            if (b != null) {
               data.shiftHeld = b;
            }

         }, (data) -> data.shiftHeld).add()).build();
      }
   }
}
