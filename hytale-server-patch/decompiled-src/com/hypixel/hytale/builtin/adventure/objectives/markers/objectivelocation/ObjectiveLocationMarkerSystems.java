...
public class ObjectiveLocationMarkerSystems {
   public static class InitSystem extends RefSystem<EntityStore> {
      ...
      public InitSystem(@Nonnull ComponentType<EntityStore, ObjectiveLocationMarker> objectiveLocationMarkerComponent, @Nonnull ComponentType<EntityStore, ModelComponent> modelComponentType, @Nonnull ComponentType<EntityStore, TransformComponent> transformComponentType, @Nonnull ComponentType<EntityStore, PrefabCopyableComponent> prefabCopyableComponentType) {
         ...
         this.query = Query.<EntityStore>and(objectiveLocationMarkerComponent, modelComponentType, transformComponentType);
      }
      ...
      public void onEntityAdded(@Nonnull Ref<EntityStore> ref, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ObjectiveLocationMarker objectiveLocationMarkerComponent = (ObjectiveLocationMarker)store.getComponent(ref, this.objectiveLocationMarkerComponent);

         assert objectiveLocationMarkerComponent != null;

         ObjectiveLocationMarkerAsset markerAsset = (ObjectiveLocationMarkerAsset)ObjectiveLocationMarkerAsset.getAssetMap().getAsset(objectiveLocationMarkerComponent.objectiveLocationMarkerId);
         if (markerAsset == null) {
   ...
   public static class EnsureNetworkSendableSystem extends HolderSystem<EntityStore> {
      ...
      public EnsureNetworkSendableSystem(@Nonnull ComponentType<EntityStore, ObjectiveLocationMarker> objectiveLocationMarkerComponentType, @Nonnull ComponentType<EntityStore, NetworkId> networkIdComponentType) {
         ...
         this.query = Query.<EntityStore>and(objectiveLocationMarkerComponentType, Query.not(networkIdComponentType));
      }
   ...
   public static class TickingSystem extends EntityTickingSystem<EntityStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ObjectiveLocationMarker objectiveLocationMarkerComponent = (ObjectiveLocationMarker)archetypeChunk.getComponent(index, this.objectiveLocationMarkerComponentType);
         ...
         if (activeObjective == null) {
            ...
            ObjectListIterator var16 = playerRefs.iterator();

            while(var16.hasNext()) {
               Ref<EntityStore> playerRef = (Ref)var16.next();
               PlayerRef playerRefComponent = (PlayerRef)commandBuffer.getComponent(playerRef, this.playerRefComponentType);
               if (playerRefComponent != null) {
                  UUIDComponent playerUuidComponent = (UUIDComponent)commandBuffer.getComponent(playerRef, this.uuidComponentType);
                  if (playerUuidComponent != null) {
                     TransformComponent playerTransformComponent = (TransformComponent)commandBuffer.getComponent(playerRef, this.transformComponentType);
                     if (playerTransformComponent != null) {
                        WeatherTracker playerWeatherTrackerComponent = (WeatherTracker)commandBuffer.getComponent(playerRef, this.weatherTrackerComponentType);
                        if (playerWeatherTrackerComponent != null && isPlayerInSpecificEnvironment(objectiveLocationMarkerComponent, playerWeatherTrackerComponent, playerTransformComponent, commandBuffer)) {
