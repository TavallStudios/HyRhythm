...
public class LookBlocksBelowProvider extends WorldLocationProvider {
   ...
   public boolean equals(@Nullable Object o) {
      if (this == o) {
         ...
         if (this.count != that.count) {
            ...
            return this.maxRange != that.maxRange ? false : Arrays.equals(this.blockTags, that.blockTags);
         }
      } else {
         return false;
      }
   ...
   public String toString() {
      ...
      return "LookBlocksBelowProvider{blockTags=" + var10000 + ", count=" + this.count + ", minRange=" + this.minRange + ", maxRange=" + this.maxRange + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(LookBlocksBelowProvider.class, LookBlocksBelowProvider::new, BASE_CODEC).append(new KeyedCodec("BlockTags", Codec.STRING_ARRAY), (lookBlocksBelowCondition, strings) -> lookBlocksBelowCondition.blockTags = strings, (lookBlocksBelowCondition) -> lookBlocksBelowCondition.blockTags).addValidator(Validators.nonEmptyArray()).addValidator(Validators.uniqueInArray()).add()).append(new KeyedCodec("Count", Codec.INTEGER), (lookBlocksBelowCondition, integer) -> lookBlocksBelowCondition.count = integer, (lookBlocksBelowCondition) -> lookBlocksBelowCondition.count).addValidator(Validators.greaterThan(0)).add()).append(new KeyedCodec("MinRange", Codec.INTEGER), (lookBlocksBelowCondition, integer) -> lookBlocksBelowCondition.minRange = integer, (lookBlocksBelowCondition) -> lookBlocksBelowCondition.minRange).addValidator(Validators.greaterThanOrEqual(0)).add()).append(new KeyedCodec("MaxRange", Codec.INTEGER), (lookBlocksBelowCondition, integer) -> lookBlocksBelowCondition.maxRange = integer, (lookBlocksBelowCondition) -> lookBlocksBelowCondition.maxRange).addValidator(Validators.greaterThanOrEqual(1)).add()).afterDecode((lookBlocksBelowCondition) -> {
