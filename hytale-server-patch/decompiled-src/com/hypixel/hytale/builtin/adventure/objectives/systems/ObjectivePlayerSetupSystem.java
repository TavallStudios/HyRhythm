...
public class ObjectivePlayerSetupSystem extends RefSystem<EntityStore> {
   ...
   public ObjectivePlayerSetupSystem(@Nonnull ComponentType<EntityStore, ObjectiveHistoryComponent> objectiveHistoryComponentType, @Nonnull ComponentType<EntityStore, Player> playerComponentType) {
      ...
      this.query = Query.<EntityStore>and(playerComponentType, this.uuidComponentType);
   }
   ...
   public void onEntityAdded(@Nonnull Ref<EntityStore> ref, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      UUIDComponent uuidComponent = (UUIDComponent)store.getComponent(ref, this.uuidComponentType);
      ...
      if (perWorldData.isFirstSpawn()) {
         ...
         if (starterObjectiveLinePerWorld != null) {
            String objectiveLineId = (String)starterObjectiveLinePerWorld.get(worldName);
            if (objectiveLineId != null) {
               objectiveModule.startObjectiveLine(store, objectiveLineId, Set.of(playerUuid), world.getWorldConfig().getUuid(), (UUID)null);
            }
         }
      }
