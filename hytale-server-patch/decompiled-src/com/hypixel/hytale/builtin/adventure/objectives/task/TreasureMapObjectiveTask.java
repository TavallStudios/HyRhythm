...
public class TreasureMapObjectiveTask extends ObjectiveTask {
   ...
   protected TransactionRecord[] setup0(@Nonnull Objective objective, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      this.eventRegistry.register((Class)TreasureChestOpeningEvent.class, world.getName(), (event) -> this.onTreasureChestOpeningEvent(objective, event));
   ...
   @Nonnull
   private TransactionRecord spawnChest(@Nonnull Objective objective, @Nonnull World world, @Nonnull TreasureMapObjectiveTaskAsset.ChestConfig chestConfig, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
   ...
   private TreasureChestState spawnChestBlock(@Nonnull World world, @Nonnull Vector3i conditionPosition, String chestBlockTypeKey, @Nonnull SpawnTreasureChestTransactionRecord transactionRecord) {
      ...
      if (worldChunk == null) {
         ...
         if (!(blockState instanceof ItemContainerState)) {
            ...
            TreasureChestState treasureChestState = (TreasureChestState)BlockStateModule.get().createBlockState(TreasureChestState.class, worldChunk, conditionPosition.clone(), blockState.getBlockType());
            if (treasureChestState == null) {
   ...
   @Nullable
   private Vector3i calculateChestSpawnPosition(@Nonnull TreasureMapObjectiveTaskAsset.ChestConfig chestConfig, @Nonnull Objective objective, @Nonnull World world, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      int currentTry = 0;
      ...
      for(conditionPosition = null; currentTry < 500 && conditionPosition == null; ++currentTry) {
         ...
         position.add((double)(radius * TrigMathUtil.cos(angle)), 0.0, (double)(radius * TrigMathUtil.sin(angle)));
         position.y = (double)world.getChunk(ChunkUtil.indexChunkFromBlock(position.x, position.z)).getHeight(MathUtil.floor(position.x), MathUtil.floor(position.z));
   ...
   public String toString() {
      int var10000 = this.currentCompletion;
      return "TreasureMapObjectiveTask{currentCompletion=" + var10000 + ", chestCount=" + this.chestCount + ", chestUUIDs=" + String.valueOf(this.chestUUIDs) + "} " + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(TreasureMapObjectiveTask.class, TreasureMapObjectiveTask::new, BASE_CODEC).append(new KeyedCodec("CurrentCompletion", Codec.INTEGER), (treasureMapObjectiveTask, integer) -> treasureMapObjectiveTask.currentCompletion = integer, (treasureMapObjectiveTask) -> treasureMapObjectiveTask.currentCompletion).add()).append(new KeyedCodec("ChestCount", Codec.INTEGER), (treasureMapObjectiveTask, integer) -> treasureMapObjectiveTask.chestCount = integer, (treasureMapObjectiveTask) -> treasureMapObjectiveTask.chestCount).add()).append(new KeyedCodec("ChestUUIDs", new ArrayCodec(Codec.UUID_BINARY, (x$0) -> new UUID[x$0])), (treasureMapObjectiveTask, uuids) -> {
         treasureMapObjectiveTask.chestUUIDs.clear();
         Collections.addAll(treasureMapObjectiveTask.chestUUIDs, uuids);
      }, (treasureMapObjectiveTask) -> (UUID[])treasureMapObjectiveTask.chestUUIDs.toArray((x$0) -> new UUID[x$0])).add()).build();
   }
}
