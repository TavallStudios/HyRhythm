...
public class ReachLocationTask extends ObjectiveTask {
   ...
   protected TransactionRecord[] setup0(@Nonnull Objective objective, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      if (!this.markers.isEmpty()) {
         ...
         store.forEachChunk(REACH_LOCATION_MARKER_COMPONENT_TYPE, (BiConsumer)((archetypeChunk, componentStoreCommandBuffer) -> {
            for(int index = 0; index < archetypeChunk.size(); ++index) {
               ReachLocationMarker reachLocationMarkerComponent = (ReachLocationMarker)archetypeChunk.getComponent(index, REACH_LOCATION_MARKER_COMPONENT_TYPE);
         ...
         if (!reachLocationMarkerEntities.isEmpty()) {
            ...
            Ref<EntityStore> closestMarker = (Ref)reachLocationMarkerEntities.getFirst();
            ...
            if (closestLocationName != null) {
               ObjectiveTaskMarker marker = new ObjectiveTaskMarker(this.getMarkerId(objective), new Transform(closestPosition), MARKER_ICON, Message.raw(closestLocationName));
      ...
      if (!this.markerLoaded) {
         ...
         if (markerId.equals(this.getAsset().getTargetLocationId())) {
            ...
            if (locationName != null) {
               ObjectiveTaskMarker marker = new ObjectiveTaskMarker(this.getMarkerId(objective), new Transform(position), MARKER_ICON, Message.raw(locationName));
               this.addMarker(marker);
               this.markerLoaded = true;
            }
         }
