...
public class TreasureChestState extends ItemContainerState implements BreakValidatedBlockState {
   ...
   public void onOpen(@Nonnull Ref<EntityStore> ref, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      IEventDispatcher<TreasureChestOpeningEvent, TreasureChestOpeningEvent> dispatcher = HytaleServer.get().getEventBus().dispatchFor(TreasureChestOpeningEvent.class, world.getName());
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(TreasureChestState.class, TreasureChestState::new, BlockState.BASE_CODEC).append(new KeyedCodec("ObjectiveUUID", Codec.UUID_BINARY), (treasureChestState, uuid) -> treasureChestState.objectiveUUID = uuid, (treasureChestState) -> treasureChestState.objectiveUUID).add()).append(new KeyedCodec("ChestUUID", Codec.UUID_BINARY), (treasureChestState, uuid) -> treasureChestState.chestUUID = uuid, (treasureChestState) -> treasureChestState.chestUUID).add()).append(new KeyedCodec("Opened", Codec.BOOLEAN), (treasureChestState, aBoolean) -> treasureChestState.opened = aBoolean, (treasureChestState) -> treasureChestState.opened).add()).build();
   }
}
