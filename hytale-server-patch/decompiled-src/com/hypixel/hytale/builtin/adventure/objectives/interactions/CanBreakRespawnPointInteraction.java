...
public class CanBreakRespawnPointInteraction extends SimpleBlockInteraction {
   ...
   protected void interactWithBlock(@Nonnull World world, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull InteractionType type, @Nonnull InteractionContext context, @Nullable ItemStack itemInHand, @Nonnull Vector3i targetBlock, @Nonnull CooldownHandler cooldownHandler) {
      ...
      if (chunk == null) {
         ...
         BlockComponentChunk blockComp = (BlockComponentChunk)chunkStore.getStore().getComponent(chunk, BlockComponentChunk.getComponentType());
         if (blockComp == null) {
            context.getState().state = InteractionState.Failed;
         } else {
            ...
            if (blockEntity == null) {
               ...
               RespawnBlock respawnState = (RespawnBlock)chunkStore.getStore().getComponent(blockEntity, RespawnBlock.getComponentType());
               if (respawnState == null) {
                  context.getState().state = InteractionState.Finished;
               } else {
                  UUIDComponent uuidComponent = (UUIDComponent)commandBuffer.getComponent(context.getOwningEntity(), UUIDComponent.getComponentType());
