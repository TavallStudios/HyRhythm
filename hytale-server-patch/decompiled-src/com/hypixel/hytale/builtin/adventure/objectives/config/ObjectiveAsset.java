...
public class ObjectiveAsset implements JsonAssetWithMap<String, DefaultAssetMap<String, ObjectiveAsset>> {
   ...
   public String toString() {
      ...
      return "ObjectiveAsset{id='" + var10000 + "', category='" + this.category + "', taskSets=" + Arrays.toString(this.taskSets) + ", completionHandlers=" + Arrays.toString(this.completionHandlers) + ", objectiveTitleKey='" + this.objectiveTitleKey + "', objectiveDescriptionKey='" + this.objectiveDescriptionKey + "', removeOnItemDrop=" + this.removeOnItemDrop + "}";
   }

   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(ObjectiveAsset.class, ObjectiveAsset::new, Codec.STRING, (objective, s) -> objective.id = s, (objective) -> objective.id, (objective, data) -> objective.extraData = data, (objective) -> objective.extraData).appendInherited(new KeyedCodec("Category", Codec.STRING), (objectiveAsset, s) -> objectiveAsset.category = s, (objectiveAsset) -> objectiveAsset.category, (objectiveAsset, parent) -> objectiveAsset.category = parent.category).add()).appendInherited(new KeyedCodec("TaskSets", new ArrayCodec(TaskSet.CODEC, (x$0) -> new TaskSet[x$0])), (objective, tasks) -> objective.taskSets = tasks, (objective) -> objective.taskSets, (objective, parent) -> objective.taskSets = parent.taskSets).addValidator(Validators.nonEmptyArray()).add()).appendInherited(new KeyedCodec("Completions", new ArrayCodec(ObjectiveCompletionAsset.CODEC, (x$0) -> new ObjectiveCompletionAsset[x$0])), (objective, rewards) -> objective.completionHandlers = rewards, (objective) -> objective.completionHandlers, (objective, parent) -> objective.completionHandlers = parent.completionHandlers).add()).appendInherited(new KeyedCodec("TitleId", Codec.STRING), (objectiveAsset, s) -> objectiveAsset.objectiveTitleKey = s, (objectiveAsset) -> objectiveAsset.objectiveTitleKey, (objectiveAsset, parent) -> objectiveAsset.objectiveTitleKey = parent.objectiveTitleKey).metadata(new UIEditor(new UIEditor.LocalizationKeyField("server.objectives.{assetId}.title", true))).add()).appendInherited(new KeyedCodec("DescriptionId", Codec.STRING), (objectiveAsset, s) -> objectiveAsset.objectiveDescriptionKey = s, (objectiveAsset) -> objectiveAsset.objectiveDescriptionKey, (objectiveAsset, parent) -> objectiveAsset.objectiveDescriptionKey = parent.objectiveDescriptionKey).metadata(new UIEditor(new UIEditor.LocalizationKeyField("server.objectives.{assetId}.desc"))).add()).appendInherited(new KeyedCodec("RemoveOnItemDrop", Codec.BOOLEAN), (objectiveAsset, aBoolean) -> objectiveAsset.removeOnItemDrop = aBoolean, (objectiveAsset) -> objectiveAsset.removeOnItemDrop, (objectiveAsset, parent) -> objectiveAsset.removeOnItemDrop = parent.removeOnItemDrop).add()).afterDecode((objectiveAsset) -> {
      ...
      VALIDATOR_CACHE = new ValidatorCache<String>(new AssetKeyValidator(ObjectiveAsset::getAssetStore));
   }
}
