...
public class ReachLocationMarkerSystems {
   ...
   public static class EntityAdded extends RefSystem<EntityStore> {
      ...
      public EntityAdded(@Nonnull ComponentType<EntityStore, ReachLocationMarker> reachLocationMarkerComponent, @Nonnull ComponentType<EntityStore, TransformComponent> transformComponentType) {
         ...
         this.query = Query.<EntityStore>and(reachLocationMarkerComponent, transformComponentType);
      }
      ...
      public void onEntityAdded(@Nonnull Ref<EntityStore> ref, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ReachLocationMarker reachLocationMarkerComponent = (ReachLocationMarker)commandBuffer.getComponent(ref, this.reachLocationMarkerComponent);
   ...
   public static class EnsureNetworkSendable extends HolderSystem<EntityStore> {
      ...
      public EnsureNetworkSendable(@Nonnull ComponentType<EntityStore, ReachLocationMarker> reachLocationMarkerComponentType, @Nonnull ComponentType<EntityStore, NetworkId> networkIdComponentType) {
         ...
         this.query = Query.<EntityStore>and(reachLocationMarkerComponentType, Query.not(networkIdComponentType));
      }

      public void onEntityAdd(@Nonnull Holder<EntityStore> holder, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store) {
         int nextNetworkId = ((EntityStore)store.getExternalData()).takeNextNetworkId();
   ...
   public static class Ticking extends EntityTickingSystem<EntityStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ReachLocationMarker reachLocationMarkerComponent = (ReachLocationMarker)archetypeChunk.getComponent(index, this.reachLocationMarkerComponent);

         assert reachLocationMarkerComponent != null;

         String markerId = reachLocationMarkerComponent.getMarkerId();
         ReachLocationMarkerAsset asset = (ReachLocationMarkerAsset)ReachLocationMarkerAsset.getAssetMap().getAsset(markerId);
         if (asset == null) {
            ...
            spatialResource.getSpatialStructure().ordered(position, (double)asset.getRadius(), results);
            ObjectiveDataStore objectiveDataStore = ObjectivePlugin.get().getObjectiveDataStore();

            for(int i = 0; i < results.size(); ++i) {
               Ref<EntityStore> otherEntityReference = (Ref)results.get(i);
               UUIDComponent otherUuidComponent = (UUIDComponent)commandBuffer.getComponent(otherEntityReference, this.uuidComponentType);
