...
public class ObjectiveLocationMarkerCommand extends AbstractCommandCollection {
   public ObjectiveLocationMarkerCommand() {
      ...
      this.addAliases(new String[]{"marker"});
      this.addSubCommand(new AddLocationMarkerCommand());
      this.addSubCommand(new EnableLocationMarkerCommand());
      this.addSubCommand(new DisableLocationMarkerCommand());
   }

   public static class AddLocationMarkerCommand extends AbstractPlayerCommand {
      @Nonnull
      private final RequiredArg<String> locationMarkerArg;

      public AddLocationMarkerCommand() {
         super("add", "server.commands.objective.locationMarker.add");
         this.locationMarkerArg = this.withRequiredArg("locationMarkerId", "server.commands.objective.locationMarker.add.arg.locationMarkerId.desc", ArgTypes.STRING);
      }
      ...
      protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
         ...
         if (playerTransformComponent != null) {
            ...
            if (ObjectiveLocationMarkerAsset.getAssetMap().getAsset(objectiveLocationMarkerId) == null) {
               ...
               context.sendMessage(Message.translation("server.general.failed.didYouMean").param("choices", StringUtil.sortByFuzzyDistance(objectiveLocationMarkerId, ObjectiveLocationMarkerAsset.getAssetMap().getAssetMap().keySet(), CommandUtil.RECOMMEND_COUNT).toString()));
            } else {
