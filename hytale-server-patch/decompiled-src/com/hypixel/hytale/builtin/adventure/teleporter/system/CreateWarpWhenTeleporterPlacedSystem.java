...
public class CreateWarpWhenTeleporterPlacedSystem extends RefChangeSystem<ChunkStore, PlacedByInteractionComponent> {
   ...
   public CreateWarpWhenTeleporterPlacedSystem(@Nonnull ComponentType<ChunkStore, PlacedByInteractionComponent> placedByInteractionComponentType, @Nonnull ComponentType<ChunkStore, Teleporter> teleporterComponentType, @Nonnull ComponentType<ChunkStore, BlockModule.BlockStateInfo> blockStateInfoComponentType, @Nonnull ComponentType<EntityStore, PlayerRef> playerRefComponentType) {
      ...
      this.query = Query.<ChunkStore>and(placedByInteractionComponentType, teleporterComponentType, blockStateInfoComponentType);
   }
   ...
   public void onComponentAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull PlacedByInteractionComponent placedBy, @Nonnull Store<ChunkStore> chunkStore, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
      ...
      if (whoPlacedRef != null && whoPlacedRef.isValid()) {
         PlayerRef playerRefComponent = (PlayerRef)entityStore.getStore().getComponent(whoPlacedRef, this.playerRefComponentType);
            ...
            if (chunkRef.isValid()) {
               ...
               if (worldChunk != null) {
                  ...
                  if (cannedName != null) {
                     ...
                     Teleporter teleporterComponent = (Teleporter)commandBuffer.getComponent(ref, this.teleporterComponentType);
   ...
   public static void createWarp(@Nonnull WorldChunk worldChunk, @Nonnull BlockModule.BlockStateInfo blockStateInfo, @Nonnull String name) {
      ...
      if (blockChunkComponent != null) {
         ...
         Vector3d warpPosition = (new Vector3d((double)x, (double)y, (double)z)).add(0.5, 0.65, 0.5);
         Transform warpTransform = new Transform(warpPosition, new Vector3f(0.0F / 0.0F, warpRotationYaw, 0.0F / 0.0F));
   ...
   public void onComponentSet(@Nonnull Ref<ChunkStore> ref, @Nullable PlacedByInteractionComponent oldComponent, @Nonnull PlacedByInteractionComponent newComponent, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
   }

   public void onComponentRemoved(@Nonnull Ref<ChunkStore> ref, @Nonnull PlacedByInteractionComponent component, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
   }
