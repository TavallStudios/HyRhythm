...
public class TurnOffTeleportersSystem extends RefSystem<ChunkStore> {
   public static final Query<ChunkStore> QUERY = Query.<ChunkStore>and(Teleporter.getComponentType(), BlockModule.BlockStateInfo.getComponentType());
   ...
   public static void updatePortalBlocksInWorld(World world) {
      ...
      AndQuery<ChunkStore> entityQuery = Query.<ChunkStore>and(Teleporter.getComponentType(), BlockModule.BlockStateInfo.getComponentType());
      store.forEachChunk(entityQuery, (BiConsumer)((archetypeChunk, commandBuffer) -> {
   ...
   private static void updatePortalBlockInWorld(Ref<ChunkStore> ref, ComponentAccessor<ChunkStore> store) {
      if (ref.isValid()) {
         Teleporter teleporter = (Teleporter)store.getComponent(ref, Teleporter.getComponentType());
   ...
   public static void updatePortalBlockInWorld(ComponentAccessor<ChunkStore> store, Teleporter teleporter, BlockModule.BlockStateInfo blockStateInfo) {
      ...
      if (chunkRef.isValid()) {
         WorldChunk worldChunkComponent = (WorldChunk)store.getComponent(chunkRef, WorldChunk.getComponentType());
         if (worldChunkComponent != null) {
            ...
            if (blockType != null) {
               ...
               Warp destinationWarp = warpId == null ? null : (Warp)TeleportPlugin.get().getWarps().get(warpId);
               ...
               if (destinationWarp == null) {
                  teleporter.setWarp((String)null);
                  blockStateInfo.markNeedsSaving(store);
               }
