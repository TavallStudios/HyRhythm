...
public class ReputationPlugin extends JavaPlugin {
   ...
   private int maxReputationValue = -2147483648;
   private int minReputationValue = 2147483647;
   ...
   protected void setup() {
      instance = this;
      AssetRegistry.register(((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)HytaleAssetStore.builder(ReputationRank.class, new DefaultAssetMap()).setPath("NPC/Reputation/Ranks")).setCodec(ReputationRank.CODEC)).setKeyFunction(ReputationRank::getId)).build());
      AssetRegistry.register(((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)((HytaleAssetStore.Builder)HytaleAssetStore.builder(ReputationGroup.class, new DefaultAssetMap()).setPath("NPC/Reputation/Groups")).setCodec(ReputationGroup.CODEC)).setKeyFunction(ReputationGroup::getId)).build());
      this.getCommandRegistry().registerCommand(new ReputationCommand());
      ChoiceRequirement.CODEC.register((String)"Reputation", ReputationRequirement.class, ReputationRequirement.CODEC);
   ...
   protected void start() {
      ...
      if (this.reputationRanks.size() > 1) {
         ...
         int previousMaxValue = ((ReputationRank)this.reputationRanks.getFirst()).getMaxValue();

         for(int i = 1; i < this.reputationRanks.size(); ++i) {
            ReputationRank reputationRank = (ReputationRank)this.reputationRanks.get(i);
            if (previousMaxValue < reputationRank.getMinValue()) {
               this.getLogger().at(Level.WARNING).log("There is a gap between the values of the ReputationRank %s and %s, please review the assets.", reputationRank.getId(), ((ReputationRank)this.reputationRanks.get(i - 1)).getId());
            }

            if (previousMaxValue > reputationRank.getMinValue()) {
               this.getLogger().at(Level.WARNING).log("Min value of rank %s is already contained in rank %s, please review the asset.", reputationRank.getId(), ((ReputationRank)this.reputationRanks.get(i - 1)).getId());
            }

            previousMaxValue = reputationRank.getMaxValue();
         }

         this.minReputationValue = ((ReputationRank)this.reputationRanks.getFirst()).getMinValue();
         this.maxReputationValue = ((ReputationRank)this.reputationRanks.getLast()).getMaxValue();
      }
   }
   ...
   public int changeReputation(@Nonnull Player player, @Nonnull Ref<EntityStore> npcRef, int value, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ReputationGroupComponent reputationGroupComponent = (ReputationGroupComponent)componentAccessor.getComponent(npcRef, this.reputationGroupComponentType);
      return reputationGroupComponent == null ? -2147483648 : this.changeReputation(player, reputationGroupComponent.getReputationGroupId(), value, componentAccessor);
   }
   ...
   public int changeReputation(@Nonnull Player player, @Nonnull String reputationGroupId, int value, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (reputationGameplayConfig.getReputationStorageType() == ReputationGameplayConfig.ReputationStorageType.PerPlayer) {
         ReputationGroup reputationGroup = (ReputationGroup)ReputationGroup.getAssetMap().getAsset(reputationGroupId);
   ...
   public int changeReputation(@Nonnull World world, @Nonnull String reputationGroupId, int value) {
      ...
      if (reputationGameplayConfig.getReputationStorageType() != ReputationGameplayConfig.ReputationStorageType.PerWorld) {
         ...
         if (reputationGroup == null) {
            ...
            ReputationDataResource reputationDataResource = (ReputationDataResource)world.getEntityStore().getStore().getResource(this.reputationDataResourceType);
   ...
   public int getReputationValue(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> playerEntityRef, @Nonnull Ref<EntityStore> npcEntityRef) {
      ReputationGroupComponent reputationGroupComponent = (ReputationGroupComponent)store.getComponent(npcEntityRef, this.reputationGroupComponentType);
      return reputationGroupComponent == null ? -2147483648 : this.getReputationValue(store, playerEntityRef, reputationGroupComponent.getReputationGroupId());
   }
   ...
   public int getReputationValue(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> playerEntityRef, @Nonnull String reputationGroupId) {
      ...
      Player playerComponent = (Player)store.getComponent(playerEntityRef, Player.getComponentType());
   ...
   public int getReputationValue(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> npcRef) {
      String reputationGroupId = ((ReputationGroupComponent)store.getComponent(npcRef, this.reputationGroupComponentType)).getReputationGroupId();
      return this.getReputationValue(store, reputationGroupId);
   }
   ...
   public ReputationRank getReputationRank(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull Ref<EntityStore> npcRef) {
      ReputationGroupComponent reputationGroupComponent = (ReputationGroupComponent)store.getComponent(npcRef, this.reputationGroupComponentType);
   ...
   public ReputationRank getReputationRankFromValue(int value) {
      if (value == -2147483648) {
         ...
         for(int i = 0; i < this.reputationRanks.size(); ++i) {
            if (((ReputationRank)this.reputationRanks.get(i)).containsValue(value)) {
               return (ReputationRank)this.reputationRanks.get(i);
            }
         }
