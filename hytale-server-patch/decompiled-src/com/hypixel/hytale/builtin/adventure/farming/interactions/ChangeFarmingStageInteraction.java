...
public class ChangeFarmingStageInteraction extends SimpleBlockInteraction {
                        ...
                        if (blockRef == null) {
                           ...
                           if (stages != null && stages.length != 0) {
                              ...
                              if (sectionRef != null && sectionRef.isValid()) {
                                 BlockSection blockSectionComponent = (BlockSection)chunkStore.getComponent(sectionRef, BlockSection.getComponentType());
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ChangeFarmingStageInteraction.class, ChangeFarmingStageInteraction::new, SimpleBlockInteraction.CODEC).documentation("Changes the farming stage of the target block.")).appendInherited(new KeyedCodec("Stage", Codec.INTEGER), (interaction, stage) -> interaction.targetStage = stage, (interaction) -> interaction.targetStage, (o, p) -> o.targetStage = p.targetStage).documentation("The stage index to set (0, 1, 2, etc.). Use -1 for the final stage. Ignored if Increase is set.").add()).appendInherited(new KeyedCodec("Increase", Codec.INTEGER), (interaction, increase) -> interaction.increaseBy = increase, (interaction) -> interaction.increaseBy, (o, p) -> o.increaseBy = p.increaseBy).documentation("Add this amount to the current stage (e.g., 1 = advance one stage, 2 = advance two stages). Takes priority over Decrease and Stage.").add()).appendInherited(new KeyedCodec("Decrease", Codec.INTEGER), (interaction, decrease) -> interaction.decreaseBy = decrease, (interaction) -> interaction.decreaseBy, (o, p) -> o.decreaseBy = p.decreaseBy).documentation("Subtract this amount from the current stage (e.g., 1 = go back one stage). Takes priority over Stage.").add()).appendInherited(new KeyedCodec("StageSet", Codec.STRING), (interaction, stageSet) -> interaction.targetStageSet = stageSet, (interaction) -> interaction.targetStageSet, (o, p) -> o.targetStageSet = p.targetStageSet).documentation("Optional. The stage set to switch to (e.g., 'Default', 'Harvested'). If not provided, uses current stage set.").add()).build();
   }
}
