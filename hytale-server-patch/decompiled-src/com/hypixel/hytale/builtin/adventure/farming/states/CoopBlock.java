...
public class CoopBlock implements Component<ChunkStore> {
   ...
   @Nonnull
   protected List<CoopResident> residents = new ObjectArrayList();
   @Nonnull
   ...
   public CoopBlock(@Nonnull String farmingCoopId, @Nonnull List<CoopResident> residents, @Nonnull ItemContainer itemContainer) {
      this.itemContainer = EmptyItemContainer.INSTANCE;
            ...
            if (!residentDeployed && residentEntityId == null) {
               ...
               if (roleBuilder != null) {
                  ...
                  if (spawningContext.set(world, residentSpawnLocation.x, residentSpawnLocation.y, residentSpawnLocation.z) && spawningContext.canSpawn() == SpawnTestResult.TEST_OK) {
                     Pair<Ref<EntityStore>, NPCEntity> npcPair = NPCPlugin.get().spawnEntity(store, npcRoleIndex, spawningContext.newPosition(), Vector3f.ZERO, (Model)null, (TriConsumer)null);
                     if (npcPair == null) {
   ...
   public void ensureNoResidentsInWorld(@Nonnull Store<EntityStore> store) {
      FarmingCoopAsset coopAsset = this.getCoopAsset();
      if (coopAsset != null) {
         ObjectArrayList<CoopResident> residentsToRemove = new ObjectArrayList();
         ...
         while(var11.hasNext()) {
            CoopResident resident = (CoopResident)var11.next();
