...
public class TilledSoilBlock implements Component<ChunkStore> {
   ...
   public String toString() {
      boolean var10000 = this.planted;
      return "TilledSoilBlock{planted=" + var10000 + ", fertilized=" + this.fertilized + ", externalWater=" + this.externalWater + ", wateredUntil=" + String.valueOf(this.wateredUntil) + ", decayTime=" + String.valueOf(this.decayTime) + "}";
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(TilledSoilBlock.class, TilledSoilBlock::new).versioned()).codecVersion(VERSION)).append(new KeyedCodec("Planted", Codec.BOOLEAN), (state, planted) -> state.planted = planted, (state) -> state.planted ? Boolean.TRUE : null).add()).append(new KeyedCodec("ModifierTimes", new MapCodec(Codec.INSTANT, Object2ObjectOpenHashMap::new, false)), (state, times) -> {
         if (times != null) {
            state.wateredUntil = (Instant)times.get("WateredUntil");
         }
      }, (state) -> null).setVersionRange(0, 0).add()).append(new KeyedCodec("Flags", Codec.STRING_ARRAY), (state, flags) -> {
         if (flags != null) {
            state.fertilized = ArrayUtil.contains(flags, "Fertilized");
            state.externalWater = ArrayUtil.contains(flags, "ExternalWater");
         }
      }, (state) -> null).setVersionRange(0, 0).add()).append(new KeyedCodec("Fertilized", Codec.BOOLEAN), (state, v) -> state.fertilized = v, (state) -> state.fertilized ? Boolean.TRUE : null).setVersionRange(1, VERSION).add()).append(new KeyedCodec("ExternalWater", Codec.BOOLEAN), (state, v) -> state.externalWater = v, (state) -> state.externalWater ? Boolean.TRUE : null).setVersionRange(1, VERSION).add()).append(new KeyedCodec("WateredUntil", Codec.INSTANT), (state, v) -> state.wateredUntil = v, (state) -> state.wateredUntil).setVersionRange(1, VERSION).add()).append(new KeyedCodec("DecayTime", Codec.INSTANT), (state, v) -> state.decayTime = v, (state) -> state.decayTime).add()).build();
   }
}
