...
public class HarvestCropInteraction extends SimpleBlockInteraction {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(HarvestCropInteraction.class, HarvestCropInteraction::new, SimpleBlockInteraction.CODEC).documentation("Harvests the resources from the target farmable block.")).appendInherited(new KeyedCodec("RequireNotBroken", Codec.BOOLEAN), (interaction, s) -> interaction.requireNotBroken = s, (interaction) -> interaction.requireNotBroken, (interaction, parent) -> interaction.requireNotBroken = parent.requireNotBroken).documentation("If true, the interaction will fail if the held item is broken (durability = 0).").add()).build();
   }
}
