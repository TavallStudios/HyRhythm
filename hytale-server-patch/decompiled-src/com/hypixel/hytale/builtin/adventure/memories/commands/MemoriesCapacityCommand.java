...
public class MemoriesCapacityCommand extends AbstractPlayerCommand {
   ...
   public MemoriesCapacityCommand() {
      ...
      this.capacityArg = this.withRequiredArg("capacity", "server.commands.memories.capacity.capacity.desc", ArgTypes.INTEGER);
   }

   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      Integer capacity = (Integer)this.capacityArg.get(context);
      ...
      if (capacity <= 0) {
         ...
         PlayerMemories playerMemories = (PlayerMemories)store.ensureAndGetComponent(ref, PlayerMemories.getComponentType());
