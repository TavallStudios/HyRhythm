...
public class CameraShakePacketGenerator extends SimpleAssetPacketGenerator<String, CameraShake, IndexedAssetMap<String, CameraShake>> {
   ...
   protected ToClientPacket generateRemovePacket(@Nonnull IndexedAssetMap<String, CameraShake> assetMap, @Nonnull Set<String> removed) {
      ...
      for(String key : removed) {
         ...
         profiles.put(index, (Object)null);
      }
   ...
   @Nonnull
   protected static ToClientPacket toCachedPacket(@Nonnull UpdateType type, @Nonnull IndexedAssetMap<String, CameraShake> assetMap, @Nonnull Map<String, CameraShake> assets) {
      Int2ObjectOpenHashMap<com.hypixel.hytale.protocol.CameraShake> profiles = new Int2ObjectOpenHashMap();

      for(Map.Entry<String, CameraShake> entry : assets.entrySet()) {
         int index = assetMap.getIndex((String)entry.getKey());
         profiles.put(index, ((CameraShake)entry.getValue()).toPacket());
      }
