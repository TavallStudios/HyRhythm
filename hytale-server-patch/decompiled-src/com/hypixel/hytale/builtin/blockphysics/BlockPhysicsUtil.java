...
public class BlockPhysicsUtil {
      ...
      if (filler != 0) {
               ...
               for(int x = 0; x < blockWidth; ++x) {
                  for(int y = 0; y < blockHeight; ++y) {
                     for(int z = 0; z < blockDepth; ++z) {
                        ...
                        int fillerSupportDistance = testBlockPhysics(chunkAccessor, neighbourBlockSection, neighbourBlockPhysics, neighbourFluidSection, fillerX, fillerY, fillerZ, blockType, neighbourRotation, neighbourFiller);
         ...
         if (supportDistance == 0) {
            ...
            switch (blockType.getSupportDropType()) {
               case BREAK -> BlockHarvestUtils.naturallyRemoveBlockByPhysics(new Vector3i(blockX, blockY, blockZ), blockType, filler, 256, chunkReference, componentAccessor, chunkStore);
               case DESTROY -> BlockHarvestUtils.naturallyRemoveBlockByPhysics(new Vector3i(blockX, blockY, blockZ), blockType, filler, 2304, chunkReference, componentAccessor, chunkStore);
            }
      ...
      if (blockType.isUnknown()) {
         ...
         if (requiredBlockFaceSupportMap != null && !requiredBlockFaceSupportMap.isEmpty()) {
                     ...
                     if (!boundingBox.containsBlock(origin, neighbourX, neighbourY, neighbourZ)) {
                        ...
                        for(RequiredBlockFaceSupport requiredBlockFaceSupport : requiredBlockFaceSupports) {
                           if (requiredBlockFaceSupport.isAppliedToFiller(blockFillerOffset)) {
                              boolean doesSatisfyRequirements = doesSatisfyRequirements(blockType, blockFillerOffset, neighbourFillerOffset, blockFace, neighbourBlockFace, neighbourBlockId, neighbourBlockType, neighbourRotation, neighbourFluidId, neighbourFluid, requiredBlockFaceSupport);
            ...
            if (!hasTestedForSupport) {
               ...
               if (lowestSupportDistance < 2147483647 && lowestSupportDistance >= 0) {
   ...
   public static boolean doesMatchFaceType(Vector3i fillerOffset, @Nonnull String faceType, BlockFace blockFace, @Nonnull Map<BlockFace, BlockFaceSupport[]> supporting) {
      ...
      BlockFaceSupport[] blockFaceSupports = (BlockFaceSupport[])supporting.get(blockFace);
