...
public class FluidPlugin extends JavaPlugin {
   ...
   protected void setup() {
      ...
      FluidTicker.CODEC.register(Priority.DEFAULT, (String)"Default", DefaultFluidTicker.class, DefaultFluidTicker.CODEC);
      FluidTicker.CODEC.register((String)"Fire", FireFluidTicker.class, FireFluidTicker.CODEC);
      FluidTicker.CODEC.register((String)"Finite", FiniteFluidTicker.class, FiniteFluidTicker.CODEC);
            ...
            if (blockChunkComponent != null) {
               ...
               if (sections != null) {
                  for(int i = 0; i < sections.length && i < 10; ++i) {
                     ...
                     FluidSection fluidSectionComponent = (FluidSection)section.getComponent(FluidSection.getComponentType());
                     if (fluidSectionComponent != null && !fluidSectionComponent.isEmpty()) {
                        BlockSection blockSectionComponent = (BlockSection)section.ensureAndGetComponent(BlockSection.getComponentType());
                              ...
                              if (fluidType == null) {
                                 ...
                                 if (FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(idx)))) {
                                    ...
                                    if (!ticker.canDemote()) {
                                       ...
                                       boolean canSpread = ChunkUtil.isBorderBlock(x, z) || fluidSectionComponent.getFluidId(x - 1, y, z) != fluidId && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x - 1, y, z))) || fluidSectionComponent.getFluidId(x + 1, y, z) != fluidId && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x + 1, y, z))) || fluidSectionComponent.getFluidId(x, y, z - 1) != fluidId && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x, y, z - 1))) || fluidSectionComponent.getFluidId(x, y, z + 1) != fluidId && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x, y, z + 1)));
                                       if (y > 0) {
                                          if (ChunkUtil.chunkCoordinate(y) == ChunkUtil.chunkCoordinate(y - 1)) {
                                             canSpread |= fluidSectionComponent.getFluidId(x, y - 1, z) != fluidId && !FluidTicker.isSolid(blockMap.getAsset(blockSectionComponent.get(x, y - 1, z)));
                                          } else {
                                             FluidSection fluidSection2 = (FluidSection)sections[i - 1].getComponent(FluidSection.getComponentType());
                                             canSpread |= fluidSection2.getFluidId(x, y - 1, z) != fluidId && !FluidTicker.isSolid(blockMap.getAsset(blockChunkComponent.getBlock(x, y - 1, z)));
                                          }
                                       }
                     ...
                     do {
                        ...
                        for(int i = 0; i < sections.length; ++i) {
                           ...
                           if (fluidSectionComponent != null && !fluidSectionComponent.isEmpty()) {
                              ...
                              blockSectionComponent.forEachTicking(accessor, fluidSectionComponent, i, (preprocesorAccessor, fluidSection1, xx, yx, zx, block) -> {
                                 ...
                                 if (fluidId == 0) {
                                    ...
                                    return fluid.getTicker().process(preprocesorAccessor.worldChunk.getWorld(), preprocesorAccessor.tick, preprocesorAccessor, fluidSection1, accessor.blockSection, fluid, fluidId, blockX, yx, blockZ);
                                 }
                              });
                           }
                        }
   ...
   public static class PreprocesorAccessor implements FluidTicker.Accessor {
      ...
      public FluidSection getFluidSection(int cx, int cy, int cz) {
         return this.blockChunk.getX() == cx && this.blockChunk.getZ() == cz && cy >= 0 && cy < this.sections.length ? (FluidSection)this.sections[cy].getComponent(FluidSection.getComponentType()) : null;
      }
