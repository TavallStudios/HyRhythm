...
public class FluidSystems {
   ...
   public static class EnsureFluidSection extends HolderSystem<ChunkStore> {
      ...
      public EnsureFluidSection(@Nonnull ComponentType<ChunkStore, ChunkSection> chunkSectionComponentType, @Nonnull ComponentType<ChunkStore, FluidSection> fluidSectionComponentType) {
         ...
         this.query = Query.<ChunkStore>and(chunkSectionComponentType, Query.not(fluidSectionComponentType));
      }
   ...
   public static class MigrateFromColumn extends ChunkColumnMigrationSystem {
      ...
      public MigrateFromColumn(@Nonnull ComponentType<ChunkStore, ChunkColumn> chunkColumnComponentType, @Nonnull ComponentType<ChunkStore, BlockChunk> blockChunkComponentType, @Nonnull ComponentType<ChunkStore, FluidSection> fluidSectionComponentType) {
         ...
         this.fluidSectionComponentType = fluidSectionComponentType;
         this.query = Query.<ChunkStore>and(chunkColumnComponentType, blockChunkComponentType);
      }

      public void onEntityAdd(@Nonnull Holder<ChunkStore> holder, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store) {
         ChunkColumn chunkColumnComponent = (ChunkColumn)holder.getComponent(this.chunkColumnComponentType);

         assert chunkColumnComponent != null;

         BlockChunk blockChunkComponent = (BlockChunk)holder.getComponent(this.blockChunkComponentType);
   ...
   public static class SetupSection extends HolderSystem<ChunkStore> {
      ...
      public SetupSection(@Nonnull ComponentType<ChunkStore, ChunkSection> chunkSectionComponentType, @Nonnull ComponentType<ChunkStore, FluidSection> fluidSectionComponentType) {
         this.dependencies = Set.of(new SystemDependency(Order.AFTER, MigrateFromColumn.class));
         this.chunkSectionComponentType = chunkSectionComponentType;
         this.fluidSectionComponentType = fluidSectionComponentType;
         this.query = Query.<ChunkStore>and(chunkSectionComponentType, fluidSectionComponentType);
      }

      public void onEntityAdd(@Nonnull Holder<ChunkStore> holder, @Nonnull AddReason reason, @Nonnull Store<ChunkStore> store) {
         ChunkSection chunkSectionComponent = (ChunkSection)holder.getComponent(this.chunkSectionComponentType);

         assert chunkSectionComponent != null;

         FluidSection fluidSectionComponent = (FluidSection)holder.getComponent(this.fluidSectionComponentType);
   ...
   public static class LoadPacketGenerator extends ChunkStore.LoadFuturePacketDataQuerySystem {
      ...
      public void fetch(int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer, PlayerRef query, @Nonnull List<CompletableFuture<ToClientPacket>> results) {
         ChunkColumn chunkColumnComponent = (ChunkColumn)archetypeChunk.getComponent(index, this.chunkColumnComponentType);

         assert chunkColumnComponent != null;

         for(Ref<ChunkStore> sectionRef : chunkColumnComponent.getSections()) {
            FluidSection fluidSectionComponent = (FluidSection)commandBuffer.getComponent(sectionRef, this.fluidSectionComponentType);
   ...
   public static class ReplicateChanges extends EntityTickingSystem<ChunkStore> implements RunWhenPausedSystem<ChunkStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         FluidSection fluidSectionComponent = (FluidSection)archetypeChunk.getComponent(index, this.fluidSectionComponentType);
         ...
         if (!changes.isEmpty()) {
            ...
            WorldChunk worldChunkComponent = (WorldChunk)commandBuffer.getComponent(chunkSectionComponent.getChunkColumnReference(), this.worldChunkComponentType);
               ...
               if (changes.size() >= 1024) {
                  ...
                  fluidSectionComponent.getCachedPacket().whenComplete((packetx, throwable) -> {
                     if (throwable != null) {
                        ...
                        while(i$.hasNext()) {
                           PlayerRef playerRef = (PlayerRef)i$.next();
   ...
   public static class Ticking extends EntityTickingSystem<ChunkStore> {
      ...
      public Ticking(@Nonnull ComponentType<ChunkStore, ChunkSection> chunkSectionComponentType, @Nonnull ComponentType<ChunkStore, FluidSection> fluidSectionComponentType, @Nonnull ComponentType<ChunkStore, BlockChunk> blockChunkComponentType) {
         this.dependencies = Set.of(new SystemDependency(Order.AFTER, ChunkBlockTickSystem.PreTick.class), new SystemDependency(Order.BEFORE, ChunkBlockTickSystem.Ticking.class));
         this.chunkSectionComponentType = chunkSectionComponentType;
         this.fluidSectionComponentType = fluidSectionComponentType;
         this.blockChunkComponentType = blockChunkComponentType;
         this.query = Query.<ChunkStore>and(fluidSectionComponentType, chunkSectionComponentType);
      }
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<ChunkStore> archetypeChunk, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
         ...
         BlockChunk blockChunkComponent = (BlockChunk)commandBuffer.getComponent(chunkRef, this.blockChunkComponentType);
