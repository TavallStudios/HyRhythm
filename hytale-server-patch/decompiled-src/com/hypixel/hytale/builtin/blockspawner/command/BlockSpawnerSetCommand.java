...
public class BlockSpawnerSetCommand extends AbstractWorldCommand {
   ...
   public BlockSpawnerSetCommand() {
      ...
      this.blockSpawnerIdArg = this.withRequiredArg("blockSpawnerId", "server.commands.blockspawner.set.blockSpawnerId.desc", BLOCK_SPAWNER_ASSET_TYPE);
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      ...
      if (this.positionArg.provided(context)) {
         RelativeIntPosition relativePosition = (RelativeIntPosition)this.positionArg.get(context);
      ...
      if (worldChunk == null) {
         ...
         if (blockRef != null && blockRef.isValid()) {
            ...
            BlockSpawner spawnerState = (BlockSpawner)chunkStore.getComponent(blockRef, BlockSpawner.getComponentType());
            if (spawnerState == null) {
               context.sendMessage(Message.translation("server.general.containerNotFound").param("block", position.toString()));
            } else {
               String spawnerId;
               if ((Boolean)this.ignoreChecksFlag.get(context)) {
