...
public final class VoidEventRefSystem extends RefSystem<EntityStore> {
   ...
   public void onEntityRemove(@Nonnull Ref<EntityStore> ref, @Nonnull RemoveReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      if (portalWorld.exists()) {
         ...
         if (forcedMusic != null) {
            AmbienceResource ambienceResource = (AmbienceResource)store.getResource(AmbienceResource.getResourceType());
            ambienceResource.setForcedMusicAmbience((String)null);
         }

         VoidEvent voidEvent = (VoidEvent)commandBuffer.getComponent(ref, VoidEvent.getComponentType());
         VoidEventStage activeStage = voidEvent.getActiveStage();
         if (activeStage != null) {
            VoidEventStagesSystem.stopStage(activeStage, store, commandBuffer);
            voidEvent.setActiveStage((VoidEventStage)null);
         }
