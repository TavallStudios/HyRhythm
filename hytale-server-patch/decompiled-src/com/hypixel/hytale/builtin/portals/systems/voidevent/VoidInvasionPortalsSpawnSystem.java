...
public class VoidInvasionPortalsSpawnSystem extends DelayedEntitySystem<EntityStore> {
   ...
   public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      VoidEvent voidEventComponent = (VoidEvent)archetypeChunk.getComponent(index, VoidEvent.getComponentType());
      ...
      if (this.findPortalSpawnPos == null) {
         ...
         Vector3d portalPos;
         try {
            portalPos = (Vector3d)this.findPortalSpawnPos.join();
            this.findPortalSpawnPos = null;
         } catch (Throwable t) {
            ((HytaleLogger.Api)LOGGER.at(Level.SEVERE).withCause(t)).log("Error trying to find a void event spawn position");
            return;
         }
         ...
         if (portalPos != null) {
            ...
            if (eventConfig == null) {
               ...
               world.getChunkAsync(chunkIndex).thenAcceptAsync((chunk) -> {
                  ...
                  if (blockType == null) {
                     LOGGER.at(Level.WARNING).log("Failed to place invasion portal block at %s, %s, %s. Block type is not configured", portalBlockPos.x, portalBlockPos.y, portalBlockPos.z);
                  } else {
   ...
   @Nullable
   private static CompletableFuture<Vector3d> findPortalSpawnPosition(@Nonnull World world, @Nonnull VoidEvent voidEvent, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      if (!portalWorld.exists()) {
         ...
         if (spawnPoint == null) {
            ...
            if (playerTransform == null) {
               ...
               return CompletableFuture.supplyAsync(() -> (Vector3d)(new SearchCone(direction, 48.0, 64.0, 90.0, 8)).filter(noNearbySpawners).filter(new NotNearPointXZ(spawnPos, 18.0)).then(new SearchBelow(12)).filter(new FitsAPortal()).execute(world, originPosition).orElse((Object)null), world);
            }
         }
      }
   }
