   ...
   public void onEntityAdded(@Nonnull Ref<EntityStore> ref, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
      ...
      if (portalWorld.exists()) {
         ...
         if (!cursedItemsInWorld.isEmpty()) {
            ItemComponent itemComponent = (ItemComponent)store.getComponent(ref, ItemComponent.getComponentType());
            ItemStack itemStack = itemComponent.getItemStack();
            if (itemStack != null) {
               String itemId = itemStack.getItemId().toString();
               if (cursedItemsInWorld.contains(itemId)) {
                  AdventureMetadata adventureMeta = (AdventureMetadata)itemStack.getFromMetadataOrDefault("Adventure", AdventureMetadata.CODEC);
