...
public class InvasionPortalConfig {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(InvasionPortalConfig.class, InvasionPortalConfig::new).append(new KeyedCodec("BlockKey", Codec.STRING), (config, o) -> config.blockKey = o, (config) -> config.blockKey).documentation("The block used for evil portals that spawn around the world during the event").add()).append(new KeyedCodec("SpawnBeacons", Codec.STRING_ARRAY), (config, o) -> config.spawnBeacons = o, (config) -> config.spawnBeacons).add()).documentation("An array of SpawnBeacon IDs, which will make mobs spawn around the evil portals")).append(new KeyedCodec("OnSpawnParticles", Codec.STRING), (config, o) -> config.onSpawnParticles = o, (config) -> config.onSpawnParticles).documentation("A particle system ID to spawn when the portal spawns, should be a temporary one.").add()).build();
   }
}
