   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull World world, @Nonnull Store<EntityStore> store) {
      ...
      if (portalWorld.exists() && portalWorld.isVoidEventActive()) {
         ...
         if (!portalWorld.exists()) {
            if (!(Boolean)this.overrideWorld.get(context)) {
               context.sendMessage(Message.translation("server.commands.portals.notInPortal"));
               return;
            }

            GameplayConfig gameplayConfig = (GameplayConfig)GameplayConfig.getAssetMap().getAsset("Portal");
            ...
            portalWorld.init((PortalType)PortalType.getAssetMap().getAsset("Hederas_Lair"), 10000, new PortalRemovalCondition(10000.0), portalGameplayConfig);
