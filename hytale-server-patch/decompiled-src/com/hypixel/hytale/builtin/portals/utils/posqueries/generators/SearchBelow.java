...
public class SearchBelow implements SpatialQuery {
   ...
   public Stream<Vector3d> createCandidates(@Nonnull World world, @Nonnull Vector3d origin, @Nullable SpatialQueryDebug debug) {
      if (debug != null) {
         ...
         debug.appendLine("Searching up to " + var10001 + " blocks below " + SpatialQueryDebug.fmt(origin) + ":");
      }

      return IntStream.rangeClosed(0, this.height).mapToObj((dy) -> origin.clone().add(0.0, (double)(-dy), 0.0));
   }
}
