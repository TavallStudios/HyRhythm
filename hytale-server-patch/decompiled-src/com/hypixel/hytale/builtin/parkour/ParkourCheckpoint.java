...
public class ParkourCheckpoint implements Component<EntityStore> {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(ParkourCheckpoint.class, ParkourCheckpoint::new).append(new KeyedCodec("CheckpointIndex", Codec.INTEGER), (parkourCheckpoint, integer) -> parkourCheckpoint.index = integer, (parkourCheckpoint) -> parkourCheckpoint.index).add()).build();
   }
}
