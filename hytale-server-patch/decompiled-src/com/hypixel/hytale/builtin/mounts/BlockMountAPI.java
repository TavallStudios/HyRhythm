...
public final class BlockMountAPI {
   ...
   @Nonnull
   public static BlockMountResult mountOnBlock(@Nonnull Ref<EntityStore> entity, @Nonnull CommandBuffer<EntityStore> commandBuffer, @Nonnull Vector3i targetBlock, @Nonnull Vector3f interactPos) {
      MountedComponent existingMounted = (MountedComponent)commandBuffer.getComponent(entity, MountedComponent.getComponentType());
      if (existingMounted != null) {
         return BlockMountAPI.DidNotMount.ALREADY_MOUNTED;
      } else {
         World world = ((EntityStore)entity.getStore().getExternalData()).getWorld();
            ...
            if (chunkRef == null) {
               ...
               if (blockComponentChunk == null) {
                  ...
                  if (blockType == null) {
                     ...
                     BlockMountComponent blockMountComponent = (BlockMountComponent)world.getChunkStore().getStore().getComponent(blockRef, BlockMountComponent.getComponentType());
   ...
   public static record Mounted(BlockType blockType, MountedComponent component) implements BlockMountResult {
   }
