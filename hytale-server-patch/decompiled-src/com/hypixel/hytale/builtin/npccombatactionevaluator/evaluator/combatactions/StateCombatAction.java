...
public class StateCombatAction extends CombatActionOption {
   ...
   public String toString() {
      ...
      return "StateCombatAction{state='" + var10000 + "', subState='" + this.subState + "'}" + super.toString();
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(StateCombatAction.class, StateCombatAction::new, CombatActionOption.BASE_CODEC).documentation("A combat action which switches the NPCs state. Using substate only will switch between combat substates, whereas including the main state can be used to transition out of combat.")).append(new KeyedCodec("State", Codec.STRING), (option, s) -> option.state = s, (option) -> option.state).documentation("The main state name.").addValidator(Validators.nonNull()).addValidator(Validators.nonEmptyString()).add()).append(new KeyedCodec("SubState", Codec.STRING), (option, s) -> option.subState = s, (option) -> option.subState).documentation("The substate name.").add()).build();
   }
}
