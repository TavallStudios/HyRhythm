...
public class SensorCombatActionEvaluator extends SensorBase {
   ...
   public SensorCombatActionEvaluator(@Nonnull BuilderSensorCombatActionEvaluator builder, @Nonnull BuilderSupport support) {
      super(builder);
      this.positionProvider = new EntityPositionProvider(this.parameterProvider);
      ...
      CombatActionEvaluatorSystems.CombatConstructionData constructionData = (CombatActionEvaluatorSystems.CombatConstructionData)holder.ensureAndGetComponent(CombatActionEvaluatorSystems.CombatConstructionData.getComponentType());
   ...
   public boolean matches(@Nonnull Ref<EntityStore> ref, @Nonnull Role role, double dt, @Nonnull Store<EntityStore> store) {
      if (!super.matches(ref, role, dt, store)) {
         ...
         if (target == null) {
            ...
            Vector3d selfPosition = ((TransformComponent)store.getComponent(ref, TRANSFORM_COMPONENT_TYPE)).getPosition();
            Vector3d targetPosition = ((TransformComponent)store.getComponent(target, TRANSFORM_COMPONENT_TYPE)).getPosition();
