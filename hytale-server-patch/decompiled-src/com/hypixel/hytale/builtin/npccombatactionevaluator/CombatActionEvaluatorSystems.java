...
public class CombatActionEvaluatorSystems {
   ...
   public static class OnAdded extends HolderSystem<EntityStore> {
      ...
      public OnAdded(ComponentType<EntityStore, CombatConstructionData> combatConstructionDataComponentType) {
         ...
         this.query = Query.<EntityStore>and(combatConstructionDataComponentType, combatConstructionDataComponentType);
      }
      ...
      public void onEntityAdd(@Nonnull Holder<EntityStore> holder, @Nonnull AddReason reason, @Nonnull Store<EntityStore> store) {
         ...
         if (role != null && role.getBalanceAsset() != null) {
            BalanceAsset balancingAsset = (BalanceAsset)BalanceAsset.getAssetMap().getAsset(role.getBalanceAsset());
            if (balancingAsset instanceof CombatBalanceAsset) {
               CombatBalanceAsset combatBalance = (CombatBalanceAsset)balancingAsset;
               CombatConstructionData constructionData = (CombatConstructionData)holder.getComponent(this.combatConstructionDataComponentType);
   ...
   public static class EvaluatorTick extends EntityTickingSystem<EntityStore> {
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         ...
         if (!archetypeChunk.getArchetype().contains(DeathComponent.getComponentType())) {
            CombatActionEvaluator evaluatorComponent = (CombatActionEvaluator)archetypeChunk.getComponent(index, this.componentType);
            ...
            if (currentState != evaluatorComponent.getRunInState()) {
               ...
               DamageMemory damageMemory = (DamageMemory)archetypeChunk.getComponent(index, this.damageMemoryComponentType);
               if (damageMemory != null) {
                  damageMemory.clearTotalDamage();
               }

            } else if (!role.getCombatSupport().isExecutingAttack()) {
               ValueStore valueStoreComponent = (ValueStore)archetypeChunk.getComponent(index, this.valueStoreComponentType);
               ...
               if (basicAttacks != null) {
                  ...
                  evaluatorComponent.setCurrentBasicAttackSet(currentSubState, (CombatActionEvaluatorConfig.BasicAttacks)null);
               }
               ...
               if (currentAction != null) {
                  ...
                  if (!evaluatorComponent.hasTimedOut(dt)) {
                     ...
                     if (targetRef != null && targetRef.isValid() && !commandBuffer.getArchetype(targetRef).contains(DeathComponent.getComponentType())) {
                        Player targetPlayerComponent = (Player)commandBuffer.getComponent(targetRef, this.playerComponentType);
               ...
               if (evaluatorComponent.getOptionsBySubState().containsKey(currentSubState)) {
                  ...
                  if (!(utility < minRunUtility)) {
                     ...
                     List<Evaluator<CombatActionOption>.OptionHolder> currentStateOptions = (List)optionLists.get(currentSubState);
   ...
   public static class CombatConstructionData implements Component<EntityStore> {
      ...
      public Component<EntityStore> clone() {
         CombatConstructionData data = new CombatConstructionData();
