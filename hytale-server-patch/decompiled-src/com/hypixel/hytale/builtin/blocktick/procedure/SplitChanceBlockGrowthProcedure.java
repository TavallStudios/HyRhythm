...
public class SplitChanceBlockGrowthProcedure extends BasicChanceBlockGrowthProcedure {
   ...
   public SplitChanceBlockGrowthProcedure(int chanceMin, int chance, @Nonnull int[] chances, @Nonnull String[] data, boolean nextTicking) {
      super(chanceMin, chance, (String)null, nextTicking);
   ...
   protected boolean executeToBlock(@Nonnull World world, int worldX, int worldY, int worldZ, String to) {
      String block = (String)RandomUtil.roll(this.getRandom().nextInt(this.sumChances), this.data, this.chances);
      return super.executeToBlock(world, worldX, worldY, worldZ, block);
   }

   @Nonnull
   public String toString() {
      int var10000 = this.chanceMin;
      return "SplitChanceBlockGrowthProcedure{chanceMin=" + var10000 + ", chance=" + this.chance + ", to=" + this.to + ", nextTicking=" + this.nextTicking + ", chances=" + Arrays.toString(this.chances) + ", data=" + Arrays.toString(this.data) + ", sumChances=" + this.sumChances + "}";
   }

   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SplitChanceBlockGrowthProcedure.class, SplitChanceBlockGrowthProcedure::new, TickProcedure.BASE_CODEC).append(new KeyedCodec("NextIds", Codec.BSON_DOCUMENT), (proc, v, extraInfo) -> {
      ...
      }).addValidator(Validators.nonNull()).add()).addField(new KeyedCodec("ChanceMin", Codec.INTEGER), (proc, v) -> proc.chanceMin = v, (proc) -> proc.chanceMin)).addField(new KeyedCodec("Data", Codec.INTEGER), (proc, v) -> proc.chance = v, (proc) -> proc.chance)).addField(new KeyedCodec("NextTicking", Codec.BOOLEAN), (proc, v) -> proc.nextTicking = v, (proc) -> proc.nextTicking)).build();
   }
}
