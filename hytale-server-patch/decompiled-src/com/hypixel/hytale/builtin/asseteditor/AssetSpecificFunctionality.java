...
public class AssetSpecificFunctionality {
   ...
   private static final Map<UUID, PlayerPreviewData> activeWeatherPreviewMapping = new ConcurrentHashMap();
   ...
   public static void setup() {
      getEventRegistry().register((Class)LoadedAssetsEvent.class, ModelAsset.class, AssetSpecificFunctionality::onModelAssetLoaded);
      getEventRegistry().register((Class)LoadedAssetsEvent.class, Item.class, AssetSpecificFunctionality::onItemAssetLoaded);
      getEventRegistry().register((Class)AssetEditorActivateButtonEvent.class, "EquipItem", AssetSpecificFunctionality::onEquipItem);
                  ...
                  if (!path.toString().isEmpty()) {
                     ...
                     if (assetType instanceof AssetStoreTypeHandler && ((AssetStoreTypeHandler)assetType).getAssetStore().getAssetClass().equals(ModelAsset.class)) {
                        ...
                        if (modelAsset.getId().equals(id)) {
                           ...
                           ((EditorClient)editor.getKey()).getPacketHandler().write((ToClientPacket)packet);
                        }
                     }
                  }
               }
                  ...
                  for(Map.Entry<EditorClient, AssetPath> editor : clientOpenAssetPathMapping.entrySet()) {
                     ...
                     if (!path.toString().isEmpty()) {
                        ...
                        if (assetType instanceof AssetStoreTypeHandler && ((AssetStoreTypeHandler)assetType).getAssetStore().getAssetClass().equals(Item.class)) {
                           String id = (String)Item.getAssetStore().decodeFilePathKey(path);
                           if (item.getId().equals(id)) {
                              AssetEditorUpdateModelPreview packet = getModelPreviewPacketForItem((AssetPath)editor.getValue(), item);
                              if (packet != null) {
                                 ((EditorClient)editor.getKey()).getPacketHandler().write((ToClientPacket)packet);
                              }
                           }
   ...
   private static void onUpdateWeatherPreviewLockEvent(@Nonnull AssetEditorUpdateWeatherPreviewLockEvent event) {
      PlayerPreviewData currentPreviewSettings = (PlayerPreviewData)activeWeatherPreviewMapping.computeIfAbsent(event.getEditorClient().getUuid(), (k) -> new PlayerPreviewData());
   ...
   private static void equipItem(@Nonnull Path assetPath, @Nonnull EditorClient editorClient) {
      ...
      if (playerRef != null) {
         ...
         String key = (String)Item.getAssetStore().decodeFilePathKey(assetPath);
         Item item = (Item)Item.getAssetMap().getAsset(key);
         if (item == null) {
         ...
         if (ref != null && ref.isValid()) {
            ...
            world.execute(() -> {
               ...
               if (modelAsset == null) {
                  ...
                  com.hypixel.hytale.server.core.asset.type.model.config.Model model = com.hypixel.hytale.server.core.asset.type.model.config.Model.createRandomScaleModel(modelAsset);
   ...
   private static void onRequestLocalizationKeyDataSet(@Nonnull AssetEditorFetchAutoCompleteDataEvent event) {
      ...
      event.setResults((String[])results.toArray((x$0) -> new String[x$0]));
   }
   ...
   private static void onRequestBlockGroupsDataSet(@Nonnull AssetEditorFetchAutoCompleteDataEvent event) {
      ...
      for(String group : com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockType.getAssetMap().getGroups()) {
   ...
   private static void onRequestItemCategoriesDataSet(@Nonnull AssetEditorRequestDataSetEvent event) {
      ...
      if (itemModule.isDisabled()) {
         ...
         event.setResults((String[])itemModule.getFlatItemCategoryList().toArray((x$0) -> new String[x$0]));
      }
   }
   ...
   private static void onSelectAsset(@Nonnull AssetEditorSelectAssetEvent event) {
      ...
      if (MODEL_ASSET_ID.equals(assetType)) {
         String key = (String)ModelAsset.getAssetStore().decodeFilePathKey(event.getAssetFilePath().path());
         ModelAsset modelAsset = (ModelAsset)ModelAsset.getAssetMap().getAsset(key);
         if (modelAsset != null) {
            Model modelPacket = com.hypixel.hytale.server.core.asset.type.model.config.Model.createUnitScaleModel(modelAsset).toPacket();
            event.getEditorClient().getPacketHandler().write((ToClientPacket)(new AssetEditorUpdateModelPreview(event.getAssetFilePath().toPacket(), modelPacket, (BlockType)null, DEFAULT_PREVIEW_CAMERA_SETTINGS)));
         }
      }

      if (ITEM_ASSET_ID.equals(assetType)) {
         AssetPath assetPath = event.getAssetFilePath();
         String key = (String)Item.getAssetStore().decodeFilePathKey(assetPath.path());
         Item item = (Item)Item.getAssetMap().getAsset(key);
         if (item != null) {
   ...
   public static AssetEditorUpdateModelPreview getModelPreviewPacketForItem(@Nonnull AssetPath assetPath, @Nullable Item item) {
      if (item == null) {
         ...
         camera.cameraOrientation = new Vector3f((float)(-Math.toRadians((double)rotation.x)), (float)(-Math.toRadians((double)rotation.y)), (float)(-Math.toRadians((double)rotation.z)));
         if (item.getBlockId() != null) {
            com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockType blockType = (com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockType)((BlockTypeAssetMap)com.hypixel.hytale.server.core.asset.type.blocktype.config.BlockType.getAssetStore().getAssetMap()).getAsset(item.getBlockId());
            if (blockType != null) {
               camera.modelScale *= blockType.getCustomModelScale();
               return new AssetEditorUpdateModelPreview(assetPath.toPacket(), (Model)null, blockType.toPacket(), camera);
            }
         }

         Model modelPacket = convertToModelPacket(item);
         return new AssetEditorUpdateModelPreview(assetPath.toPacket(), modelPacket, (BlockType)null, camera);
      }
   }
   ...
   public static AssetIconProperties getDefaultItemIconProperties(@Nonnull Item item) {
      if (item.getWeapon() != null) {
         ...
         AssetIconProperties var10000;
         switch (item.getArmor().getArmorSlot()) {
            case Chest -> var10000 = new AssetIconProperties(0.5F, new Vector2f(0.0F, -5.0F), new Vector3f(22.5F, 45.0F, 22.5F));
            case Head -> var10000 = new AssetIconProperties(0.5F, new Vector2f(0.0F, -3.0F), new Vector3f(22.5F, 45.0F, 22.5F));
            case Legs -> var10000 = new AssetIconProperties(0.5F, new Vector2f(0.0F, -25.8F), new Vector3f(22.5F, 45.0F, 22.5F));
            case Hands -> var10000 = new AssetIconProperties(0.92F, new Vector2f(0.0F, -10.8F), new Vector3f(22.5F, 45.0F, 22.5F));
