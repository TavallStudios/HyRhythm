...
public class AssetEditorPacketHandler extends GenericPacketHandler {
   ...
   public String getIdentifier() {
      ...
      return "{Editor(" + var10000 + "), " + String.valueOf(this.editorClient.getUuid()) + ", " + this.editorClient.getUsername() + "}";
   }
   ...
   public void handle(@Nonnull AssetEditorCreateAsset packet) {
      if (!this.lacksPermission(packet.token)) {
         ...
         AssetEditorPlugin.get().handleCreateAsset(this.editorClient, new AssetPath(packet.path.pack, Path.of(packet.path.path)), packet.data, packet.rebuildCaches, packet.buttonId, packet.token);
      }
   }
   ...
   public void handle(@Nonnull AssetEditorFetchJsonAssetWithParents packet) {
      if (!this.lacksPermission(packet.token)) {
         ...
         AssetEditorPlugin.get().handleFetchJsonAssetWithParents(this.editorClient, new AssetPath(packet.path.pack, Path.of(packet.path.path)), packet.isFromOpenedTab, packet.token);
      }
   }
   ...
   public void handle(@Nonnull AssetEditorUpdateJsonAsset packet) {
      if (!this.lacksPermission(packet.token)) {
         ...
         AssetEditorPlugin.get().handleJsonAssetUpdate(this.editorClient, new AssetPath(packet.path.pack, Path.of(packet.path.path)), packet.assetType, packet.assetIndex, packet.commands, packet.token);
      }
   }

   public void handle(@Nonnull AssetEditorFetchAutoCompleteData packet) {
      if (!this.lacksPermission(packet.token)) {
         CompletableFutureUtil._catch(((CompletableFuture)HytaleServer.get().getEventBus().dispatchForAsync(AssetEditorFetchAutoCompleteDataEvent.class, packet.dataset).dispatch(new AssetEditorFetchAutoCompleteDataEvent(this.editorClient, packet.dataset, packet.query))).thenAccept((event) -> {
   ...
   public void handle(@Nonnull AssetEditorActivateButton packet) {
      if (!this.lacksPermission()) {
         ...
         IEventDispatcher<AssetEditorActivateButtonEvent, AssetEditorActivateButtonEvent> dispatch = HytaleServer.get().getEventBus().dispatchFor(AssetEditorActivateButtonEvent.class, packet.buttonId);
   ...
   public void handle(@Nonnull AssetEditorRequestDataset packet) {
      if (!this.lacksPermission()) {
         CompletableFutureUtil._catch(((CompletableFuture)HytaleServer.get().getEventBus().dispatchForAsync(AssetEditorRequestDataSetEvent.class, packet.name).dispatch(new AssetEditorRequestDataSetEvent(this.editorClient, packet.name, (String[])null))).thenAccept((event) -> {
            if (event.getResults() == null) {
   ...
   public void handle(@Nonnull AssetEditorUpdateWeatherPreviewLock packet) {
      if (!this.lacksPermission()) {
         IEventDispatcher<AssetEditorUpdateWeatherPreviewLockEvent, AssetEditorUpdateWeatherPreviewLockEvent> dispatch = HytaleServer.get().getEventBus().dispatchFor(AssetEditorUpdateWeatherPreviewLockEvent.class);
         if (dispatch.hasListener()) {
   ...
   public void handle(@Nonnull Disconnect packet) {
      switch (packet.type) {
         case Disconnect -> this.disconnectReason.setClientDisconnectType(DisconnectType.Disconnect);
         case Crash -> this.disconnectReason.setClientDisconnectType(DisconnectType.Crash);
      }
