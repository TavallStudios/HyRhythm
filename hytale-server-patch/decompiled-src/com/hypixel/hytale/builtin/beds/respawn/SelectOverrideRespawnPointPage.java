...
public class SelectOverrideRespawnPointPage extends RespawnPointPage {
   ...
   @Nonnull
   private static final Value<String> DEFAULT_RESPAWN_BUTTON_STYLE = Value.<String>ref("Pages/OverrideRespawnPointButton.ui", "DefaultRespawnButtonStyle");
   @Nonnull
   private static final Value<String> SELECTED_RESPAWN_BUTTON_STYLE = Value.<String>ref("Pages/OverrideRespawnPointButton.ui", "SelectedRespawnButtonStyle");
   @Nonnull
   ...
   public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, @Nonnull Store<EntityStore> store) {
      ...
      if (playerRefComponent != null) {
         HeadRotation rotationComponent = (HeadRotation)store.getComponent(ref, HeadRotation.getComponentType());
         if (rotationComponent != null) {
            float lookYaw = rotationComponent.getRotation().getYaw();
            double direction = Math.toDegrees((double)lookYaw);
            ...
            for(int i = 0; i < this.respawnPoints.length; ++i) {
               ...
               double angle = Math.atan2((double)(respawnPointPosition.z - this.respawnPointToAddPosition.z), (double)(respawnPointPosition.x - this.respawnPointToAddPosition.x));
   ...
   public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull RespawnPointPage.RespawnPointEventData data) {
      if (data.getIndex() != -1) {
         ...
         this.setRespawnPointForPlayer(ref, store, this.respawnPointToAddPosition, this.respawnPointToAdd, data.getRespawnPointName(), new PlayerRespawnPointData[]{this.respawnPoints[this.selectedRespawnPointIndex]});
