...
public abstract class RespawnPointPage extends InteractiveCustomUIPage<RespawnPointEventData> {
   @Nonnull
   ...
   public RespawnPointPage(@Nonnull PlayerRef playerRef, @Nonnull InteractionType interactionType) {
      super(playerRef, interactionType == InteractionType.Use ? CustomPageLifetime.CanDismissOrCloseThroughInteraction : CustomPageLifetime.CanDismiss, RespawnPointPage.RespawnPointEventData.CODEC);
   }
         ...
         if (playerComponent != null) {
            ...
            if (chunk != null) {
               ...
               if (blockType != null) {
                  ...
                  BlockBoundingBoxes blockBoundingBoxAsset = (BlockBoundingBoxes)BlockBoundingBoxes.getAssetMap().getAsset(blockType.getHitboxTypeIndex());
         ...
         for(int i = 0; i < respawnPointsToRemove.length; ++i) {
            ...
            for(int j = 0; j < respawnPoints.length; ++j) {
               ...
               if (respawnPoint.getBlockPosition().equals(respawnPointToRemove.getBlockPosition())) {
                  respawnPoints = (PlayerRespawnPointData[])ArrayUtil.remove(respawnPoints, j);
                  break;
               }
            }
            ...
            if (chunkReference != null && chunkReference.isValid()) {
               BlockComponentChunk blockComponentChunk = (BlockComponentChunk)chunkStore.getStore().getComponent(chunkReference, BlockComponentChunk.getComponentType());
               if (blockComponentChunk != null) {
                  int blockIndex = ChunkUtil.indexBlockInColumn(position.x, position.y, position.z);
                  Ref<ChunkStore> blockRef = blockComponentChunk.getEntityReference(blockIndex);
                  if (blockRef != null && blockRef.isValid()) {
                     RespawnBlock respawnBlock = (RespawnBlock)chunkStore.getStore().getComponent(blockRef, RespawnBlock.getComponentType());
                     if (respawnBlock != null) {
                        respawnBlock.setOwnerUUID((UUID)null);
                        WorldChunk worldChunk = (WorldChunk)chunkStore.getStore().getComponent(chunkReference, WorldChunk.getComponentType());
                        if (worldChunk != null) {
   ...
   public static class RespawnPointEventData {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(RespawnPointEventData.class, RespawnPointEventData::new).append(new KeyedCodec("Action", Codec.STRING), (entry, s) -> entry.action = s, (entry) -> entry.action).add()).append(new KeyedCodec("Index", Codec.STRING), (entry, s) -> {
            entry.indexStr = s;
            entry.index = Integer.parseInt(s);
         }, (entry) -> entry.indexStr).add()).append(new KeyedCodec("@RespawnPointName", Codec.STRING), (entry, s) -> entry.respawnPointName = s, (entry) -> entry.respawnPointName).add()).build();
      }
   }
}
