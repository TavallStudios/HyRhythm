...
public class DeployableOwnerComponent implements Component<EntityStore> {
      ...
      if (current > limit) {
         ...
         for(Pair<String, Ref<EntityStore>> deployablePair : this.deployables) {
            if (((String)deployablePair.key()).equals(id)) {
               this.deployablesForDestruction.add((Ref)deployablePair.value());
   ...
   private void handleGlobalDeployableLimit(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> owner) {
      ...
      for(Pair<String, Ref<EntityStore>> deployablePair : this.deployables) {
         DeployableComponent deployableComponent = (DeployableComponent)store.getComponent((Ref)deployablePair.value(), DeployableComponent.getComponentType());
      ...
      if (current > 1) {
         ...
         for(Pair<String, Ref<EntityStore>> deployablePair : this.deployables) {
            Ref<EntityStore> deployableRef = (Ref)deployablePair.value();
