...
public class SpawnDeployableFromRaycastInteraction extends SimpleInstantInteraction {
   ...
   protected void firstRun(@Nonnull InteractionType type, @Nonnull InteractionContext context, @Nonnull CooldownHandler cooldownHandler) {
      ...
      if (!this.canAfford(context.getEntity(), commandBuffer)) {
         ...
         if (raycastHit == null) {
            ...
            raycastHit = new Position((double)((float)position.x), (double)((float)position.y), (double)((float)position.z));
         }
         ...
         com.hypixel.hytale.protocol.Vector3f spawnPosition = new com.hypixel.hytale.protocol.Vector3f((float)raycastHit.x, (float)raycastHit.y, (float)raycastHit.z);
         Vector3f norm = new Vector3f(raycastNormal.x, raycastNormal.y, raycastNormal.z);
         if (correctedRaycastDistance > 0.0F && correctedRaycastDistance <= this.maxPlacementDistance && (this.config.getAllowPlaceOnWalls() || isSurface(norm))) {
            Direction attackerRot = clientState.attackerRot;
            Vector3f rot = new Vector3f(0.0F, attackerRot.yaw, 0.0F);
            DeployablesUtils.spawnDeployable(commandBuffer, store, this.config, entityRef, new Vector3f(spawnPosition.x, spawnPosition.y, spawnPosition.z), rot, "UP");
         }
   ...
   protected boolean canAfford(@Nonnull Ref<EntityStore> entityRef, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      if (this.entityStats != null && !this.entityStats.isEmpty()) {
         EntityStatMap entityStatMapComponent = (EntityStatMap)componentAccessor.getComponent(entityRef, EntityStatMap.getComponentType());
