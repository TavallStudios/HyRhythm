...
public abstract class DeployableConfig implements NetworkSerializable<com.hypixel.hytale.protocol.DeployableConfig> {
   @Nonnull
   public static final CodecMapCodec<DeployableConfig> CODEC = new CodecMapCodec<DeployableConfig>("Type");
   @Nonnull
   ...
   private String id;
   private int maxLiveCount = 2147483647;
   ...
   private static void processConfig(@Nonnull DeployableConfig config) {
      ...
      if (config.generatedModel != null) {
         config.generatedModel = Model.createScaledModel((ModelAsset)ModelAsset.getAssetMap().getAsset(config.model), config.modelScale);
      }

      if (config.generatedModelPreview != null) {
         config.generatedModelPreview = Model.createScaledModel((ModelAsset)ModelAsset.getAssetMap().getAsset(config.modelPreview), config.modelScale);
      }
   ...
   protected static void playAnimation(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull DeployableConfig config, @Nonnull String animationSetKey) {
      NetworkId networkIdComponent = (NetworkId)store.getComponent(ref, NetworkId.getComponentType());
      if (networkIdComponent != null) {
         DeployablesUtils.playAnimation(store, networkIdComponent.getId(), ref, config, AnimationSlot.Action, (String)null, animationSetKey);
      }
   }
   ...
   protected static void stopAnimation(@Nonnull Store<EntityStore> store, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, int index) {
      ...
      if (ref.isValid()) {
         NetworkId networkIdComponent = (NetworkId)archetypeChunk.getComponent(index, NetworkId.getComponentType());
         if (networkIdComponent != null) {
   ...
   public Model getModel() {
      if (this.generatedModel != null) {
         ...
         this.generatedModel = Model.createScaledModel((ModelAsset)ModelAsset.getAssetMap().getAsset(this.model), this.modelScale);
         return this.generatedModel;
      }
   }
   ...
   public Model getModelPreview() {
      if (this.modelPreview == null) {
         ...
         this.generatedModelPreview = Model.createScaledModel((ModelAsset)ModelAsset.getAssetMap().getAsset(this.modelPreview), this.modelScale);
         return this.generatedModelPreview;
      }
   }
   ...
   public static class StatConfig {
      ...
      private float max;
      private float initial = 3.4028235E38F;
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(StatConfig.class, StatConfig::new).documentation("Initial and maximum values for a stat.")).append(new KeyedCodec("Max", Codec.FLOAT), (config, f) -> config.max = f, (config) -> config.max).addValidator(Validators.nonNull()).addValidator(Validators.greaterThan(0.0F)).documentation("The maximum value for the stat.").add()).append(new KeyedCodec("Initial", Codec.FLOAT), (config, f) -> config.initial = f, (config) -> config.initial).documentation("The initial value for the stat. If omitted, will be set to max.").add()).build();
      }
   }
}
