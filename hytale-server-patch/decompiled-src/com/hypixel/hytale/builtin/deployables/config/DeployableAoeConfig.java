...
public class DeployableAoeConfig extends DeployableConfig {
   ...
   protected void handleDetection(@Nonnull final Store<EntityStore> store, @Nonnull final CommandBuffer<EntityStore> commandBuffer, @Nonnull final Ref<EntityStore> deployableRef, @Nonnull DeployableComponent deployableComponent, @Nonnull Vector3d position, float radius, @Nonnull final DamageCause damageCause) {
   ...
   protected void applyEffectToTarget(@Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> targetRef) {
      if (this.effectsToApply != null) {
         EffectControllerComponent effectControllerComponent = (EffectControllerComponent)store.getComponent(targetRef, EffectControllerComponent.getComponentType());
