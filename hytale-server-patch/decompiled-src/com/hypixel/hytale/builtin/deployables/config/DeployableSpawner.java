...
public class DeployableSpawner implements JsonAssetWithMap<String, DefaultAssetMap<String, DeployableSpawner>> {
   ...
   public static DefaultAssetMap<String, DeployableSpawner> getAssetMap() {
      if (ASSET_MAP == null) {
         ASSET_MAP = (DefaultAssetMap)AssetRegistry.getAssetStore(DeployableSpawner.class).getAssetMap();
      }
   ...
   static {
      CODEC = ((AssetBuilderCodec.Builder)((AssetBuilderCodec.Builder)AssetBuilderCodec.builder(DeployableSpawner.class, DeployableSpawner::new, Codec.STRING, (t, k) -> t.id = k, (t) -> t.id, (asset, data) -> asset.data = data, (asset) -> asset.data).append(new KeyedCodec("Config", DeployableConfig.CODEC), (i, s) -> i.config = s, (i) -> i.config).addValidator(Validators.nonNull()).add()).append(new KeyedCodec("PositionOffsets", new ArrayCodec(Vector3d.CODEC, (x$0) -> new Vector3d[x$0])), (i, s) -> i.positionOffsets = s, (i) -> i.positionOffsets).add()).build();
   }
}
