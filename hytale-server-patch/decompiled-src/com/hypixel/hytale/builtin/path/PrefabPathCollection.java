...
public class PrefabPathCollection {
   ...
   private final Int2ObjectMap<PathSet> pathsByFriendlyName = new Int2ObjectOpenHashMap();
   ...
   public IPrefabPath getNearestPrefabPath(int nameIndex, @Nonnull Vector3d position, Set<UUID> disallowedPaths, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      PathSet set = (PathSet)this.pathsByFriendlyName.getOrDefault(nameIndex, (Object)null);
      return set == null ? null : set.getNearestPath(position, disallowedPaths, componentAccessor);
   }

   public IPrefabPath getPath(UUID id) {
      return (IPrefabPath)this.paths.getOrDefault(id, (Object)null);
   }
   ...
   public IPrefabPath getOrConstructPath(@Nonnull UUID id, @Nonnull String name, @Nonnull Int2ObjectConcurrentHashMap.IntBiObjFunction<UUID, String, IPrefabPath> pathGenerator) {
      IPrefabPath path = (IPrefabPath)this.paths.computeIfAbsent(id, (k) -> {
      ...
      PathSet set = (PathSet)this.pathsByFriendlyName.computeIfAbsent(nameIndex, (s) -> new PathSet());
      set.add(path);
      return path;
   }
   ...
   public void removePath(UUID id) {
      IPrefabPath removed = (IPrefabPath)this.paths.remove(id);
   ...
   private static class PathSet {
      ...
      public IPrefabPath getNearestPath(@Nonnull Vector3d position, @Nullable Set<UUID> disallowedPaths, ComponentAccessor<EntityStore> componentAccessor) {
         ...
         for(int i = 0; i < this.paths.size(); ++i) {
            IPrefabPath path = (IPrefabPath)this.paths.get(i);
            if (disallowedPaths == null || !disallowedPaths.contains(path.getId())) {
