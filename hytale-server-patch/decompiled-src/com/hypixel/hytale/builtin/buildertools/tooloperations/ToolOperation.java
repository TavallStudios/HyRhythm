...
public abstract class ToolOperation implements TriIntObjPredicate<Void> {
   ...
   public static PrototypePlayerBuilderToolSettings getOrCreatePrototypeSettings(UUID playerUuid) {
      PrototypePlayerBuilderToolSettings settings = (PrototypePlayerBuilderToolSettings)PROTOTYPE_TOOL_SETTINGS.get(playerUuid);
      if (settings == null) {
   ...
   @Nonnull
   public static List<Vector3i> calculateInterpolatedPositions(@Nullable Vector3i lastPosition, @Nonnull Vector3i currentPosition, int brushWidth, int brushHeight, int brushSpacing) {
      ArrayList<Vector3i> positions = new ArrayList();
      if (lastPosition == null) {
         positions.add(currentPosition);
         return positions;
      } else {
         double dx = (double)(currentPosition.getX() - lastPosition.getX());
         double dy = (double)(currentPosition.getY() - lastPosition.getY());
         double dz = (double)(currentPosition.getZ() - lastPosition.getZ());
         double distance = Math.sqrt(dx * dx + dy * dy + dz * dz);
         if (brushSpacing == 0) {
            float maxBrushDimension = (float)Math.max(brushWidth, brushHeight);
            float spacingThreshold = Math.max(1.0F, maxBrushDimension * 0.5F);
            if (distance <= (double)spacingThreshold) {
               positions.add(currentPosition);
               return positions;
            }

            int steps = (int)Math.ceil(distance / (double)spacingThreshold);

            for(int i = 1; i <= steps; ++i) {
               float t = (float)i / (float)steps;
               int interpX = (int)Math.round((double)lastPosition.getX() + dx * (double)t);
               int interpY = (int)Math.round((double)lastPosition.getY() + dy * (double)t);
               int interpZ = (int)Math.round((double)lastPosition.getZ() + dz * (double)t);
               positions.add(new Vector3i(interpX, interpY, interpZ));
            }
         } else if (distance >= (double)brushSpacing) {
            positions.add(currentPosition);
         }
   ...
   public int getBrushDensity() {
      Object densityValue = this.args.tool().get("BrushDensity");
      if (densityValue instanceof Number number) {
      ...
      if (shapeRange <= 1 && shapeHeight <= 1) {
         ...
         switch (shape) {
            ...
            case Torus:
               ...
               BlockTorusUtil.forEachBlock(x, y, z, radiusXZ, minorRadius, shapeThickness, capped, (Object)null, operation);
         }
      }
   ...
   @Nonnull
   private static Vector3i getOffsets(int width, int height, boolean originRotation, BrushOrigin origin, @Nonnull Transform transform, @Nonnull Vector3i vector, boolean applyBottomOriginFix) {
   ...
   @Nonnull
   public static ToolOperation fromPacket(@Nonnull Ref<EntityStore> ref, @Nonnull Player player, @Nonnull BuilderToolOnUseInteraction packet, @Nonnull ComponentAccessor<EntityStore> componentAccessor) throws Exception {
      ...
      if (builderTool == null) {
         ...
         OperationFactory factory = (OperationFactory)OPERATIONS.get(toolType);
         if (factory == null) {
