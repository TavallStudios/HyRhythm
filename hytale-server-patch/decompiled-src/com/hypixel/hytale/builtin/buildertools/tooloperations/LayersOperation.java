...
public class LayersOperation extends ToolOperation {
   ...
   public LayersOperation(@Nonnull Ref<EntityStore> ref, @Nonnull Player player, @Nonnull BuilderToolOnUseInteraction packet, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      switch ((String)this.args.tool().get("aDirection")) {
         case "Up" -> this.depthDirection = Vector3i.UP;
         case "Down" -> this.depthDirection = Vector3i.DOWN;
         case "North" -> this.depthDirection = Vector3i.NORTH;
         case "South" -> this.depthDirection = Vector3i.SOUTH;
         case "East" -> this.depthDirection = Vector3i.EAST;
         case "West" -> this.depthDirection = Vector3i.WEST;
         case "Camera" -> this.depthDirection = headRotationComponent.getAxisDirection();
         default -> this.depthDirection = Vector3i.DOWN;
      }
