...
public class LayerCommand extends AbstractPlayerCommand {
   ...
   public LayerCommand() {
      ...
      this.layerDirectionArg = this.withRequiredArg("direction", "server.commands.layer.direction.desc", ArgTypes.STRING);
      this.layersArg = this.withListRequiredArg("layers", "server.commands.layer.layers.desc", ArgTypes.LAYER_ENTRY_TYPE);
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      ...
      if (PrototypePlayerBuilderToolSettings.isOkayToDoCommandsOnSelection(ref, playerComponent, store)) {
         String direction = ((String)this.layerDirectionArg.get(context)).toLowerCase();
         List<Pair<Integer, String>> layers = (List)this.layersArg.get(context);
            ...
            if (directionValid) {
               BuilderToolsPlugin.addToQueue(playerComponent, playerRef, (r, s, componentAccessor) -> {
                  ...
                  if (direction.equalsIgnoreCase("camera")) {
                     ...
                     layerDirection = (Vector3i)directions.get(direction);
                  }
   ...
   static {
      directions = Map.of("up", Vector3i.UP, "down", Vector3i.DOWN, "north", Vector3i.NORTH, "south", Vector3i.SOUTH, "east", Vector3i.EAST, "west", Vector3i.WEST);
   }
}
