...
public class HotbarSwitchCommand extends AbstractPlayerCommand {
   ...
   public HotbarSwitchCommand() {
      ...
      this.hotbarSlotArg = (RequiredArg)this.withRequiredArg("hotbarSlot", "server.commands.hotbar.hotbarSlot.desc", ArgTypes.INTEGER).addValidator(Validators.range(0, 9));
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      ...
      if ((Boolean)this.saveInsteadOfLoadArg.get(context)) {
         hotbarManager.saveHotbar(ref, ((Integer)this.hotbarSlotArg.get(context)).shortValue(), store);
      } else {
         hotbarManager.loadHotbar(ref, ((Integer)this.hotbarSlotArg.get(context)).shortValue(), store);
      }
