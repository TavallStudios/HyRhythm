...
public class RedoCommand extends AbstractPlayerCommand {
   public RedoCommand() {
      ...
      this.addUsageVariant(new RedoWithCountCommand());
   }
   ...
   private static class RedoWithCountCommand extends AbstractPlayerCommand {
      ...
      public RedoWithCountCommand() {
         ...
         this.countArg = this.withRequiredArg("count", "server.commands.redo.count.desc", ArgTypes.INTEGER);
         this.setPermissionGroup(GameMode.Creative);
         this.requirePermission("hytale.editor.history");
      }

      protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
         RedoCommand.executeRedo(store, ref, (Integer)this.countArg.get(context));
      }
   }
}
