...
public class FlipCommand extends AbstractPlayerCommand {
   public FlipCommand() {
      ...
      this.addUsageVariant(new FlipWithDirectionCommand());
   }

   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      executeFlip(store, ref, playerRef, (RelativeDirection)null);
   }
   ...
   private static class FlipWithDirectionCommand extends AbstractPlayerCommand {
      ...
      public FlipWithDirectionCommand() {
         ...
         this.directionArg = this.withRequiredArg("direction", "server.commands.flip.direction.desc", RelativeDirection.ARGUMENT_TYPE);
         this.setPermissionGroup(GameMode.Creative);
      }

      protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
         FlipCommand.executeFlip(store, ref, playerRef, (RelativeDirection)this.directionArg.get(context));
      }
   }
}
