...
public class ContractSelectionCommand extends AbstractPlayerCommand {
   ...
   public ContractSelectionCommand() {
      ...
      this.distanceArg = this.withRequiredArg("distance", "server.commands.contract.arg.distance.desc", ArgTypes.INTEGER);
      this.axisArg = this.withListOptionalArg("axis", "server.commands.contract.arg.axis.desc", ArgTypes.forEnum("Axis", Axis.class));
      this.setPermissionGroup(GameMode.Creative);
      this.addAliases(new String[]{"contract"});
   }
   ...
   protected void execute(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      ...
      if (PrototypePlayerBuilderToolSettings.isOkayToDoCommandsOnSelection(ref, playerComponent, store)) {
         ...
         int distance = (Integer)this.distanceArg.get(context);
         List<Vector3i> directions = new ObjectArrayList();
         if (this.axisArg.provided(context)) {
            for(Axis axis : (List)this.axisArg.get(context)) {
