...
public class BuilderToolsUserData implements Component<EntityStore> {
   ...
   public static BuilderToolsUserData get(@Nonnull Player player) {
      BuilderToolsUserData userData = (BuilderToolsUserData)player.toHolder().getComponent(getComponentType());
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(BuilderToolsUserData.class, BuilderToolsUserData::new).append(new KeyedCodec("SelectionHistory", Codec.BOOLEAN), BuilderToolsUserData::setRecordSelectionHistory, BuilderToolsUserData::isRecordingSelectionHistory).addValidator(Validators.nonNull()).documentation("Controls whether changes to the block selection box are recorded in the undo/redo history.").add()).build();
   }
}
