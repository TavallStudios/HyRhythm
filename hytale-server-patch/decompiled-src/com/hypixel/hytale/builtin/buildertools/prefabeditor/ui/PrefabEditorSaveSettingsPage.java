...
public class PrefabEditorSaveSettingsPage extends InteractiveCustomUIPage<PageData> {
   ...
   private static final Value<String> BUTTON_SELECTED = Value.<String>ref("Pages/BasicTextButton.ui", "SelectedLabelStyle");
   @Nonnull
   ...
   public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder, @Nonnull Store<EntityStore> store) {
      ...
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#MainPage #SaveButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.Save.name()).append("@PrefabsToSave", "#MainPage #PrefabsToSave #Input.Value").append("@Entities", "#MainPage #Entities #CheckBox.Value").append("@Empty", "#MainPage #Empty #CheckBox.Value").append("@Overwrite", "#MainPage #Overwrite #CheckBox.Value").append("@ClearSupport", "#MainPage #ClearSupport #CheckBox.Value"));
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#MainPage #CancelButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.Cancel.name()));
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#MainPage #PrefabsToSave #BrowseButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.OpenBrowser.name()));
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#MainPage #PrefabsToSave #SelectAllButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.SelectAll.name()));
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#MainPage #PrefabsToSave #SelectEditedButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.SelectEdited.name()));
      eventBuilder.addEventBinding(CustomUIEventBindingType.ValueChanged, "#BrowserPage #SearchInput", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.BrowserSearch.name()).append("@BrowserSearch", "#BrowserPage #SearchInput.Value"), false);
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#BrowserPage #BrowserButtons #SelectAllBrowserButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.BrowserSelectAll.name()));
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#BrowserPage #BrowserButtons #ConfirmButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.ConfirmBrowser.name()));
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#BrowserPage #BrowserButtons #CancelButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.CancelBrowser.name()));
      eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#SavingPage #BackButton", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.BackFromSaving.name()));
   ...
   public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull PageData data) {
      ...
      switch (data.action.ordinal()) {
         case 0:
            ...
            CompletableFuture.allOf(saveFutures).thenAccept((unused) -> {
               int successes = 0;
               int failures = 0;

               for(CompletableFuture<Boolean> future : saveFutures) {
                  if ((Boolean)future.join()) {
                     ++successes;
                  } else {
                     ++failures;
                  }
         ...
         case 1:
            ...
            break;
         case 2:
            String allPaths = (String)this.prefabEditSession.getLoadedPrefabMetadata().values().stream().map((m) -> m.getPrefabPath().toString().replace('\\', '/')).collect(Collectors.joining(","));
            UICommandBuilder commandBuilder = new UICommandBuilder();
            commandBuilder.set("#MainPage #PrefabsToSave #Input.Value", allPaths);
            this.sendUpdate(commandBuilder);
            break;
         case 3:
            String editedPaths = (String)this.prefabEditSession.getLoadedPrefabMetadata().values().stream().filter(PrefabEditingMetadata::isDirty).map((m) -> m.getPrefabPath().toString().replace('\\', '/')).collect(Collectors.joining(","));
            UICommandBuilder commandBuilder = new UICommandBuilder();
   ...
   private void buildPrefabList(@Nonnull UICommandBuilder commandBuilder, @Nonnull UIEventBuilder eventBuilder) {
      ...
      if (!loadedPrefabs.isEmpty()) {
         ...
         if (!this.browserSearchQuery.isEmpty()) {
            ...
            Stream var10000 = matchScores.keySet().stream();
            Objects.requireNonNull(matchScores);
            prefabsToDisplay = (List)var10000.sorted(Comparator.comparingInt(matchScores::getInt).reversed()).collect(Collectors.toList());
         } else {
            prefabsToDisplay = (List)loadedPrefabs.values().stream().sorted(Comparator.comparing((m) -> m.getPrefabPath().getFileName().toString().toLowerCase())).collect(Collectors.toList());
         }
         ...
         for(PrefabEditingMetadata metadata : prefabsToDisplay) {
            ...
            eventBuilder.addEventBinding(CustomUIEventBindingType.Activating, "#BrowserPage #FileList[" + buttonIndex + "]", (new EventData()).append("Action", PrefabEditorSaveSettingsPage.Action.BrowserTogglePrefab.name()).append("PrefabUuid", metadata.getUuid().toString()));
            ++buttonIndex;
         }
   ...
   protected static class PageData {
      ...
      static {
         CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(PageData.class, PageData::new).append(new KeyedCodec("Action", new EnumCodec(Action.class, EnumCodec.EnumStyle.LEGACY)), (o, action) -> o.action = action, (o) -> o.action).add()).append(new KeyedCodec("@PrefabsToSave", Codec.STRING), (o, prefabsToSave) -> o.prefabsToSave = prefabsToSave, (o) -> o.prefabsToSave).add()).append(new KeyedCodec("@Entities", Codec.BOOLEAN), (o, entities) -> o.entities = entities, (o) -> o.entities).add()).append(new KeyedCodec("@Empty", Codec.BOOLEAN), (o, empty) -> o.empty = empty, (o) -> o.empty).add()).append(new KeyedCodec("@Overwrite", Codec.BOOLEAN), (o, overwrite) -> o.overwrite = overwrite, (o) -> o.overwrite).add()).append(new KeyedCodec("@ClearSupport", Codec.BOOLEAN), (o, clearSupport) -> o.clearSupport = clearSupport, (o) -> o.clearSupport).add()).append(new KeyedCodec("@BrowserSearch", Codec.STRING), (o, browserSearchStr) -> o.browserSearchStr = browserSearchStr, (o) -> o.browserSearchStr).add()).append(new KeyedCodec("PrefabUuid", Codec.STRING), (o, prefabUuid) -> o.prefabUuid = prefabUuid, (o) -> o.prefabUuid).add()).build();
      }
   }
