...
public class PrefabEditSession implements Resource<EntityStore> {
   ...
   public void addPrefab(@Nonnull Path prefabPath, @Nonnull Vector3i minPoint, @Nonnull Vector3i maxPoint, @Nonnull Vector3i anchorPoint, @Nonnull Vector3i pastePosition) {
      ...
      PrefabEditingMetadata prefabEditingMetadata = new PrefabEditingMetadata(prefabPath, minPoint, maxPoint, anchorPoint, pastePosition, Universe.get().getWorld(this.worldName));
   ...
   public PrefabEditingMetadata updatePrefabBounds(@Nonnull UUID prefab, @Nonnull Vector3i newMin, @Nonnull Vector3i newMax) {
      PrefabEditingMetadata prefabEditingMetadata = (PrefabEditingMetadata)this.loadedPrefabMetadata.get(prefab);
   ...
   public MapMarker[] createPrefabMarkers() {
      return (MapMarker[])this.loadedPrefabMetadata.values().stream().map(PrefabEditSession::createPrefabMarker).toArray((x$0) -> new MapMarker[x$0]);
   }
   ...
   public static MapMarker createPrefabMarker(@Nonnull PrefabEditingMetadata metadata) {
      ...
      return (new MapMarkerBuilder("prefab-" + String.valueOf(metadata.getUuid()), "Prefab.png", transform)).withCustomName(prefabName).build();
   }
   ...
   static {
      ...
      }, (editSession) -> (PrefabEditingMetadata[])editSession.loadedPrefabMetadata.values().toArray((x$0) -> new PrefabEditingMetadata[x$0])).add()).afterDecode((editSession) -> {
