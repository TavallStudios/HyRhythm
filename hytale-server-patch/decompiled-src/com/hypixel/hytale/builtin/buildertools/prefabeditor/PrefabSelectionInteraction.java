...
public class PrefabSelectionInteraction extends SimpleInstantInteraction {
   ...
   private static Vector3i getTargetLocation(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (targetBlock != null) {
         ...
         if (targetEntityRef != null && targetEntityRef.isValid()) {
            TransformComponent entityTransformComponent = (TransformComponent)componentAccessor.getComponent(targetEntityRef, TransformComponent.getComponentType());
   ...
   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(PrefabSelectionInteraction.class, PrefabSelectionInteraction::new, SimpleInstantInteraction.CODEC).documentation("Interaction that handles the selection functionally for the prefab selection tool.")).build();
   }
}
