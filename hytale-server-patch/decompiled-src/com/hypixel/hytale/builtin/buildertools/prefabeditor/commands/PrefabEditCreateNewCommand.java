...
public class PrefabEditCreateNewCommand extends AbstractAsyncPlayerCommand {
   ...
   public PrefabEditCreateNewCommand() {
      ...
      this.flatNumBlocksBelowArg = (DefaultArg)this.withDefaultArg("numBlocksToSurface", "server.commands.editprefab.load.numBlocksToSurface.desc", ArgTypes.INTEGER, 0, "server.commands.editprefab.load.numBlocksToSurface.default.desc").addValidator(Validators.range(0, 120));
   ...
   protected CompletableFuture<Void> executeAsync(@Nonnull CommandContext context, @Nonnull Store<EntityStore> store, @Nonnull Ref<EntityStore> ref, @Nonnull PlayerRef playerRef, @Nonnull World world) {
      ...
      Path prefabBaseDirectory = ((PrefabRootDirectory)this.prefabPathArg.get(context)).getPrefabPath();
      String prefabName = (String)this.prefabNameArg.get(context);
