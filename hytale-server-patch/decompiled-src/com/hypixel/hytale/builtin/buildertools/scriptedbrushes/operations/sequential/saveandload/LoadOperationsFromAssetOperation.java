...
public class LoadOperationsFromAssetOperation extends SequenceBrushOperation {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(LoadOperationsFromAssetOperation.class, LoadOperationsFromAssetOperation::new).append(new KeyedCodec("AssetId", Codec.STRING), (op, val) -> op.assetId = val, (op) -> op.assetId).documentation("The ID of the ScriptedBrushAsset to load operations from").add()).documentation("Load and inline operations from another ScriptedBrushAsset")).build();
   }
}
