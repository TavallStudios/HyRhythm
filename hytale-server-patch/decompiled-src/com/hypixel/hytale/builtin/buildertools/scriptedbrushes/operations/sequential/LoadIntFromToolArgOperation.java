...
public class LoadIntFromToolArgOperation extends SequenceBrushOperation {
   ...
   public LoadIntFromToolArgOperation() {
      ...
      this.targetFieldArg = LoadIntFromToolArgOperation.TargetField.Width;
   }
         ...
         if (itemStack == null) {
            ...
            if (toolArgs != null && toolArgs.containsKey(this.argNameArg)) {
               ...
               if (argValue instanceof Integer) {
                  ...
                  brushConfig.setErrorFlag("LoadInt: Tool arg '" + var10001 + "' is not an Int type (found " + argValue.getClass().getSimpleName() + ")");
               }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(LoadIntFromToolArgOperation.class, LoadIntFromToolArgOperation::new).append(new KeyedCodec("ArgName", Codec.STRING, true), (op, val) -> op.argNameArg = val, (op) -> op.argNameArg).documentation("The name of the Int tool arg to load the value from").add()).append(new KeyedCodec("TargetField", new EnumCodec(TargetField.class)), (op, val) -> op.targetFieldArg = val, (op) -> op.targetFieldArg).documentation("The brush config field to set (Width, Height, Density, Thickness, OffsetX, OffsetY, OffsetZ)").add()).append(new KeyedCodec("Relative", Codec.BOOLEAN), (op, val) -> op.relativeArg = val, (op) -> op.relativeArg).documentation("When true, adds the loaded value to the current field value instead of replacing it").add()).append(new KeyedCodec("Negate", Codec.BOOLEAN), (op, val) -> op.negateArg = val, (op) -> op.negateArg).documentation("When true, turns the sign of the value to negative").add()).documentation("Load an integer from an Int tool arg and apply it to a brush config field")).build();
   }

   public static enum TargetField {
      None((Function)null, (BiConsumer)null),
      ...
      public int getValue(BrushConfig brushConfig) {
         return (Integer)this.getter.apply(brushConfig);
      }
