...
public class BrushConfigCommandExecutor {
   ...
   public void execute(@Nonnull Ref<EntityStore> ref, World world, Vector3i origin, boolean isHoldDownInteraction, InteractionType interactionType, ComponentAccessor<EntityStore> componentAccessor) {
      this.execute(ref, world, origin, isHoldDownInteraction, interactionType, (Consumer)null, componentAccessor);
   }
   ...
   public BrushConfig.BCExecutionStatus step(Ref<EntityStore> ref, boolean placePreviewAfterStep, ComponentAccessor<EntityStore> componentAccessor) {
      if (!this.brushConfig.isCurrentlyExecuting()) {
         ...
         try {
            SequenceBrushOperation brushOperation = (SequenceBrushOperation)this.sequentialOperations.get(this.currentOperationIndex);
   ...
   public void exitExecution(Ref<EntityStore> ref, ComponentAccessor<EntityStore> componentAccessor) {
      ...
      if (this.brushConfig.isHasExecutionContextEncounteredError() && this.currentOperationIndex < this.sequentialOperations.size()) {
         this.sendExecutionErrorMessage(playerRefComponent, (SequenceBrushOperation)this.sequentialOperations.get(this.currentOperationIndex));
      }
   ...
   private void sendExecutionErrorMessage(PlayerRef playerRef, @Nonnull SequenceBrushOperation brushOperation) {
      ...
      Set<Message> items = (Set)brushOperation.getRegisteredOperationSettings().values().stream().map((value) -> Message.translation("server.builderTools.brushConfig.settings.item").param("name", value.getName()).param("value", value.getValueString())).collect(Collectors.toSet());
   ...
   public void loadOperatingIndex(String name, boolean allowFutureJump) {
      ...
      if (!this.storedIndexes.containsKey(name)) {
         ...
         int newIndex = (Integer)this.storedIndexes.get(name);
   ...
   public int getPersistentVariableOrDefault(String variableName, int defaultValue) {
      variableName = variableName.toLowerCase();
      return !this.persistentStoredVariables.containsKey(variableName) ? defaultValue : (Integer)this.persistentStoredVariables.get(variableName);
   }
   ...
   public void loadBrushConfigSnapshot(String name, @Nonnull BrushConfig.DataSettingFlags... dataToLoad) {
      ...
      if (!this.brushConfigStoredSnapshots.containsKey(name)) {
         ...
         BrushConfig loadedBrushConfig = (BrushConfig)this.brushConfigStoredSnapshots.get(name);
