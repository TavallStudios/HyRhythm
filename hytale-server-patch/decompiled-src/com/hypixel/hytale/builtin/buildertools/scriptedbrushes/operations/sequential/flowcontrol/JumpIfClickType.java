...
public class JumpIfClickType extends SequenceBrushOperation {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(JumpIfClickType.class, JumpIfClickType::new).append(new KeyedCodec("StoredIndexName", Codec.STRING), (op, val) -> op.indexVariableNameArg = val, (op) -> op.indexVariableNameArg).documentation("The labeled index to jump to, previous or future").add()).append(new KeyedCodec("ClickType", new EnumCodec(ClickType.class)), (op, val) -> op.clickTypeArg = val, (op) -> op.clickTypeArg).documentation("The click type (left or right) to compare with to jump").add()).documentation("Jump the execution of the stack based on the click type")).build();
   }
