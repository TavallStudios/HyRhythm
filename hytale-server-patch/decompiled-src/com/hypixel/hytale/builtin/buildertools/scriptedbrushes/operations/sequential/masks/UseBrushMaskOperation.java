...
public class UseBrushMaskOperation extends SequenceBrushOperation {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(UseBrushMaskOperation.class, UseBrushMaskOperation::new).append(new KeyedCodec("UseBrushMask", Codec.BOOLEAN), (op, val) -> op.useBrushMask = val, (op) -> op.useBrushMask).documentation("Enables or disables the brush's mask").add()).documentation("Enable the brush tool's mask (the mask placed on the tool)")).build();
   }
}
