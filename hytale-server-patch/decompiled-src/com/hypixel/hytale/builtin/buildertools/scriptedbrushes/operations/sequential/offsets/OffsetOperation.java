...
public class OffsetOperation extends SequenceBrushOperation {
   ...
   public OffsetOperation() {
      ...
      this.offsetArg = RelativeVector3i.ZERO;
      this.targetFieldArg = LoadIntFromToolArgOperation.TargetField.None;
      this.negateArg = false;
   }
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(OffsetOperation.class, OffsetOperation::new).append(new KeyedCodec("Offset", RelativeVector3i.CODEC), (op, val) -> op.offsetArg = val, (op) -> op.offsetArg).documentation("Sets the offset in 3 dimensions, each value is optionally relative by prefixing it with a tilde").add()).documentation("Offset the brush location by a specified amount from the clicked origin")).append(new KeyedCodec("TargetField", new EnumCodec(LoadIntFromToolArgOperation.TargetField.class)), (op, val) -> op.targetFieldArg = val, (op) -> op.targetFieldArg).documentation("The brush config field to set (Width, Height, Density, Thickness, OffsetX, OffsetY, OffsetZ)").add()).append(new KeyedCodec("Negate", Codec.BOOLEAN, true), (op, val) -> op.negateArg = val, (op) -> op.negateArg).documentation("Whether to invert the sign of the relative field").add()).build();
   }
}
