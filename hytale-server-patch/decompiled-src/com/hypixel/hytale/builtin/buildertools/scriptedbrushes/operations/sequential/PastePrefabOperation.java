...
public class PastePrefabOperation extends SequenceBrushOperation {
   ...
   public boolean modifyBlocks(@Nonnull Ref<EntityStore> ref, @Nonnull BrushConfig brushConfig, BrushConfigCommandExecutor brushConfigCommandExecutor, BrushConfigEditStore edit, int x, int y, int z, ComponentAccessor<EntityStore> componentAccessor) {
      if (this.hasBeenPlacedAlready) {
         ...
         PrefabListAsset prefabListAsset = this.prefabListAssetId != null ? (PrefabListAsset)PrefabListAsset.getAssetMap().getAsset(this.prefabListAssetId) : null;
         if (prefabListAsset == null) {
            ...
            if (prefabPath == null) {
               ...
               double xLength = (double)(accessor.getMaxX() - accessor.getMinX());
               double zLength = (double)(accessor.getMaxZ() - accessor.getMinZ());
               ...
               accessor.forEach(IPrefabBuffer.iterateAllColumns(), (xi, yi, zi, blockId, holder, supportValue, rotation, filler, call, fluidId, fluidLevel) -> {
                  int bx = x + xi;
                  int by = y + yi;
                  int bz = z + zi;
                  ...
                  if (filler == 0) {
                     ...
                     if (supportValue != 0) {
                        ...
                        column = (ChunkColumn)store.getComponent(chunkRef, ChunkColumn.getComponentType());
