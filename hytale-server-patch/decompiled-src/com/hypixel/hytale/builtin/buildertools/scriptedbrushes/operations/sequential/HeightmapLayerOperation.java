...
public class HeightmapLayerOperation extends SequenceBrushOperation {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(HeightmapLayerOperation.class, HeightmapLayerOperation::new).append(new KeyedCodec("Layers", new ArrayCodec(LayerEntryCodec.CODEC, (x$0) -> new LayerEntryCodec[x$0])), (op, val) -> op.layerArgs = (List<LayerEntryCodec>)(val != null ? new ArrayList(Arrays.asList(val)) : List.of()), (op) -> op.layerArgs != null ? (LayerEntryCodec[])op.layerArgs.toArray(new LayerEntryCodec[0]) : new LayerEntryCodec[0]).documentation("The layers to set").add()).documentation("Replace blocks according to the specified layers in terms of their depth from the tallest block in its column")).build();
   }
}
