...
public class BrushConfigChunkAccessor extends LocalCachedChunkAccessor {
   ...
   public int getBlock(@Nonnull Vector3i pos) {
      return this.editOperation.getAfter().hasBlockAtWorldPos(pos.x, pos.y, pos.z) ? this.editOperation.getAfter().getBlockAtWorldPos(pos.x, pos.y, pos.z) : this.getBlockIgnoringHistory(pos);
   }

   public int getBlock(int x, int y, int z) {
      return this.editOperation.getAfter().hasBlockAtWorldPos(x, y, z) ? this.editOperation.getAfter().getBlockAtWorldPos(x, y, z) : this.getBlockIgnoringHistory(x, y, z);
   }
   ...
   public int getBlockIgnoringHistory(int x, int y, int z) {
      return this.editOperation.getBefore().hasBlockAtWorldPos(x, y, z) ? this.editOperation.getBefore().getBlockAtWorldPos(x, y, z) : this.getChunk(ChunkUtil.indexChunkFromBlock(x, z)).getBlock(x, y, z);
   }
