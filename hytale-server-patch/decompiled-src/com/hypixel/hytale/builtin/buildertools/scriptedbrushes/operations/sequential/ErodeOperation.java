...
public class ErodeOperation extends SequenceBrushOperation {
   public static final BuilderCodec<ErodeOperation> CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(ErodeOperation.class, ErodeOperation::new).append(new KeyedCodec("ErodePreset", new EnumCodec(ErodePreset.class)), (op, val) -> op.erodePresetArg = val, (op) -> op.erodePresetArg).documentation("An erosion preset to use with the operation").add()).documentation("Erodes blocks following a preset")).build();
   ...
   public ErodeOperation() {
      ...
      this.erodePresetArg = ErodeOperation.ErodePreset.Default;
   }
   ...
   private void iterateFill(@Nonnull BrushConfigEditStore edit, int x, int y, int z) {
      ...
      if (block == 0) {
         ...
         if (numFacesFound >= this.erodePresetArg.fillFaces) {
            ...
            IntIterator var15 = blockCount.keySet().iterator();

            while(var15.hasNext()) {
               int blockId = (Integer)var15.next();
