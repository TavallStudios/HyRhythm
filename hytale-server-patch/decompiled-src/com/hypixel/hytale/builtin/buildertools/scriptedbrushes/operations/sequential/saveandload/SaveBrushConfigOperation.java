...
public class SaveBrushConfigOperation extends SequenceBrushOperation {
   ...
   static {
      CODEC = ((BuilderCodec.Builder)((BuilderCodec.Builder)BuilderCodec.builder(SaveBrushConfigOperation.class, SaveBrushConfigOperation::new).append(new KeyedCodec("StoredName", Codec.STRING), (op, val) -> op.variableNameArg = val, (op) -> op.variableNameArg).documentation("The name to store the snapshot of this brush config under").add()).documentation("Save a snapshot of the current brush config in order to restore it later")).build();
   }
}
