...
public class WeatherSystem {
   ...
   public static class WorldAddedSystem extends StoreSystem<EntityStore> {
      ...
      public void onSystemAddedToStore(@Nonnull Store<EntityStore> store) {
         String forcedWeather = ((EntityStore)store.getExternalData()).getWorld().getWorldConfig().getForcedWeather();
         ((WeatherResource)store.getResource(this.weatherResourceType)).setForcedWeather(forcedWeather);
      }
   ...
   public static class PlayerAddedSystem extends HolderSystem<EntityStore> {
      ...
      public void onEntityRemoved(@Nonnull Holder<EntityStore> holder, @Nonnull RemoveReason reason, @Nonnull Store<EntityStore> store) {
         WeatherTracker weatherTrackerComponent = (WeatherTracker)holder.ensureAndGetComponent(WEATHER_TRACKER_COMPONENT_TYPE);
         weatherTrackerComponent.clear();
      }

      static {
         QUERY = Archetype.of(PLAYER_REF_COMPONENT_TYPE, TRANSFORM_COMPONENT_TYPE);
      }
   }
   ...
   public static class TickingSystem extends EntityTickingSystem<EntityStore> {
      ...
      public void tick(float dt, int systemIndex, @Nonnull Store<EntityStore> store) {
         WeatherResource weatherResource = (WeatherResource)store.getResource(WEATHER_RESOURCE_TYPE);
            ...
            if (weatherResource.getForcedWeatherIndex() == 0) {
               ...
               if (weatherResource.compareAndSwapHour(currentHour)) {
                  ...
                  for(Map.Entry<String, Environment> entry : assetMap.getAssetMap().entrySet()) {
                     ...
                     IWeightedMap<WeatherForecast> weatherForecast = ((Environment)entry.getValue()).getWeatherForecast(currentHour);
                     int selectedWeatherIndex = ((WeatherForecast)weatherForecast.get((Random)random)).getWeatherIndex();
      ...
      public void tick(float dt, int index, @Nonnull ArchetypeChunk<EntityStore> archetypeChunk, @Nonnull Store<EntityStore> store, @Nonnull CommandBuffer<EntityStore> commandBuffer) {
         WeatherResource weatherResource = (WeatherResource)store.getResource(WEATHER_RESOURCE_TYPE);
         PlayerRef playerRefComponent = (PlayerRef)archetypeChunk.getComponent(index, PLAYER_REF_COMPONENT_TYPE);
