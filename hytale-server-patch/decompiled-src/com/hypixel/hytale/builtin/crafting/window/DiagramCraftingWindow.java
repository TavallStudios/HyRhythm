...
public class DiagramCraftingWindow extends CraftingWindow implements ItemContainerWindow {
   ...
   public boolean onOpen0(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store) {
      ...
      this.updateInput((ItemContainer)null, ref, store);
   ...
   public void handleAction(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull WindowAction action) {
      ...
      byte var8 = 0;
      //$FF: var8->value
      //0->com/hypixel/hytale/protocol/packets/window/CancelCraftingAction
      //1->com/hypixel/hytale/protocol/packets/window/UpdateCategoryAction
      //2->com/hypixel/hytale/protocol/packets/window/CraftItemAction
      switch (action.typeSwitch<invokedynamic>(action, var8)) {
         case 0:
            CancelCraftingAction ignored = (CancelCraftingAction)action;
            craftingManagerComponent.cancelAllCrafting(ref, store);
            break;
         case 1:
            UpdateCategoryAction updateAction = (UpdateCategoryAction)action;
            ...
            break;
         case 2:
            CraftItemAction ignored = (CraftItemAction)action;
            ...
            if (this.bench.getCompletedSoundEventIndex() != 0) {
               SoundUtil.playSoundEvent3d(this.bench.getCompletedSoundEventIndex(), SoundCategory.SFX, (double)this.x + 0.5, (double)this.y + 0.5, (double)this.z + 0.5, store);
            }
   ...
   private void updateInventory(@Nonnull Ref<EntityStore> ref, @Nonnull ComponentAccessor<EntityStore> componentAccessor, @Nonnull CraftingBench.BenchItemCategory benchItemCategory) {
      ...
      this.combinedInputItemContainer = new CombinedItemContainer(new ItemContainer[]{this.inputPrimaryContainer, this.inputSecondaryContainer});
   ...
   private void updateInput(@Nullable ItemContainer container, @Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store) {
      ...
      if (primaryItemStack != null && !primaryItemStack.isEmpty()) {
         ...
         for(short i = 0; i < this.inputSecondaryContainer.getCapacity(); ++i) {
            ...
            if (itemStack != null && !itemStack.isEmpty()) {
               this.inputSecondaryContainer.setSlotFilter(FilterActionType.ADD, i, (SlotFilter)null);
      ...
      if (recipes.size() == 1 && allSlotsFull) {
         CraftingRecipe recipe = (CraftingRecipe)recipes.getFirst();
         ItemStack output = (ItemStack)CraftingManager.getOutputItemStacks(recipe).getFirst();
      ...
      if (primaryItemStack != null && !primaryItemStack.isEmpty()) {
         ...
         for(CraftingRecipe recipe : this.getBenchRecipes()) {
            if (recipe.getInput().length != inputCapacity && (!this.benchItemCategory.isSpecialSlot() || recipe.getInput().length != inputCapacity - 1)) {
               LOGGER.at(Level.WARNING).log("Recipe for %s has different input length than the diagram! %s - %s, %s, %s", recipe.getId(), recipe, this.bench, this.category, this.itemCategory);
