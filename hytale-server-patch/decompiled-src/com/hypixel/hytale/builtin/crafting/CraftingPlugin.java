...
public class CraftingPlugin extends JavaPlugin {
   ...
   public static Set<String> getAvailableRecipesForCategory(@Nonnull String benchId, @Nonnull String benchCategoryId) {
      BenchRecipeRegistry benchRecipeRegistry = (BenchRecipeRegistry)registries.get(benchId);
   ...
   public static boolean isValidCraftingMaterialForBench(@Nonnull BenchState benchState, @Nonnull ItemStack itemStack) {
      BenchRecipeRegistry benchRecipeRegistry = (BenchRecipeRegistry)registries.get(benchState.getBench().getId());
   ...
   protected void setup() {
      AssetRegistry.getAssetStore(Interaction.class).loadAssets("Hytale:Hytale", List.of(OpenBenchPageInteraction.SIMPLE_CRAFTING, OpenBenchPageInteraction.DIAGRAM_CRAFTING, OpenBenchPageInteraction.STRUCTURAL_CRAFTING));
      ...
      eventRegistry.register((Class)LoadedAssetsEvent.class, CraftingRecipe.class, CraftingPlugin::onRecipeLoad);
      eventRegistry.register((Class)RemovedAssetsEvent.class, CraftingRecipe.class, CraftingPlugin::onRecipeRemove);
      eventRegistry.register((Class)LoadedAssetsEvent.class, Item.class, CraftingPlugin::onItemAssetLoad);
      eventRegistry.register((Class)RemovedAssetsEvent.class, Item.class, CraftingPlugin::onItemAssetRemove);
      ...
      entityStoreRegistry.registerSystem(new PlayerAddedSystem(playerComponentType, playerRefComponentType));
   }
   ...
   private static void onItemAssetLoad(@Nonnull LoadedAssetsEvent<String, Item, DefaultAssetMap<String, Item>> event) {
      ...
      for(Item item : event.getLoadedAssets().values()) {
         if (item.hasRecipesToGenerate()) {
            ...
            itemGeneratedRecipes.put(item.getId(), (String[])generatedIds.toArray((x$0) -> new String[x$0]));
   ...
   private static void onItemAssetRemove(@Nonnull RemovedAssetsEvent<String, Item, DefaultAssetMap<String, Item>> event) {
      for(String id : event.getRemovedAssets()) {
         String[] generatedRecipes = (String[])itemGeneratedRecipes.get(id);
      ...
      for(CraftingRecipe recipe : event.getLoadedAssets().values()) {
         ...
         if (recipe.getBenchRequirement() != null) {
            for(BenchRequirement benchRequirement : recipe.getBenchRequirement()) {
               BenchRecipeRegistry benchRecipeRegistry = (BenchRecipeRegistry)registries.computeIfAbsent(benchRequirement.id, BenchRecipeRegistry::new);
   ...
   public static List<CraftingRecipe> getBenchRecipes(BenchType benchType, String name) {
      return getBenchRecipes(benchType, name, (String)null);
   }

   @Nonnull
   public static List<CraftingRecipe> getBenchRecipes(BenchType benchType, String benchId, @Nullable String category) {
      BenchRecipeRegistry registry = (BenchRecipeRegistry)registries.get(benchId);
      if (registry == null) {
         return List.of();
      } else {
