...
public class AssetRegistry {
   ...
   public static <K, T extends JsonAssetWithMap<K, M>, M extends AssetMap<K, T>> AssetStore<K, T, M> getAssetStore(Class<T> tClass) {
      return (AssetStore)storeMap.get(tClass);
   }
   ...
   public static <K, T extends JsonAssetWithMap<K, M>, M extends AssetMap<K, T>, S extends AssetStore<K, T, M>> S register(@Nonnull S assetStore) {
      ...
      try {
         if (storeMap.putIfAbsent(((AssetStore)assetStore).getAssetClass(), assetStore) != null) {
            throw new IllegalArgumentException("Asset Store already exists for " + String.valueOf(((AssetStore)assetStore).getAssetClass()));
         }
      } finally {
   ...
   public static boolean registerClientTag(@Nonnull String tag) {
      if (tag == null) {
         ...
         boolean var3;
         try {
            var3 = CLIENT_TAG_MAP.put(tag, TAG_MAP.computeIfAbsent(tag, (k) -> NEXT_TAG_INDEX.getAndIncrement())) == -2147483648;
         } finally {
            TAG_LOCK.unlockWrite(stamp);
         }
   ...
   static {
      ...
      TAG_MAP = new Object2IntOpenHashMap();
      CLIENT_TAG_MAP = new Object2IntOpenHashMap();
      TAG_MAP.defaultReturnValue(-2147483648);
      CLIENT_TAG_MAP.defaultReturnValue(-2147483648);
   }
}
