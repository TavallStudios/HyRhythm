...
public class AssetCodecMapCodec<K, T extends JsonAsset<K>> extends StringCodecMapCodec<T, AssetBuilderCodec<K, T>> implements AssetCodec<K, T> {
   ...
   public AssetCodecMapCodec<K, T> register(@Nonnull Priority priority, @Nonnull String id, Class<? extends T> aClass, BuilderCodec<? extends T> codec) {
      AssetBuilderCodec<K, T> assetCodec = AssetBuilderCodec.<K, T>wrap(codec, this.idCodec.getChildCodec(), this.idSetter, this.idGetter, this.dataSetter, this.dataGetter);
      super.register(priority, (String)id, aClass, assetCodec);
      return this;
   }

   public T decodeAndInherit(@Nonnull BsonDocument document, T parent, ExtraInfo extraInfo) {
      BsonValue id = document.get(this.key);
      AssetBuilderCodec<K, T> codec = (AssetBuilderCodec)this.idToCodec.get(id == null ? null : id.asString().getValue());
      if (codec == null) {
         AssetBuilderCodec<K, T> defaultCodec = (AssetBuilderCodec)this.getDefaultCodec();
         if (defaultCodec == null) {
            String var10002 = this.key;
            throw new ACodecMapCodec.UnknownIdException("No codec registered with for '" + var10002 + "': " + String.valueOf(id));
         } else {
            return (T)(defaultCodec.decodeAndInherit(document, parent, extraInfo));
         }
      } else {
         return (T)(codec.decodeAndInherit(document, parent, extraInfo));
      }
   }
   ...
   public T decodeAndInheritJson(@Nonnull RawJsonReader reader, @Nullable T parent, @Nonnull ExtraInfo extraInfo) throws IOException {
      ...
      try {
         ...
         if (codec != null) {
            JsonAsset var11 = (JsonAsset)codec.decodeAndInheritJson(reader, parent, extraInfo);
            return (T)var11;
         }
   ...
   protected void mutateChildSchema(String key, @Nonnull SchemaContext context, BuilderCodec<? extends T> c, @Nonnull ObjectSchema objectSchema) {
      ...
      AssetBuilderCodec<K, T> def = (AssetBuilderCodec)this.getDefaultCodec();
      if (!this.allowDefault || def != c) {
         ...
         Schema parentSchema = (Schema)objectSchema.getProperties().get(bc.getParentCodec().getKey());
         if (parentSchema != null) {
            Schema.InheritSettings settings = parentSchema.getHytaleParent();
            settings.setMapKey(this.key);
            settings.setMapKeyValue(key);
            objectSchema.setOneOf(new Schema[]{idField, parentField});
         }
      }
