...
public class Store<ECS_TYPE> implements ComponentAccessor<ECS_TYPE> {
   ...
   @Nonnull
   private final ParallelTask<EntityTickingSystem.SystemTaskData<ECS_TYPE>> parallelTask = new ParallelTask<EntityTickingSystem.SystemTaskData<ECS_TYPE>>(EntityTickingSystem.SystemTaskData::new);
   @Nonnull
   private final ParallelTask<ForEachTaskData<ECS_TYPE>> forEachTask = new ParallelTask<ForEachTaskData<ECS_TYPE>>(ForEachTaskData::new);
   @Nonnull
   private final ParallelTask<EntityDataSystem.SystemTaskData<ECS_TYPE, ?, ?>> fetchTask = new ParallelTask<EntityDataSystem.SystemTaskData<ECS_TYPE, ?, ?>>(EntityDataSystem.SystemTaskData::new);
   @Nonnull
   private final ProcessingCounter processing = new ProcessingCounter();
   ...
   Store(@Nonnull ComponentRegistry<ECS_TYPE> registry, int storeIndex, @Nonnull ECS_TYPE externalData, @Nonnull IResourceStorage resourceStorage) {
      ...
      this.archetypeToIndexMap = new Object2IntOpenHashMap();
      this.archetypeChunkReuse = new BitSet();
      this.archetypeChunks = ArchetypeChunk.<ECS_TYPE>emptyArray();
      ...
      this.archetypeToIndexMap.defaultReturnValue(-2147483648);
      Arrays.fill(this.entityToArchetypeChunk, -2147483648);
      Arrays.fill(this.entityChunkIndex, -2147483648);
   }
   ...
   CommandBuffer<ECS_TYPE> takeCommandBuffer() {
      ...
      if (this.commandBuffers.isEmpty()) {
         ...
         CommandBuffer<ECS_TYPE> buffer = (CommandBuffer)this.commandBuffers.pop();
   ...
   void onAdd(@Nonnull ComponentRegistry.Data<ECS_TYPE> data) {
      ...
      this.resources = (Resource[])Arrays.copyOf(this.resources, resourceSize);

      for(int index = 0; index < resourceSize; ++index) {
         ResourceType<ECS_TYPE, ? extends Resource<ECS_TYPE>> resourceType = data.getResourceType(index);
      ...
      this.systemMetrics = (HistoricMetric[])Arrays.copyOf(this.systemMetrics, data.getSystemSize());
      ...
      for(int systemIndex = -1; (systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0; this.systemMetrics[systemIndex] = HistoricMetric.builder(33333333L, TimeUnit.NANOSECONDS).addPeriod(1L, TimeUnit.SECONDS).addPeriod(1L, TimeUnit.MINUTES).addPeriod(5L, TimeUnit.MINUTES).build()) {
      }
   ...
   public int getEntityCountFor(@Nonnull Query<ECS_TYPE> query) {
      ...
      if (query instanceof ExactArchetypeQuery<ECS_TYPE> exactQuery) {
         ...
         return archetypeIndex != -2147483648 ? this.archetypeChunks[archetypeIndex].size() : 0;
      } else {
         int count = 0;
   ...
   public int getEntityCountFor(int systemIndex) {
      ...
      for(int index = -1; (index = indexes.nextSetBit(index + 1)) >= 0; count += this.archetypeChunks[index].size()) {
      }
   ...
   public ArchetypeChunkData[] collectArchetypeChunkData() {
      ...
      for(int i = 0; i < this.archetypeSize; ++i) {
         ...
         if (chunk != null) {
            ...
            for(int j = archetype.getMinIndex(); j < archetype.length(); ++j) {
               ComponentType<ECS_TYPE, ? extends Component<ECS_TYPE>> componentType = archetype.get(j);
      ...
      return (ArchetypeChunkData[])result.toArray((x$0) -> new ArchetypeChunkData[x$0]);
   }
      ...
      if (ref.isValid()) {
         ...
         try {
            ...
            if (oldLength <= entityIndex) {
               ...
               this.refs = (Ref[])Arrays.copyOf(this.refs, systemIndex);
               ...
               Arrays.fill(this.entityToArchetypeChunk, oldLength, systemIndex, -2147483648);
               Arrays.fill(this.entityChunkIndex, oldLength, systemIndex, -2147483648);
            }
            ...
            while((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
               for(int i = refStart; i < refEnd; ++i) {
                  ...
                  if (entityProcessedBySystemIndexes.get(systemIndex)) {
                     ...
                     system.onEntityAdded(ref, reason, this, commandBuffer);
                     if (commandBuffer.consumeWasTrackedRefRemoved()) {
                        int remaining = refEnd - i;
                        if (remaining > 1) {
                           System.arraycopy(refs, i + 1, refs, i, remaining - 1);
                           refs[refEnd - 1] = ref;
                           --i;
                        }
   ...
   public Holder<ECS_TYPE> removeEntity(@Nonnull Ref<ECS_TYPE> ref, @Nonnull Holder<ECS_TYPE> holder, @Nonnull RemoveReason reason) {
      return this.removeEntity(ref, holder, reason, (Throwable)null);
   }
   ...
   Holder<ECS_TYPE> removeEntity(@Nonnull Ref<ECS_TYPE> ref, @Nonnull Holder<ECS_TYPE> holder, @Nonnull RemoveReason reason, @Nullable Throwable proxyReason) {
      ...
      try {
         ...
         while((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
            if (entityProcessedBySystemIndexes.get(systemIndex)) {
               ((RefSystem)data.getSystem(systemIndex, systemType)).onEntityRemove(ref, reason, this, commandBuffer);
            }
         }
         ...
         this.entityToArchetypeChunk[lastIndex] = -2147483648;
         this.entityChunkIndex[lastIndex] = -2147483648;
      ...
      if (refArr.length < refEnd) {
         ...
         try {
            ...
            while((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
               for(int i = refStart; i < refEnd; ++i) {
                  ...
                  BitSet entityProcessedBySystemIndexes = this.archetypeChunkIndexesToSystemIndex[archetypeIndex];
                  if (entityProcessedBySystemIndexes.get(systemIndex)) {
                     ((RefSystem)data.getSystem(systemIndex, systemType)).onEntityRemove(refArr[i], reason, this, commandBuffer);
                  }
               }
            }
         ...
         try {
            ...
            while((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
               ...
               for(int i = holderStart; i < holderEnd; ++i) {
                  if (system.test(this.registry, holders[i].getArchetype())) {
                     system.onEntityRemoved(holders[i], reason, this);
                  }
               }
            }
         } finally {
   ...
   public <T extends Component<ECS_TYPE>> T getComponent(@Nonnull Ref<ECS_TYPE> ref, @Nonnull ComponentType<ECS_TYPE, T> componentType) {
      this.assertThread();
      return (T)this.__internal_getComponent(ref, componentType);
   }
   ...
   protected <T extends Component<ECS_TYPE>> T __internal_getComponent(@Nonnull Ref<ECS_TYPE> ref, @Nonnull ComponentType<ECS_TYPE, T> componentType) {
      ...
      int archetypeIndex = this.entityToArchetypeChunk[ref.getIndex()];
      ArchetypeChunk<ECS_TYPE> archetypeChunk = this.archetypeChunks[archetypeIndex];
      return (T)archetypeChunk.getComponent(this.entityChunkIndex[ref.getIndex()], componentType);
   }
      ...
      if (this.shutdown) {
         ...
         try {
            if (query instanceof ExactArchetypeQuery) {
               ...
               for(int archetypeIndex = 0; archetypeIndex < this.archetypeSize; ++archetypeIndex) {
                  ArchetypeChunk<ECS_TYPE> archetypeChunk = this.archetypeChunks[archetypeIndex];
                  if (archetypeChunk != null && query.test(archetypeChunk.getArchetype()) && predicate.test(this.archetypeChunks[archetypeIndex], commandBuffer)) {
                     result = true;
                     break;
                  }
               }
      ...
      if (this.shutdown) {
         ...
         try {
            ...
            while((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
               ...
               while((index = indexes.nextSetBit(index + 1)) >= 0) {
                  ((ArchetypeDataSystem)system).fetch(this.archetypeChunks[index], this, commandBuffer, query, results);
               }
            }
            ...
            while((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
               ...
               for(Ref<ECS_TYPE> ref : refs) {
                  ...
                  if (entityProcessedBySystemIndexes.get(systemIndex)) {
                     ...
                     ArchetypeChunk<ECS_TYPE> archetypeChunk = this.archetypeChunks[archetypeIndex];
                     ((EntityDataSystem)system).fetch(index, archetypeChunk, this, commandBuffer, query, results);
                  }
               }
            }
   ...
   private void updateArchetypeIndexes(@Nonnull ComponentRegistry.Data<ECS_TYPE> data) {
      ...
      if (oldLength < systemSize) {
         this.systemIndexToArchetypeChunkIndexes = (BitSet[])Arrays.copyOf(this.systemIndexToArchetypeChunkIndexes, systemSize);
   ...
   public void assertThread() {
      ...
      if (!currentThread.equals(this.thread) && this.thread.isAlive()) {
         String var10002 = String.valueOf(this.thread);
         throw new IllegalStateException("Assert not in thread! " + var10002 + " but was in " + String.valueOf(currentThread));
      }
   }
   ...
   private int findOrCreateArchetypeChunk(@Nonnull Archetype<ECS_TYPE> archetype) {
      ...
      if (archetypeIndex != -2147483648) {
         ...
         if (oldLength <= archetypeIndex) {
            ...
            this.archetypeChunks = (ArchetypeChunk[])Arrays.copyOf(this.archetypeChunks, newLength);
            this.archetypeChunkIndexesToSystemIndex = (BitSet[])Arrays.copyOf(this.archetypeChunkIndexesToSystemIndex, newLength);
            int systemSize = data.getSystemSize();
         ...
         ArchetypeChunk<ECS_TYPE> archetypeChunk = new ArchetypeChunk<ECS_TYPE>(this, archetype);
   ...
   static {
      METRICS_REGISTRY = (new MetricsRegistry()).register("ArchetypeChunkCount", Store::getArchetypeChunkCount, Codec.INTEGER).register("EntityCount", Store::getEntityCount, Codec.INTEGER).register("Systems", (componentStore) -> {
         ...
         for(int systemIndex = 0; systemIndex < data.getSystemSize(); ++systemIndex) {
            ...
            systemMetricData[systemIndex] = new SystemMetricData(system.getClass().getName(), componentStore.getArchetypeChunkCountFor(systemIndex), componentStore.getEntityCountFor(systemIndex), system instanceof TickingSystem ? systemMetrics[systemIndex] : null, metrics);
         }

         return systemMetricData;
      }, new ArrayCodec(SystemMetricData.CODEC, (x$0) -> new SystemMetricData[x$0])).register("ArchetypeChunks", Store::collectArchetypeChunkData, new ArrayCodec(ArchetypeChunkData.CODEC, (x$0) -> new ArchetypeChunkData[x$0]));
   }
