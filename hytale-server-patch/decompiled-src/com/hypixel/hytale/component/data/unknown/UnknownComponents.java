...
public class UnknownComponents<ECS_TYPE> implements Component<ECS_TYPE> {
   ...
   public <T extends Component<ECS_TYPE>> T removeComponent(@Nonnull String componentId, @Nonnull Codec<T> codec) {
      BsonDocument bsonDocument = (BsonDocument)this.unknownComponents.remove(componentId);
      if (bsonDocument == null) {
         return null;
      } else {
         ExtraInfo extraInfo = (ExtraInfo)ExtraInfo.THREAD_LOCAL.get();
         T component = codec.decode(bsonDocument, extraInfo);
         extraInfo.getValidationResults().logOrThrowValidatorExceptions(LOGGER);
         return component;
      }
   }
   ...
   public Component<ECS_TYPE> clone() {
      return new UnknownComponents<ECS_TYPE>(new Object2ObjectOpenHashMap(this.unknownComponents));
   }

   static {
      CODEC = ((BuilderCodec.Builder)BuilderCodec.builder(UnknownComponents.class, UnknownComponents::new).addField(new KeyedCodec("Components", new MapCodec(Codec.BSON_DOCUMENT, Object2ObjectOpenHashMap::new, false)), (o, map) -> o.unknownComponents = map, (o) -> o.unknownComponents)).build();
   }
}
