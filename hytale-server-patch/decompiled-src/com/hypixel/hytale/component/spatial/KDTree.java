...
public class KDTree<T> implements SpatialStructure<T> {
   @Nonnull
   private final List<Node<T>> nodePool = new ObjectArrayList();
   ...
   public void rebuild(@Nonnull SpatialData<T> spatialData) {
      ...
      if (spatialDataSize != 0) {
         for(int i = 0; i < this.dataListPoolIndex; ++i) {
            ((List)this.dataListPool.get(i)).clear();
         }
   ...
   public T closest(@Nonnull Vector3d point) {
      ClosestState<T> closestState = new ClosestState<T>((Node)null, 1.7976931348623157E308);
      this.closest0(closestState, this.root, point, 0);
      return (T)(closestState.node == null ? null : closestState.node.data.getFirst());
   }
   ...
   public void ordered(@Nonnull Vector3d center, double radius, @Nonnull List<T> results) {
      double distanceSq = radius * radius;
      ObjectArrayList<OrderedEntry<T>> entryResults = new ObjectArrayList();
      this.ordered0(entryResults, this.root, center, distanceSq, 0);
      entryResults.sort(Comparator.comparingDouble((o) -> o.distanceSq));
      ObjectListIterator var8 = entryResults.iterator();

      while(var8.hasNext()) {
         OrderedEntry<T> entry = (OrderedEntry)var8.next();
         int i = 0;

         for(int bound = entry.values.size(); i < bound; ++i) {
            T data = (T)entry.values.get(i);
   ...
   public void ordered3DAxis(@Nonnull Vector3d center, double xSearchRadius, double YSearchRadius, double zSearchRadius, @Nonnull List<T> results) {
      ...
      entryResults.sort(Comparator.comparingDouble((o) -> o.distanceSq));
      ObjectListIterator var10 = entryResults.iterator();

      while(var10.hasNext()) {
         OrderedEntry<T> entry = (OrderedEntry)var10.next();
         int i = 0;
   ...
   private List<T> getPooledDataList() {
      if (this.dataListPoolIndex < this.dataListPool.size()) {
         return (List)this.dataListPool.get(this.dataListPoolIndex++);
      } else {
   ...
   private void _internal_ordered3DAxis(@Nonnull List<OrderedEntry<T>> results, @Nullable Node<T> node, @Nonnull Vector3d center, double xSearchRadius, double ySearchRadius, double zSearchRadius, int depth) {
      if (node != null) {
         int axis = depth % 3;
         boolean inCuboid = node.vector.x >= center.x - xSearchRadius && node.vector.x <= center.x + xSearchRadius && node.vector.y >= center.y - ySearchRadius && node.vector.y <= center.y + ySearchRadius && node.vector.z >= center.z - zSearchRadius && node.vector.z <= center.z + zSearchRadius;
         if (inCuboid) {
         ...
         int newDepth = depth + 1;
         int compare = compare(node.vector, center, axis);
         Node<T> primary = compare < 0 ? node.one : node.two;
         Node<T> secondary = compare < 0 ? node.two : node.one;
   ...
   private static int compare(@Nonnull Vector3d v1, @Nonnull Vector3d v2, int axis) {
      int var10000;
      switch (axis) {
         case 0 -> var10000 = Double.compare(v1.x, v2.x);
         case 1 -> var10000 = Double.compare(v1.z, v2.z);
         case 2 -> var10000 = Double.compare(v1.y, v2.y);
