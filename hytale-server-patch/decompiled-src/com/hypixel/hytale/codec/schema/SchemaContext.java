...
public class SchemaContext {
   ...
   public <T> Schema refDefinition(@Nonnull SchemaConvertable<T> convertable, @Nullable T def) {
      ...
      if (ref != null) {
         return ref;
      } else if (convertable instanceof BuilderCodec) {
         ...
         return c;
      } else if (convertable instanceof NamedSchema) {
         NamedSchema namedSchema = (NamedSchema)convertable;
   ...
   public Schema getRawDefinition(@Nonnull BuilderCodec<?> codec) {
      ...
      return (Schema)this.definitions.get(name);
   }

   @Nullable
   public Schema getRawDefinition(@Nonnull NamedSchema namedSchema) {
      return (Schema)this.otherDefinitions.get(this.resolveName(namedSchema));
   }
   ...
   private String resolveName(@Nonnull NamedSchema namedSchema) {
      return (String)this.nameMap.computeIfAbsent(namedSchema, (key) -> {
   ...
   private String resolveName(@Nonnull BuilderCodec<?> codec) {
      return (String)this.nameMap.computeIfAbsent(codec.getInnerClass(), (key) -> {
