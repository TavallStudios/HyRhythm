...
public class RawJsonReader implements AutoCloseable {
      ...
      if (this.markIndex <= -1) {
         ...
         if (spaceInBuffer > 16384) {
            ...
            if (this.markIndex > 16384) {
               ...
               int var10001 = this.buffer.length;
               System.err.println("Reallocate: " + var10001 + " to " + newSize);
   ...
   public long skip(long skip) throws IOException {
      if (skip < 0L) {
         ...
         if (skip < (long)negativeBufferIndex) {
            this.bufferIndex = 0;
            return (long)negativeBufferIndex;
         } else {
   ...
   public boolean tryConsume(@Nonnull String str, int start) throws IOException {
      ...
      while(true) {
         if (start < str.length()) {
            ...
            while(true) {
               if (start >= str.length() || this.bufferIndex >= this.bufferSize) {
                  continue label25;
               }
   ...
   public long readStringPartAsLong(int count) throws IOException {
      ...
      return UnsafeUtil.UNSAFE != null && this.bufferIndex + count <= this.bufferSize ? this.readStringPartAsLongUnsafe(count) : this.readStringPartAsLongSlow(count);
   }
   ...
   protected long readStringPartAsLongUnsafe(int count) throws IOException {
      ...
      long value = UnsafeUtil.UNSAFE.getLong(this.buffer, (long)offset);
   ...
   public boolean readBooleanValue() throws IOException {
      ...
      switch (read) {
         ...
         case 't':
            ...
            break;
         default:
            throw this.expecting(read, "true' or 'false");
      }
   ...
   public byte readByteValue(int radix) throws IOException {
      return (byte)((int)this.readLongValue(radix));
   }
   ...
   private IOException unexpectedChar(char read) {
      String var10002 = Integer.toHexString(read);
      return new IOException("Unexpected character: " + var10002 + ", '" + read + "'!");
   }
   ...
   private IOException expecting(char read, @Nonnull String expected, int index) {
      String var10002 = Integer.toHexString(read);
      return new IOException("Unexpected character: " + var10002 + ", '" + read + "' when consuming string '" + expected + "' expected '" + expected.substring(index - 1) + "'!");
   }
   ...
   public String toString() {
      if (this.buffer == null) {
         ...
         return this.in == null ? "Buffer RawJsonReader: " + String.valueOf(s) : "Streamed RawJsonReader: " + String.valueOf(s);
      }
   }
   ...
   public static BsonValue readBsonValue(@Nonnull RawJsonReader reader) throws IOException {
      ...
      if (read == -1) {
         ...
         switch (read) {
            case 34:
               var10000 = new BsonString(reader.readString());
               break;
            case 43:
            case 45:
               var10000 = new BsonDouble(reader.readDoubleValue());
               break;
            case 70:
            case 84:
            case 102:
   ...
   public static <T> T readSync(@Nonnull Path path, @Nonnull Codec<T> codec, @Nonnull HytaleLogger logger) throws IOException {
      char[] buffer = (char[])READ_BUFFER.get();
   ...
   public static <T> T readSyncWithBak(@Nonnull Path path, @Nonnull Codec<T> codec, @Nonnull HytaleLogger logger) {
      try {
         return (T)readSync(path, codec, logger);
      } catch (IOException var8) {
         Path backupPath = path.resolveSibling(String.valueOf(path.getFileName()) + ".bak");
         if (var8 instanceof NoSuchFileException && !Files.exists(backupPath, new LinkOption[0])) {
            return null;
         } else {
            if (Sentry.isEnabled()) {
            ...
            try {
               T value = (T)readSync(backupPath, codec, logger);
