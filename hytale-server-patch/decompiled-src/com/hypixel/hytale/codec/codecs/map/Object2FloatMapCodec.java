...
public class Object2FloatMapCodec<T> implements Codec<Object2FloatMap<T>>, WrappedCodec<T> {
   ...
   public Object2FloatMap<T> decode(@Nonnull BsonValue bsonValue, ExtraInfo extraInfo) {
      ...
      Object2FloatMap<T> map = (Object2FloatMap)this.supplier.get();

      for(Map.Entry<String, BsonValue> stringBsonValueEntry : bsonDocument.entrySet()) {
         T decodedKey = this.keyCodec.decode(new BsonString((String)stringBsonValueEntry.getKey()), extraInfo);
         map.put(decodedKey, (float)((BsonValue)stringBsonValueEntry.getValue()).asNumber().doubleValue());
      }
   ...
   public BsonValue encode(@Nonnull Object2FloatMap<T> map, ExtraInfo extraInfo) {
      ...
      ObjectIterator var4 = map.keySet().iterator();

      while(var4.hasNext()) {
         T key = (T)var4.next();
         String encodedKey = this.keyCodec.encode(key, extraInfo).asString().getValue();
         bsonDocument.put(encodedKey, new BsonDouble((double)map.getFloat(key)));
      }
