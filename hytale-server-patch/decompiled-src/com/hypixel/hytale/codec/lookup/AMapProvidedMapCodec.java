...
public abstract class AMapProvidedMapCodec<K, V, P, M extends Map<K, V>> implements Codec<M>, ValidatableCodec<M> {
   ...
   public M decode(@Nonnull BsonValue bsonValue, @Nonnull ExtraInfo extraInfo) {
      ...
      for(Map.Entry<String, BsonValue> entry : bsonDocument.entrySet()) {
         ...
         try {
            K key = (K)this.getKeyForId((String)entry.getKey());
            if (key == null) {
               this.handleUnknown(map, (String)entry.getKey(), (BsonValue)entry.getValue(), extraInfo);
            } else {
   ...
   public M decodeJson(@Nonnull RawJsonReader reader, @Nonnull ExtraInfo extraInfo) throws IOException {
      ...
      if (reader.tryConsume('}')) {
         return (M)(this.unmodifiable ? this.emptyMap() : this.createMap());
      } else {
         M map = this.createMap();
   ...
   public Schema toSchema(@Nonnull SchemaContext context) {
      ...
      LinkedHashMap<String, Schema> props = (LinkedHashMap)this.codecProvider.keySet().stream().map((key) -> {
         Codec<V> codec = this.getCodecFor(key);
         return Map.entry(this.getIdForKey(key), codec.toSchema(context));
      }).sorted(Entry.comparingByKey()).collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (a, b) -> a, LinkedHashMap::new));
   ...
   public void validateDefaults(ExtraInfo extraInfo, @Nonnull Set<Codec<?>> tested) {
      if (tested.add(this)) {
         for(P value : this.codecProvider.values()) {
            Codec<V> codec = (Codec)this.mapper.apply(value);
   ...
   private Codec<V> getCodecFor(K key) {
      return (Codec)this.mapper.apply(this.codecProvider.get(key));
   }
