...
public class ApplyForceInteraction extends SimpleInteraction {
   ...
   public ApplyForceInteraction() {
      this.changeVelocityType = ChangeVelocityType.Add;
   ...
   public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
      if (buffer.readableBytes() - offset < 104) {
         ...
         if ((nullBits & 8) != 0) {
            ...
            int pos = offset + 104 + settingsOffset;
            if (pos >= buffer.writerIndex()) {
         ...
         if ((nullBits & 16) != 0) {
            ...
            int pos = offset + 104 + rulesOffset;
            if (pos >= buffer.writerIndex()) {
         ...
         if ((nullBits & 64) != 0) {
            ...
            int pos = offset + 104 + cameraOffset;
            if (pos >= buffer.writerIndex()) {
         ...
         if ((nullBits & 128) != 0) {
            ...
            int pos = offset + 104 + forcesOffset;
            if (pos >= buffer.writerIndex()) {
   ...
   public ApplyForceInteraction clone() {
      ...
      copy.forces = this.forces != null ? (AppliedForce[])Arrays.stream(this.forces).map((ex) -> ex.clone()).toArray((x$0) -> new AppliedForce[x$0]) : null;
   ...
   public int hashCode() {
      ...
      result = 31 * result + Objects.hashCode(this.raycastMode);
      return result;
   }
}
