...
public class BuilderToolSelectionToolReplyWithClipboard implements Packet, ToClientPacket {
   ...
   public BuilderToolSelectionToolReplyWithClipboard clone() {
      ...
      copy.blocksChange = this.blocksChange != null ? (BlockChange[])Arrays.stream(this.blocksChange).map((e) -> e.clone()).toArray((x$0) -> new BlockChange[x$0]) : null;
      copy.fluidsChange = this.fluidsChange != null ? (FluidChange[])Arrays.stream(this.fluidsChange).map((e) -> e.clone()).toArray((x$0) -> new FluidChange[x$0]) : null;
      return copy;
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof BuilderToolSelectionToolReplyWithClipboard)) {
         return false;
      } else {
