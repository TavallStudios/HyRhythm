...
public class BuilderToolBrushData {
   ...
   public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
      if (buffer.readableBytes() - offset < 84) {
         ...
         if ((nullBits[1] & 32) != 0) {
            ...
            for(int i = 0; i < favoriteMaterialsCount; ++i) {
               ValidationResult structResult = BuilderToolBlockArg.validateStructure(buffer, pos);
         ...
         if ((nullBits[2] & 16) != 0) {
            ...
            for(int i = 0; i < maskCommandsCount; ++i) {
               ValidationResult structResult = BuilderToolStringArg.validateStructure(buffer, pos);
               if (!structResult.isValid()) {
   ...
   public BuilderToolBrushData clone() {
      ...
      copy.favoriteMaterials = this.favoriteMaterials != null ? (BuilderToolBlockArg[])Arrays.stream(this.favoriteMaterials).map((e) -> e.clone()).toArray((x$0) -> new BuilderToolBlockArg[x$0]) : null;
      ...
      copy.maskCommands = this.maskCommands != null ? (BuilderToolStringArg[])Arrays.stream(this.maskCommands).map((e) -> e.clone()).toArray((x$0) -> new BuilderToolStringArg[x$0]) : null;
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof BuilderToolBrushData)) {
         return false;
      } else {
