...
public class ServerSetBlocks implements Packet, ToClientPacket {
   ...
   public int computeSize() {
      int size = 12;
      size += VarInt.size(this.cmds.length) + this.cmds.length * 9;
      return size;
   }
   ...
   public ServerSetBlocks clone() {
      ...
      copy.y = this.y;
      copy.z = this.z;
      copy.cmds = (SetBlockCmd[])Arrays.stream(this.cmds).map((e) -> e.clone()).toArray((x$0) -> new SetBlockCmd[x$0]);
      return copy;
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof ServerSetBlocks)) {
         return false;
      } else {
         ServerSetBlocks other = (ServerSetBlocks)obj;
