...
public class UpdateCameraShake implements Packet, ToClientPacket {
   ...
   public void serialize(@Nonnull ByteBuf buf) {
      ...
      if (this.profiles != null) {
         ...
         for(Map.Entry<Integer, CameraShake> e : this.profiles.entrySet()) {
            buf.writeIntLE((Integer)e.getKey());
            ((CameraShake)e.getValue()).serialize(buf);
         }
      }
   ...
   public int computeSize() {
      ...
      if (this.profiles != null) {
         ...
         for(Map.Entry<Integer, CameraShake> kvp : this.profiles.entrySet()) {
            profilesSize += 4 + ((CameraShake)kvp.getValue()).computeSize();
         }
   ...
   public UpdateCameraShake clone() {
      ...
      if (this.profiles != null) {
         ...
         for(Map.Entry<Integer, CameraShake> e : this.profiles.entrySet()) {
            m.put((Integer)e.getKey(), ((CameraShake)e.getValue()).clone());
         }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof UpdateCameraShake)) {
         return false;
      } else {
         UpdateCameraShake other = (UpdateCameraShake)obj;
