...
public class UpdateItems implements Packet, ToClientPacket {
   ...
   public void serialize(@Nonnull ByteBuf buf) {
      ...
      if (this.items != null) {
         ...
         for(Map.Entry<String, ItemBase> e : this.items.entrySet()) {
            PacketIO.writeVarString(buf, (String)e.getKey(), 4096000);
            ((ItemBase)e.getValue()).serialize(buf);
         }
      } else {
   ...
   public int computeSize() {
      ...
      if (this.items != null) {
         ...
         for(Map.Entry<String, ItemBase> kvp : this.items.entrySet()) {
            itemsSize += PacketIO.stringSize((String)kvp.getKey()) + ((ItemBase)kvp.getValue()).computeSize();
         }
   ...
   public UpdateItems clone() {
      ...
      if (this.items != null) {
         ...
         for(Map.Entry<String, ItemBase> e : this.items.entrySet()) {
            m.put((String)e.getKey(), ((ItemBase)e.getValue()).clone());
         }

         copy.items = m;
      }

      copy.removedItems = this.removedItems != null ? (String[])Arrays.copyOf(this.removedItems, this.removedItems.length) : null;
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof UpdateItems)) {
         return false;
      } else {
         UpdateItems other = (UpdateItems)obj;
