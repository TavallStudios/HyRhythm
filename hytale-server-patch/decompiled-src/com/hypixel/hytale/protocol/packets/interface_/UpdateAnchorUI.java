...
public class UpdateAnchorUI implements Packet, ToClientPacket {
   ...
   public UpdateAnchorUI clone() {
      ...
      copy.commands = this.commands != null ? (CustomUICommand[])Arrays.stream(this.commands).map((e) -> e.clone()).toArray((x$0) -> new CustomUICommand[x$0]) : null;
      copy.eventBindings = this.eventBindings != null ? (CustomUIEventBinding[])Arrays.stream(this.eventBindings).map((e) -> e.clone()).toArray((x$0) -> new CustomUIEventBinding[x$0]) : null;
      return copy;
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof UpdateAnchorUI)) {
         return false;
      } else {
