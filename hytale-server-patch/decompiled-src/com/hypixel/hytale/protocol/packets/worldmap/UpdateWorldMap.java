...
public class UpdateWorldMap implements Packet, ToClientPacket {
   ...
   public UpdateWorldMap clone() {
      ...
      copy.chunks = this.chunks != null ? (MapChunk[])Arrays.stream(this.chunks).map((e) -> e.clone()).toArray((x$0) -> new MapChunk[x$0]) : null;
      copy.addedMarkers = this.addedMarkers != null ? (MapMarker[])Arrays.stream(this.addedMarkers).map((e) -> e.clone()).toArray((x$0) -> new MapMarker[x$0]) : null;
      copy.removedMarkers = this.removedMarkers != null ? (String[])Arrays.copyOf(this.removedMarkers, this.removedMarkers.length) : null;
      return copy;
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof UpdateWorldMap)) {
         return false;
      } else {
