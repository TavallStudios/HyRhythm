...
public class FormattedMessage {
   ...
   public void serialize(@Nonnull ByteBuf buf) {
      ...
      if (this.params != null) {
         ...
         for(Map.Entry<String, ParamValue> e : this.params.entrySet()) {
            PacketIO.writeVarString(buf, (String)e.getKey(), 4096000);
            ((ParamValue)e.getValue()).serializeWithTypeId(buf);
         }
      } else {
      ...
      if (this.messageParams != null) {
         ...
         for(Map.Entry<String, FormattedMessage> e : this.messageParams.entrySet()) {
            PacketIO.writeVarString(buf, (String)e.getKey(), 4096000);
            ((FormattedMessage)e.getValue()).serialize(buf);
         }
      } else {
   ...
   public int computeSize() {
      ...
      if (this.params != null) {
         ...
         for(Map.Entry<String, ParamValue> kvp : this.params.entrySet()) {
            paramsSize += PacketIO.stringSize((String)kvp.getKey()) + ((ParamValue)kvp.getValue()).computeSizeWithTypeId();
         }
      ...
      if (this.messageParams != null) {
         ...
         for(Map.Entry<String, FormattedMessage> kvp : this.messageParams.entrySet()) {
            messageParamsSize += PacketIO.stringSize((String)kvp.getKey()) + ((FormattedMessage)kvp.getValue()).computeSize();
         }
   ...
   public FormattedMessage clone() {
      ...
      copy.children = this.children != null ? (FormattedMessage[])Arrays.stream(this.children).map((ex) -> ex.clone()).toArray((x$0) -> new FormattedMessage[x$0]) : null;
      copy.params = this.params != null ? new HashMap(this.params) : null;
      if (this.messageParams != null) {
         Map<String, FormattedMessage> m = new HashMap();

         for(Map.Entry<String, FormattedMessage> e : this.messageParams.entrySet()) {
            m.put((String)e.getKey(), ((FormattedMessage)e.getValue()).clone());
         }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof FormattedMessage)) {
         return false;
      } else {
