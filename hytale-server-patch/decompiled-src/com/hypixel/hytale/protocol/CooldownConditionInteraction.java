...
public class CooldownConditionInteraction extends SimpleInteraction {
   ...
   public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
      if (buffer.readableBytes() - offset < 43) {
         ...
         if ((nullBits & 32) != 0) {
            int cooldownIdOffset = buffer.getIntLE(offset + 39);
            if (cooldownIdOffset < 0) {
               return ValidationResult.error("Invalid offset for CooldownId");
            }

            int pos = offset + 43 + cooldownIdOffset;
            if (pos >= buffer.writerIndex()) {
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof CooldownConditionInteraction)) {
         return false;
      } else {
         CooldownConditionInteraction other = (CooldownConditionInteraction)obj;
