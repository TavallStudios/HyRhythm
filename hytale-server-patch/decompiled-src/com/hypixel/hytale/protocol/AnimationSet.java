...
public class AnimationSet {
   ...
   public static AnimationSet deserialize(@Nonnull ByteBuf buf, int offset) {
      ...
      if ((nullBits & 4) != 0) {
         ...
         if ((long)(varPos1 + varIntLen) + (long)animationsCount * 22L > (long)buf.readableBytes()) {
   ...
   public AnimationSet clone() {
      ...
      copy.id = this.id;
      copy.animations = this.animations != null ? (Animation[])Arrays.stream(this.animations).map((e) -> e.clone()).toArray((x$0) -> new Animation[x$0]) : null;
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof AnimationSet)) {
         return false;
      } else {
         AnimationSet other = (AnimationSet)obj;
