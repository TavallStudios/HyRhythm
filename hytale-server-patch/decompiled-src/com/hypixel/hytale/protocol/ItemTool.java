...
public class ItemTool {
   ...
   public static ItemTool deserialize(@Nonnull ByteBuf buf, int offset) {
      ...
      if ((nullBits & 1) != 0) {
         ...
         if ((long)(pos + specsVarLen) + (long)specsCount * 9L > (long)buf.readableBytes()) {
   ...
   public ItemTool clone() {
      ...
      copy.specs = this.specs != null ? (ItemToolSpec[])Arrays.stream(this.specs).map((e) -> e.clone()).toArray((x$0) -> new ItemToolSpec[x$0]) : null;
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof ItemTool)) {
         return false;
      } else {
         ItemTool other = (ItemTool)obj;
