...
public class ItemLibrary {
   ...
   public static ItemLibrary deserialize(@Nonnull ByteBuf buf, int offset) {
      ...
      if ((nullBits & 1) != 0) {
         ...
         if ((long)(varPos0 + varIntLen) + (long)itemsCount * 147L > (long)buf.readableBytes()) {
   ...
   public int computeSize() {
      ...
      if (this.blockMap != null) {
         ...
         for(Map<Integer, String> elem : this.blockMap) {
            blockMapSize += VarInt.size(elem.size()) + elem.entrySet().stream().mapToInt((kvpInner) -> 4 + PacketIO.stringSize((String)kvpInner.getValue())).sum();
         }
   ...
   public ItemLibrary clone() {
      ...
      copy.items = this.items != null ? (ItemBase[])Arrays.stream(this.items).map((e) -> e.clone()).toArray((x$0) -> new ItemBase[x$0]) : null;
      copy.blockMap = this.blockMap != null ? (Map[])Arrays.stream(this.blockMap).map((d) -> new HashMap(d)).toArray((x$0) -> new Map[x$0]) : null;
      return copy;
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof ItemLibrary)) {
         return false;
      } else {
         ItemLibrary other = (ItemLibrary)obj;
