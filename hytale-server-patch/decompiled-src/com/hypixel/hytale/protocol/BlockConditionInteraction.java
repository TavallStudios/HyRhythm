...
public class BlockConditionInteraction extends SimpleBlockInteraction {
   ...
   public static BlockConditionInteraction deserialize(@Nonnull ByteBuf buf, int offset) {
      ...
      if ((nullBits & 32) != 0) {
         ...
         if ((long)(varPos5 + varIntLen) + (long)matchersCount * 3L > (long)buf.readableBytes()) {
   ...
   public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
      if (buffer.readableBytes() - offset < 44) {
         ...
         if ((nullBits & 32) != 0) {
            ...
            for(int i = 0; i < matchersCount; ++i) {
               ValidationResult structResult = BlockMatcher.validateStructure(buffer, pos);
   ...
   public BlockConditionInteraction clone() {
      ...
      copy.matchers = this.matchers != null ? (BlockMatcher[])Arrays.stream(this.matchers).map((ex) -> ex.clone()).toArray((x$0) -> new BlockMatcher[x$0]) : null;
      return copy;
   }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof BlockConditionInteraction)) {
         return false;
      } else {
         BlockConditionInteraction other = (BlockConditionInteraction)obj;
