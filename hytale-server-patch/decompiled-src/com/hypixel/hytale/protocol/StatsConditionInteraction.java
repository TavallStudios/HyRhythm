...
public class StatsConditionInteraction extends SimpleInteraction {
   ...
   public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
      if (buffer.readableBytes() - offset < 46) {
         ...
         if ((nullBits & 32) != 0) {
            int costsOffset = buffer.getIntLE(offset + 42);
            if (costsOffset < 0) {
               return ValidationResult.error("Invalid offset for Costs");
            }

            int pos = offset + 46 + costsOffset;
            if (pos >= buffer.writerIndex()) {
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof StatsConditionInteraction)) {
         return false;
      } else {
         StatsConditionInteraction other = (StatsConditionInteraction)obj;
