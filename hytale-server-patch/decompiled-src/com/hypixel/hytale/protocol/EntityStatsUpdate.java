...
public class EntityStatsUpdate extends ComponentUpdate {
   ...
   public int serialize(@Nonnull ByteBuf buf) {
      ...
      if (this.entityStatUpdates.size() > 4096000) {
         ...
         for(Map.Entry<Integer, EntityStatUpdate[]> e : this.entityStatUpdates.entrySet()) {
            ...
            VarInt.write(buf, ((EntityStatUpdate[])e.getValue()).length);

            for(EntityStatUpdate arrItem : (EntityStatUpdate[])e.getValue()) {
   ...
   public int computeSize() {
      ...
      for(Map.Entry<Integer, EntityStatUpdate[]> kvp : this.entityStatUpdates.entrySet()) {
         entityStatUpdatesSize += 4 + VarInt.size(((EntityStatUpdate[])kvp.getValue()).length) + Arrays.stream((EntityStatUpdate[])kvp.getValue()).mapToInt((inner) -> inner.computeSize()).sum();
      }
   ...
   public EntityStatsUpdate clone() {
      ...
      for(Map.Entry<Integer, EntityStatUpdate[]> e : this.entityStatUpdates.entrySet()) {
         m.put((Integer)e.getKey(), (EntityStatUpdate[])Arrays.stream((EntityStatUpdate[])e.getValue()).map((x) -> x.clone()).toArray((x$0) -> new EntityStatUpdate[x$0]));
      }
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (obj instanceof EntityStatsUpdate) {
   ...
   public int hashCode() {
      return Objects.hash(new Object[]{this.entityStatUpdates});
   }
}
