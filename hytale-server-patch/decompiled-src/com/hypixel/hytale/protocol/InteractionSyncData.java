...
public class InteractionSyncData {
   ...
   public InteractionSyncData() {
      this.state = InteractionState.Finished;
      this.enteredRootInteraction = -2147483648;
      this.blockFace = BlockFace.None;
      this.placedBlockId = -2147483648;
      this.chargeValue = -1.0F;
      this.chainingIndex = -1;
      this.flagIndex = -1;
      this.movementDirection = MovementDirection.None;
      this.applyForceState = ApplyForceState.Waiting;
   ...
   public static InteractionSyncData deserialize(@Nonnull ByteBuf buf, int offset) {
      ...
      if ((nullBits[1] & 1) != 0) {
         ...
         if ((long)(varPos1 + varIntLen) + (long)hitEntitiesCount * 53L > (long)buf.readableBytes()) {
   ...
   public void serialize(@Nonnull ByteBuf buf) {
      ...
      if (this.forkCounts != null) {
         ...
         for(Map.Entry<InteractionType, Integer> e : this.forkCounts.entrySet()) {
   ...
   public InteractionSyncData clone() {
      ...
      copy.hitEntities = this.hitEntities != null ? (SelectedHitEntity[])Arrays.stream(this.hitEntities).map((e) -> e.clone()).toArray((x$0) -> new SelectedHitEntity[x$0]) : null;
   ...
   public int hashCode() {
      ...
      result = 31 * result + Objects.hashCode(this.generatedUUID);
      return result;
   }
}
