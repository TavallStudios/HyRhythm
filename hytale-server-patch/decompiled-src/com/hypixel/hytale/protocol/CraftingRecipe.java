...
public class CraftingRecipe {
   ...
   public CraftingRecipe clone() {
      ...
      copy.id = this.id;
      copy.inputs = this.inputs != null ? (MaterialQuantity[])Arrays.stream(this.inputs).map((e) -> e.clone()).toArray((x$0) -> new MaterialQuantity[x$0]) : null;
      copy.outputs = this.outputs != null ? (MaterialQuantity[])Arrays.stream(this.outputs).map((e) -> e.clone()).toArray((x$0) -> new MaterialQuantity[x$0]) : null;
      copy.primaryOutput = this.primaryOutput != null ? this.primaryOutput.clone() : null;
      copy.benchRequirement = this.benchRequirement != null ? (BenchRequirement[])Arrays.stream(this.benchRequirement).map((e) -> e.clone()).toArray((x$0) -> new BenchRequirement[x$0]) : null;
