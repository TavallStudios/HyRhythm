...
public class SemverRange implements SemverSatisfies {
      ...
      if (!str.isBlank() && !"*".equals(str)) {
         ...
         for(int i = 0; i < split.length; ++i) {
            ...
            if (subRange.contains(" - ")) {
               ...
               comparators[i] = new SemverRange(new SemverSatisfies[]{new SemverComparator(SemverComparator.ComparisonType.GTE, Semver.fromString(range[0], strict)), new SemverComparator(SemverComparator.ComparisonType.LTE, Semver.fromString(range[1], strict))}, true);
            } else if (subRange.charAt(0) == '~') {
               Semver semver = Semver.fromString(subRange.substring(1), strict);
               if (semver.getMinor() > 0L) {
                  comparators[i] = new SemverRange(new SemverSatisfies[]{new SemverComparator(SemverComparator.ComparisonType.GTE, semver), new SemverComparator(SemverComparator.ComparisonType.LT, new Semver(semver.getMajor(), semver.getMinor() + 1L, 0L, (String[])null, (String)null))}, true);
               } else {
                  comparators[i] = new SemverRange(new SemverSatisfies[]{new SemverComparator(SemverComparator.ComparisonType.GTE, semver), new SemverComparator(SemverComparator.ComparisonType.LT, new Semver(semver.getMajor() + 1L, 0L, 0L, (String[])null, (String)null))}, true);
               }
            } else if (subRange.charAt(0) == '^') {
               Semver semver = Semver.fromString(subRange.substring(1), strict);
               if (semver.getMajor() > 0L) {
                  ...
                  comparators[i] = new SemverRange(new SemverSatisfies[]{new SemverComparator(SemverComparator.ComparisonType.GTE, semver), new SemverComparator(SemverComparator.ComparisonType.LT, new Semver(0L, semver.getMinor() + 1L, 0L, (String[])null, (String)null))}, true);
               } else {
                  comparators[i] = new SemverRange(new SemverSatisfies[]{new SemverComparator(SemverComparator.ComparisonType.GTE, semver), new SemverComparator(SemverComparator.ComparisonType.LT, new Semver(0L, 0L, semver.getPatch() + 1L, (String[])null, (String)null))}, true);
               }
   ...
   static {
      CODEC = new FunctionCodec(Codec.STRING, SemverRange::fromString, SemverRange::toString);
      WILDCARD = new SemverRange(new SemverSatisfies[0], true);
   }
}
