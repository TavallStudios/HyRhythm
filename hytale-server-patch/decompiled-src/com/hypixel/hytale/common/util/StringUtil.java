...
public class StringUtil {
   ...
   public static <V extends Enum<V>> V parseEnum(@Nonnull V[] enumConstants, String str) {
      return (V)parseEnum(enumConstants, str, StringUtil.MatchType.EQUALS);
   }
   ...
   public static <V extends Enum<V>> V parseEnum(@Nonnull V[] enumConstants, String str, MatchType matchType) {
      if (matchType == StringUtil.MatchType.EQUALS) {
         ...
         for(V enumValue : enumConstants) {
            int index = StringCompareUtil.indexOfDifference(str, enumValue.name().toLowerCase());
   ...
   public static String[] parseArgs(String rawString, @Nonnull Map<String, String> argOptions) {
         ...
         switch (c) {
            ...
            case '\\':
               ...
               switch (c1) {
                  ...
                  case '\'':
                  case '\\':
                     String var10000 = argsStr.substring(0, i);
                     argsStr = var10000 + argsStr.substring(i + 1);
                     ++i;
                     break;
                  default:
      ...
      if (quote != 0) {
         ...
         if (hasRaw) {
            ...
            return (String[])argsList.toArray((x$0) -> new String[x$0]);
         }
      }
   }
   ...
   public static void generateGraph(@Nonnull StringBuilder sb, int width, int height, long minX, long maxX, double minY, double maxY, @Nonnull DoubleFunction<String> labelFormatFunc, int historyLength, @Nonnull IntToLongFunction timestampFunc, @Nonnull IntToDoubleFunction valueFunc) {
      ...
      for(int i = 0; i < width; ++i) {
         ...
         for(long nextAggTimestamp = maxX - (lengthX - (long)(colAggLength * (double)i)); historyIndex < historyLength && timestampFunc.applyAsLong(historyIndex) < nextAggTimestamp; ++historyIndex) {
      ...
      for(int row = 0; row < height; ++row) {
         ...
         String label = (String)labelFormatFunc.apply(rowMaxValue);
         labels[row] = label;
      ...
      String var10000 = " ".repeat(yLabelWidth);
      String bar = var10000 + " " + "#".repeat(width + 2);
