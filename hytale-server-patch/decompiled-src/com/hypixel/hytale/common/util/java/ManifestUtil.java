...
public class ManifestUtil {
   ...
   private static final CachedSupplier<Manifest> MANIFEST = SupplierUtil.<Manifest>cache(() -> {
      try {
   ...
   private static final CachedSupplier<String> IMPLEMENTATION_VERSION = SupplierUtil.<String>cache(() -> {
      try {
         Manifest localManifest = MANIFEST.get();
         return localManifest == null ? "NoJar" : (String)Objects.requireNonNull(localManifest.getMainAttributes().getValue("Implementation-Version"), "Null implementation version!");
      } catch (Throwable t) {
         HytaleLogger.getLogger().at(Level.WARNING).log("Exception was thrown getting implementation version!", t);
         return "UNKNOWN";
      }
   });
   private static final CachedSupplier<String> IMPLEMENTATION_REVISION_ID = SupplierUtil.<String>cache(() -> {
      try {
         Manifest localManifest = MANIFEST.get();
         return localManifest == null ? "NoJar" : (String)Objects.requireNonNull(localManifest.getMainAttributes().getValue("Implementation-Revision-Id"), "Null implementation revision id!");
      } catch (Throwable t) {
         HytaleLogger.getLogger().at(Level.WARNING).log("Exception was thrown getting implementation revision id!", t);
         return "UNKNOWN";
      }
   });
   private static final CachedSupplier<String> IMPLEMENTATION_PATCHLINE = SupplierUtil.<String>cache(() -> {
      try {
         Manifest localManifest = MANIFEST.get();
   ...
   private static final CachedSupplier<String> VERSION = SupplierUtil.<String>cache(() -> {
